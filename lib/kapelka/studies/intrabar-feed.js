...
export class IntrabarFeed {
  ...
  reconcile(needed) {
    for (const k in this.cache)
      if (!needed.has(k)) {
        this._dropSub(this.cache[k].subId);
        delete this.cache[k];
      }
  }
  ...
  /** @param {string} lowerTfId @param {import('../core/types.js').Bar[]} bars @param {number} chartSec @returns {any} */
    ...
    const wantFrom = /** @type {number} */ (bars[startIdx].time) * 1000,
      wantTo = /** @type {number} */ (lastBar.time) * 1000 + chartMs;
    ...
    if (e.subId == null && this.opts.subscribeIntrabar) {
      ...
      e.subId = this.opts.subscribeIntrabar(
        lowerTfId,
        liveFrom,
        (/** @type {import('../core/types.js').Bar[]} */ subBars) => {
  ...
  /** @param {any} id */
  _dropSub(id) {
    if (id != null && this.opts.dropIntrabar) {
      try {
        this.opts.dropIntrabar(id);
      } catch (_) {}
    }
  }
  ...
  _fetch(lowerTfId, e, fromMs, toMs) {
    e.loading = true;
    Promise.resolve(/** @type {Function} */ (this.opts.intrabar)(lowerTfId, fromMs, toMs))
      .then((/** @type {import('../core/types.js').Bar[]} */ subBars) => {
        e.loading = false;
        e.subBars = mergeBars(e.subBars, subBars || []);
