...
export function registerCustomPlot(type, view) {
  if (type && view) CUSTOM_PLOTS[type] = view;
}
/** @param {string} type */
export function unregisterCustomPlot(type) {
  delete CUSTOM_PLOTS[type];
}
/** @param {string} type */
...
export const defaultsFor = (study) => {
  /** @type {Record<string, any>} */
  const d = {};
  (study.inputs || []).forEach((/** @type {import('./types.js').StudyInput} */ i) => {
    d[i.key] = i.default;
  });
  return d;
};
...
export function styleToOptions(eff, show) {
  ...
  const base = {
    ...
    axisId: eff.priceScaleId,
    margins: eff.scaleMargins,
    overlayLog: eff.overlayLog,
  };
  ...
  if (eff.type === 'area')
    return {
      ...base,
      lineColor: eff.color,
      lineWidth: eff.lineWidth,
      lineStyle: ls,
      lineType: eff.lineType,
      topColor: rgba(eff.color, eff.fillOpacity),
      bottomColor: rgba(eff.color, 0),
      showCursorMarker: false,
      showPointMarkers: !!eff.markers,
    };
  if (eff.type === 'baseline')
    return {
      ...base,
      lineWidth: eff.lineWidth,
      lineStyle: ls,
      topLineColor: eff.colorUp,
      bottomLineColor: eff.colorDown,
      topFillColor1: rgba(eff.colorUp, eff.fillOpacity),
      topFillColor2: rgba(eff.colorUp, 0),
      bottomFillColor1: rgba(eff.colorDown, eff.fillOpacity),
      bottomFillColor2: rgba(eff.colorDown, 0),
      showCursorMarker: false,
      showPointMarkers: !!eff.markers,
    };
  return {
...
export function applyStacking(plots) {
  ...
  plots.forEach((pl) => {
    if (pl.stack) {
      if (!groups.has(pl.stack)) groups.set(pl.stack, []);
      /** @type {any[]} */ (groups.get(pl.stack)).push(pl);
    }
  });
...
export function buildFillBands(fills, plots) {
  ...
  const byKey = {};
  plots.forEach((pl) => {
    byKey[pl.key] = pl.data || [];
  });
  return /** @type {Array<{ top: string, bottom: string, color: string, gradient: any, points: any[] }>} */ (
    fills
      .map((/** @type {any} */ f) => {
        const top = byKey[f.top],
          bot = byKey[f.bottom];
        ...
        return {
          top: f.top,
          bottom: f.bottom,
          color: f.color || 'rgba(38,166,154,0.12)',
...
export function shapesToMarks(shapes) {
  ...
  for (const s of shapes || []) {
    ...
    if (Array.isArray(s.marks)) {
      for (const m of s.marks) if (m) out.push(m);
      continue;
    } // raw geometry, pass-through
    if (s.shape) {
      for (const m of resolveShape(s)) if (m) out.push(m);
      continue;
    } // named recipe from the shape library
    if (s.type === 'band') {
      out.push({
        back: true,
        closed: true,
        fill: s.color || 'rgba(120,120,120,0.12)',
        path: [
          { t: s.from, vp: 0 },
          { t: s.to, vp: 0 },
          { t: s.to, vp: 1 },
      ...
      if (s.label)
        out.push({
          text: s.label,
          at: { t: s.from, p: s.top, dx: 4, dy: 2 },
          color: s.borderColor || s.color,
          size: 11,
        });
    } else if (s.type === 'vline') {
      out.push({
        stroke: s.color || '#787b86',
        width: s.width || 1,
        dash: s.lineStyle,
        path: [
          { t: s.time, vp: 0 },
...
export function scaleProvider(scale) {
  ...
  if (scale && typeof scale === 'object') {
    const hasMax = scale.max != null,
      hasMin = scale.min != null;
...
export function bucketIntrabar(bars, subBars, chartSec) {
  ...
  for (let i = 0; i < bars.length; i++) {
    const start = /** @type {number} */ (bars[i].time),
      end = /** @type {number} */ (i + 1 < bars.length ? bars[i + 1].time : start + chartSec);
    while (j < subBars.length && /** @type {number} */ (subBars[j].time) < start) j++;
    while (j < subBars.length && /** @type {number} */ (subBars[j].time) < end) {
      out[i].push(subBars[j]);
      j++;
    }
  }
  return out;
