...
export function runStepBatch(entries, sh) {
  const runs = entries.map((e) => {
    ...
    return {
      ...
      /** @type {Map<string, number>} */ idx: new Map(
        metas.map((/** @type {any} */ m, /** @type {number} */ i) => [m.key, i]),
      ),
    };
  });

  for (let i = 0; i < sh.n; i++) {
    // <-- ONE traversal, shared by every study
  ...
  return runs.map((run) => {
    /** @type {any} */
    const res = {
      plots: run.metas.map((/** @type {any} */ m, /** @type {number} */ i) => ({ ...m, data: run.data[i] })),
    };
