...
export class StudyWorker {
  ...
  _ensure() {
    ...
    w.onmessage = (e) => {
      ...
      if (d.error) t.reject(new Error(d.error)); else t.resolve(d.results);
    };
    ...
    w.onerror = (/** @type {any} */ e) => {
      const msg = 'study worker: ' + ((e && (e.message || e.filename)) || 'load failed') + (e && e.lineno ? ' @' + e.lineno : '');
      try { console.error('[study-worker]', msg, e); } catch (_) {}
   ...
   */
  send(msg) { this._ensure().postMessage(msg); }
  ...
  destroy() {
    if (this._w) { try { this._w.terminate(); } catch (_) {} this._w = null; }
