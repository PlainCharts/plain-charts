...
const dashArr = (dash, w) => {
  ...
  return dash === 'dashed'
    ? [Math.max(4, w * 2), Math.max(4, w * 2)]
    : dash === 'dotted'
      ? [Math.max(1, w), Math.max(2, w * 2)]
      : [];
};
...
export function paintMark(c, m, scope) {
  ...
  if (m.path) {
    ...
    if (m.stroke) {
      ...
      c.lineWidth = m.width || 1;
      c.setLineDash(dashArr(m.dash, m.width || 1));
      c.stroke();
    }
    c.restore();
  } else if (m.text != null) {
    const q = resolveVertex(m.at, scope);
    if (!q) return;
    c.save();
    ...
    const lines = String(m.text).split('\n'),
      ...
      base = m.baseline || 'top';
    const y0 =
      base === 'bottom' ? q.y - (lines.length - 1) * lh : base === 'middle' ? q.y - ((lines.length - 1) * lh) / 2 : q.y;
...
export function paintMarks(c, marks, scope) {
  if (marks) for (const m of marks) paintMark(c, m, scope);
}
...
export function createMarkPrimitive(chart, getBarTimes, getSeries, getMarks) {
  ...
  const painter = (back) => (c, W, H) => {
    ...
    const scope = {
      timeToX: xOfTime,
      priceToY: (/** @type {number} */ p) => (series ? series.priceToY(p) : null),
      width: W,
      height: H,
    };
    ...
    if (marks)
      for (const m of marks) {
        if (!!m.back === back) paintMark(c, m, scope);
      }
  };
  ...
  const makeView = (zOrder, paint, isEmpty) => ({
    renderer: () => ({
      /** @param {any} target */
      draw(target) {
        target.useMediaCoordinateSpace((/** @type {any} */ s) =>
          paint(s.context, s.mediaSize.width, s.mediaSize.height),
        );
      },
    }),
    zOrder: () => zOrder,
  ...
  return {
    ...
    paneViews() {
      return [
        makeView('bottom', painter(true), () => {
          const ms = getMarks();
          return !ms || !ms.some((/** @type {any} */ m) => m.back);
        }),
        makeView('top', painter(false)),
      ];
    },
