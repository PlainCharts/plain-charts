...
export function createBandPrimitive(chart, getBarTimes, getSeries, getBands) {
  ...
  const paint = (c, W, H) => {
    ...
    getBands().forEach((/** @type {Band} */ band) => {
      ...
      if (g && Array.isArray(g.at) && Array.isArray(g.colors) && g.colors.length && c.createLinearGradient) {
        const yA = series.priceToY(g.at[0]),
          yB = series.priceToY(g.at[g.at.length - 1]);
        if (yA != null && yB != null && yA !== yB) {
          const gr = c.createLinearGradient(0, yA, 0, yB),
            n = g.colors.length;
          g.colors.forEach((/** @type {string} */ col, /** @type {number} */ i) =>
            gr.addColorStop(n === 1 ? 0 : i / (n - 1), col),
          );
      ...
      const flush = () => {
        if (run.length >= 2) {
          ...
          for (let i = 0; i < run.length; i++) {
            const p = run[i];
            (i === 0 ? c.moveTo : c.lineTo).call(c, p.x, p.yTop);
          }
    ...
    renderer: () => ({
      ...
      draw(target) {
        target.useMediaCoordinateSpace((/** @type {any} */ scope) =>
          painter(scope.context, scope.mediaSize.width, scope.mediaSize.height),
        );
      },
    }),
    zOrder: () => zOrder,
  ...
  return {
    updateAllViews() {},
    paneViews() {
      return [makeView('bottom', paint)];
    }, // behind the plot lines
