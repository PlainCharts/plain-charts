...
export function paneIndexOf(chart, series) {
  try { const ps = chart.panes(); for (let i = 0; i < ps.length; i++) { if (ps[i].getSeries().indexOf(series) !== -1) return i; } } catch (_) {}
  return -1;
}
...
export function applyPaneMode(chart, mainSeries, series, mode, state) {
  let ps; try { ps = chart.panes(); } catch (_) { return; }
  const idx = paneIndexOf(chart, series); if (idx < 0) return;
  const mainIdx = paneIndexOf(chart, mainSeries);
  const sf = (/** @type {any} */ p) => { try { return p.getStretchFactor(); } catch (_) { try { return p.getHeight(); } catch (_2) { return 1; } } };
  const px = (/** @type {any} */ p) => { try { return p.getHeight() || 0; } catch (_) { return 0; } };
  const set = (/** @type {any} */ p, /** @type {number} */ f) => { try { p.setStretchFactor(Math.max(0.0001, f)); } catch (_) {} };
  ...
  if (mode === 'max') {
    ...
    const h = ps.map(px), grow = Math.max(0, state.collapsedPx - h[idx]);
