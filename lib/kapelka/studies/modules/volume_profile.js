...
//
// Gallery copy of the app's studies/modules/volume_profile.js — identical algorithm; the only change
...
Studies.register({
  ...
  calc(bars, p) {
    ...
    /** @param {number} a1 @param {number} a2 @param {number} b1 @param {number} b2 @param {number} height @param {number} vol */
    ...
    return {
      plots: [{ key: 'vp', name: 'VP', type: 'hbar', legend: false, side: p.side || 'right', widthFrac: (p.width || 30) / 100, data }],
      shapes: [
