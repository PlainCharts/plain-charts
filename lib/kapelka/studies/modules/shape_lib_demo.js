...
registerShape('zone', {
  ...
  marks: [
    ...
    {
      text: "=$up ? 'accumulation' : 'distribution'",
      at: { t: '$from', p: '=$top - $pad', dx: 4, dy: 2 },
...
registerShape('pin', (/** @type {any} */ d) => {
  const col = d.color || '#e0a030',
    H = d.height || 30,
    r = d.dot || 4;
  const dot = /** @type {any[]} */ ([]);
  for (let k = 0; k <= 16; k++) {
    const a = (k / 16) * Math.PI * 2;
    dot.push({ t: d.t, p: d.p, dx: r * Math.cos(a), dy: -H + r * Math.sin(a) });
  }
  return [
    {
      path: [
...
Studies.register({
  ...
  calc(bars, p) {
    ...
    for (let i = start; i < N; i++) {
      ...
      if (bars[i].low < loV) loV = bars[i].low;
    }
