...
Studies.register({
  ...
  calc(bars, p) {
    ...
    const from = bars[N - k].time, to = bars[N - 1].time, lastT = bars[N - 1].time;
    ...
    return {
      ...
      shapes: [
        ...
        { marks: [{ closed: true, fill: '#4dd0e1', path: [
          { t: lastT, p: lastV, dy: -6 }, { t: lastT, p: lastV, dx: 6 },
          { t: lastT, p: lastV, dy: 6 }, { t: lastT, p: lastV, dx: -6 },
        ] }] },

        // --- cross-pane: drawn on the MAIN price chart (overlay:true) ---
        // INLINE MARKS: a box around the last k bars' high/low range
        { overlay: true, marks: [{ closed: true, fill: 'rgba(77,208,225,0.10)', stroke: '#4dd0e1', width: 1, path: [
          { t: from, p: top }, { t: to, p: top }, { t: to, p: bot }, { t: from, p: bot },
        ] }] },
