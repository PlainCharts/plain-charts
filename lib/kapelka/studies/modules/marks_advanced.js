...
Studies.register({
  ...
  calc(bars) {
    ...
    const len = 20,
      sma = /** @type {{ time: number, value: number }[]} */ ([]);
    let sum = 0;
    for (let i = 0; i < N; i++) {
      sum += bars[i].close;
    ...
    const hiT = bars[hiI].time,
      loT = bars[loI].time;
    const lastT = bars[N - 1].time,
      lastC = bars[N - 1].close;
    ...
    //    The engine has no curves; the author makes one out of data.
    {
      const x0 = loT,
        ...
        y2 = hiV;
      const cx = (x0 + x2) / 2,
        cy = Math.max(hiV, loV) + (hiV - loV) * 0.55;
    ...
    {
      const r = 20,
        pts = /** @type {import('../primitives/marks.js').Vertex[]} */ ([]);
      for (let k = 0; k <= 40; k++) {
        const a = (k / 40) * Math.PI * 2;
        pts.push({ t: lastT, p: lastC, dx: r * Math.cos(a), dy: r * Math.sin(a) });
      }
    ...
    {
      const dx = 48,
        dy = 46; // tail offset from the tip, in pixels
      m.push({
        path: [
          { t: hiT, p: hiV, dx, dy },
          { t: hiT, p: hiV },
    ...
    {
      const zA = start + Math.floor(R * 0.12),
        zB = start + Math.floor(R * 0.34);
      let zTop = -Infinity,
        zBot = Infinity;
      for (let i = zA; i <= zB; i++) {
        if (bars[i].high > zTop) zTop = bars[i].high;
        if (bars[i].low < zBot) zBot = bars[i].low;
      }
      const zF = bars[zA].time,
        zT = bars[zB].time,
        span = zT - zF;
      m.push({
        closed: true,
        dash: 'dashed',
        stroke: 'rgba(150,152,161,0.7)',
        width: 1,
        path: [
          { t: zF, p: zTop },
          { t: zT, p: zTop },
      ...
      for (let k = 0; k <= M; k++) {
        const u = k / M,
          ub = Math.max(0, Math.min(1, u - 0.3));
        m.push({
          path: [
            { t: zF + u * span, p: zTop },
            { t: zF + ub * span, p: zBot },
          ],
          stroke: 'rgba(150,152,161,0.20)',
          width: 1,
        });
      }
      m.push({
        text: 'consolidation',
        at: { t: zF, p: zTop, dx: 4, dy: -14 },
        color: 'rgba(150,152,161,0.9)',
        size: 11,
      });
    }
