...
export function snapshotState(s) {
  ...
  for (const k in s) { const v = /** @type {any} */ (s)[k]; out[k] = (v && typeof v === 'object') ? snapshotState(v) : v; }
  return out;
}
...
export function restoreState(live, snap) {
  if (!live || !snap) return;
  for (const k in snap) { const v = snap[k]; live[k] = (v && typeof v === 'object') ? snapshotState(v) : v; }
}
...
function assembleRow(row, time, data, idx) {
  ...
  const pts = {};
  for (const k in row) {
    const pi = idx.get(k); if (pi === undefined) continue;
    const v = row[k];
    const p = (v && typeof v === 'object') ? { time, ...v } : { time, value: v };
    data[pi].push(p); pts[k] = p;
  }
  return pts;
}
...
export function stepFull(sess, sh) {
  ...
  sess.state = state; sess.metas = metas; sess.shapes = shapes; sess.fills = fills; sess.n = sh.n;
  /** @type {any} */
  const res = { plots: metas.map((/** @type {any} */ m, /** @type {number} */ i) => ({ ...m, data: data[i] })) };
  if (shapes) res.shapes = shapes; if (fills) res.fills = fills; if (scale) res.scale = scale;
  return res;
}
...
export function stepForming(sess, sh) {
  ...
  const i = sh.n - 1;
  const pts = assembleRow(sess.study.step(i, sh, sess.params, sess.ctx, sess.state), sh.time[i], sess.metas.map(() => []), idx);
  sess.n = sh.n;
...
export function stepAppend(sess, sh) {
  ...
  const closedPts = assembleRow(sess.study.step(closedI, sh, sess.params, sess.ctx, sess.state), sh.time[closedI], sess.metas.map(() => []), idx);
  sess.checkpoint = snapshotState(sess.state);   // new checkpoint now includes the just-closed bar (0..n_old-1)
  const formI = sh.n - 1;
  const formPts = assembleRow(sess.study.step(formI, sh, sess.params, sess.ctx, sess.state), sh.time[formI], sess.metas.map(() => []), idx);
  sess.n = sh.n;
...
function tailFrom(metas, rows) {
  ...
  for (const m of metas) {
    /** @type {any[]} */
    const points = [];
    for (const r of rows) { if (r && r[m.key]) points.push(r[m.key]); }
