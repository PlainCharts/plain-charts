...
function mergeByTime(cur, inc) {
  ...
  cur.length = 0; for (const r of merged) cur.push(r);   // rewrite in place: the resident state holds this ref
  return cur;
}
...
export function applyResident(state, msg) {
  ...
  switch (msg.type) {
    ...
    case 'update-forming-bar': {   // a live tick on the forming (last) bar -> replace it in place
      const b = msg.bar, n = state.bars.length;
      if (b) { if (n && T2(state.bars[n - 1]) === T2(b)) state.bars[n - 1] = b; else if (!n || T2(b) > T2(state.bars[n - 1])) state.bars.push(b); }
      return state;
    }

    case 'append-bar': {   // a bar closed and a new one opened -> push it (the prior last was finalized first)
      const b = msg.bar, n = state.bars.length;
      if (b) { if (!n || T2(b) > T2(state.bars[n - 1])) state.bars.push(b); else if (T2(state.bars[n - 1]) === T2(b)) state.bars[n - 1] = b; }
      return state;
    }

    case 'append-intrabar': {   // sub-bars for a lower timeframe: the live delta (or the initial windowed fetch)
