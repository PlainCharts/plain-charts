...
export class StudyHost {
  ...
  constructor(chart, opts = {}) {
    ...
    this._intrabarFeed = new IntrabarFeed(opts, {
      toWorker: (/** @type {any} */ msg) => {
        if (this._worker) this._worker.send(msg);
      },
      onData: () =>
        this.attached.forEach((a) => {
          if (a.study && (a.study.intrabar || a.study.lowerTimeframe)) this.recompute(a);
        }),
    });
  ...
  setData(bars) {
    ...
    if (this._worker) {
      if (kind === 'forming') this._worker.send({ type: 'update-forming-bar', bar: bars[bars.length - 1] });
        ...
        this._worker.send({ type: 'update-forming-bar', bar: bars[bars.length - 2] });
        this._worker.send({ type: 'append-bar', bar: bars[bars.length - 1] });
  ...
  setVisibleRange(range) {
    ...
    if (!range) return;
    this.attached.forEach((a) => {
      if (a.study && a.study.viewport) this.recompute(a);
    });
  }
  ...
  resetIntrabar() {
    ...
    if (this._worker) {
      this._worker.send({ type: 'reset' });
      this._worker.send({ type: 'set-bars', bars: this.bars });
      this._syncBars = this.bars;
    }
  }
  ...
  _reconcileIntrabar() {
    ...
    for (const a of this.attached)
      if (this._show(a) && a.study && (a.study.intrabar || a.study.lowerTimeframe))
        needed.add(this._resolveLowerTf(a.study));
  ...
  refreshVisibility() {
    ...
    this.attached.forEach((a) => {
      if (this._show(a) && a._stale) this.recompute(a);
    });
  }
  ...
  add(id, params, opts = {}) {
    ...
    const a = {
      study,
      params: { ...defaultsFor(study), ...(params || {}) },
      hidden: !!opts.hidden,
      collapsed: false,
      overlay: study.overlay !== false,
      plots: new Map(),
      seriesType: new Map(),
      paneIndex: null,
      stretched: false,
      ...
      shapesSeries: null,
      mainShapesPrim: null,
      mainShapesSeries: null, // sub-pane study's `overlay:true` shapes, on the price pane
      fillBands: [],
      fillPrim: null,
      fillSeries: null,
      markers: [],
  ...
  remove(i) {
    ...
    this._onRemoved(a);
    this._detachStudy(a);
    a.plots.forEach((/** @type {any} */ s) => {
      try {
        this.chart.removePlot(s);
      } catch (_) {}
    });
    if (ci >= 0) {
      try {
        const ps = this.chart.panes();
        if (ps[ci] && ps[ci].getSeries().length === 0) this.chart.removePane(ci);
      } catch (_) {}
    }
    ...
    if (this._worker && this._workerEligible(a) && !this.attached.some((x) => this._workerEligible(x)))
      this.disposeStudyWorker();
    this._persist();
  }
  clearAll() {
  ...
  applyTemplate(studies) {
    this.clearAll();
    (studies || []).forEach((/** @type {any} */ s) => this.add(s.id, s.params, { hidden: s.hidden, style: s.style }));
  }

  // ---- visibility ----
  /** @param {Attachment} a */
  _show(a) {
    return !a.hidden && !a.collapsed && !this._isHidden();
  }
  /** @param {number} i */
  toggleHidden(i) {
    const a = this.attached[i];
  ...
  _restyle(a) {
    (a.plotMeta || []).forEach((/** @type {any} */ pm) => {
      ...
      if (s) {
        try {
          s.configure(styleToOptions(effectiveStyle(a.style, pm), this._show(a)));
        } catch (_) {}
      }
    });
  }
  ...
  async recompute(a) {
    ...
    if (eligible) {
      ...
      if (!url) {
        a.error = 'study worker: no URL for ' + a.study.id;
        this._onError(a, a.error);
        return;
      }
      ...
      const meta = {
        ...
        chartSec: this._chartSec(),
      };
      if (ctx.lowerTimeframe !== undefined) {
        meta.lowerTimeframe = ctx.lowerTimeframe;
        meta.intrabarLoading = ctx.intrabarLoading;
      }
    ...
    try {
      out = await a.study.calc(bars, a.params, ctx);
    } catch (e) {
      a.error = (e && /** @type {any} */ (e).message) || String(e);
      this._onError(a, a.error);
      return;
    }
  ...
  _queueExec(a, studyId, url, params, meta, seq) {
    ...
    this._pending.push({
      ...
      a,
      seq,
      entry: { eid, studyId, studyUrl: url, params, meta, sid: a._sid, mode: this._tickMode },
    });
    if (!this._flushScheduled) {
      this._flushScheduled = true;
      Promise.resolve().then(() => this._flushExec());
    }
  }
  async _flushExec() {
    this._flushScheduled = false;
    ...
    let results;
    try {
      results = await this._studyWorker().execBatch(batch.map((b) => b.entry));
    } catch (e) {
      const msg = (e && /** @type {any} */ (e).message) || String(e);
      batch.forEach((b) => {
        if (b.a._calcSeq === b.seq) {
          b.a.error = msg;
          this._onError(b.a, msg);
        }
      });
      return;
    }
    ...
    for (const r of results || []) {
      ...
      if (r.error) {
        if (b.a._calcSeq === b.seq) {
          b.a.error = r.error;
          this._onError(b.a, r.error);
        }
        continue;
      }
      if (r.tail)
        this._applyIncremental(b.a, r.tail, b.seq); // a checkpoint tick: only the changed last point(s)
  ...
  _applyIncremental(a, tail, seq) {
    applyIncremental(this, a, tail, seq);
  }
  /** @param {Attachment} a @param {any} out @param {number} seq */
  _applyResult(a, out, seq) {
    applyResult(this, a, out, seq);
  }
  ...
  _applyPaneMode(series, mode, state) {
    applyPaneMode(this.chart, this._mainSeries(), series, mode, state);
  }
  /** @param {Attachment} a */
  _applyPreserve(a) {
    applyPreserve(this.chart, a, this._isHidden());
  }
  // re-derive each sub-pane study's paneIndex from where its series actually lives (after a move)
  _reindexPanes() {
    reindexPanes(this.chart, this.attached);
  }
  ...
  _ensureIntrabar(lowerTfId) {
    return this._intrabarFeed.ensure(lowerTfId, this.bars, this._chartSec());
  }
  // average bar spacing in seconds (for the last bar's open-ended bucket window + the lower-tf heuristic)
  _chartSec() {
    const b = this.bars;
    if (b.length < 2) return 60;
    const last = /** @type {number} */ (b[b.length - 1].time),
      first = /** @type {number} */ (b[0].time);
    return (last - first) / (b.length - 1) || 60;
  }

  // ---- context accessors (override or feed via opts) ----
  _priceDecimals() {
    return this.opts.priceDecimals != null ? this.opts.priceDecimals : 2;
  }
  ...
  _tickSize() {
    return this.opts.tickSize != null ? this.opts.tickSize : null;
  }
  // chart candle up/down colors -> ctx.candle for studies that opt to match them (else null)
  _candleStyle() {
    return this.opts.candleStyle ? this.opts.candleStyle() : null;
  }
  _isHidden() {
    return this.opts.isHidden ? this.opts.isHidden() : false;
  }
  _mainSeries() {
    return this.opts.mainSeries ? this.opts.mainSeries() : null;
  }
  /** @returns {number[]} */
  _barTimes() {
    return this.opts.barTimes ? this.opts.barTimes() : /** @type {number[]} */ (this.bars.map((b) => b.time));
  }
  /** @param {any} series */
  _paneIndexOf(series) {
    return paneIndexOf(this.chart, series);
  }
  _persist() {
    if (this.opts.persist)
      try {
        this.opts.persist(this.serialize());
      } catch (_) {}
  }
  ...
  /** @param {Attachment} a */
  _workerEligible(a) {
    return !!(this.opts.worker && a && a.study && a.study.worker !== false && !a.study.requestFrames);
  }
  ...
  _studyWorker() {
    if (!this._worker) {
      ...
      this._worker.send({ type: 'set-bars', bars: this.bars });
      for (const k in this._intrabarFeed.cache) {
        const sb = this._intrabarFeed.cache[k].subBars;
        if (sb && sb.length) this._worker.send({ type: 'append-intrabar', tf: k, sub: sb });
      }
    }
    return this._worker;
  }
  ...
  _onAdded(a, opts) {
    this._emit('added', a, opts);
  }
  /** @param {Attachment} a */
  _onComputed(a) {
    this._emit('computed', a);
  }
  /** @param {Attachment} a */
  _onRemoved(a) {
    this._emit('removed', a);
  }
  /** @param {Attachment} a @param {string} msg */
  _onError(a, msg) {
    this._emit('error', a, msg);
  }
}
