...
function sharedWindow() {
  if (cachedVersion !== dataVersion) {
    cachedShared = sharedFromBars(resident.bars);
...
scope.onmessage = async (/** @type {MessageEvent} */ e) => {
  ...
  if (
    msg.type === 'set-bars' ||
    msg.type === 'update-forming-bar' ||
    msg.type === 'append-bar' ||
    msg.type === 'append-intrabar' ||
    msg.type === 'reset'
  ) {
    try {
      applyResident(resident, msg);
      dataVersion++;
      if (msg.type === 'reset') sessions.clear();
    } catch (err) {
      try {
        console.error('[study-worker]', (err && /** @type {any} */ (err).message) || err);
      } catch (_) {}
    }
    return;
  }

  if (msg.type !== 'exec-batch') {
    scope.postMessage({ id: msg.id, error: 'study worker: unknown message ' + msg.type });
    return;
  }
  ...
  const subShFor = (/** @type {string} */ tf, /** @type {number} */ chartSec) => {
    if (!subShCache[tf])
      subShCache[tf] = Object.assign({}, baseSh, {
        sub: bucketIntrabar(resident.bars, resident.sub[tf] || [], chartSec),
      });
    return subShCache[tf];
  };
  for (const en of entries || []) {
    try {
      if (en.studyUrl && !loaded.has(en.studyUrl)) {
        await import(/* @vite-ignore */ en.studyUrl);
        loaded.add(en.studyUrl);
      }
      ...
      if (typeof study.step !== 'function') {
        ...
        if (intra)
          ctx.intrabar = bucketIntrabar(resident.bars, resident.sub[en.meta.lowerTimeframe] || [], en.meta.chartSec);
      ...
      if (!sess || sess.paramsKey !== paramsKey) {
        sess = createStepSession(study, en.params, ctx);
        sess.paramsKey = paramsKey;
        sessions.set(en.sid, sess);
      } else {
        sess.ctx = ctx;
        sess.study = study;
      } // refresh ctx (decimals/candle) + study ref (a relink swaps it)
      ...
      results.push({ eid: en.eid, error: (err && /** @type {any} */ (err).message) || String(err) });
    }
  }
...
function buildCtx(params, meta) {
  ...
  if (meta.lowerTimeframe !== undefined) {
    ctx.lowerTimeframe = meta.lowerTimeframe;
    ctx.intrabarLoading = !!meta.intrabarLoading;
  }
  return ctx;
}
