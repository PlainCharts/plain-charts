...
import {
  SERIES_CTOR, effectiveStyle, styleToOptions,
  applyStacking, buildFillBands, scaleProvider, getCustomPlot, shapesToMarks,
} from './channels.js';
...
export function feedPoint(arr, p) {
  ...
  else { const i = arr.findIndex((x) => x.time >= p.time); if (i >= 0 && arr[i].time === p.time) arr[i] = p; else arr.splice(i < 0 ? n : i, 0, p); }
}
...
export function applyIncremental(host, a, tail, seq) {
  ...
  if (a.error) { a.error = null; a.errLogged = null; }
  ...
  for (const t of tail) {
    ...
    if (s) for (const p of t.points) { try { s.feedBar(p); } catch (_) {} }
  ...
  if (a.fillBands && a.fillBands.length) {
    ...
    for (const band of a.fillBands) {
      const tops = tailByKey[band.top], bots = tailByKey[band.bottom];
...
export function applyResult(host, a, out, seq) {
  ...
  a.fillBands = /** @type {any[]} */ (buildFillBands(Array.isArray(out.fills) ? out.fills : [], plots)).concat(stackBands);
  ...
  a.plotMeta = plots.map((/** @type {any} */ pl) => ({
    key: pl.key, name: pl.name || pl.key, type: pl.type || 'line',
    color: pl.color, colorUp: pl.colorUp, colorDown: pl.colorDown,
    lineWidth: pl.lineWidth, lineStyle: pl.lineStyle, lineType: pl.lineType,
    markers: pl.markers, fillOpacity: pl.fillOpacity, visible: pl.visible,
    legend: pl.legend, precision: pl.precision,
  ...
  if (!a.overlay && !a.stretched && a.paneIndex > 0) {   // a pane-0 study (chart-less) fills the pane, no shrink
    try { const ps = host.chart.panes(); if (ps[a.paneIndex]) { ps[a.paneIndex].setStretchFactor(0.3); a.stretched = true; } } catch (_) {}
  }
...
function reconcilePlots(host, a, plots, paneIndex) {
  ...
  plots.forEach((/** @type {any} */ pl) => {
    ...
    if (s && a.seriesType.get(pl.key) !== eff.type) {   // a type change can't apply in place — recreate
      try { host.chart.removePlot(s); } catch (_) {}
      a.plots.delete(pl.key); a.seriesType.delete(pl.key); s = null;
    }
    if (!s) {
      ...
      s = view
        ...
        : host.chart.addPlot(/** @type {Record<string, any>} */ (SERIES_CTOR)[eff.type] || Line, styleToOptions(eff, host._show(a)), paneIndex);
      a.plots.set(pl.key, s); a.seriesType.set(pl.key, eff.type);
    } else {
  ...
  [...a.plots.keys()].forEach((k) => {
    if (!seen.has(k)) { try { host.chart.removePlot(a.plots.get(k)); } catch (_) {} a.plots.delete(k); a.seriesType.delete(k); }
  });
}
...
function attachResultPrimitives(host, a) {
  if (a.shapes.length && !a.shapesPrim) {
    ...
    if (a.shapesSeries) { a.shapesPrim = createMarkPrimitive(host.chart, () => host._barTimes(), () => a.shapesSeries, () => shapesToMarks(host._show(a) ? (a.overlay ? a.shapes : a.shapes.filter((/** @type {any} */ s) => !s.overlay)) : [])); try { a.shapesSeries.addLayer(a.shapesPrim); } catch (_) {} }
  }
  ...
  if (!a.overlay && a.shapes.some((/** @type {any} */ s) => s.overlay) && !a.mainShapesPrim) {
    ...
    if (a.mainShapesSeries) { a.mainShapesPrim = createMarkPrimitive(host.chart, () => host._barTimes(), () => a.mainShapesSeries, () => shapesToMarks(host._show(a) ? a.shapes.filter((/** @type {any} */ s) => s.overlay) : [])); try { a.mainShapesSeries.addLayer(a.mainShapesPrim); } catch (_) {} }
  }
  ...
  if (a.fillBands.length && !a.fillPrim) {
    ...
    if (a.fillSeries) { a.fillPrim = createBandPrimitive(host.chart, () => host._barTimes(), () => a.fillSeries, () => (host._show(a) ? (a.fillBands || []) : [])); try { a.fillSeries.addLayer(a.fillPrim); } catch (_) {} }
  }
...
export function applyMarkers(host, a) {
  ...
  try { s.setMarkers(a.markers || []); } catch (_) {}
}
...
export function applyScale(host, a, paneIndex, scale) {
  ...
  if (a._scalePane != null && (a._scalePane !== paneIndex || !fn)) { try { host.chart.setPaneScale(a._scalePane, null); } catch (_) {} a._scalePane = null; }
  if (fn) { try { host.chart.setPaneScale(paneIndex, fn); a._scalePane = paneIndex; } catch (_) {} }
}

/** @param {any} host @param {Attachment} a */
export function detachStudy(host, a) {
  a._animStep = null; a._animRunning = false;   // stop any in-flight tween loop
  if (a.shapesPrim && a.shapesSeries) { try { a.shapesSeries.removeLayer(a.shapesPrim); } catch (_) {} }
  a.shapesPrim = null; a.shapesSeries = null;
  if (a.mainShapesPrim && a.mainShapesSeries) { try { a.mainShapesSeries.removeLayer(a.mainShapesPrim); } catch (_) {} }
  a.mainShapesPrim = null; a.mainShapesSeries = null;
  if (a.fillPrim && a.fillSeries) { try { a.fillSeries.removeLayer(a.fillPrim); } catch (_) {} }
  a.fillPrim = null; a.fillSeries = null;
  if (a._scalePane != null) { try { host.chart.setPaneScale(a._scalePane, null); } catch (_) {} a._scalePane = null; }
}
