...
export function attachControls(skin) {
  ...
  const build = (a) => {
    ...
    if (!host._noRemove)
      btns.push(
        mk('🗑', 'Remove', () => {
          const i = host.attached.indexOf(a);
          if (i >= 0) host.remove(i);
        }),
      );
    ctrls.append(...btns);
  ...
  const position = (a) => {
    ...
    if (!chromeVisible(host, a)) {
      rec.ctrlsEl.style.display = 'none';
      return;
    ...
    rec.ctrlsEl.style.display = '';
    rec.ctrlsEl.style.top = top + 4 + 'px';
    rec.ctrlsEl.style.right = scaleWidth(chart) + 6 + 'px';
  };
  ...
  host.on('panemode', (/** @type {any} */ a, /** @type {string} */ mode) => {
    ...
    if (rec.colBtn) {
      ...
      rec.colBtn.textContent = c ? '⌃' : '⌄';
      rec.colBtn.title = c ? 'Expand' : 'Collapse';
    }
