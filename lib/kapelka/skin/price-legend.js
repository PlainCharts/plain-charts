...
export function attachPriceLegend(skin) {
  ...
  const mainSeries = () => {
    try {
      return host._mainSeries ? host._mainSeries() : null;
    } catch (_) {
      return null;
    }
  };
  ...
  const render = (seriesData) => {
    ...
    const fmt = (v) => {
      if (v == null) return '—';
      try {
        return s && s.formatPrice ? s.formatPrice().format(v) : String(v);
      } catch (_) {
        return String(v);
      }
    };
    ...
    if (title) {
      ...
      t.className = 'skin-price-title';
      t.textContent = title;
      box.appendChild(t);
    }
    if (bar) {
      const vals = document.createElement('span');
      vals.className = 'skin-price-vals';
      /**
       * @param {string} k
       * @param {any} v
       */
      const cell = (k, v) => {
        ...
        sp.className = 'skin-price-v';
        sp.textContent = k + ' ' + fmt(v);
        return sp;
      };
  ...
  skin.setPriceLegend = (/** @type {any} */ patch) => {
    skin.priceLegend = { ...(skin.priceLegend || {}), ...(patch || {}) };
    render(null);
  };
