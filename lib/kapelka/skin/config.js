...
const row = (label) => {
  const r = mk('div', 'skin-row');
  const c = mk('span', 'skin-row-c');
  r.append(mk('span', 'skin-row-l', label), c);
  return { r, c };
};
...
export function attachConfig(skin) {
  ...
  const setParam = (a, k, v) => {
    const i = idx(a);
    if (i >= 0) host.setParam(i, k, v);
  };
  /** @param {any} a @param {string} k @param {any} patch */
  const setStyle = (a, k, patch) => {
    const i = idx(a);
    if (i >= 0) host.setStyle(i, k, patch);
  };
  ...
  function renderInputs(body, a) {
    ...
    if (!ins.length) {
      body.appendChild(mk('div', 'skin-note', 'No inputs.'));
      return;
    }
    ins.forEach((/** @type {any} */ inp) => {
      ...
      if (inp.type === 'bool') {
        ...
        ctrl.type = 'checkbox';
        ctrl.checked = !!val;
        ctrl.onchange = () => setParam(a, inp.key, ctrl.checked);
      } else if (inp.type === 'source' || inp.type === 'select') {
        ctrl = mk('select');
        (inp.type === 'source' ? SOURCES : inp.options || []).forEach((/** @type {any} */ s) => {
          const o = mk('option', null, s.name || s.key);
          o.value = s.key;
          if (s.key === val) o.selected = true;
          ctrl.appendChild(o);
        });
        ctrl.onchange = () => setParam(a, inp.key, ctrl.value);
      } else if (inp.type === 'color') {
        ctrl = mk('input');
        ctrl.type = 'color';
        ctrl.value = toHex(val);
        ctrl.oninput = () => setParam(a, inp.key, ctrl.value);
      } else if (inp.type === 'number') {
        ctrl = mk('input');
        ctrl.type = 'number';
        ctrl.value = val;
        if (inp.min != null) ctrl.min = inp.min;
        if (inp.max != null) ctrl.max = inp.max;
        if (inp.step != null) ctrl.step = inp.step;
        ctrl.oninput = () => {
          const v = parseFloat(ctrl.value);
          if (!Number.isNaN(v)) setParam(a, inp.key, v);
        };
      } else {
        ctrl = mk('input');
        ctrl.type = 'text';
  ...
  function renderStyle(body, a) {
    ...
    if (!metas.length) {
      body.appendChild(mk('div', 'skin-note', 'No plots.'));
      return;
    }
    metas.forEach((/** @type {any} */ pm) => {
      ...
      const vis = mk('input');
      vis.type = 'checkbox';
      vis.checked = eff.visible !== false;
      vis.onchange = () => setStyle(a, pm.key, { visible: vis.checked });
      c.appendChild(vis);
      if (eff.type !== 'segmented') {
        // segmented colors live per-segment in the data
        const color = mk('input');
        color.type = 'color';
        color.value = toHex(eff.color);
        color.oninput = () => setStyle(a, pm.key, { color: color.value });
        const width = mk('input', 'skin-in');
        width.type = 'number';
        width.min = 1;
        width.max = 8;
        width.value = eff.lineWidth || 2;
        width.style.width = '46px';
        width.oninput = () => {
          const v = parseFloat(width.value);
          if (!Number.isNaN(v)) setStyle(a, pm.key, { lineWidth: v });
        };
  ...
  function build(a) {
    ...
    tIn.onclick = () => {
      tIn.classList.add('skin-on');
      tSt.classList.remove('skin-on');
      renderInputs(body, a);
    };
    tSt.onclick = () => {
      tSt.classList.add('skin-on');
      tIn.classList.remove('skin-on');
      renderStyle(body, a);
    };
  ...
  skin.openSettings = (/** @type {any} */ a) => {
    close();
    win = build(a);
    document.body.appendChild(win);
  };
  // re-render the open window if its study recomputed (e.g. params changed live)
  host.on('computed', (/** @type {any} */ a) => {
    /* values update via the legends; window stays as-is */
  });
}
