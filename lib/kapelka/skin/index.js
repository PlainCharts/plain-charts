...
function rgbaFrom(color, op) {
  ...
  if (m) { const c = m[1]; const x = (/** @type {number} */ i) => parseInt(c[i] + c[i], 16); return `rgba(${x(0)},${x(1)},${x(2)},${op})`; }
  return color;
}
...
export function createSkin(host, opts = {}) {
  ...
  const priceLegend = (opts.priceLegend && typeof opts.priceLegend === 'object') ? { ...opts.priceLegend }
    : (opts.priceLegend ? {} : null);

  const skin = {
    host, chart, container, priceLegend,
    ...
    reposition() { this._positioners.forEach((/** @type {() => void} */ fn) => { try { fn(); } catch (_) {} }); },
    // re-render + reposition every piece — call after a host-side visibility change (hide/global-hide)
    refresh() { this._refreshers.forEach((/** @type {() => void} */ fn) => { try { fn(); } catch (_) {} }); this.reposition(); },
    ...
    setLegendOptions(patch) {
      ...
      if (this.container) this.container.style.setProperty('--skin-legend-bg', o.bg ? (rgbaFrom(o.bgColor, o.bgOpacity != null ? o.bgOpacity : 0.5) || 'transparent') : 'transparent');
      this.refresh();
    },
    destroy() {
      this._offs.forEach((/** @type {() => void} */ fn) => { try { fn(); } catch (_) {} }); this._offs = [];
      this.per.forEach((/** @type {Record<string, any>} */ rec) => Object.values(rec).forEach((/** @type {any} */ el) => { if (el && el.remove) try { el.remove(); } catch (_) {} }));
      this.per.clear();
    },
  };
  ...
  if (container) {
    let raf = 0;
    const sched = () => { if (raf) return; raf = requestAnimationFrame(() => { raf = 0; skin.reposition(); }); };
    if (typeof ResizeObserver !== 'undefined') { const ro = new ResizeObserver(sched); ro.observe(container); skin._offs.push(() => ro.disconnect()); }
  ...
  host.on('added', (/** @type {any} */ a) => { if (!skin.per.has(a)) skin.per.set(a, {}); });
  host.attached.forEach((/** @type {any} */ a) => { if (!skin.per.has(a)) skin.per.set(a, {}); });
  ...
  host.on('removed', (/** @type {any} */ a) => { skin.per.delete(a); });
