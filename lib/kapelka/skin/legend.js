...
export function studyLabel(a, opts = {}) {
  ...
  const nums = (a.study.inputs || [])
    .filter((/** @type {any} */ i) => i.type === 'number' && !i.hidden && i.legend === true)
    .map((/** @type {any} */ i) => a.params[i.key])
...
export function attachStudyLegend(skin) {
  ...
  const build = (a) => {
    ...
    const rec = skin.per.get(a);
    if (!rec || rec.legendEl) return;
    const wrap = document.createElement('div');
    wrap.className = 'skin-legend';
    const name = document.createElement('span');
    name.className = 'skin-legend-name';
    name.title = 'Settings';
    name.onclick = () => {
      if (skin.openSettings) skin.openSettings(a);
    };
    const vals = document.createElement('span');
    vals.className = 'skin-legend-vals';
  ...
  const render = (a, seriesData) => {
    ...
    (a.plotMeta || []).forEach((/** @type {any} */ pm) => {
      ...
      span.className = 'skin-legend-v';
      span.style.color = eff.color;
      span.textContent = fmtVal(v, pm.precision);
  ...
  const position = (a) => {
    const rec = skin.per.get(a);
    if (!rec || !rec.legendEl) return;
    if (!chromeVisible(host, a)) {
      rec.legendEl.style.display = 'none';
      return;
    } // per-study / global hide
    const { idx, top } = paneGeom(chart, a.plots.values().next().value);
    if (idx < 0) {
      rec.legendEl.style.display = 'none';
      return;
    }
    rec.legendEl.style.display = '';
    rec.legendEl.style.top = top + 4 + 'px';
    rec.legendEl.style.left = '8px';
  };
  const positionAll = () => host.attached.forEach(position);
  skin._positioners.push(positionAll);
  skin._refreshers.push(() =>
    host.attached.forEach((/** @type {any} */ a) => {
      if (!a.overlay) render(a, null);
    }),
  );
