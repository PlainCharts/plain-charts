...
export function attachOverlayLegend(skin) {
  ...
  const box = document.createElement('div'); box.className = 'skin-ovl';
  box.style.top = (skin.overlayTop != null ? skin.overlayTop : 8) + 'px'; box.style.left = '8px';
  ...
  const icon = (txt, title, fn, always) => {
    const s = document.createElement('span'); s.className = 'skin-ovl-ico' + (always ? ' skin-always' : '');
    s.textContent = txt; s.title = title; s.onclick = (e) => { e.stopPropagation(); fn(); }; return s;
  };
  ...
  const structureSig = (overlays) => {
    ...
    return overlays.map((/** @type {any} */ a) => {
      const plots = (a.plotMeta || []).map((/** @type {any} */ pm) => {
        const eff = effectiveStyle(a.style, pm);
        return pm.key + ':' + (pm.legend === false ? 'x' : '') + (eff.visible === false ? 'x' : '') + eff.color + ':' + pm.precision;
      }).join(',');
      return studyLabel(a, { inputs: o.inputs !== false }) + '|' + (a.hidden ? 1 : 0) + '|' + (a.error || '') + '|' + plots;
    }).join(';') + '#' + (o.title === false ? 't' : '') + (o.values === false ? 'v' : '');
  };
  ...
  const render = (seriesData) => {
    ...
    if (sig === lastSig) { updateValues(seriesData); return; }
    lastSig = sig;
    slots = [];
    box.innerHTML = '';
    if (!overlays.length) { box.style.display = 'none'; skin.overlayOpen = false; return; }
    box.style.display = '';

    // collapsed -> a count chip
    if (collapsible && !skin.overlayOpen) {
      const chip = document.createElement('div'); chip.className = 'skin-ovl-chip';
      const tw = document.createElement('span'); tw.className = 'skin-ovl-tw'; tw.textContent = '▾';
      const ct = document.createElement('span'); ct.className = 'skin-ovl-count'; ct.textContent = String(overlays.length);
      chip.append(tw, ct);
      chip.title = overlays.length + ' indicator' + (overlays.length > 1 ? 's' : '') + ' on this chart';
      chip.onclick = (e) => { e.stopPropagation(); skin.overlayOpen = true; render(null); };
    ...
    overlays.forEach((/** @type {any} */ a) => {
      const row = document.createElement('div'); row.className = 'skin-ovl-row' + (a.hidden ? ' skin-dim' : '');
      if (o.title !== false) {
        const name = document.createElement('span'); name.className = 'skin-ovl-name'; name.textContent = studyLabel(a, { inputs: o.inputs !== false });
        name.onclick = () => { if (skin.openSettings) skin.openSettings(a); };
        row.appendChild(name);
      }
      if (a.error) { const warn = icon('⚠', a.error, () => {}, true); warn.style.color = '#e0a030'; row.appendChild(warn); }
      if (o.values !== false) (a.plotMeta || []).forEach((/** @type {any} */ pm) => {
        if (pm.legend === false) return;
        const eff = effectiveStyle(a.style, pm); if (eff.visible === false) return;
        const series = a.plots.get(pm.key);
        let v = (seriesData && series) ? (seriesData.get(series) || {}).value : null;
        if (v == null) v = pm.last;
        const sp = document.createElement('span'); sp.className = 'skin-ovl-v'; sp.style.color = eff.color; sp.textContent = fmtVal(v, pm.precision);
        slots.push({ span: sp, a, key: pm.key });
        row.appendChild(sp);
      });
      // controls (hover-revealed; the crossed-eye stays visible while hidden)
      const eye = icon(a.hidden ? '◌' : '◉', a.hidden ? 'Show' : 'Hide', () => { host.toggleHidden(host.attached.indexOf(a)); render(null); }, a.hidden);
      const gear = icon('⚙', 'Settings', () => { if (skin.openSettings) skin.openSettings(a); });
    ...
    if (collapsible) {
      const col = document.createElement('div'); col.className = 'skin-ovl-collapse'; col.textContent = '▴'; col.title = 'Collapse';
      col.onclick = (e) => { e.stopPropagation(); skin.overlayOpen = false; render(null); };
      box.appendChild(col);
    }
  };
