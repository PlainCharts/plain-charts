...
import { applyOvStyle, refreshOverlay } from './core/components/renderers/candles.js'; // main candle overlay
import { buildLayout } from './core/build_layout.js';
import { yTransformsByPos, layersMetaByPos, offchartDescriptors } from './core/layout-inputs.js'; // chart state -> buildLayout inputs
...
import { hitTest as hitTestPrimitives } from './core/components/primitives-host.js'; // series-attached primitives
...
import {
  ...
  resetPanes,
  panes as panesApi,
  removePane as removePaneApi,
  movePane,
  addPane as addPaneApi,
} from './core/panes.js';
...
function deepMerge(a, b) {
  const o = isObj(a) ? { ...a } : {};
  for (const k in b) o[k] = isObj(a[k]) && isObj(b[k]) ? deepMerge(a[k], b[k]) : b[k];
  return o;
}
...
class Chart {
  ...
  constructor(el, options = {}) {
    ...
    this._gridShaders = /** @type {any[]} */ ([]);
    this._sbShaders = /** @type {any[]} */ ([]);
    this._noShaders = /** @type {any[]} */ ([]); // targeted shaders (price-tag -> main sidebar)
    ...
    this._showTime = true; // price-scale / time-scale visibility
    this._showLastValue = true;
    this._sbPx = 0;
    this._chartLeftPx = 0; // sidebar width + chart left-offset (left-scale) in CSS px
  ...
  _mkcv() {
    ...
    c.style.cssText = 'position:absolute;display:block;';
  ...
  _buildComp() {
    ...
    const $props = {
      ...
      shaders: [],
      timezone: (this._options.timeAxis && this._options.timeAxis.timezone) || 0,
      dayRoll: (this._options.timeAxis && this._options.timeAxis.dayRoll) || 0,
      meta: {},
    };
    this._comp = {
      config: Const.ChartConfig,
  ...
  _schedule() {
    ...
    if (ms > 0) {
      ...
      if (wait > 0) {
        setTimeout(() => requestAnimationFrame(() => this._flushPaint()), wait);
        return;
      }
    }
  ...
  _scheduleObjects(prim) {
    if (prim && this._primHasActiveBelow(prim)) {
      this._schedule();
      return;
    }
  ...
  _primHasActiveBelow(prim) {
    try {
      ...
      return true;
    } // a misbehaving view -> stay safe, paint fully
    return false;
  }
  _flushPaint() {
    if (this._paused) return;
    const full = this._dirty,
      obj = this._objDirty,
      cur = this._cursorDirty;
    this._dirty = false;
    this._objDirty = false;
    this._cursorDirty = false;
    if (!full && !obj && !cur) return; // flags consumed by an earlier flush (double-rAF)
    this._lastPaint = nowMs();
    if (full) this._redraw();
    else if (obj) this._redrawObjects();
    else this._redrawCursor();
  }
  ...
  setPaused(on) {
    ...
    this._paused = p;
    if (!p && (this._dirty || this._objDirty || this._cursorDirty)) {
      this._dirty = false;
      this._objDirty = false;
      this._cursorDirty = false;
      this._schedule();
    } // replay the deferred paint (full covers all tiers)
    return this;
  }
  _cs() {
    return this._series.find((s) => s._isCandle() && s._rows.length);
  }
  ...
  _ds() {
    return this._cs() || this._series.find((s) => s._rows.length);
  }
  /** @param {import('./core/series.js').Series} s */
  _restyle(s) {
    if (s === this._cs()) {
      this._style = s._style;
      if (this._ov) applyOvStyle(this);
    }
  }
  ...
  _iv() {
    const cs = this._ds();
    return (cs && Utils.detect_interval(cs._rows)) || this._interval || 60000;
  }
  ...
  _fitToData() {
    return fitToData(this);
  }
  /** @param {import('./core/series.js').Series} s  @param {number|null} oldLastTime */
  _autoScrollAppend(s, oldLastTime) {
    return autoScrollAppend(this, s, oldLastTime);
  }
  ...
  _timeToLogical(t) {
    return timeToLogical(this, t);
  }
  /** @param {number} i @returns {number} */
  _logicalToTime(i) {
    return logicalToTime(this, i);
  }

  /** @param {number} k pane ID */
  _yOf(k) {
    return this._y[k] || (this._y[k] = { auto: true, range: null, zoom: 1 });
  }
  /** @param {number} k pane ID */
  _resetPaneAuto(k) {
    const y = this._yOf(k);
    ...
    y.zoom = 1;
    this._invalidate();
  } // price scale -> auto-fit (autoScale)
  /** @param {number} y CSS px @returns {number} grid POSITION under y, or -1 */
  _paneAt(y) {
    const L = this._comp.$props.layout;
    if (!L) return -1;
    const gs = L.grids;
    for (let k = 0; k < gs.length; k++) {
      const g = gs[k];
      if (y >= g.offset && y < g.offset + g.height) return k;
    }
    return -1;
  }
  ...
  _separatorAt(y) {
    const L = this._comp.$props.layout;
    if (!L || !L.grids || L.grids.length < 2) return -1;
    const grids = L.grids;
  ...
  _resizePanes(clientY) {
    const d = this._sepDrag,
      L = this._comp.$props.layout;
    if (!d || !L || !L.grids) return;
    const off = clientY - d.y0,
      nh1 = d.h1 + off,
      nh2 = d.h2 - off,
      MIN = 30;
    if (nh1 < MIN || nh2 < MIN) return;
    const px = L.grids.map((/** @type {any} */ g) => g.height);
    px[d.k - 1] = nh1;
    px[d.k] = nh2;
  ...
  _ensureManual(pos) {
    const y = this._yOf(this._idAt(pos));
    if (y.auto) {
      const g = this._gridAt(pos);
      y.range = g ? [g.$_hi, g.$_lo] : null;
      y.auto = false;
    }
  }
  ...
  _rebuild() {
    ...
    const yTransforms = yTransformsByPos(this); // manual Y windows, pane ID -> grid POSITION
    const layersMeta = layersMetaByPos(this); // per-pane auto-range shaping (votes + scale provider)
    ...
    const { layout, sub, interval } = buildLayout({
      rows: cs._rows,
      range: /** @type {[number, number]} */ (this._range),
      width: this._w,
      height: this._h,
      colors,
      font: fontFor(this._options),
      timezone: this._comp.$props.timezone,
      dayRoll: this._comp.$props.dayRoll,
      yTransforms,
      layersMeta,
      offcharts: offchartDescriptors(this),
      logScale: this._modeOf(0) === 1,
      scaleMode: this._modeOf(0),
      scaleMargins: /** @type {any} */ (this._mainMargins),
      ib: this._ib,
      mainGridHeight: this._stretch[0],
      minMove: mainPf.minMove,
      tickPrec: mainPf.precision,
      future: /** @type {any} */ (this._future), // future whitespace rows -> gapless session-following axis past the last bar (ib)
      hidePrice: !this._showPrice,
      hideTime: !this._showTime, // reclaim freed space when a scale is hidden
    ...
    this._chartW = g.width;
    this._chartH = g.height;
    ...
    p.layout = layout;
    p.sub = sub;
    p.range = /** @type {[number, number]} */ (this._range);
    p.interval = interval;
    p.colors = colors;
    p.width = this._w;
    p.height = this._h;
    p.font = fontFor(this._options);
    ...
    this._comp._layout = layout;
    this._comp.interval = interval;
    this._comp.main_section.sub = sub;
    ...
    for (const pane of this._panes) {
      ...
      buildPaneOverlays(this, pane); // compose the pane's z-ordered paint stack (data/objects sheets)
    }
  ...
  _seriesInPane(k) {
    const main = this._cs();
    return this._series.filter((s) => s._pane === k && s._rows.length && s !== main && s._opts.visible !== false);
  }
  ...
  _drawSeries(ctx, k, s) {
    const base = this._gridAt(k);
    if (!base || !s._rows.length) return;
    ...
    switch (s._type.type) {
      case 'Area':
        return SeriesRenderers.drawArea(ctx, g, s);
      case 'Baseline':
        return SeriesRenderers.drawBaseline(ctx, g, s);
      case 'Histogram':
        return SeriesRenderers.drawHistogram(ctx, g, s);
      case 'Segmented':
        return SeriesRenderers.drawSegmented(ctx, g, s);
      case 'HBar':
        return SeriesRenderers.drawHBar(ctx, base, s); // horizontal: price->y via the pane scale
      default:
        return SeriesRenderers.drawLine(ctx, g, s); // Line
    }
  }
  ...
  _drawCustom(ctx, g, s) {
    const view = s._view;
    if (!view || typeof view.draw !== 'function') return;
    ...
    try {
      ...
      if (!(/** @type {any} */ (s)._drawWarned)) {
        /** @type {any} */ (s)._drawWarned = true;
        try {
          console.error('[kapelka] custom series draw() threw:', e);
        } catch (_) {}
      }
    }
  }
  ...
  hitTest(x, y) {
    return hitTestPrimitives(this, x, y);
  }
  ...
  _syncCursor(grids) {
    ...
    if (m && gx >= 0 && gx < this._chartW) {
      for (let k = 0; k < grids.length; k++) {
        const gk = grids[k];
        if (m.y >= gk.offset && m.y < gk.offset + gk.height) {
          active = k;
          break;
        }
      }
    ...
    if (active >= 0) {
      ...
      const g0 = grids[0],
        cn0 = g0 && g0.candles;
      if (cn0 && cn0.length && gx > cn0[cn0.length - 1].x) {
        c.x = gx;
        c.t = g0.screen2t(gx);
      }
      ...
      if (this._cursorSnapX != null && g0) {
        const sx = typeof this._cursorSnapX === 'function' ? this._cursorSnapX() : this._cursorSnapX;
      ...
      this._root.style.cursor =
        this._sepHover > 0 ? 'row-resize' : hit && hit.cursorStyle ? hit.cursorStyle : 'crosshair';
      // sourceEvent only on the redraw right after a real pointer move (distinguishes user vs programmatic)
      const srcEv = this._pointerMoved ? this._lastPointerEvent : undefined;
      this._pointerMoved = false;
      emitCross(
        this,
        /** @type {any} */ ({
          time: c.t != null ? this._i2t(c.t) / 1000 : null,
          point: { x: c.x, y: c.y },
          hoveredObjectId: hit ? hit.externalId : undefined,
          sourceEvent: srcEv,
        }),
      );
    } else if (this._forcedCross && this._forcedCross.time != null) {
      // programmatic cross (setCursor, cross-pane sync)
      const fc = this._forcedCross,
        pos = fc.series ? this._posOf(fc.series._pane) : 0,
        gk = grids[pos];
      const c = this._comp.cursor;
      ...
      c.grid_id = pos;
      c.t = this._ib ? this._timeToLogical(fc.time * 1000) : fc.time * 1000;
      c.x = gk ? gk.t2screen(c.t) : undefined;
      c.y$ = fc.price;
      c.y = gk && fc.price != null ? gk.$2screen(fc.price) : undefined;
      for (const pane of this._panes) pane.cross.visible = true;
      this._crossActive = true;
  ...
  _paintAxes() {
    ...
    for (const pane of this._panes) {
      if (this._showPrice) {
        // skip the price axis entirely when hidden (its space is already reclaimed)
        ...
        p.shaders = this._noShaders;
        pane.sb.update();
        drawAxisViews(this, pane);
      }
    }
    cur.y$ = sY$;
    if (!showTimeLbl) cur.t = undefined;
    if (this._showTime) {
      p.shaders = this._gridShaders;
      this._bb.update();
      drawTimeAxisViews(this);
    }
    cur.t = sT;
  }
  ...
  _idAt(pos) {
    const v = this._paneIds[pos];
    return v != null ? v : pos;
  }
  /** @param {number} id pane ID @returns {number} grid position, or -1 if hidden */
  _posOf(id) {
    return this._paneIds.indexOf(id);
  } // grid position, or -1 if the pane is hidden (no grid)
  /** @param {number} id pane ID @returns {any} grid for a pane ID (null when hidden) */
  _gridOf(id) {
    return this._gridAt(this._posOf(id));
  } // grid for a pane ID (null when hidden)
  _sbWidth() {
    return this._showPrice && this._g ? /** @type {any} */ (this._g).sb : 0;
  }
  ...
  _inSidebarZone(x) {
    return this._showPrice && (this._scaleSide === 'left' ? x < this._sbPx : x >= this._chartLeftPx + this._chartW);
  }
  /** @param {number} id pane ID @param {number} price @returns {number|null} */
  _priceToCoord(id, price) {
    const g = this._gridOf(id);
    return g ? Math.floor(price * g.A + g.B) : null;
  } // id = pane ID
  /** @param {number} id pane ID @param {number} y @returns {number|null} */
  _coordToPrice(id, y) {
    const g = this._gridOf(id);
    return g ? (y - g.B) / g.A : null;
  }
  ...
  _maxVZoom() {
    return maxVZoom(this);
  }
  /** @param {number} id pane ID @returns {number[]|null} [hi, lo] */
  _visibleHiLo(id) {
    return visibleHiLo(this, id);
  }
  /** @param {number[]|null} range [hi, lo] @param {number} id pane ID @returns {number[]|null} */
  _clampVZoom(range, id) {
    return clampVZoom(this, range, id);
  }
  // candle body width as a fraction of the bar step (CANDLEW); user-settable, default 0.7.
  _candleWidth() {
    const v = this._options.candleWidth;
    return v > 0 ? v : Const.ChartConfig.CANDLEW;
  }
  _labelGap() {
    const t = this._options.timeAxis;
    const v = t && t.labelGap;
    return v > 0 ? v : Const.ChartConfig.MIN_LABEL_PX;
  }
  /** @param {[number, number]|null} r [t0, t1] @param {number} [anchor] @returns {[number, number]|null} */
  _clampZoom(r, anchor) {
    return clampZoom(this, r, anchor);
  }
  _visibleCount() {
    return visibleCount(this);
  }
  _emitRange() {
    return emitRange(this);
  }

  // ---- ENGINE_API.md surface ----
  /** @param {import('./core/types.js').SeriesType} type  @param {any} [opts]  @param {number} [paneIndex] */
  addPlot(type, opts = {}, paneIndex = 0) {
    const s = new Series(this, type, opts, paneIndex);
    this._series.push(s);
    this._invalidate();
    return s;
  }
  ...
  /** @param {import('./core/series.js').Series} s */
  removePlot(s) {
    const i = this._series.indexOf(s);
    if (i >= 0) this._series.splice(i, 1);
    if (s && s._view && typeof s._view.destroy === 'function') {
      try {
        s._view.destroy();
      } catch (_) {}
    }
  ...
  setPaneScale(paneId, fn) {
    if (fn) this._scaleProviders[paneId] = fn;
    else delete this._scaleProviders[paneId];
    this._invalidate();
  }
  ...
  configure(o = {}) {
    ...
    // session day-roll (ms) for tick-mark classification on daily+ timeframes: bars stamped at the
    // session OPEN (the evening before their trade date) are rolled forward before the day/month/year
    // boundary tests, so the "new month" tick lands on the bar that TRADES the 1st.
    if (o.timeAxis && o.timeAxis.dayRoll != null && this._comp) this._comp.$props.dayRoll = o.timeAxis.dayRoll;
    ...
    if (o.timeAxis && o.timeAxis.rightOffset != null && this._range) {
      ...
      if (d) {
        const shift = this._ib ? d : d * this._iv();
        this._range = [this._range[0] + shift, this._range[1] + shift];
        this._emitRange();
      }
    }
  ...
  getConfig() {
    return { ...this._options, localization: this._options.localization || {} };
  } // chart options snapshot
  ...
  _readScaleOpts(o) {
    const rp = o.rightPriceAxis,
      lp = o.leftPriceAxis,
      t = o.timeAxis;
    if (rp && rp.mode != null)
      this._setPaneMode(0, rp.mode); // also forces auto-fit on switch to %/indexed
    else if (lp && lp.mode != null) this._setPaneMode(0, lp.mode);
    if (rp && rp.margins)
      this._mainMargins = rp.margins; // main-scale margins (top/bottom empty fraction)
    else if (lp && lp.margins) this._mainMargins = lp.margins;
    if ((rp && rp.invert != null) || (lp && lp.invert != null))
      this._invert = !!((rp && rp.invert) || (lp && lp.invert));
    if (lp || rp) {
      const leftVis = lp && lp.visible === true,
        rightVis = rp && rp.visible === true;
      if (leftVis && !rightVis) {
        this._scaleSide = 'left';
        this._showPrice = true;
      } else if (rightVis && !leftVis) {
        this._scaleSide = 'right';
        this._showPrice = true;
  ...
  panes() {
    return panesApi(this);
  }
  /** @param {number} index pane ID */
  removePane(index) {
    return removePaneApi(this, index);
  }
  /** @param {number} from  @param {number} to */
  _movePane(from, to) {
    return movePane(this, from, to);
  }
  ...
  _modeOf(k) {
    return this._scaleMode[k] || 0;
  }
  /** @param {number} k pane ID */
  _paneLogOf(k) {
    return this._modeOf(k) === 1;
  } // log branch (y-drag zoom) — only mode 1
  ...
  /** @param {number} pos grid POSITION */
  _scaleLockedAt(pos) {
    const m = this._modeOf(this._idAt(pos));
    return m === 2 || m === 3;
  }
  ...
  addPane() {
    return addPaneApi(this);
  }
  /** @param {number} p price @returns {number|null} */
  priceToY(p) {
    return this._priceToCoord(0, p);
  }
  /** @param {number} y @returns {number|null} price */
  yToPrice(y) {
    return this._coordToPrice(0, y);
  }
  /** @param {Function} cb */
  onCursorMove(cb) {
    this._cbs.crosshair.add(cb);
  }
  /** @param {Function} cb */
  offCursorMove(cb) {
    this._cbs.crosshair.delete(cb);
  }
  /** @param {Function} cb */
  onClick(cb) {
    this._cbs.click.add(cb);
  }
  /** @param {Function} cb */
  offClick(cb) {
    this._cbs.click.delete(cb);
  }
  ...
  setFutureWhitespace(times) {
    this._future =
      this._ib && Array.isArray(times) && times.length
  ...
  setCursor(price, time, series) {
    this._forcedCross = { price, time, series };
    this._scheduleCursor();
  }
  clearCursor() {
    this._forcedCross = null;
    this._scheduleCursor();
  }
  ...
  priceAxis(id) {
    ...
    return {
      ...
      configure: (/** @type {any} */ o = {}) => {
        if (o.mode != null) c._setPaneMode(0, o.mode);
      },
      getConfig: () => ({ mode: c._modeOf(0) }),
    };
  }
  destroy() {
    try {
    ...
    try {
      this.el.removeChild(this._root);
    } catch (_) {}
    for (const k in this._cbs) {
      try {
        /** @type {Record<string, Set<Function>>} */ (this._cbs)[k].clear();
      } catch (_) {}
    } // drop all subscriptions (no lingering callbacks)
  }
}
...
export function mountChart(el, options) {
