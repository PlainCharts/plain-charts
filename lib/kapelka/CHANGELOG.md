...
- The DataCube's script engine and standard indicator library. Its interpreter compiled each study from an expression string, and a library of stock indicators shipped alongside it.
...
- Shift + wheel scrolls the time window left and right without zooming. Wheel down moves forward about 5% of the visible window per notch, matching touch and drag pan. Gated by `handleScroll`.
...
- Studies compute off the render thread in a Web Worker, on a vanilla-JS reimplementation of the DataCube data-flow model with no expression interpreter. The worker owns the bar data. It is primed once, then fed small mutations (replace the forming bar, append a closed bar) rather than a full snapshot per study, and it builds one shared column window (time, OHLC, volume, open interest) per data change that every study reads. On a live tick a series study restores its checkpoint at the last closed bar, steps only the forming bar, and emits just the last point, so no panes, scales, or shapes are rebuilt. Eligible studies run in the worker only, and a worker failure surfaces as a study error.
- Two study forms coexist by design. A series study implements `step` and streams one bar per tick. A geometry study implements `calc` and recomputes the whole array for multi-bar shapes such as boxes, session strips, and terrain. The step form adds channels computed once: `shapes`, `fills` (a shaded band over two plot keys), `scale` (lock the pane range), and `init` for a window-wide precompute.
...
- Gapless future axis. `chart.setFutureWhitespace(times)` feeds future whitespace rows into the indexed stream, so the axis resolves future times by lookup instead of extrapolating clock time. The future now collapses weekend and session gaps exactly like the past. Whitespace rows paint no candle and stay out of the price auto-scale.
...
- `setPaused(bool)` gates painting. While paused the data model still updates, but no paint runs, and resuming replays one catch-up paint if anything changed. Lets a host stop an off-screen chart from burning render CPU.
- `setCursorSnapX(fn|number|null)` pins the crosshair's vertical line to a screen x while the horizontal line still tracks the mouse. It snaps the crosshair onto a grabbed time-anchored object instead of letting it wander in the whitespace. Pass a live getter, a fixed number, or null to release.
- Full unsubscribe surface, and `destroy()` clears all subscriptions.
- Primitive autoscale votes. An attached primitive can implement `autoscaleInfo()`, and a price line can set `autoscale: true`, so both extend the pane's auto-range like data. It runs before a study's `scale` locks the range, and never against a manually dragged scale. Opt-in, off by default.
- Hit-test arbitration. A primitive hit can carry `hitTestPriority` (point over line over range) and `distance` from the cursor, so overlapping objects resolve deterministically by priority, then distance, then zOrder. Hits without the fields behave as before.
...
- Anchored objects paint on their own sheet. Each pane now stacks three canvases: data (gridlines, candles, series, markers), objects (price lines and primitives), and crosshair. Dragging a price line or moving the cursor repaints only its own sheet plus the axes, so the candles keep their last paint. Full paints still refresh all three sheets, and `snapshot()` no longer includes the crosshair.
...
- Intrabar (lower-timeframe) data follows a push model. History is fetched once over a bounded window (the last N main bars, default 750), and live sub-bars arrive through a streaming subscription (`opts.subscribeIntrabar`) instead of a timer re-download. An idle chart makes zero intrabar requests.
