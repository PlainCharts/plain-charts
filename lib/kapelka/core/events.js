...
function seriesDataMap(c, tms) {
  ...
  for (const s of c._series) {
    ...
    if (s._view && r[3] != null) { try { const pv = s._view.priceValues(r[3]); val = pv[pv.length - 1]; } catch (_) {} }   // custom: last priceValue
    map.set(s, s._isCandle() ? { time: r[0] / 1000, open: r[1], high: r[2], low: r[3], close: r[4], value: r[4] } : { time: r[0] / 1000, value: val });
  }
  return map;
}
...
export function emitCross(c, p, leaveEvent) {
  ...
  c._cbs.crosshair.forEach((/** @type {(a: any) => void} */ cb) => { try { cb(arg); } catch (_) {} });
}
...
export function emitClick(c, px, py) {
  ...
  c._cbs.click.forEach((/** @type {(a: any) => void} */ cb) => { try { cb(arg); } catch (_) {} });
}
