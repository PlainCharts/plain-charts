...
export function polyline(ctx, g, rows, lineType) {
  ...
  for (let i = 0; i < n; i++) { xs[i] = g.t2screen(rows[i][0]); ys[i] = g.$2screen(rows[i][1]); }
  ctx.moveTo(xs[0], ys[0]);
  if (lineType === 1) {                     // stepped: horizontal hold to the next x, then vertical jump
    for (let i = 1; i < n; i++) { ctx.lineTo(xs[i], ys[i - 1]); ctx.lineTo(xs[i], ys[i]); }
  } else if (lineType === 2 && n > 2) {     // curved: Catmull-Rom -> cubic bezier through each point pair
    for (let i = 0; i < n - 1; i++) {
      const a = i > 0 ? i - 1 : 0, d = i + 2 < n ? i + 2 : n - 1;
      ctx.bezierCurveTo(xs[i] + (xs[i + 1] - xs[a]) / 6, ys[i] + (ys[i + 1] - ys[a]) / 6,
                        xs[i + 1] - (xs[d] - xs[i]) / 6, ys[i + 1] - (ys[d] - ys[i]) / 6,
                        xs[i + 1], ys[i + 1]);
    }
  } else {                                  // simple
...
export function drawLine(ctx, g, s) {
  const o = s._opts;
  ctx.beginPath(); polyline(ctx, g, s._rows, o.lineType);
...
export function drawArea(ctx, g, s) {
  ...
  ctx.beginPath(); polyline(ctx, g, rows, o.lineType);
...
export function drawBaseline(ctx, g, s) {
  ...
  if (ctx.createLinearGradient) {
    ctx.beginPath(); polyline(ctx, g, rows); ctx.lineTo(x1, yBase); ctx.lineTo(x0, yBase); ctx.closePath();
...
export function drawHBar(ctx, g, s) {
  const o = s._opts, rows = s._rows; if (!rows.length) return;
  const W = g.width, side = o.side || 'right';
  ...
  if (!thick && rows.length > 1) thick = Math.abs(g.$2screen(rows[1][0]) - g.$2screen(rows[0][0])) * (o.fill != null ? o.fill : 0.82);
  thick = Math.max(1, Math.round(thick || 3));
  const dft = o.color || 'rgba(120,120,120,0.5)';
  let maxV = 0; for (const r of rows) if (r[1] > maxV) maxV = r[1];   // r[1] = total per row
  if (maxV <= 0) return;
  const maxLen = frac * W, anchor = side === 'left' ? 0 : W, dir = side === 'left' ? 1 : -1;
  const px = (/** @type {number} */ v) => Math.max(0, (v / maxV) * maxLen);
  for (const r of rows) {
    const y = Math.round(g.$2screen(r[0]) - thick / 2);   // price -> y (pane price scale)
    const b = r[2];
    const segs = (b && b.segments) ? b.segments : [{ value: (b && b.value) != null ? b.value : r[1], color: b && b.color }];
...
export function drawSegmented(ctx, g, s) {
  ...
  const span = (/** @type {number} */ cx, /** @type {number|undefined} */ f) => { const w = Math.max(1, Math.floor(bw * (f != null ? f : 1))); return [cx - Math.floor(w / 2), w]; };
  for (const r of rows) {
    ...
    for (const seg of (p.segments || [])) {
      ...
      if (seg.fill === false) {
        ctx.strokeStyle = seg.color || '#888'; ctx.lineWidth = seg.lineWidth || 1;
        ctx.strokeRect(left + 0.5, top + 0.5, Math.max(1, w - 1), Math.max(1, h - 1));
      } else {
        ctx.fillStyle = seg.color || '#888'; ctx.fillRect(left, top, w, h);
      }
    }
...
export function drawCandleSeries(ctx, g, s, candleW) {
  const rows = s._rows, w = (g.px_step || 6) * candleW;
  for (const r of rows) {
