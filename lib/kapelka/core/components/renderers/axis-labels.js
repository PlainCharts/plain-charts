...
export function drawAxisViews(c, pane) {
  ...
  for (const s of c._series) {
    ...
    if (s._isCandle() && s._rows.length && s._opts.showLastValue !== false) {
      const last = s._rows[s._rows.length - 1],
        price = last[4],
        st = s._style;
      add(
        g.$2screen(price),
        fmt(g, price),
        '#fff',
        s._opts.priceLineColor || (price >= last[1] ? st.colorCandleUp : st.colorCandleDw),
      );
    }
    for (const pl of s._priceLines) {
      const o = pl._opts;
      if (o.showAxisLabel === false || o.price == null) continue;
      // the title becomes a side chip; the box shows only the price (full width, readable)
      add(
        g.$2screen(o.price),
        fmt(g, o.price),
        o.axisLabelTextColor || '#fff',
        o.axisLabelColor || o.color || '#363a45',
        o.title || null,
      );
    }
    for (const prim of s._primitives) {
      const views = /** @type {any[]} */ (
        [].concat(
          prim.priceAxisViews ? prim.priceAxisViews() : [],
          prim.priceAxisPaneViews ? prim.priceAxisPaneViews() : [],
        )
      );
      for (const v of views) {
        if (!v || (v.visible && !v.visible())) continue;
        add(
          v.coordinate ? v.coordinate() : null,
          v.text ? v.text() : '',
          v.textColor ? v.textColor() : '#fff',
          v.backColor ? v.backColor() : '#363a45',
        );
      }
    }
  }
  ...
  const ctx = pane.sbCv.getContext('2d');
  ctx.font = c._comp.$props.font;
  // chip goes on objCv (the objects sheet), NOT gridCv (the data sheet): a quote tick repaints via the
  // objects tier, which clears objCv every frame right before this runs, so the chip clears+redraws in
  // lockstep with the price line. gridCv is cleared only on a full redraw, so a chip drawn there would
  // stack a fresh copy each tick and never erase the old y -- the "triple tag" ghost.
  const gctx = pane.objCv.getContext('2d');
  gctx.font = c._comp.$props.font;
  const sb = g.sb,
    left = c._scaleSide === 'left',
    r = 3;
  for (const t of tags) {
    const ty = Math.round(t.y);
    // price label fills the price-scale box (full width -> stays readable)
    ctx.fillStyle = t.bg;
    ctx.fillRect(0, ty - h / 2, sb, h);
    ctx.fillStyle = t.fg;
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText(t.text, 6, ty);
    // title chip: a rounded badge butted against the scale, poking into the chart (on objCv, see above)
    if (t.chip) {
...
export function drawTimeAxisViews(c) {
  const g = c._g;
  if (!g) return;
  const ctx = c._cv.bb.getContext('2d');
  ctx.font = c._comp.$props.font;
  /**
   * @param {number|null} x
   * @param {string} text
  ...
  const tag = (x, text, fg, bg) => {
    ...
    const w = ctx.measureText(text).width + 10,
      tx = Math.round(x),
      h = Const.ChartConfig.PANHEIGHT;
    ctx.fillStyle = bg || '#363a45';
    ctx.fillRect(tx - w / 2, 0, w, h); // match the crosshair time panel height (not a squat 16px)
  ...
  for (const s of c._series)
    for (const prim of s._primitives) {
      ...
      for (const v of views) {
        if (v.visible && !v.visible()) continue;
        tag(
          v.coordinate ? v.coordinate() : null,
          v.text ? v.text() : '',
          v.textColor ? v.textColor() : '#fff',
