...
export function buildOverlay(chart) {
  ...
  const ov = {
    $props: {
      ...
      colors: chart._comp.$props.colors,
      last: null,
      meta: {},
      tf: undefined,
    },
    data: [],
    $emit: (/** @type {string} */ ev, /** @type {any} */ s) => {
      if (ev === 'new-shader') (s.target === 'sidebar' ? chart._sbShaders : chart._gridShaders).push(s);
    },
    price_line: true,
  };
  chart._ov = ov;
...
export function applyOvStyle(chart) {
  const ov = chart._ov,
    st = chart._style;
  ov.colorCandleUp = st.colorCandleUp;
  ov.colorCandleDw = st.colorCandleDw;
  ov.colorWickUp = st.colorWickUp;
  ov.colorWickDw = st.colorWickDw;
  ov.colorCandleBorderUp = st.colorCandleBorderUp;
  ov.colorCandleBorderDw = st.colorCandleBorderDw;
}
...
export function refreshOverlay(chart, g, rows, colors) {
  ...
  ov.$props.layout = g;
  ov.$props.colors = colors;
  ov.$props.data = rows;
  ov.data = rows;
  ov.$props.last = last;
  ov.$props.meta = { last };
  const cs = chart._cs();
  ov.price_line = !cs || cs._opts.showPriceLine !== false; // series showPriceLine toggle
  applyOvStyle(chart);
}
...
function conflateCandles(cands) {
  ...
  for (const c of cands) {
    ...
    if (px !== col) {
      ...
      m.c = c.c;
      m.raw[4] = c.raw[4]; // extend close to the last bar in this pixel
...
function drawCurrentPriceLine(chart, ctx, g) {
  const cs = chart._cs();
  if (!cs || !cs._rows.length) return;
  const o = cs._opts;
  if (o.showPriceLine === false) return;
  const last = cs._rows[cs._rows.length - 1],
    price = last[4];
