 ...
 */
export function drawPriceLines(c, ctx, k) {   // k = grid POSITION
  const g = c._gridAt(k); if (!g) return;
  const id = c._idAt(k);
  for (const s of c._series) {
    if (s._pane !== id || !s._priceLines.length) continue;
    for (const pl of s._priceLines) {
      const o = pl._opts; if (o.showLine === false || o.price == null) continue;
...
export function drawPrimitives(c, ctx, pane, band = 'above') {
  ...
  for (const s of c._series) {
    ...
    for (const prim of s._primitives) {
      try {
        ...
        for (const v of views.slice().filter(inBand).sort((/** @type {any} */ a, /** @type {any} */ b) => zval(a) - zval(b))) {
          const r = v.renderer && v.renderer(); if (r) renderers.push(r);
        }
  ...
  for (const r of renderers) { if (r.drawBackground) { try { r.drawBackground(target); } catch (_) {} } }
  for (const r of renderers) { if (r.draw) { try { r.draw(target); } catch (_) {} } }
}
...
export function hitTest(c, x, y) {
  const L = c._comp.$props.layout; if (!L) return null;
