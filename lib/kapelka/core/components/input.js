...
export function wireInput(c) {
  ...
  root.addEventListener(
    ...
    (/** @type {WheelEvent} */ e) => {
      ...
      // Guard on the visible SPAN (bar slots on screen), not the real-bar count: with a short
      // dataset (e.g. 15 monthly bars) the two diverge and a real-bar guard deadlocks the wheel --
      // zoom-in refused because fewer bars exist than minZoom while the window spans hundreds of
      // slots, and zoom-out steps (proportional to the count) shrink toward zero. The span IS the
      // slot count in ib mode; in time mode it's span / interval.
      const n = c._ib ? c._range[1] - c._range[0] : (c._range[1] - c._range[0]) / c._iv();
      if (n < 2) return;
      const out = e.deltaY > 0;
      if (out && n > c._maxZoom()) return; // can't zoom out past maxZoom bar slots (over-compression)
      if (!out && n <= c._minZoom()) return; // can't zoom in past minZoom bar slots (over-zoom limit)
