...
class CursorUpdater {
    ...
    /** @param {{ x: number, y: number, grid_id: number }} e */
    sync(e) {
    ...
    overlay_data(grid, e) {
        ...
        if (grid.id > 0) {
            // Sequential grids
            let d = data.filter((/** @type {any} */ x) => x.grid.id === undefined)
            // grids with custom ids (for merging)
            let m = data.filter((/** @type {any} */ x) => x.grid.id === grid.id)
            data = [d[grid.id - 1], ...m]
        }

        const t = grid.screen2t(e.x)
        let ids = /** @type {Record<string, number>} */ ({}), res = /** @type {Record<string, any>} */ ({})
        for (var d of data) {
            let ts = d.data.map((/** @type {any} */ x) => x[0])
    ...
    cursor_data(grid, e) {
        ...
        let xs = data.map((/** @type {any} */ x) => grid.t2screen(x[0]) + 0.5)
    ...
    /** @param {any} grid @param {any} mouse @param {any} candle */
