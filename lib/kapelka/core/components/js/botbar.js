...
export default class Botbar {
    ...
    /**
     * The author's time-axis (X-axis) render class.
    ...
    constructor(canvas, comp) {
        ...
        this.comp = comp
        this.$p = comp.$props
        /** @type {any} the author's built layout object (open-shaped: grids, botbar, ...) */
    ...
    update() {
        ...
        for (var p of this.layout.botbar.xs) {
            ...
            this.ctx.fillText(/** @type {string} */ (lbl), p[0], 18)
    ...
    panel() {
        ...
        let cursor = /** @type {number} */ (this.$p.cursor.x)
    ...
    /**
     * @param {any[]} p  a botbar tick entry [x_px, [t_ms, ...], weight]
     * @returns {string|number}
     */
    format_date(p) {
        let t = p[1][0]
        t = this.grid_0.ti_map.i2t(t)
        // app-supplied axis tick formatter (tickMarkFormatter(seconds, tickMarkType))
        const tmf = /** @type {any} */ (this.$p).tickMarkFormatter
        if (tmf) {
            let type
            // Honor ONLY grid_maker's weight (p[2]) -- it classifies year/month/day boundaries in the
            // DISPLAY timezone (p_t). The old raw-UTC fallbacks (year_start(t)/day_start(t) on the
            // unshifted t) fire at UTC midnight too, so with a tz offset they double-stamped a date on
            // the UTC-midnight tick (e.g. a spurious day label at 20:00 for UTC-4). (TODO(*) resolved.)
            if (p[2] === YEAR) type = 0
            else if (p[2] === MONTH) type = 1
            else if (p[2] === DAY) type = 2
            else type = 3
    ...
    format_cursor_x() {
        ...
        t = this.grid_0.ti_map.i2t(t)
        // app-supplied crosshair time formatter (localization.timeFormatter(seconds))
        const tf = /** @type {any} */ (this.$p).timeFormatter
        if (tf) return tf(/** @type {number} */ (t) / 1000)
        ...
        //t += new Date(t).getTimezoneOffset() * MINUTE
        let d = new Date(/** @type {number} */ (t) + k * this.$p.timezone * HOUR)
