...
export default class TI {
    ...
    /** @param {any} params  the overlay-build ctx (sub/interval/meta/$props/…) @param {import('../../types.js').Row[]} res */
    init(params, res) {
        let {
            sub, interval, meta, $props:$p, interval_ms, sub_start, ib
        } = params

        // These state members are assigned only here (not in the constructor), so TS would
        // otherwise widen them to `T | undefined` and flag every downstream indexed access;
        // they are the dynamic ti_map state -> typed `any` per the migration boundary rule.
        /** @type {any} sparse t->i map (indexed by time) */
        this.ti_map = []
        /** @type {any} sparse i->t map (indexed by index) */
        this.it_map = []
        /** @type {any} the main subset re-keyed by index */
        this.sub_i = []
        this.ib = ib
        /** @type {any} the main subset rows (t-keyed) */
        this.sub = res
        this.ss = sub_start
        this.tf = interval_ms
    ...
    /** @param {import('../../types.js').Row[]} data @param {string} [mode] @returns {import('../../types.js').Row[]} */
    parse(data, mode) {
    ...
    /** @param {number} i @param {string} [mode] */
    i2t_mode(i, mode) {
    ...
    // Used by tv.goto()
    /** @param {number} smth @param {import('../../types.js').Row[]} ohlcv */
    gt2i(smth, ohlcv) {
        if (smth > MAX_ARR) {
