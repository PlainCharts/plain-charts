...
export function createGridX(G) {
  const { self, sub, interval, range, $p, ti_map, timezone, dayRoll, master_grid, minLabelPx } = G;
  ...
  function insert_line(prev, p, x, m0) {
    ...
    if (ti_map.tf < DAY) {
      ...
      // Daily+ bars stamped at the session OPEN (evening before their trade date): roll them into
      // their trade day -- plus the display offset, matching the app's label formatting -- before the
      // year/month/day boundary tests, so "new month" lands on the bar that TRADES the 1st. Without
      // dayRoll the legacy behavior (raw UTC timestamps) is unchanged.
      prev_t += dayRoll + timezone * HOUR;
      p_t += dayRoll + timezone * HOUR;
    }
    let d = timezone * HOUR;
