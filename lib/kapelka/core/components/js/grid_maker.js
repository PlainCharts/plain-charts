...
function GridMaker(id, params, master_grid = null) {
    ...
    var mt = manualY ? 0 : (grid.marginTop || 0), mb = manualY ? 0 : (grid.marginBottom || 0)
    ...
    const fmtDisp = p => {
        let v = toDisp(p), d = self.disp_prec != null ? self.disp_prec : 2
    ...
    /** @type {any} */
    const G = {
        self, sub, interval, range, $p, height, ti_map, timezone,
        master_grid, minLabelPx, toDisp, fromDisp,
    }
    ...
    function calc_$range() {
        if (!master_grid) {
            ...
            if (sub.length && sub[0].length >= 5) {
                ...
                for (var i = 0, n = sub.length; i < n; i++) {
                    let x = sub[i]
                    for (var j = 1; j < x.length; j++) { let v = x[j]; if (v == null) continue; if (v > hi) hi = v; if (v < lo) lo = v }
                }
            }
            if (y_range_fn) { var [hi, lo, exp] = /** @type {[number, number, any]} */ (y_range_fn(hi, lo)) }
        } else {
    ...
    function calc_positions() {
        ...
        var rTop = mt * height, rH = height * (1 - mt - mb) || height
        if (!grid.logScale) {
            self.A = - rH / (self.$_hi - self.$_lo)
            self.B = rTop - self.$_hi * self.A
        } else {
            self.A = - rH / (math.log(self.$_hi) -
                       math.log(self.$_lo))
