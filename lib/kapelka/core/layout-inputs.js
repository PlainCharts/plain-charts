...
export function yTransformsByPos(c) {
  ...
  for (const k in c._y) {
    const y = c._y[k];
    if (!y.auto && y.range) {
      const pos = c._paneIds.indexOf(+k);
      if (pos >= 0) yTransforms[pos] = { auto: false, range: y.range };
    }
  }
  return yTransforms;
}
  ...
  for (let pos = 0; pos < c._paneIds.length; pos++) {
    ...
    for (const s of c._series) {
      ...
      for (const prim of s._primitives) {
        ...
        try {
          const a = prim.autoscaleInfo();
          const r = a && a.priceRange;
      ...
      for (const pl of s._priceLines) {
        const o = pl._opts;
        if (o.autoscale && o.price != null && isFinite(o.price)) votes.push({ minValue: o.price, maxValue: o.price });
      }
    }
    if (!scaleFn && !votes.length) continue;
    layersMeta[pos] = {
      scale: {
        y_range: (/** @type {number} */ hi, /** @type {number} */ lo) => {
          for (const v of votes) {
            if (v.maxValue > hi) hi = v.maxValue;
            if (v.minValue < lo) lo = v.minValue;
          }
