...
export function buildPaneOverlays(c, pane) {
  /** @type {any[]} */
  const overlays = [];
  overlays.push({ z: -1, display: true, renderer: { draw: (/** @type {any} */ ctx) => drawPrimitives(c, ctx, pane, 'below') } });   // drawings sent behind the candles
  if (pane.k === 0) overlays.push({ z: 0, display: true, renderer: { draw: (/** @type {any} */ ctx) => drawCandles(c, ctx) } });
  for (const s of pane.series) overlays.push({ z: 10, display: true, renderer: { draw: (/** @type {any} */ ctx) => c._drawSeries(ctx, pane.k, s) } });
  ...
  overlays.push({ z: 20, display: true, renderer: { draw: (/** @type {any} */ ctx) => { const cs = c._cs(); const list = (cs && cs._pane === pane.id && cs._markers && cs._markers.length) ? [cs, ...pane.series] : pane.series; SeriesRenderers.drawMarkers(ctx, c._gridAt(pane.k), list, c._comp.$props.font); } } });
  overlays.push({ z: 1e5, display: true, renderer: { draw: (/** @type {any} */ ctx) => drawPriceLines(c, ctx, pane.k) } });
  overlays.push({ z: 1e6, display: true, renderer: { draw: (/** @type {any} */ ctx) => drawPrimitives(c, ctx, pane, 'above') } });
