...
import { buildOverlay, drawCandles } from './components/renderers/candles.js';
import * as SeriesRenderers from './components/renderers/series.js'; // value-series painters
import { drawPriceLines, drawPrimitives } from './components/primitives-host.js'; // series-attached primitives
...
export function ensurePanes(c, n) {
  ...
  while (c._panes.length > n) {
    const pane = c._panes.pop();
    c._destroyPane(pane);
  }
}
...
export function makePane(c, k) {
  ...
  const crossComp = {
    $props: { layout: c._comp.$props.layout.grids[k], cursor: c._comp.cursor, colors: c._comp.$props.colors },
  };
...
/** @param {any} c @param {any} pane the pane render bundle (series already assigned) */
export function buildPaneOverlays(c, pane) {
  /** @type {any[]} */
  const overlays = [];
  overlays.push({
    z: -1,
    display: true,
    renderer: { draw: (/** @type {any} */ ctx) => drawPrimitives(c, ctx, pane, 'below') },
  }); // drawings sent behind the candles
  if (pane.k === 0)
    overlays.push({ z: 0, display: true, renderer: { draw: (/** @type {any} */ ctx) => drawCandles(c, ctx) } });
  for (const s of pane.series)
    overlays.push({
      z: 10,
      display: true,
      renderer: { draw: (/** @type {any} */ ctx) => c._drawSeries(ctx, pane.k, s) },
    });
  ...
  overlays.push({
    ...
    renderer: {
      draw: (/** @type {any} */ ctx) => {
        const cs = c._cs();
        const list =
          cs && cs._pane === pane.id && cs._markers && cs._markers.length ? [cs, ...pane.series] : pane.series;
        SeriesRenderers.drawMarkers(ctx, c._gridAt(pane.k), list, c._comp.$props.font);
      },
    },
  });
  overlays.push({
    z: 1e5,
    display: true,
    renderer: { draw: (/** @type {any} */ ctx) => drawPriceLines(c, ctx, pane.k) },
  });
  overlays.push({
    z: 1e6,
    display: true,
    renderer: { draw: (/** @type {any} */ ctx) => drawPrimitives(c, ctx, pane, 'above') },
  });
...
export function destroyPane(c, pane) {
  try {
    c._root.removeChild(pane.gridCv);
    c._root.removeChild(pane.objCv);
    c._root.removeChild(pane.crossCv);
    c._root.removeChild(pane.sbCv);
  } catch (_) {}
}
...
/** @param {any} c */
export function resetPanes(c) {
  for (const p of c._panes) c._destroyPane(p);
  c._panes = [];
}
...
export function panes(c) {
  ...
  return [...ids]
    ...
    .map((k, pos) => ({
      ...
      setStretchFactor: (/** @type {number} */ f) => {
        c._stretch[k] = Math.max(0.0001, +f || 1);
        c._invalidate();
      },
      moveTo: (/** @type {number} */ idx) => c._movePane(pos, idx), // pos = this pane's index in panes() (= _movePane's order)
      setPreserveEmptyPane: (/** @type {boolean} */ on) => {
        if (on) c._preserve[k] = true;
        else delete c._preserve[k];
        c._invalidate();
      },
...
export function removePane(c, index) {
  // drop every series in that grid; the now-empty grid disappears on rebuild
  c._series = c._series.filter((/** @type {any} */ s) => s._pane !== index);
  delete c._scaleMode[index];
  delete c._y[index];
  delete c._stretch[index];
  c._invalidate();
}
...
export function movePane(c, from, to) {
  ...
  for (const s of c._series) if (s._pane > 0) allIds.add(s._pane);
  ...
  const remap = {};
  order.forEach((oldP, newIdx) => {
    remap[oldP] = newIdx;
  });
  /** @param {Record<string, any>} obj @returns {Record<string, any>} */
  const reKey = (obj) => {
    const o = /** @type {Record<string, any>} */ ({});
    for (const k in obj) o[remap[k] != null ? remap[k] : k] = obj[k];
    return o;
  };
  for (const s of c._series) if (remap[s._pane] != null) s._pane = remap[s._pane];
  c._scaleMode = /** @type {any} */ (reKey(c._scaleMode));
  c._y = /** @type {any} */ (reKey(c._y));
  c._stretch = /** @type {any} */ (reKey(c._stretch));
  c._invalidate();
}

/** @param {any} c @returns {any} */
export function addPane(c) {
  const idx = c._panes.length;
