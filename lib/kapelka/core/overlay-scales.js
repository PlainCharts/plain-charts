...
export function isOverlayId(id) { return id != null && id !== '' && id !== 'right' && id !== 'left'; }
...
export function makeScaleView(g, o) {
  const H = g.height, mt = o.top || 0, mb = o.bottom || 0;
  const regionTop = mt * H, regionBot = (1 - mb) * H, regionH = Math.max(1, regionBot - regionTop);
  let hi = o.hi, lo = o.lo; if (hi === lo) { hi += 1; lo -= 1; }
  const log = o.log, tf = (/** @type {number} */ v) => (log ? Math.log(v) : v);
  const A = -regionH / (tf(hi) - tf(lo)), B = regionTop - tf(hi) * A;
  return {
    A, B, height: regionBot, width: g.width, px_step: g.px_step, ti_map: g.ti_map,
    $2screen: (v) => /** @type {import('./types.js').YPx} */ (Math.floor(tf(v) * A + B) - 0.5),
    screen2$: (y) => /** @type {import('./types.js').Price} */ (log ? Math.exp((y - B) / A) : (y - B) / A),
    t2screen: (t) => g.t2screen(t), screen2t: (x) => g.screen2t(x),
  };
}
...
export function scanScaleRange(list, isHist) {
  ...
  for (const s of list) for (const r of s._rows) {
    if (s._isCandle()) { if (r[2] > hi) hi = r[2]; if (r[3] < lo) lo = r[3]; }
    else if (s._type.type === 'Segmented' || s._view) { const a = r[1], b = r[2]; const mx = Math.max(a, b), mn = Math.min(a, b); if (mx > hi) hi = mx; if (mn < lo) lo = mn; }
    else { const v = r[1]; if (v != null) { if (v > hi) hi = v; if (v < lo) lo = v; } }
  }
...
export function buildScaleViews(g, seriesList) {
  ...
  for (const s of seriesList) {
    if (isOverlayId(s._priceScaleId)) { /** @type {import('./series.js').Series[]} */ (groups.get(s._priceScaleId) || groups.set(s._priceScaleId, []).get(s._priceScaleId)).push(s); }
    else s._scaleView = null;
  }
  groups.forEach((list) => {
    const isHist = list.some((s) => s._type.type === 'Histogram');
    const range = scanScaleRange(list, isHist);
    if (!range) { list.forEach((s) => { s._scaleView = null; }); return; }
    ...
    list.forEach((s) => { s._scaleView = view; });
  });
}
