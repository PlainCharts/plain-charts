...
export class Series {
  ...
  _row(b) {
    ...
    if (this._view) {
      ...
      let mn = 0, mx = 0;
      try { const pv = this._view.priceValues ? this._view.priceValues(b) : [b.value]; mn = Math.min(...pv); mx = Math.max(...pv); } catch (_) {}
      if (!isFinite(mn) || !isFinite(mx)) { mn = mx = 0; }
      return [t, mn, mx, b];
    }
    ...
    if (b.segments || b.wick || b.lines) {
      ...
      const ext = (/** @type {number|undefined} */ v) => { if (v != null && isFinite(v)) { if (v < lo) lo = v; if (v > hi) hi = v; } };
  ...
  feed(bars) {
    ...
    let sorted = true;
    for (let i = 1; i < rows.length; i++) if (rows[i][0] < rows[i - 1][0]) { sorted = false; break; }
  ...
  feedBar(b) {
    ...
    else if (!n || row[0] > d[n - 1][0]) { const prevLast = n ? d[n - 1][0] : null; d.push(row); this._chart._autoScrollAppend(this, prevLast); }   // append -> maybe auto-scroll
  ...
  configure(o = {}) {
    ...
    this._chart._restyle(this); this._chart._invalidate();
    return this;
  }
  getConfig() { return this._opts; }
  /** @param {number} p */
  priceToY(p) { return this._chart._priceToCoord(this._pane, p); }
  /** @param {number} y */
  yToPrice(y) { return this._chart._coordToPrice(this._pane, y); }
  formatPrice() {
  ...
  priceAxis() {
    ...
    return { width: () => c._sbWidth(), configure: (/** @type {Record<string, any>} */ o = {}) => { if (o.mode != null) c._setPaneMode(k, o.mode); if (o.autoScale) c._resetPaneAuto(k); if (o.range && o.range.length === 2) { const y = c._yOf(k); y.auto = false; y.range = [o.range[0], o.range[1]]; y.zoom = 1; c._invalidate(); } }, getConfig: () => ({ mode: c._modeOf(k) }) };
  }
  // price lines paint on the objects sheet -- adding/moving one repaints that sheet only, not the data
  /** @param {Record<string, any>} [opts] */
  addLevel(opts = {}) {
    const line = { _opts: { ...opts }, configure: (/** @type {any} */ o) => { Object.assign(line._opts, o); this._chart._scheduleObjects(); return line; }, getConfig: () => line._opts };
    this._priceLines.push(line); this._chart._scheduleObjects(); return line;
  }
  /** @param {any} line */
  removeLevel(line) { const i = this._priceLines.indexOf(line); if (i >= 0) this._priceLines.splice(i, 1); this._chart._scheduleObjects(); }
  ...
  addLayer(prim) {
    ...
    if (prim.attached) prim.attached({ chart: this._chart, series: this, requestUpdate: () => this._chart._scheduleObjects(prim) });
