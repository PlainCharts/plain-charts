...
export function buildLayout({
  ...
  const ctx = {
    chart: {
      grid: {
        logScale: !!logScale,
        scaleMode: scaleMode || 0,
        height: mainGridHeight,
        // scaleMargins on the MAIN price scale (undefined -> full height, keeps default EXPAND padding)
        marginTop: scaleMargins ? scaleMargins.top || 0 : undefined,
        marginBottom: scaleMargins ? scaleMargins.bottom || 0 : undefined,
        minMove,
        tickPrec,
      },
    }, // instrument tick grid -> price labels on the main scale quantize to it
    ohlcv: rows,
    sub,
