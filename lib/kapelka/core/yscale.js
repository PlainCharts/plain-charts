...
/** @param {{ y0: number, z: number, height: number }} drag @param {number} curY @returns {number} */
...
/** @param {[number, number]} startRange @param {number} z @param {{ A?: number, B: number, height: number }|null} [log] @returns {number[]} */
export function calcRange(startRange, z, log) {
  if (log && log.A != null) {
    ...
    const f = (/** @type {number} */ y) => math.exp((y - log.B) / /** @type {number} */ (log.A));
