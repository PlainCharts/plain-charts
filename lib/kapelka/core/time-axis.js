...
export function i2t(c, v) {
  const g = c._g;
  return c._ib && g && g.ti_map ? g.ti_map.i2t(v) : v;
}
/** @param {any} c @param {number} v @returns {number} */
export function t2i(c, v) {
  const g = c._g;
  return c._ib && g && g.ti_map ? g.ti_map.t2i(v) : v;
}
...
export function logicalToTime(c, i) {
  ...
  return d[k][0] + (i - k) * (d[k + 1][0] - d[k][0] || iv);
}
...
export function fitToData(c) {
  ...
  if (c._ib) {
    // index units: last index = n-1 (array position)
    const i1 = n - 1 + off;
    c._range = [i1 - 150, i1];
    return;
  }
...
export function autoScrollAppend(c, s, oldLastTime) {
  ...
  if (c._ib) {
    let appended = 0;
    for (let i = n - 1; i >= 0 && rows[i][0] > oldLastTime; i--) appended++; // count new bars
    const oldLastIdx = n - 1 - appended;
    if (c._range[1] >= oldLastIdx) {
      c._range = [c._range[0] + appended, c._range[1] + appended];
      c._emitRange();
    }
  } else if (c._range[1] >= oldLastTime) {
    const d = rows[n - 1][0] - oldLastTime;
    c._range = [c._range[0] + d, c._range[1] + d];
    c._emitRange();
  }
}
...
export function emitRange(c) {
  const ts = c.timeAxis();
  const tr = ts.timeWindow();
  c._cbs.range.forEach((/** @type {Function} */ cb) => {
    try {
      cb(tr);
    } catch (_) {}
  });
  const lr = ts.barWindow();
  c._cbs.logical.forEach((/** @type {Function} */ cb) => {
    try {
      cb(lr);
    } catch (_) {}
  });
...
/** @param {any} c */
export function maxZoom(c) {
  const v = c._options.maxZoom;
  return v > 0 ? v : Const.ChartConfig.MAX_ZOOM;
}
/** @param {any} c */
export function minZoom(c) {
  const v = c._options.minZoom;
  return v > 0 ? v : Const.ChartConfig.MIN_ZOOM;
}
...
/** @param {any} c */
export function maxVZoom(c) {
  const v = c._options.maxVZoom;
  return v > 0 ? v : 3;
}
...
export function visibleHiLo(c, id) {
  const main =
    c._series.find((/** @type {any} */ s) => s._pane === id && s._isCandle() && s._rows.length) ||
    c._series.find((/** @type {any} */ s) => s._pane === id && s._rows.length && !isOverlayId(s._priceScaleId));
  if (!main || !c._range) return null;
  const rows = main._rows,
    n = rows.length,
    cand = main._isCandle();
  let hi = -Infinity,
    lo = Infinity;
  if (c._ib) {
    const a = Math.max(0, Math.floor(c._range[0])),
      b = Math.min(n - 1, Math.ceil(c._range[1]));
    for (let i = a; i <= b; i++) {
      const r = rows[i];
      if (cand) {
        if (r[2] > hi) hi = r[2];
...
export function clampVZoom(c, range, id) {
  ...
  const dr = c._visibleHiLo(id);
  if (!dr) return range;
  const dSpan = dr[0] - dr[1];
  if (!(dSpan > 0)) return range;
  const hi = range[0],
    lo = range[1],
    span = hi - lo;
...
export function clampZoom(c, r, anchor) {
  ...
  const max = c._maxZoom() * unit,
    min = c._minZoom() * unit;
  let want = span;
  if (span > max) want = max;
  else if (span < min) want = min;
  else return r;
  ...
  return {
    ...
    timeWindow: () =>
      !c._range
        ? null
        : c._ib
          ? { from: c._logicalToTime(c._range[0]) / 1000, to: c._logicalToTime(c._range[1]) / 1000 }
          : { from: c._range[0] / 1000, to: c._range[1] / 1000 },
    setTimeWindow: (/** @type {{ from: number, to: number }|null} */ r) => {
      if (!r) return;
      c._range = c._ib
        ? [c._timeToLogical(r.from * 1000), c._timeToLogical(r.to * 1000)]
        : [r.from * 1000, r.to * 1000];
      c._invalidate();
    },
    ...
    zoomTimeWindow: (/** @type {{ from: number, to: number }|null} */ r) => {
      if (!r) return;
      c._range = c._clampZoom(
        c._ib ? [c._timeToLogical(r.from * 1000), c._timeToLogical(r.to * 1000)] : [r.from * 1000, r.to * 1000],
      );
      c._emitRange();
      c._invalidate();
    },
    barWindow: () =>
      !c._range
        ? null
        : c._ib
          ? { from: c._range[0], to: c._range[1] }
          : { from: c._timeToLogical(c._range[0]), to: c._timeToLogical(c._range[1]) },
    setBarWindow: (/** @type {{ from: number, to: number }|null} */ r) => {
      if (!r) return;
      c._range = c._clampZoom(c._ib ? [r.from, r.to] : [c._logicalToTime(r.from), c._logicalToTime(r.to)]);
      c._emitRange();
      c._invalidate();
    },
    ...
    xToBar: (/** @type {number} */ x) => {
      if (!c._g) return null;
      const t = c._g.screen2t(x);
      return c._ib ? t : c._timeToLogical(t);
    },
    fitAll: () => {
      c._range = null;
      c._fitToData();
    ...
    width: () => c._chartW,
    height: () => {
      if (!c._showTime) return 0;
      const L = c._comp.$props.layout;
      return (L && L.botbar && L.botbar.height) || 0;
    },
    ...
    configure: (/** @type {any} */ o = {}) => {
      ...
      if (o.barSpacing != null && o.barSpacing > 0 && c._chartW > 0) {
        ...
        if (cs && cs._rows.length) {
          const off = c._options.timeAxis.rightOffset != null ? c._options.timeAxis.rightOffset : 6,
            wB = c._chartW / o.barSpacing,
            n = cs._rows.length;
          if (c._ib) {
            const last = n - 1;
            c._range = [last + off - wB, last + off];
          } else {
            const iv = c._iv(),
              last = cs._rows[n - 1][0],
              t1 = last + off * iv;
            c._range = [t1 - wB * iv, t1];
          }
          c._invalidate();
        }
      }
