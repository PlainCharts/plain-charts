...
function readStatus() { try { return JSON.parse(fs.readFileSync(STATUS_FILE, 'utf8')); } catch (_) { return {}; } }
function bumpRev(s, id) { s._rev = s._rev || {}; s._rev[id] = (s._rev[id] || 0) + 1; }   // tells the runtime to reload this addon

function listIds() {
  let entries = [];
  try { entries = fs.readdirSync(ADDONS_DIR, { withFileTypes: true }); } catch (_) {}
  return entries.filter((d) => d.isDirectory() && fs.existsSync(path.join(ADDONS_DIR, d.name, 'index.js'))).map((d) => d.name);
}
...
function readAddonLocales(dir) {
  ...
  const out = {};
  let files = [];
  try { files = fs.readdirSync(path.join(dir, 'locales')).filter((f) => f.endsWith('.json')); } catch (_) { return null; }
  for (const f of files) {
    ...
    } catch (_) { /* skip a malformed file */ }
  }
  return Object.keys(out).length ? out : null;
}

module.exports = {
  init() { /* execution lives in the renderer runtime — nothing to start here */ },

  list() {
    const state = readState(), status = readStatus();
  ...
  config(id, cfg) {
    ...
    const s = readState(); bumpRev(s, sid); writeState(s);
    return { ok: true };
  },
  reload(id) { const s = readState(); bumpRev(s, sanitizeId(id)); writeState(s); return { ok: true }; },

  save(name, code) {
    const id = sanitizeId(name);
    if (!id) return { error: 'invalid name' };
    if (typeof code !== 'string') return { error: 'no code' };
    try { new Function('module', 'exports', 'require', '__dirname', '__filename', code); }
    catch (e) { return { error: 'Syntax: ' + ((e && e.message) || e) }; }
    try {
      const dir = path.join(ADDONS_DIR, id);
      fs.mkdirSync(dir, { recursive: true });
      fs.writeFileSync(path.join(dir, 'index.js'), code);
    } catch (e) { return { error: String((e && e.message) || e) }; }
    const s = readState(); bumpRev(s, id); writeState(s);   // runtime reloads if it's enabled
    return { ok: true, id };
  },
  ...
  installPackage(id, files) {
    ...
    const s = readState(); bumpRev(s, sid); writeState(s);   // runtime reloads if it's enabled
    return { ok: true, id: sid };
  },
  read(id) {
    const sid = sanitizeId(id);
    try { return { code: fs.readFileSync(path.join(ADDONS_DIR, sid, 'index.js'), 'utf8') }; }
    catch (_) { return { error: 'not found' }; }
  },
  ...
  removeIcon(id) {   // revert to the letter badge
    const sid = sanitizeId(id);
    if (sid) { try { fs.unlinkSync(path.join(ADDONS_DIR, sid, 'icon.png')); } catch (_) {} }
    return { ok: true };
  },
  remove(id) {
    const sid = sanitizeId(id);
    const s = readState(); delete s[sid]; if (s._rev) delete s._rev[sid]; writeState(s);
    try { fs.rmSync(path.join(ADDONS_DIR, sid), { recursive: true, force: true }); } catch (_) {}
    return { ok: true };
  },
};
