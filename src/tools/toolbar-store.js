...
const store = createStore('/api/toolbar', {
  tools: DEFAULT_TOOLS,
  icons: {},
  hotkeys: {},
...
const save = () => {
  store.set('tools', data.tools);
  store.set('icons', data.icons);
  store.set('hotkeys', data.hotkeys);
  store.set('features', data.features);
  store.set('newDrawingSync', data.newDrawingSync);
  store.set('magnet', data.magnet);
  store.set('hideDrawings', data.hideDrawings);
  store.set('hideIndicators', data.hideIndicators);
  store.set('lockDrawings', data.lockDrawings);
};
...
export function setNewDrawingSync(mode) {
  data.newDrawingSync = mode || 'none';
  save();
}
...
/** @param {any} v */
export function setHideDrawings(v) {
  data.hideDrawings = !!v;
  save();
}
/** @param {any} v */
export function setHideIndicators(v) {
  data.hideIndicators = !!v;
  save();
}
...
/** @param {any} v */
export function setLockDrawings(v) {
  data.lockDrawings = !!v;
  save();
}
...
export function setOnBar(id, on) {
  if (on) {
    ...
    data.tools = data.tools.filter((x) => x !== id);
  }
  save();
}
...
export function setToolHotkey(id, combo) {
  if (!combo) {
    delete data.hotkeys[id];
    save();
    return;
  }
  // a combo maps to one tool only — clear it from any other tool first
  Object.keys(data.hotkeys).forEach((k) => {
    if (data.hotkeys[k] === combo) delete data.hotkeys[k];
  });
