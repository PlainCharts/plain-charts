...
export const toolIdForFile = (folder) => {
  for (const [id, f] of folderOf) if (f === folder) return id;
  return null;
};
...
/** @param {string} id */
export const clearToolIconUrl = (id) => {
  iconOf.delete(id);
};
...
async function importFolder(folder, hasVocab) {
  ...
  try {
    await import('/packages/tools/' + folder + '/' + folder + '.js?t=' + Date.now());
  } finally {
  ...
  let meta = null;
  try {
    const r = await fetch('/packages/tools/' + folder + '/meta.json', { cache: 'no-store' });
  ...
  if (hasVocab) {
    ...
    try {
      const v = await fetch('/packages/tools/' + folder + '/vocab.json').then((r) => r.json());
      registerVocab(v && v.words ? v.words : v);
    } catch (_) {}
  }
}
...
export async function loadUserTools() {
  ...
  let tools = [];
  try {
    const d = await fetch('/api/user-tools').then((r) => r.json());
    tools = d.tools || [];
  } catch (_) {}
  for (const t of tools) {
    try {
      await importFolder(t.folder, t.hasVocab);
    } catch (/** @type {any} */ e) {
      log('Tool "' + t.folder + '" failed to load: ' + (e.message || e), true);
    }
  }
}
...
export async function reloadUserToolFile(folder) {
  [...folderOf.entries()].forEach(([id, f]) => {
    if (f === folder) folderOf.delete(id);
  });
