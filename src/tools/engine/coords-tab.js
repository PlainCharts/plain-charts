...
function barToTime(pane, bar) {
  ...
  if (x == null) return null;
  let t = ts.xToTime(x);
  if (t == null) { const r = ts.timeWindow(); if (r) t = bar < 0 ? r.from : r.to; }
  return t;
}
...
export function renderCoordsTab(body, d, pane, preview) {
  ...
  d.points.forEach((/** @type {Point} */ p, /** @type {number} */ i) => {
    ...
    if (hasPrice) {
      ...
      const price = /** @type {HTMLInputElement} */ (el('input')); price.type = 'number'; price.className = 'set-coord-in';
      price.value = Number(pt.price).toFixed(dec);
      price.oninput = () => { const v = parseFloat(price.value); if (!Number.isNaN(v)) { pt.price = v; preview(); } };
    ...
    const date = /** @type {HTMLInputElement} */ (el('input')); date.type = 'date'; date.className = 'set-coord-in set-coord-txt'; date.spellcheck = false;
    const bar = /** @type {HTMLInputElement} */ (el('input')); bar.type = 'number'; bar.className = 'set-coord-in';
    ...
    const timeWidget = segTime(h24, (nt) => {
      const p = partsOf(/** @type {number} */ (pt.time), off); p.h = nt.h; p.mi = nt.mi; p.s = nt.s;
      ...
      const b = timeToBar(pane, pt.time); bar.value = b != null ? /** @type {any} */ (b) : '';
      preview();
    });
    ...
    refresh = () => {
      ...
      timeWidget.set(p);
      const b = timeToBar(pane, /** @type {number} */ (pt.time)); bar.value = b != null ? /** @type {any} */ (b) : '';
    };
    refresh();
    ...
    date.onchange = () => {
      ...
      const p = partsOf(/** @type {number} */ (pt.time), off); p.y = nd.y; p.mo = nd.mo; p.d = nd.d;
      pt.time = timeFromParts(p, off); refresh(); preview();
    };
    bar.onchange = () => {
      const v = parseInt(bar.value, 10);
      if (Number.isNaN(v)) return refresh();
      const t = barToTime(pane, v); if (t != null) { pt.time = t; refresh(); preview(); }
    };
    [date, bar].forEach((inp) => inp.addEventListener('keydown', (e) => { if (e.key === 'Enter') inp.blur(); }));

    /** @param {string} label @param {HTMLElement} input */
    const mkRow = (label, input) => {
      const r = el('div', 'set-row');
      r.appendChild(el('div', 'set-row-left', tag + label));
      const c = el('div', 'set-controls'); c.appendChild(input);
