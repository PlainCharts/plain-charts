...
function applyDefault(engine, id) {
  const d = /** @type {Drawing | undefined} */ (engine.get(id)); const tool = d && getTool(d.tool); if (!d || !tool) return;
...
export function buildTemplateItem(engine, id, opts = {}) {
  const d = engine.get(id); const tool = d && getTool(d.tool);
  ...
  const row = (label, fn) => { const r = el('div', 'dwg-item'); r.append(el('span', 'dwg-check', ''), el('span', 'dwg-label', label)); r.onclick = () => { close(); fn(); }; return r; };
...
export function closeSaveTemplateDialog() { if (saveOverlay) { saveOverlay.remove(); saveOverlay = null; } }
...
export function openSaveTemplateDialog(engine, id) {
  ...
  const overlay = el('div', 'modal open'); saveOverlay = overlay; overlay.style.zIndex = '100';
  overlay.onclick = (e) => { if (e.target === overlay) closeSaveTemplateDialog(); };
  const dlg = el('div', 'dialog tmpl-save');

  const head = el('div', 'set-head');
  const x = el('span', 'lib-x', '✕'); x.onclick = closeSaveTemplateDialog;
  ...
  const inp = /** @type {HTMLInputElement} */ (el('input', 'tmpl-save-input')); inp.type = 'text'; inp.placeholder = 'Template name';
  ...
  const doSave = async () => {
    ...
    if (listToolTemplates(tool.id).some((t) => t.name === name)) {   // overwrite → confirm first
      const ok = await confirmDialog({ message: `Drawing Template '${name}' already exists. Do you really want to replace it?` });
      if (!ok) { inp.focus(); return; }
    }
    saveToolTemplate(tool.id, name, preset);
    closeSaveTemplateDialog();
  };
  const cancel = el('button', null, 'Cancel'); cancel.onclick = closeSaveTemplateDialog;
  const save = el('button', 'primary', 'Save'); save.onclick = doSave;
  ...
  const renderSuggest = () => {
    ...
    const names = !q ? [] : listToolTemplates(tool.id).map((t) => t.name)
      .filter((n) => { const nl = n.toLowerCase(); return nl.startsWith(q) && nl !== q; });
    suggest.innerHTML = '';
    names.forEach((n) => {
      const r = el('div', 'tmpl-suggest-row', n);
      r.onmousedown = (e) => { e.preventDefault(); inp.value = n; renderSuggest(); inp.focus(); };   // pick existing → overwrite on save
      suggest.appendChild(r);
    });
    suggest.style.display = names.length ? 'block' : 'none';
  };
  inp.oninput = renderSuggest;
  inp.onkeydown = (e) => { if (e.key === 'Enter') { e.preventDefault(); doSave(); } else if (e.key === 'Escape') closeSaveTemplateDialog(); };
  renderSuggest();
