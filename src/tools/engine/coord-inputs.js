...
export function fmtDateField(p) { return p.y + '-' + pad2(p.mo + 1) + '-' + pad2(p.d); }
...
const clampInt = (v, lo, hi) => { const n = parseInt(v, 10); return Number.isNaN(n) ? lo : Math.max(lo, Math.min(hi, n)); };
/**
 * @param {boolean} h24
...
export function segTime(h24, onCommit, opts) {
  ...
  const mk = () => { const i = /** @type {HTMLInputElement} */ (el('input', 'seg')); i.type = 'text'; i.inputMode = 'numeric'; i.maxLength = 2; i.spellcheck = false; return i; };
  const hh = mk(), mm = mk(), ss = seconds ? mk() : null;
  box.append(hh, el('span', 'seg-sep', ':'), mm);
  if (ss) box.append(el('span', 'seg-sep', ':'), ss);
  /** @type {HTMLInputElement | null} */
  let ap = null;
  if (!h24) { ap = /** @type {HTMLInputElement} */ (el('input', 'seg seg-ap')); ap.type = 'text'; ap.maxLength = 2; ap.spellcheck = false; box.append(el('span', 'seg-sep', ' '), ap); }
  ...
  const read = () => {
    ...
    const mi = clampInt(mm.value, 0, 59), s = ss ? clampInt(ss.value, 0, 59) : 0;
    if (!h24) { const pm = ((/** @type {HTMLInputElement} */ (ap)).value || 'AM').toUpperCase().charAt(0) === 'P'; h = (h % 12) + (pm ? 12 : 0); }
    return { h, mi, s };
  };
  /** @param {HmsParts} p */
  const set = (p) => {
    let disp = p.h;
    if (!h24) { if (ap) ap.value = p.h < 12 ? 'AM' : 'PM'; disp = p.h % 12 === 0 ? 12 : p.h % 12; }
    hh.value = pad2(disp); mm.value = pad2(p.mi); if (ss) ss.value = pad2(p.s);
  };
  ...
  nums.forEach((seg) => {
    ...
    seg.addEventListener('input', () => {
      ...
      const v = seg.value, hi = hiOf(seg);
      // advance when the segment is full, or when a single digit can't be the tens of any valid value
      if (v.length === 2) { const n = nextOf(seg); if (n) { n.focus(); n.select && n.select(); } }
      else if (v.length === 1 && +v > Math.floor(hi / 10)) { seg.value = pad2(+v); const n = nextOf(seg); if (n) { n.focus(); n.select && n.select(); } }
    });
    seg.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
        ...
        const lo = loOf(seg), hi = hiOf(seg), span = hi - lo + 1;
        const nv = lo + (((cur - lo) + (e.key === 'ArrowUp' ? 1 : -1)) % span + span) % span;
        seg.value = pad2(nv); commit();
      } else if (e.key === ':' || e.key === ' ') { e.preventDefault(); const n = nextOf(seg); if (n) { n.focus(); n.select && n.select(); } }
      else if (e.key === 'ArrowRight' && atEnd) { const n = nextOf(seg); if (n) { e.preventDefault(); n.focus(); n.select && n.select(); } }
      else if (e.key === 'ArrowLeft' && atStart) { const p = prevOf(seg); if (p) { e.preventDefault(); p.focus(); p.select && p.select(); } }
      else if (e.key === 'Backspace' && seg.value === '') { const p = prevOf(seg); if (p) { e.preventDefault(); p.focus(); } }
      else if (e.key === 'Enter') seg.blur();
    });
    seg.addEventListener('blur', () => { if (seg.value !== '') seg.value = pad2(clampInt(seg.value, loOf(seg), hiOf(seg))); commit(); });
  });
  ...
  if (ap) {
    ...
    ap.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowUp' || e.key === 'ArrowDown' || /^[aApP]$/.test(e.key)) {
        e.preventDefault();
        /** @type {HTMLInputElement} */ (ap).value = /^[pP]$/.test(e.key) ? 'PM' : /^[aA]$/.test(e.key) ? 'AM' : ((/** @type {HTMLInputElement} */ (ap)).value.toUpperCase().charAt(0) === 'P' ? 'AM' : 'PM');
        commit();
      } else if (e.key === 'ArrowLeft') { e.preventDefault(); const p = prevOf(/** @type {HTMLInputElement} */ (ap)); if (p) { p.focus(); p.select(); } }
...
export function segDate(onCommit) {
  ...
  const read = () => {
    const mo = clampInt(MM.value, 1, 12) - 1, d = clampInt(DD.value, 1, 31), yy = clampInt(YY.value, 0, 99);
    return { y: yy < 70 ? 2000 + yy : 1900 + yy, mo, d };
  };
  /** @param {DateParts} p */
  const set = (p) => { MM.value = pad2(p.mo + 1); DD.value = pad2(p.d); YY.value = pad2(((p.y % 100) + 100) % 100); };
  const commit = () => onCommit(read());
  segs.forEach((seg) => {
    seg.addEventListener('focus', () => seg.select());
    seg.addEventListener('input', () => {
    ...
    seg.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
        ...
        const nv = lo + (((cur - lo) + (e.key === 'ArrowUp' ? 1 : -1)) % span + span) % span;
        seg.value = pad2(nv); commit();
      } else if (e.key === '/' || e.key === '.' || e.key === '-' || e.key === ' ') { e.preventDefault(); const n = nextOf(seg); if (n) { n.focus(); n.select && n.select(); } }
      else if (e.key === 'ArrowRight' && atEnd) { const n = nextOf(seg); if (n) { e.preventDefault(); n.focus(); n.select && n.select(); } }
      else if (e.key === 'ArrowLeft' && atStart) { const p = prevOf(seg); if (p) { e.preventDefault(); p.focus(); p.select && p.select(); } }
      else if (e.key === 'Backspace' && seg.value === '') { const p = prevOf(seg); if (p) { e.preventDefault(); p.focus(); } }
      else if (e.key === 'Enter') seg.blur();
