...
export function buildVisRow(c, cat, onChange) {
  ...
  chk.type = 'checkbox';
  chk.checked = c.on !== false;
  const ctrls = el('div', 'set-vis-ctrls');
  const sync = () => {
    ctrls.style.opacity = chk.checked ? '1' : '.4';
    ctrls.style.pointerEvents = chk.checked ? '' : 'none';
  };
  chk.onchange = () => {
    c.on = chk.checked;
    onChange();
  ...
  const minIn = /** @type {HTMLInputElement} */ (el('input', 'set-vis-num'));
  minIn.type = 'number';
  minIn.min = /** @type {any} */ (cat.min);
  minIn.max = /** @type {any} */ (cat.max);
  const maxIn = /** @type {HTMLInputElement} */ (el('input', 'set-vis-num'));
  maxIn.type = 'number';
  maxIn.min = /** @type {any} */ (cat.min);
  maxIn.max = /** @type {any} */ (cat.max);
  ...
  const paint = () => {
    ...
    loH.style.left = a + '%';
    hiH.style.left = b + '%';
    minIn.value = /** @type {any} */ (c.min);
    maxIn.value = /** @type {any} */ (c.max);
  };
  ...
  minIn.onchange = () => {
    let n = clamp(parseInt(minIn.value, 10));
    if (Number.isNaN(n)) n = c.min;
    if (n > c.max) n = c.max;
    c.min = n;
    paint();
    onChange();
  ...
  maxIn.onchange = () => {
    let n = clamp(parseInt(maxIn.value, 10));
    if (Number.isNaN(n)) n = c.max;
    if (n < c.min) n = c.min;
    c.max = n;
    paint();
    onChange();
...
export function buildVisibilityRows(visibility, onChange, hintText) {
  VIS_CATEGORIES.forEach((cat) => {
    if (!visibility[cat.key]) visibility[cat.key] = { on: true, min: cat.min, max: cat.max };
  });
