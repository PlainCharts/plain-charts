...
export function closeMultiMenu() {
  if (multiMenu) { multiMenu.remove(); multiMenu = null; multiBtn = null; document.removeEventListener('pointerdown', multiAway, true); }
}
...
function fibLevelsControl(d, preview) {
  ...
  const render = () => {
    ...
    d.style.levels.forEach((/** @type {any} */ lv, /** @type {number} */ i) => {
      ...
      const chk = /** @type {HTMLInputElement} */ (el('input')); chk.type = 'checkbox'; chk.checked = lv.on !== false;
      chk.onchange = () => { lv.on = chk.checked; preview(); };
      const val = /** @type {HTMLInputElement} */ (el('input', 'fib-lv-val')); val.type = 'number'; val.step = 'any'; val.value = lv.value;
      val.onchange = () => { const v = parseFloat(val.value); if (!Number.isNaN(v)) { lv.value = v; preview(); } };
      // per-level stroke: colour + width + line style (overrides the global Levels line)
      const acc = (/** @type {string} */ k, /** @type {any} */ dflt) => ({ get: () => (lv[k] != null ? lv[k] : dflt), set: (/** @type {any} */ v) => { lv[k] = v; preview(); } });
      const sw = strokeSwatch({ color: acc('color', '#787b86'), width: acc('width', sdef.lineWidth || 1), lineStyle: acc('lineStyle', sdef.lineStyle || 'solid') });
      const del = el('span', 'fib-lv-del', '✕'); del.title = 'Remove level';
      del.onclick = () => { d.style.levels.splice(i, 1); render(); preview(); };
      rowc.append(chk, val, sw, del);
      grid.appendChild(rowc);
    });
  };
  render();
  const add = /** @type {HTMLButtonElement} */ (el('button', 'fib-lv-add')); add.type = 'button'; add.textContent = '+ Add level';
  add.onclick = () => { d.style.levels.push({ value: 0, on: true, color: '#787b86' }); render(); preview(); };
...
function controlWidget(d, ctrl, preview) {
  const acc = (/** @type {string} */ key) => ({ get: () => d.style[key], set: (/** @type {any} */ v) => { d.style[key] = v; preview(); } });
  if (ctrl.type === 'color' && (ctrl.width || ctrl.lineStyle)) {
    return strokeSwatch({ color: acc(ctrl.key), width: ctrl.width ? acc(ctrl.width) : null, lineStyle: ctrl.lineStyle ? acc(ctrl.lineStyle) : null });
  }
  if (ctrl.type === 'color' && (ctrl.size || ctrl.bold || ctrl.italic)) {
    return textSwatch({ color: acc(ctrl.key), size: ctrl.size ? acc(ctrl.size) : null, bold: ctrl.bold ? acc(ctrl.bold) : null, italic: ctrl.italic ? acc(ctrl.italic) : null });
  }
  return buildControl(ctrl, () => d.style[ctrl.key], (v) => { d.style[ctrl.key] = v; preview(); });
}
...
export function buildRow(d, row, preview) {
  ...
  if (row.fields) {
    ...
    row.fields.forEach((/** @type {any} */ f) => {
      ...
      if (f.toggle) {
        const chk = /** @type {HTMLInputElement} */ (el('input')); chk.type = 'checkbox';
        chk.checked = f.toggleDefault === true ? d.style[f.toggle] !== false : !!d.style[f.toggle];
        const sync = () => { w.style.opacity = chk.checked ? '1' : '.4'; w.style.pointerEvents = chk.checked ? '' : 'none'; };
        chk.onchange = () => { d.style[f.toggle] = chk.checked; preview(); sync(); };
  ...
  if (row.toggle) {
    ...
    const sync = () => { controls.style.opacity = chk.checked ? '1' : '.4'; controls.style.pointerEvents = chk.checked ? '' : 'none'; };
    chk.onchange = () => { d.style[row.toggle] = chk.checked; preview(); sync(); };
...
function multiSelectControl(ctrl, get, set) {
  ...
  const btn = /** @type {HTMLButtonElement} */ (el('button', 'set-multi')); btn.type = 'button';
  const lbl = el('span'); const car = el('span', null, '⌄'); car.style.color = 'var(--tx-dim)';
  btn.append(lbl, car);
  const summary = () => {
    const sel = get() || [];
  ...
  summary();
  btn.onclick = () => {
    if (multiMenu) { closeMultiMenu(); return; }
    const menu = el('div', 'dwg-menu'); multiMenu = menu; multiBtn = btn;
    options.forEach((/** @type {any} */ o) => {
      const row = el('div', 'dwg-item');
      const ck = el('span', 'dwg-check');
      const paint = () => { ck.textContent = (get() || []).includes(o.key) ? '✓' : ''; };
...
function buildControl(ctrl, get, set, extras) {
  ...
  if (ctrl.type === 'range') {
    const i = /** @type {HTMLInputElement} */ (el('input')); i.type = 'range';
    i.min = ctrl.min != null ? ctrl.min : 0; i.max = ctrl.max != null ? ctrl.max : 1; i.step = ctrl.step != null ? ctrl.step : 0.05;
  ...
  if (ctrl.type === 'text') {
    const i = /** @type {HTMLInputElement} */ (el('input')); i.type = 'text'; i.value = get() || '';
    if (ctrl.placeholder) i.placeholder = ctrl.placeholder;
    i.oninput = () => set(i.value);
    return i;
  }
  if (ctrl.type === 'bool') { const i = /** @type {HTMLInputElement} */ (el('input')); i.type = 'checkbox'; i.checked = !!get(); i.onchange = () => set(i.checked); return i; }
  if (ctrl.type === 'select') {
    const s = /** @type {HTMLSelectElement} */ (el('select')); (ctrl.options || []).forEach((/** @type {any} */ o) => { const op = /** @type {HTMLOptionElement} */ (el('option', null, o.name)); op.value = o.key; s.appendChild(op); });
    s.value = get(); s.onchange = () => set(s.value); return s;
  }
  const i = /** @type {HTMLInputElement} */ (el('input')); i.type = 'number';
  if (ctrl.min != null) i.min = ctrl.min; if (ctrl.max != null) i.max = ctrl.max;
  i.value = get(); i.oninput = () => { const v = parseFloat(i.value); if (!Number.isNaN(v)) set(v); };
  return i;
}
