...
export class DrawingEngine {
  ...
  constructor(pane, series, opts = {}) {
    ...
    try {
      this.series.addLayer(this.primitive);
    } catch (_) {}
  ...
  /** @returns {Drawing[]} */
  _synced() {
    return this.isolated ? [] : /** @type {Drawing[]} */ (syncStore.forSymbol(this.pane.symbol));
  }
  /** @returns {Drawing[]} */
  allItems() {
    return this.list.concat(this._synced());
  }

  /** @param {(() => void)|null} fn */
  _setRequestUpdate(fn) {
    this._requestUpdate = fn;
  }
  requestUpdate() {
  ...
  syncPriceLabels() {
    if (drawingsHidden()) {
      for (const id of [...this.priceLabelLines.keys()]) this._clearPriceLabels(id);
      return;
    }
    ...
    items.forEach((d) => {
      ...
      const want =
        tool && tool.kind === 'draw' && !d.hidden && visibleOnTf(d, tf) && d.points && d.style && d.style.priceLabels;
      let handles = this.priceLabelLines.get(d.id);
      if (!want) {
        if (handles) {
          handles.forEach((h) => {
            try {
              this.series.removeLevel(h);
            } catch (_) {}
          });
          this.priceLabelLines.delete(d.id);
        }
        return;
      }
      if (!handles) {
        handles = [];
        this.priceLabelLines.set(d.id, handles);
      }
  ...
  _clearPriceLabels(id) {
    ...
    if (handles) {
      ...
      this.priceLabelLines.delete(id);
    }
  }
  ctx() {
    return { pane: this.pane, chart: this.pane.chart, series: this.series };
  }
  ...
  /** @returns {Drawing[]} */
  canvasItems() {
    if (this.suppressed || drawingsHidden()) return [];
    const tf = this.pane.tf();
    const eff = this._effectiveState().hidden;
    return this.allItems()
      .filter((d) => {
        const t = /** @type {Tool|undefined} */ (getTool(d.tool));
        return t && t.kind === 'draw' && !d.hidden && !eff.has(d.id) && visibleOnTf(d, tf);
      })
  ...
  _effectiveState() {
    ...
    if (this.isolated)
      walk(this.getTree(), false, false); // sub-pane: folders only, no layers
  ...
  isLocked(id) {
    const d = this.get(id);
    if (!d) return false;
    return d.locked || this._effectiveState().locked.has(id);
  }
  /** @returns {number} */
  nextZ() {
    return this.allItems().reduce((m, d) => Math.max(m, d.z || 0), 0) + 1;
  }
  ...
  reorder(id, where) {
    ...
    if (where === 'front') d.z = /** @type {number} */ (ordered[n - 1].z) + 1;
      ...
      if (idx < n - 1) {
        const nb = ordered[idx + 1],
          t = d.z;
        d.z = nb.z;
        nb.z = t;
      } else d.z = /** @type {number} */ (d.z) + 1;
  ...
  add(toolId, params) {
    ...
    if (sync === 'none') {
      ...
      syncStore.add(sync, this.pane.symbol, d);
    } // emits sync:changed -> all panes render it
    this._objectsChanged();
  ...
  _render(d) {
    ...
    if (tool && typeof tool.render === 'function') {
      try {
        this.priceLines.set(d.id, tool.render(d, this.ctx()));
      } catch (_) {}
    }
  }
  ...
  liveUpdate(d) {
    if (d && d.sync && d.sync !== 'none') bus.emit('sync:changed');
    else this.requestUpdate();
  }

  /** @param {string} id */
  startSlice(id) {
    if (this.interaction) this.interaction.startSlice(id);
  ...
  objects() {
    return this.allItems();
  } // raw drawings; the object tree provides its own (organizational) order
  ...
  layers() {
    return this.isolated || this.pane.board ? null : syncStore.getLayers(this.pane.symbol);
  }
  activeLayerId() {
    return this.isolated ? null : syncStore.getLayers(this.pane.symbol).active;
  }
  /** @param {string} id */
  setActiveLayer(id) {
    if (!this.isolated) {
      syncStore.setActiveLayer(this.pane.symbol, id);
      this._objectsChanged();
    }
  }
  /** @param {string} [name] */
  addLayer(name) {
    ...
    const ly = syncStore.addLayer(this.pane.symbol, name);
    this._objectsChanged();
    return ly;
  }
  ...
  renameLayer(id, name) {
    if (!this.isolated) {
      syncStore.renameLayer(this.pane.symbol, id, name);
      this._objectsChanged();
    }
  }
  ...
  setLayerFlag(id, flag, val) {
    if (!this.isolated) {
      syncStore.setLayerFlag(this.pane.symbol, id, flag, val);
      this._objectsChanged();
      this.requestUpdate();
    }
  }
  /** @param {string} id */
  removeLayer(id) {
    // removes the layer AND every drawing in it
    if (this.isolated) return;
    const L = syncStore.getLayers(this.pane.symbol);
    const ly = L.list.find((x) => x.id === id);
    if (!ly || L.list.length <= 1) return;
    /** @type {string[]} */
    const ids = [];
    (
      /** @param {any[]} nodes */ function collect(nodes) {
        (nodes || []).forEach((n) => {
  ...
  layersSnapshot() {
    const L = this.layers();
    return L ? { active: L.active, list: L.list } : null;
  }
  /** @param {import('./sync-store.js').LayerSet} L */
  loadLayerSet(L) {
    if (this.isolated || this.pane.board) return;
    syncStore.setLayers(this.pane.symbol, L);
    this._objectsChanged();
    this.requestUpdate();
  }
  saveTree() {
    // persist + re-render + undo boundary
    if (this.isolated) {
    ...
    syncStore.setTree(this.pane.symbol, this.getTree());
    this._objectsChanged();
    bus.emit('drawings:committed', this.pane);
  }
  _objectsChanged() {
    bus.emit('objects:changed', { pane: this.pane });
  }
  ...
  get(id) {
    return this.list.find((d) => d.id === id) || this._synced().find((d) => d.id === id);
  }
  ...
  _hitView() {
    return {
      width: this._plotW || 0,
      height: this._plotH || 0,
      priceDecimals: this.pane.priceDecimals,
  ...
  removeDrawing(id) {
    ...
    if (local) {
      ...
      if (tool && tool.kind !== 'draw') {
        ...
        if (h != null && tool.remove) {
          try {
            tool.remove(h, this.ctx());
          } catch (_) {}
        }
    ...
    if (syncStore.find(id)) {
      // shared drawing — remove everywhere
      syncStore.remove(id);
      this.selection.delete(id);
      if (this.selectedId === id) this.selectedId = [...this.selection].pop() || null;
      this.requestUpdate();
  ...
  clear() {
    // local + synced drawings shown on this pane's symbol
    [...this.list].forEach((d) => this.removeDrawing(d.id));
    syncStore
      .forSymbol(this.pane.symbol)
      .slice()
      .forEach((d) => this.removeDrawing(d.id));
  }
  clearLocal() {
    [...this.list].forEach((d) => this.removeDrawing(d.id));
  } // this pane's own drawings only
  localCount() {
  ...
  persist() {
    if (this.isolated) {
      if (this.store) this.store.save(this.list.map((d) => ({ ...d })));
      this.requestUpdate();
      return;
    }
  ...
  restore() {
    ...
    (this.pane.settings.drawings || []).forEach((/** @type {Drawing} */ d) => {
      if (!d.sync) d.sync = 'none';
      this.list.push(d);
      this._render(d);
    });
    this.requestUpdate();
  ...
  destroy() {
    ...
    this.list.forEach((d) => {
      const t = /** @type {Tool|undefined} */ (getTool(d.tool));
      if (t && t.kind !== 'draw') {
        const h = this.priceLines.get(d.id);
        if (h != null && t.remove) {
          try {
            t.remove(h, this.ctx());
          } catch (_) {}
    ...
    this.priceLabelLines.forEach((hs) =>
      hs.forEach((h) => {
        try {
          this.series.removeLevel(h);
        } catch (_) {}
    ...
    try {
      this.series.removeLayer(this.primitive);
    } catch (_) {}
    this.list = [];
    this._requestUpdate = null;
  }
