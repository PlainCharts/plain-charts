...
export function createDrawingPrimitive(engine) {
  ...
  const viewFor = (scope) => (
    ...
    (engine._plotW = scope.mediaSize.width), (engine._plotH = scope.mediaSize.height),
    {
    width: scope.mediaSize.width, height: scope.mediaSize.height, priceDecimals: engine.pane.priceDecimals,
  ...
  const paint = (c, view, d) => {
    ...
    if (typeof tool.marks === 'function') {
      ...
      if (d.text && engine._editingId !== d.id && (!tool.textEnabled || tool.textEnabled(d))) {
        ...
        if (pts.every(Boolean)) { const dt = typeof tool.drawText === 'function' ? tool.drawText : drawText; dt.call(tool, c, pts, d, view); }
      }
      return;
    }
    ...
    if (d.text && engine._editingId !== d.id && (!tool.textEnabled || tool.textEnabled(d))) { const dt = typeof tool.drawText === 'function' ? tool.drawText : drawText; dt.call(tool, c, pts, d, view); }
  };
  ...
  const hasAlert = (d) => {
    ...
    for (const a of mirror.all()) { if (a && a.objectId === d.id && (!a.symbol || a.symbol === sym)) return true; }
    return false;
  };
    ...
    const renderer = {
      ...
      draw(target) {
        target.useMediaCoordinateSpace((/** @type {DrawScope} */ scope) => {
          ...
          engine.canvasItems().forEach((/** @type {Drawing} */ d) => { if (inPlane(d.z || 0)) paint(c, view, d); });   // painter's order (z asc)
          ...
          (engine.pane.surfaces || []).forEach((/** @type {any} */ s) => {
            ...
            /** @type {Drawing[]} */
            let items; try { items = s.engine.canvasItems(); } catch (_) { items = []; }
            items.forEach((/** @type {Drawing} */ d) => {
              const tool = /** @type {Tool|undefined} */ (getTool(d.tool));
              if (!tool || !tool.spanPanes || d.hidden || !d.points || !d.points.length) return;
              if (typeof tool.marks === 'function') {
                const scope = { timeToX: view.timeToX, priceToY: view.priceToY, width: view.width, height: view.height, snapX: view.snapX };
          ...
          engine.canvasItems().forEach((/** @type {Drawing} */ sel) => {
            ...
            const solid = /** @type {ScreenPoint[]} */ (pts);   // guarded above: none is null
            if (tool.snapToBar) pts = solid.map((p) => ({ ...p, x: snapXToBar(engine.pane, p.x) }));
            drawHandles(c, typeof tool.handles === 'function' ? tool.handles(/** @type {ScreenPoint[]} */ (pts), sel) : /** @type {ScreenPoint[]} */ (pts));
          });
          ...
          if (engine.marqueeRect) {
            ...
            c.save();
            c.fillStyle = 'rgba(41,98,255,0.12)'; c.strokeStyle = 'rgba(41,98,255,0.7)'; c.lineWidth = 1;
          ...
          if (engine.sliceGuide) {
            ...
            c.save(); c.fillStyle = '#ffffff'; c.strokeStyle = '#2962ff'; c.lineWidth = 2;
            c.beginPath(); c.arc(sg.x, sg.y, 6, 0, Math.PI * 2); c.fill(); c.stroke(); c.restore();
          }
          ...
          if (g) {
            ...
            if (tool && typeof tool.marks === 'function') {
              ...
              let pts = /** @type {import('./geometry.js').DataPoint[]} */ (g.points).map((p) => toScreen(engine.pane, p, engine.series));
  ...
  /** @type {any} */ (bottomPlane).isEmpty = () => !engine.canvasItems().some((/** @type {Drawing} */ d) => (d.z || 0) < 0);
  ...
  function drawText(c, pts, d) {
    ...
    const size = ts.size || 14, pad = 5;
    const ha = ts.hAlign || 'center', va = ts.vAlign || 'middle';
    const lines = String(d.text).split('\n'), lh = size * 1.25;
    c.save();
    c.font = (ts.italic ? 'italic ' : '') + (ts.bold ? 'bold ' : '') + size + 'px sans-serif';
    c.fillStyle = ts.color || '#787b86';
    let x;
    if (ha === 'left') { x = x1 + pad; c.textAlign = 'left'; }
    else if (ha === 'right') { x = x2 - pad; c.textAlign = 'right'; }
    else { x = (x1 + x2) / 2; c.textAlign = 'center'; }
    let y0;
    if (va === 'top') { c.textBaseline = 'bottom'; y0 = y1 - pad - (lines.length - 1) * lh; }        // above the top edge
    else if (va === 'bottom') { c.textBaseline = 'top'; y0 = y2 + pad; }                              // below the bottom edge
    else { c.textBaseline = 'middle'; y0 = (y1 + y2) / 2 - (lines.length - 1) * lh / 2; }             // centered inside
  ...
  function computeTextBox(c, d, tool, view) {
    ...
    if (typeof tool.textGeom === 'function') {
      let pts = /** @type {import('./geometry.js').DataPoint[]} */ (d.points).map((p) => toScreen(engine.pane, p, engine.series));
      if (pts.some((p) => !p)) return null;
      if (tool.snapToBar) pts = /** @type {ScreenPoint[]} */ (pts).map((p) => ({ ...p, x: snapXToBar(engine.pane, p.x) }));
      ...
      if (!g) return null;
      const PAD = 4, cos = Math.cos(g.angle), sin = Math.sin(g.angle);
      const lx0 = g.lx0 - PAD, ly0 = g.ly0 - PAD, lx1 = g.lx1 + PAD, ly1 = g.ly1 + PAD;
      const corners = [[lx0, ly0], [lx1, ly0], [lx1, ly1], [lx0, ly1]]
        .map(([lx, ly]) => ({ x: g.cx + lx * cos - ly * sin, y: g.cy + lx * sin + ly * cos }));
      const xs = corners.map((p) => p.x), ys = corners.map((p) => p.y);
      return {
        x: g.cx, yTop: g.cy, ha: g.tAlign, va: g.va, size: g.size, w: g.w, totalH: g.totalH,
        angle: g.angle, cx: g.cx, cy: g.cy, baseline: g.baseline, lx0, ly0, lx1, ly1, editor: g.editor,
        x0: Math.min(...xs), y0: Math.min(...ys), x1: Math.max(...xs), y1: Math.max(...ys),
      };
    }
    const cfg = tool.settings.text; const def = cfg.defaults || {}; const ts = d.textStyle || {};
    const va = ts.vAlign || def.vAlign || 'middle', ha = ts.hAlign || def.hAlign || 'center';
    let pts = /** @type {(ScreenPoint|null)[]} */ (/** @type {import('./geometry.js').DataPoint[]} */ (d.points).map((p) => toScreen(engine.pane, p, engine.series)));
    if (pts.some((p) => !p)) return null;
    if (tool.snapToBar) pts = /** @type {ScreenPoint[]} */ (pts).map((p) => ({ ...p, x: snapXToBar(engine.pane, p.x) }));
    const solid = /** @type {ScreenPoint[]} */ (pts);   // guarded above: none is null
    const xs = solid.map((p) => p.x), ys = solid.map((p) => p.y);
    const x1 = Math.min(...xs), x2 = Math.max(...xs), y1 = Math.min(...ys), y2 = Math.max(...ys);
    const size = ts.size || 14, pad = 5;
  ...
  function drawHandles(c, pts) {
    c.save();
    c.fillStyle = '#ffffff'; c.strokeStyle = '#2962ff'; c.lineWidth = 2;
    pts.forEach((p) => { c.beginPath(); c.arc(p.x, p.y, R, 0, Math.PI * 2); c.fill(); c.stroke(); });
    c.restore();
  }
  ...
  function timeAxisViews() {
    ...
    /** @type {any} */
    let fmt; try { fmt = engine.pane.chart.getConfig().localization.timeFormatter; } catch (_) {}
    /** @type {any[]} */
    const out = [];
    engine.canvasItems().forEach((/** @type {Drawing} */ d) => {
      ...
      d.points.forEach((/** @type {import('./geometry.js').DataPoint} */ p) => {
        ...
        out.push({
          coordinate: () => x, text: () => text,
          textColor: () => '#fff', backColor: () => d.style.color,
  ...
  return {
    ...
    paneViews() { return [bottomPlane, topPlane]; },
    timeAxisViews,
    /** @param {{ requestUpdate: () => void }} p */
    attached(p) { engine._setRequestUpdate(p.requestUpdate); },
    detached() { engine._setRequestUpdate(null); try { alertUnsub(); } catch (_) {} },
  };
}
