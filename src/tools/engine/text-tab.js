...
export function renderTextTab(body, d, preview) {
  ...
  if (!cfg) { body.appendChild(el('div', 'set-soon', 'No text options for this tool.')); return; }
  if (!d.textStyle) {
  ...
  row1.appendChild(colorSwatch(ts.color, (/** @type {any} */ v) => { ts.color = v; up(); }));
  const size = /** @type {HTMLSelectElement} */ (el('select', 'set-fontsize')); [10, 11, 12, 14, 16, 18, 20, 24, 28, 32, 40].forEach((s) => { const o = /** @type {HTMLOptionElement} */ (el('option', null, String(s))); o.value = /** @type {any} */ (s); size.appendChild(o); });
  size.value = /** @type {any} */ (ts.size || 14); size.onchange = () => { ts.size = parseInt(size.value, 10); up(); };
  row1.appendChild(size);
  const bold = /** @type {HTMLButtonElement} */ (el('button', 'set-tbtn' + (ts.bold ? ' on' : ''))); bold.type = 'button'; bold.textContent = 'B'; bold.style.fontWeight = '700';
  bold.onclick = () => { ts.bold = !ts.bold; bold.classList.toggle('on', ts.bold); up(); };
  const ital = /** @type {HTMLButtonElement} */ (el('button', 'set-tbtn' + (ts.italic ? ' on' : ''))); ital.type = 'button'; ital.textContent = 'I'; ital.style.fontStyle = 'italic';
  ital.onclick = () => { ts.italic = !ts.italic; ital.classList.toggle('on', ts.italic); up(); };
  row1.append(bold, ital);
  body.appendChild(row1);

  const ta = /** @type {HTMLTextAreaElement} */ (el('textarea', 'set-text-area')); ta.value = d.text || ''; ta.placeholder = 'Text…';
  ta.oninput = () => { d.text = ta.value; up(); };
  body.appendChild(ta);

  /** @param {any[]} opts @param {any} val @param {(v: any) => void} onset */
  const sel = (opts, val, onset) => { const s = /** @type {HTMLSelectElement} */ (el('select')); (opts || []).forEach((/** @type {any} */ o) => { const op = /** @type {HTMLOptionElement} */ (el('option', null, o.name)); op.value = o.key; s.appendChild(op); }); s.value = val; s.onchange = () => onset(s.value); return s; };
  ...
  const nameOf = (opts, k) => { const o = (opts || []).find((/** @type {any} */ x) => x.key === k); return o ? o.name : k; };
  ...
  const renderGrid = () => {
    ...
    ['top', 'middle', 'bottom'].forEach((v) => ['left', 'center', 'right'].forEach((h) => {
      const cell = /** @type {HTMLButtonElement} */ (el('button', 'set-align-cell')); cell.type = 'button';
      ...
      if (!vKeys.includes(v) || !hKeys.includes(h)) { cell.classList.add('off'); cell.disabled = true; }
        ...
        cell.onclick = () => { ts.vAlign = v; ts.hAlign = h; renderGrid(); up(); };
      }
  ...
  if (cfg.orientation) {
    ...
    orow.appendChild(sel(cfg.orientation, ov, (/** @type {any} */ v) => { ts.orientation = v; up(); }));
