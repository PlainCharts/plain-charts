...
export const marqueeZoomMethods = {
  ...
  _applyMarquee() {
    ...
    const xlo = Math.min(m.x0, m.x1),
      xhi = Math.max(m.x0, m.x1);
    const ylo = Math.min(m.y0, m.y1),
      yhi = Math.max(m.y0, m.y1);
    ...
    this.engine.canvasItems().forEach((d) => {
      ...
      const s = /** @type {ScreenPoint[]} */ (
        d.points
          .map((p) => toScreen(this.pane, /** @type {{ time: number, price: number }} */ (p), this.engine.series))
          .filter(Boolean)
      );
      if (!s.length) return;
      let bx0 = Math.min(...s.map((p) => p.x)),
        bx1 = Math.max(...s.map((p) => p.x));
      let by0 = Math.min(...s.map((p) => p.y)),
        by1 = Math.max(...s.map((p) => p.y));
      ...
      if (tool && tool.timeOnly) {
        by0 = 0;
        by1 = R.height;
      }
      if (tool && tool.priceOnly) {
        bx0 = 0;
        bx1 = R.width;
      }
  ...
  _applyZoom() {
    ...
    if (m && Math.abs(m.x1 - m.x0) > 4 && Math.abs(m.y1 - m.y0) > 4) {
      const c1 = toData(this.pane, m.x0, m.y0),
        c2 = toData(this.pane, m.x1, m.y1);
      if (c1 && c2 && c1.time != null && c2.time != null && c1.price != null && c2.price != null) {
        const from = Math.min(c1.time, c2.time),
          to = Math.max(c1.time, c2.time);
        const lo = Math.min(c1.price, c2.price),
          hi = Math.max(c1.price, c2.price);
        try {
          this.pane.chart.timeAxis().zoomTimeWindow({ from, to });
        } catch (_) {}
        try {
          this.pane.series.priceAxis().configure({ range: [hi, lo] });
        } catch (_) {}
        zoomed = true;
      }
    }
