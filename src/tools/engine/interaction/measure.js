...
export const measureMethods = {
  ...
  _beginMeasure(e) {
    ...
    const mt = /** @type {Tool|undefined} */ (getTool('priceTimeRange'));
    if (!mt) return;
    ...
    this.engine.draft = {
      tool: /** @type {string} */ (mt.id),
      points: [data, { ...data }],
      style: mergeToolStyle(
        /** @type {{ defaultStyle: Record<string, any>, identityStyle?: string[] }} */ (mt),
        (getToolDefaults(mt.id) || {}).style,
      ),
    };
    this.mode = 'measuring';
