...
export const coordsMethods = {
  ...
  _localXY(e) {
    ...
    const off = this._active
      ? this._active.yOffset
        ? this._active.yOffset()
        : /** @type {() => number} */ (this._active.top)()
      : 0;
  ...
  _anchorXY(e, refDataPoint, tool) {
    if (e.shiftKey && tool && tool.shiftConstrain === 'angle' && refDataPoint)
      return this._constrainXY(e, refDataPoint, tool);
    const p = this._localXY(e);
    const m = magnetMode();
