...
function uploadIcon(id, after) {
  ...
  inp.onchange = async () => {
    ...
    try {
      ...
      alert(tr('Could not read that image.'));
      return;
    }
    const folder = fileForTool(id) || id; // the tool's PACKAGE folder (parent) — the icon lands inside it, not a phantom folder
    const r = await fetch('/api/tool-icon', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ folder, id, dataUrl }),
    })
      .then((x) => x.json())
      .catch((e) => ({ error: String(e) }));
    if (r.error) {
      alert(tr('Icon upload failed:') + ' ' + r.error);
      return;
    }
...
const toolOnSave = async (fn, code, api) => {
  ...
  const r = await fetch('/api/user-tools', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name: fn, code }),
  })
    .then((x) => x.json())
    .catch((e) => ({ error: String(e) }));
  if (r.error) {
    api.setCon('✗ ' + r.error, 'err');
    return;
  }
  try {
    await reloadUserToolFile(r.folder);
  } catch (/** @type {any} */ e) {
  ...
  if (!tool) {
    api.setCon('✗ Loaded, but no tool registered.\nDid you call Tools.register({ id, name, ... }) ?', 'err');
    return;
  }
  api.lockName();
...
function writeNewTool() {
  openCodeEditor({
    title: tr('New tool'),
    name: '',
    code: TOOL_TEMPLATE,
    saveLabel: tr('Save tool'),
    onSave: toolOnSave,
  });
}
/** @param {string} file */
async function editTool(file) {
  ...
  const r = await fetch('/api/user-tools/file?name=' + encodeURIComponent(base))
    .then((x) => x.json())
    .catch(() => ({}));
  openCodeEditor({
    title: tr('Edit tool'),
    name: base,
    code: r.code || TOOL_TEMPLATE,
    saveLabel: tr('Save tool'),
    onSave: toolOnSave,
  });
}
...
async function deleteTool(file, id) {
  if (
    !(await confirmDialog({
      title: tr('Delete tool'),
      message:
        tr('Delete the tool') + ` "${id}"? ` + tr('This permanently removes its package folder from packages/tools.'),
      yes: tr('Delete'),
      no: tr('Cancel'),
    }))
  )
    return;
  const base = (file || '').replace(/\.js$/, '');
  await fetch('/api/user-tools/delete', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export function openManager(refresh, FEATURES) {
  ...
  const render = () => {
    ...
    actions.className = 'tool-mgr-actions';
    const write = document.createElement('span');
    write.className = 'tool-mgr-action';
    write.textContent = '✎ ' + tr('Write new tool');
    write.onclick = writeNewTool;
    const fld = document.createElement('span');
    fld.className = 'tool-mgr-action';
    fld.textContent = '⊞ ' + tr('Open folder');
    ...
    const ordered = /** @type {import('./registry.js').ToolDef[]} */ ([
      ...toolbarTools()
    ...
    ordered.forEach((tool) => {
      ...
      row.className = 'tool-mgr-row';
      const chk = document.createElement('span');
      chk.className = 'tool-chk' + (onBar(tool.id) ? ' on' : '');
      chk.textContent = onBar(tool.id) ? '☑' : '☐';
      chk.onclick = () => {
        setOnBar(tool.id, !onBar(tool.id));
        render();
        refreshBar();
      };

      const iconCell = document.createElement('div');
      iconCell.className = 'tool-icon-cell';
      const cur = iconFor(tool.id) || toolIconUrl(tool.id);
      const prev = document.createElement('span');
      prev.className = 'tool-icon-prev';
      if (isImg(cur)) prev.appendChild(themeIcon(/** @type {string} */ (cur), 20));
      else prev.textContent = cur || tool.glyph || '•';
      const upBtn = document.createElement('span');
      upBtn.className = 'tool-ico';
      upBtn.textContent = '🖼';
      upBtn.title = tr('Upload PNG icon');
      upBtn.onclick = () =>
        uploadIcon(tool.id, () => {
          render();
          refreshBar();
      ...
      if (isImg(cur)) {
        ...
        clr.className = 'tool-ico';
        clr.textContent = '✕';
        clr.title = tr('Remove icon');
      ...
      name.className = 'tool-mgr-name';
      name.textContent = tr(tool.name);

      const hk = hotkeyCell(tool.id, () => {
        render();
        refreshBar();
      });

      // on-bar tools reorder by drag (off-bar tools have no position on the bar)
      const grip = el2('span', 'tool-mgr-grip', '⠿');
      grip.title = tr('Drag to reorder');
      if (onBar(tool.id))
        wireRowDrag(
          row,
          tool.id,
      ...
      if (file) {
        ...
        edit.className = 'tool-ico';
        edit.textContent = '✎';
        edit.title = tr('Edit code');
        edit.onclick = () => editTool(file);
        const del = document.createElement('span');
        del.className = 'tool-ico';
        del.textContent = '✕';
        del.title = tr('Delete tool');
    ...
    featureOrder().forEach((id) => {
      ...
      const grip = el2('span', 'tool-mgr-grip', '⠿');
      grip.title = tr('Drag to reorder');
      const noChk = el2('span', 'tool-chk');
      noChk.style.visibility = 'hidden'; // no add/remove for features
      const iconCell = document.createElement('div');
      iconCell.className = 'tool-icon-cell';
      const prev = el2('span', 'tool-icon-prev');
      prev.appendChild(themeIcon(f.icon, 20));
      iconCell.appendChild(prev);
