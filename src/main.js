...
async function start() {
  try {
    ...
    log('Init failed: ' + (/** @type {any} */ (e).message || e), true);
    throw e;
  }
  ...
  if (auto.length) {
    auto.forEach((acct) => { log('Auto-connecting to ' + acct.name + '…'); broker.connect(acct); });
    return;
  }
  ...
  if (getSetting('autoConnect')) {
    ...
    if (acct) { log('Auto-connecting to ' + acct.name + '…'); broker.connect(acct); return; }
  }
...
function initChartOrderButton() {
  ...
  if (!d || !d.openOrderTicket) { btn.style.display = 'none'; return; }
  btn.onclick = () => { const p = /** @type {any} */ (getActivePane()); d.openOrderTicket(p && p.symbol ? { symbol: p.symbol, broker: p.broker } : {}); };
}
...
function initChartProjectToggle() {
  ...
  if (!btn) return;
  const ctx = () => { const p = /** @type {any} */ (getActivePane()); return p && p.symbol ? { b: p.broker || '', s: p.symbol } : null;  };
  const sync = () => { const c = ctx(); btn.classList.toggle('active', !!(c && isProjecting(c.b, c.s))); };
  btn.onclick = () => { const c = ctx(); if (!c) return; setProjecting(c.b, c.s, !isProjecting(c.b, c.s)); sync(); };
