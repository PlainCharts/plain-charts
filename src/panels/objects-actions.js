...
/** @param {string} id */
export function startRename(id) { closeMenu(); state.renamingId = id; render(); }
...
export function renameInput(id, value, commitFn) {
  ...
  inp.value = value;
  const commit = () => { const v = inp.value.trim(); state.renamingId = null; commitFn(v); };
  inp.onkeydown = (ev) => { if (ev.key === 'Enter') { ev.preventDefault(); commit(); } else if (ev.key === 'Escape') { state.renamingId = null; render(); } };
...
export function removeSelection() {
  ...
  const byEng = new Map();
  [...state.selectedIds].forEach((id) => { const e = engineOf(id); if (!e) return; if (!byEng.has(e)) byEng.set(e, []); /** @type {string[]} */ (byEng.get(e)).push(id); });
  byEng.forEach((ids, e) => {
    ids.forEach((id) => { const r = findNode(e.getTree(), id); if (r && r.node.type === 'folder') removeFolderDeep(e, r.node); else e.removeDrawing(id); });
    e.saveTree();
  });
...
export function removeFolderDeep(e, folder) {
  [...(folder.children || [])].forEach((n) => { if (n.type === 'folder') removeFolderDeep(e, n); else e.removeDrawing(n.id); });   // copy: removeDrawing may prune children
...
export function createFolder(ids, e) {
  ids = ids || [];
  e = e || engineOf(ids[0]) || eng(); if (!e) return;
  ...
  state.selectedIds = new Set([fid]); state.anchorId = fid;
  e.saveTree();
  startRename(fid);
}
/** @param {string} id @param {Engine=} e */
export function deleteFolder(id, e) {            // remove the folder, move its contents up in place
  e = e || engineOf(id); const tree = e.getTree();
  const r = findNode(tree, id); if (!r || r.node.type !== 'folder') return;
