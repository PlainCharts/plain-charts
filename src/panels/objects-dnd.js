...
export function clearDrop() {
  if (state.listEl)
    state.listEl
      ...
      .forEach((r) => r.classList.remove('drop-into', 'drop-before', 'drop-after'));
}
...
export function rowDnd(e, row, id, isFolder) {
  ...
  row.dataset.id = id;
  row._eng = e;
  row.ondragstart = (/** @type {DragEvent} */ ev) => {
    state.dragId = id;
    state.dragEngine = e;
    /** @type {DataTransfer} */ (ev.dataTransfer).effectAllowed = 'move';
    try {
      /** @type {DataTransfer} */ (ev.dataTransfer).setData('text/plain', id);
    } catch (_) {}
  ...
  row.ondragover = (/** @type {DragEvent} */ ev) => {
    ...
    if (isFolder && y > h * 0.25 && y < h * 0.75) {
      state.dropTarget = { id, mode: 'into' };
      row.classList.add('drop-into');
    } else if (y < h / 2) {
      state.dropTarget = { id, mode: 'before' };
      row.classList.add('drop-before');
    } else {
      state.dropTarget = { id, mode: 'after' };
      row.classList.add('drop-after');
    }
  };
  row.ondrop = (/** @type {DragEvent} */ ev) => {
    ev.preventDefault();
    ev.stopPropagation();
    doDrop();
  };
...
export function doDrop() {
  ...
  ids = ids.filter(
    (id) =>
      !ids.some((o) => {
        ...
        const r = findNode(tree, o);
        return r && r.node.type === 'folder' && inSubtree(r.node, id);
      }),
  );
  ...
  if (
    t &&
    ids.some((id) => {
      const n = (findNode(tree, id) || {}).node;
      return n && n.type === 'folder' && inSubtree(n, t.id);
    })
  )
    return;
  ...
  if (!t) {
    ...
    if (!f || f.type !== 'folder')
      tree.push(...nodes); // fallback → root end
    else {
      (f.children || (f.children = [])).unshift(...nodes);
      f.expanded = true;
    }
  } else {
    const loc = findNode(tree, t.id);
    if (!loc) tree.push(...nodes);
    else loc.list.splice(t.mode === 'after' ? loc.index + 1 : loc.index, 0, ...nodes);
  }
  e.saveTree();
}
