...
const activeOff = () => {
  ...
  return p ? p.tzOffset() : 0;
};

// corner-bracket icons (plain SVG, currentColor so they follow the theme)
const SVG_MAX =
  '<svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2 6V2h4M14 6V2h-4M2 10v4h4M14 10v4h-4"/></svg>';
const SVG_RESTORE =
  '<svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2v4H2M10 2v4h4M6 14v-4H2M10 14v-4h4"/></svg>';
...
/** @param {number} offMin */
const offLabel = (offMin) => {
  const h = offMin / 60;
  return `UTC${h >= 0 ? '+' : '-'}${Number.isInteger(h) ? Math.abs(h) : Math.abs(h).toFixed(1)}`;
};
...
export function initBottomBar() {
  ...
  clock.className = 'bb-clock';
  const times = document.createElement('div');
  times.className = 'bb-times';
  const chip = document.createElement('button');
  chip.className = 'bb-tz';
  clock.append(times, chip);

  // maximize / restore the active chart pane, on the far right
  const maxBtn = document.createElement('button');
  maxBtn.className = 'bb-max';
  /** @param {boolean} maxed */
  const paintMax = (maxed) => {
    maxBtn.innerHTML = maxed ? SVG_RESTORE : SVG_MAX;
    maxBtn.title = maxed ? 'Restore chart' : 'Maximize chart';
    maxBtn.classList.toggle('active', !!maxed);
  };
  maxBtn.onclick = () => {
    const p = getActivePane();
    if (p) bus.emit('pane:maximize', p);
  };
  ...
  document.addEventListener('click', (e) => {
    ...
    if (popover && !popover.contains(t) && t !== chip) popover.classList.remove('open');
  });
...
function openTz(anchor) {
  ...
  h.className = 'tz-head';
  h.textContent = 'Time zone — UTC offset (hours)';

  const row = document.createElement('div');
  row.className = 'tz-input';
  ...
  /** @param {number} v */
  const setH = (v) => {
    v = Math.max(-12, Math.min(14, v));
    setTz(v * 60);
    inp.value = /** @type {any} */ (v);
  };
  inp.onchange = () => {
  ...
  const mk = (label, dir) => {
    ...
    b.onclick = (e) => {
      e.stopPropagation();
      setH(Math.round(activeOff() / 60) + dir);
    };
    return b;
  };
  ...
  const preset = (label, min) => {
    ...
    b.onclick = (e) => {
      e.stopPropagation();
      setTz(min);
      inp.value = /** @type {any} */ (Math.round(min / 60));
    };
    return b;
  };
