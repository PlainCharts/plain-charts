...
import { loadAddonBar, iconFor, setIcon, hotkeyFor, setHotkey, orderedIds, placeAddon, fileToIcon } from '../addons/toolbar-store.js';
...
const postJSON = (u, b) => fetch(u, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(b) }).then((r) => r.json()).catch(() => ({}));
/**
 * @param {string} tag
...
export async function initAddons() {
  ...
  bus.on('vocab:changed', () => { applyAddonLocale(); renderAddonRail(); if (mgrOpen && listEl) renderManager(); });
...
async function restoreDocked() {
  const state = getSetting('dockedAddons'); if (!state || typeof state !== 'object') return;
  restoring = true;
  try {
    for (const [id, shown] of Object.entries(state)) {
      const a = cache.find((x) => x.id === id && x.enabled);
      if (a && !panels.has(id)) { try { await openAddonUI(a, null, shown !== false); } catch (_) {} }
    }
  } finally { restoring = false; renderAddonRail(); }
}

async function refreshCache() {
  const d = await getJSON('/api/addons'); cache = d.addons || [];
  applyAddonLocale();
}
...
function applyAddonLocale() {
  ...
  const merged = {};
  if (code) for (const a of cache) { const w = a.enabled && a.locales && a.locales[code]; if (w) Object.assign(merged, w); }
  setAddonWords(merged);
}

// ---------------------------------------------------------------- right-rail icons
function ensureAddonRail() {
  const rail = $('rightrail'); if (!rail) return null;
  let sp = rail.querySelector('.rail-spacer');
  if (!sp) { sp = el('div', 'rail-spacer'); rail.appendChild(sp); }   // pin our icons to the bottom cluster
...
function renderAddonRail() {
  const wrap = ensureAddonRail(); if (!wrap) return;
  wrap.innerHTML = '';
  const enabled = cache.filter((a) => a.enabled);
  orderedIds(enabled.map((a) => a.id)).forEach((id) => {
    const a = enabled.find((x) => x.id === id); if (!a) return;
    ...
    paintIcon(btn, a);
    btn.onclick = (e) => { e.stopPropagation(); openAddonUI(a, btn); };
...
function paintIcon(node, a) {
  ...
  else { const s = el('span', 'glyph-ico', badgeText(a.name)); s.style.cssText = 'font-weight:600;font-size:15px;'; node.appendChild(s); }
}
...
function comboOf(e) {
  /** @type {string[]} */
  const p = [];
  if (e.ctrlKey) p.push('Ctrl'); if (e.metaKey) p.push('Meta'); if (e.altKey) p.push('Alt'); if (e.shiftKey) p.push('Shift');
...
function onHotkey(e) {
  ...
  if (a) { e.preventDefault(); e.stopPropagation(); openAddonUI(a); }
}
...
function openAddons() {
  ...
  const dlg = el('div', 'dialog addons-mgr'); panel = dlg; dlg.style.zIndex = '60';
  dlg.style.width = '600px'; dlg.style.maxWidth = '94vw';
  const head = el('div', 'lib-head');
  const x = el('span', 'lib-x', '✕'); x.onclick = closeAddons;
  head.append(Object.assign(document.createElement('h3'), { textContent: t('Addons') }), x);
  const list = el('div', 'tool-mgr-list addon-mgr-list'); listEl = list;
  dlg.append(head, list); document.body.appendChild(dlg);
...
function closeAddons() {
  ...
  mgrOpen = false; listEl = null;
}
async function refresh() {
  await refreshCache();
  for (const aid of [...panels.keys()]) { const a = cache.find((x) => x.id === aid); if (!a || !a.enabled) destroyPanel(aid); }   // disabled/removed -> drop its docked panel
...
/** @type {string | null} */
let dragAddon = null;
function clearAddonDropMarks() { if (listEl) listEl.querySelectorAll('.tool-mgr-drop-above, .tool-mgr-drop-below').forEach((e) => e.classList.remove('tool-mgr-drop-above', 'tool-mgr-drop-below')); }
/** @param {HTMLElement} row @param {string} id @param {string[]} allIds */
function wireAddonRowDrag(row, id, allIds) {
  row.draggable = true;
  row.ondragstart = (e) => { dragAddon = id; row.classList.add('tool-mgr-dragging'); try { const dt = /** @type {DataTransfer} */ (e.dataTransfer); dt.effectAllowed = 'move'; dt.setData('text/plain', id); } catch (_) {} };
  row.ondragend = () => { dragAddon = null; row.classList.remove('tool-mgr-dragging'); clearAddonDropMarks(); };
  row.ondragover = (e) => { if (!dragAddon || dragAddon === id) return; e.preventDefault(); const r = row.getBoundingClientRect(); const below = e.clientY > r.top + r.height / 2; row.classList.toggle('tool-mgr-drop-below', below); row.classList.toggle('tool-mgr-drop-above', !below); };
  row.ondragleave = () => row.classList.remove('tool-mgr-drop-above', 'tool-mgr-drop-below');
  row.ondrop = (e) => { e.preventDefault(); const after = row.classList.contains('tool-mgr-drop-below'); clearAddonDropMarks(); if (!dragAddon || dragAddon === id) return; placeAddon(dragAddon, id, after, allIds); dragAddon = null; renderManager(); renderAddonRail(); };
}
...
function renderManager() {
  ...
  const write = el('span', 'tool-mgr-action', '✎ ' + t('Write new addon')); write.onclick = writeNewAddon;
  ...
  acts.append(write, fld); listEl.appendChild(acts);
  listEl.appendChild(el('div', 'tool-mgr-sep'));

  if (!cache.length) { listEl.appendChild(el('div', 'addon-empty', t('No addons yet — write one or import a .js file.'))); return; }
  ...
  orderedIds(allIds).forEach((id) => {
    ...
    const grip = el('span', 'tool-mgr-grip', '⠿'); grip.title = t('Drag to reorder');

    const chk = el('span', 'tool-chk' + (a.enabled ? ' on' : ''), a.enabled ? '☑' : '☐');
    chk.title = a.enabled ? t('Disable') : t('Enable');
    chk.onclick = async () => { await postJSON('/api/addons/toggle', { id: a.id, enabled: !a.enabled }); await refresh(); };
    ...
    if (ic) { const clr = el('span', 'tool-ico', '✕'); clr.title = t('Remove icon'); clr.onclick = async () => { setIcon(a.id, ''); await fetch('/api/addon-icon?id=' + encodeURIComponent(a.id), { method: 'DELETE' }).catch(() => {}); await refresh(); }; iconCell.append(clr); }

    const name = el('span', 'tool-mgr-name', a.name);
    const hk = hotkeyCell(a.id);

    const open = el('span', 'tool-ico', '⧉'); open.title = t('Open UI'); open.onclick = () => openAddonUI(a);
    const edit = el('span', 'tool-ico', '✎'); edit.title = t('Edit code'); edit.onclick = () => editAddon(a.id);
    const reload = el('span', 'tool-ico', '⟳'); reload.title = t('Reload'); reload.onclick = async () => { await postJSON('/api/addons/reload', { id: a.id }); await refresh(); };
    const del = el('span', 'tool-ico', '🗑'); del.title = t('Delete'); del.onclick = async () => { if (confirm(t('Delete addon "{n}"? This removes its folder.').replace('{n}', a.name))) { await postJSON('/api/addons/delete', { id: a.id }); await refresh(); } };
...
function pickIcon(id) {
  ...
  inp.onchange = () => { const f = inp.files && inp.files[0]; if (!f) return; fileToIcon(f, async (/** @type {string|null} */ url) => {
    if (!url) return;
    const r = await postJSON('/api/addon-icon', { id, dataUrl: url });
    if (r && r.error) { alert(t('Icon upload failed:') + ' ' + r.error); return; }
...
function hotkeyCell(id) {
  ...
  cell.onclick = (e) => {
    ...
    cell.classList.remove('empty'); cell.classList.add('rec'); cell.textContent = t('Press keys…');
    /** @param {KeyboardEvent} ev */
    const onKey = (ev) => {
      ev.preventDefault(); ev.stopPropagation();
      if (ev.key === 'Escape') { done(false); return; }
      if (ev.key === 'Backspace' || ev.key === 'Delete') { setHotkey(id, ''); done(true); return; }
      if (['Control', 'Alt', 'Shift', 'Meta'].includes(ev.key)) return;
      if (!(ev.ctrlKey || ev.metaKey || ev.altKey)) { cell.textContent = t('Use Ctrl/Alt + key'); return; }
      setHotkey(id, comboOf(ev)); done(true);
    };
    /** @param {boolean} changed */
    const done = (changed) => {
      document.removeEventListener('keydown', onKey, true); hkRecording = null;
      if (changed) { renderManager(); renderAddonRail(); return; }
      cell.classList.remove('rec'); cell.classList.toggle('empty', !cur); cell.textContent = cur || t('Set hotkey');
    };
...
async function openAddonUI(a, btn, show = true) {
  const id = 'addon:' + a.id;
  if (popupOpen && popupOpen.id === a.id) { closePopup(); renderAddonRail(); return; }   // popup open -> toggle off
  if (panels.has(a.id)) { rp.toggle(id); persistDocked(); renderAddonRail(); return; }   // docked -> show/hide

  const r = await getJSON('/api/addons/file?id=' + encodeURIComponent(a.id));
  /** @type {any} */
  const mod = evalAddon(r.code || '');
  if (mod && mod.popup) { openAddonPopup(a, mod, btn); renderAddonRail(); return; }   // dropdown mode

  /** @type {Array<() => void>} */
  const closeCbs = [];
  const content = el('div', 'addon-panel');
  const head = el('div', 'addon-panel-head');
  const x = el('span', 'lib-x', '✕'); x.onclick = () => { rp.toggle(id, false); persistDocked(); renderAddonRail(); };
  ...
  const api = {
    ...
    log: (...m) => { console.log('[addon:' + a.id + ']', ...m); platform.console.post({ cat: 'addon', src: a.id, msg: m.map((x) => typeof x === 'string' ? x : JSON.stringify(x)).join(' ') }); },
    save: async (cfg) => { await postJSON('/api/addons/config', { id: a.id, config: cfg }); await refresh(); },
    close: () => { rp.toggle(id, false); persistDocked(); renderAddonRail(); },
    onClose: (fn) => closeCbs.push(fn),
    t, localizeDom, registerVocab,   // translation capability: look up words, localize a subtree, ship own defaults
  };

  const uiFn = mod && (mod.ui || mod.settings);
  if (typeof uiFn === 'function') {
    try { uiFn(root, api); } catch (e) { root.textContent = 'ui() error: ' + ((e && /** @type {any} */ (e).message) || e); }
  } else if (a.inputs && a.inputs.length) {
    root.appendChild(buildForm(a, api));
  } else {
    root.className += ' addon-set-empty'; root.textContent = t('This addon has no UI. Define exports.ui(root, api) to build your own.');
  }
...
function destroyPanel(addonId) {
  ...
  if (!p) return;
  p.closeCbs.forEach((f) => { try { f(); } catch (_) {} });
...
function openAddonPopup(a, mod, btn) {
  ...
  menu.style.cssText = 'position:fixed;z-index:6000;background:var(--panel);color:var(--tx);border:1px solid var(--bd);border-radius:8px;box-shadow:0 8px 30px rgba(0,0,0,.45);min-width:180px;max-width:320px;padding:8px;';
  const head = el('div', null, a.name); head.style.cssText = 'font-size:11px;color:var(--tx-dim);letter-spacing:.04em;margin:0 2px 7px;';
  const root = el('div'); root.style.cssText = 'display:block;';
  menu.append(head, root); document.body.appendChild(menu);

  /** @type {AddonApi} */
  const api = {
    id: a.id, name: a.name, config: { ...(a.config || {}) }, data: broker,
    ...
    log: (...m) => { console.log('[addon:' + a.id + ']', ...m); platform.console.post({ cat: 'addon', src: a.id, msg: m.map((x) => typeof x === 'string' ? x : JSON.stringify(x)).join(' ') }); },
    save: async (cfg) => { await postJSON('/api/addons/config', { id: a.id, config: cfg }); await refresh(); },
    close: () => { closePopup(); renderAddonRail(); },
    onClose: (fn) => closeCbs.push(fn),
    t, localizeDom, registerVocab,   // translation capability: look up words, localize a subtree, ship own defaults
  };
  const uiFn = mod && (mod.ui || mod.settings);
  if (typeof uiFn === 'function') { try { uiFn(root, api); } catch (e) { root.textContent = 'ui() error: ' + ((e && /** @type {any} */ (e).message) || e); } }
  ...
  const rb = btn ? btn.getBoundingClientRect() : { left: window.innerWidth - 54, right: window.innerWidth - 16, top: 80 };
  let left = rb.left - menu.offsetWidth - 8; if (left < 8) left = rb.right + 8;
  menu.style.left = left + 'px';
  menu.style.top = Math.min(rb.top, window.innerHeight - menu.offsetHeight - 8) + 'px';

  const away = (/** @type {PointerEvent} */ e) => { const tgt = /** @type {Node} */ (e.target); if (menu && !menu.contains(tgt) && (!btn || (tgt !== btn && !btn.contains(tgt)))) { closePopup(); renderAddonRail(); } };
  setTimeout(() => document.addEventListener('pointerdown', away, true), 0);
  popupOpen = { id: a.id, menu, closeCbs, away };
}
function closePopup() {
  if (!popupOpen) return;
  popupOpen.closeCbs.forEach((f) => { try { f(); } catch (_) {} });
  if (popupOpen.away) document.removeEventListener('pointerdown', popupOpen.away, true);
  try { popupOpen.menu.remove(); } catch (_) {}
  popupOpen = null;
}
...
function evalAddon(code) {
  ...
  const shim = () => stub;
  try { new Function('module', 'exports', 'require', code)(m, m.exports, shim); }
  catch (e) { return { __error: String((e && /** @type {any} */ (e).message) || e) }; }
  return m.exports;
}
...
function buildForm(a, api) {
  ...
  /** @type {any[]} */ (a.inputs).forEach((inp) => {
    ...
    /** @type {any} */
    let ctl;
    if (ty === 'bool') { ctl = document.createElement('input'); ctl.type = 'checkbox'; ctl.checked = !!cfg[inp.key]; ctl.onchange = () => { cfg[inp.key] = ctl.checked; }; }
    else if (ty === 'number') { ctl = document.createElement('input'); ctl.type = 'number'; ctl.value = cfg[inp.key] != null ? cfg[inp.key] : ''; ctl.onchange = () => { cfg[inp.key] = ctl.value === '' ? null : parseFloat(ctl.value); }; }
    else if (ty === 'color') { ctl = document.createElement('input'); ctl.type = 'color'; ctl.value = cfg[inp.key] || '#888888'; ctl.onchange = () => { cfg[inp.key] = ctl.value; }; }
    else if (ty === 'select') { ctl = document.createElement('select'); (inp.options || []).forEach((/** @type {any} */ o) => { const v = (o && o.value != null) ? o.value : o; const op = document.createElement('option'); op.value = v; op.textContent = t((o && o.label) || v); ctl.appendChild(op); }); ctl.value = cfg[inp.key]; ctl.onchange = () => { cfg[inp.key] = ctl.value; }; }
    else { ctl = document.createElement('input'); ctl.type = 'text'; ctl.value = cfg[inp.key] != null ? cfg[inp.key] : ''; ctl.onchange = () => { cfg[inp.key] = ctl.value; }; }
    ctl.className = 'addon-set-ctl';
    row.append(lbl, ctl); sec.appendChild(row);
  });
  const save = el('button', 'addon-reload addon-set-save', t('Save settings'));
  save.onclick = async () => { save.textContent = t('Saving…'); await api.save(cfg); save.textContent = t('Saved'); };
...
const addonOnSave = async (fn, code, eapi) => {
  ...
  if (!r.ok) { eapi.setCon('✗ ' + (r.error || 'save failed'), 'err'); return; }
  eapi.lockName();
...
async function editAddon(id) {
  ...
  openCodeEditor({ title: 'Edit addon', name: id, code: r.code || ADDON_TEMPLATE, saveLabel: 'Save addon', onSave: addonOnSave });
}
