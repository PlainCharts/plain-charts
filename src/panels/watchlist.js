...
export async function initWatchlist() {
  ...
  const closeX = el('span', 'lib-x', '✕');
  closeX.onclick = () => rp.toggle('watchlist', false);
  head.append(listBtn, closeX);
  const bar = el('div', 'obj-bar');
  const add = el('button', 'obj-bar-btn', '＋');
  add.title = t('Add symbol');
  add.onclick = () =>
    openSymbolSearch((/** @type {string} */ brokerId, /** @type {string} */ symbol) => addItem(brokerId, symbol));
  const sec = el('button', 'obj-bar-btn', '＋☰');
  sec.title = t('New section');
  sec.onclick = () => addSection();
  const colBtn = el('button', 'obj-bar-btn', '⋯');
  colBtn.title = t('Customize columns');
  ...
  rp.addView({
    id: 'watchlist',
    icon: themeIcon('/images/menu.png', 18),
    title: t('Watchlist'),
    content: view,
    width: 300,
  });
  ...
  bus.on('vocab:changed', () => {
    renderListBtn();
    renderHeader();
    if (rp.isShown('watchlist')) render();
  ...
  renderListBtn();
  renderHeader();
  if (rp.isShown('watchlist')) {
    render();
    startAll();
  }
  engineBus.on('logon', () => {
    if (rp.isShown('watchlist')) startAll();
  }); // a broker connected → pull data
}
...
function normItems(arr) {
  return /** @type {WlItem[]} */ (
    (Array.isArray(arr) ? arr : [])
      .map((/** @type {any} */ e) => {
        if (e && e.type === 'section')
          return { type: 'section', id: e.id || newId(), name: e.name || 'Section', collapsed: !!e.collapsed };
...
function normList(l) {
  return l && l.id ? { id: l.id, name: l.name || 'Watchlist', items: normItems(l.items) } : null;
}
...
function removeSection(id) {
  // drop the header only — its symbols stay (uncategorized / merge up)
...
function addSectionBefore(it) {
  ...
  /** @type {SectionItem} */
  const s = { type: 'section', id: newId(), name: 'New section', collapsed: false };
  if (idx < 0) state.items.push(s);
  else state.items.splice(idx, 0, s);
  persist();
  startRename(s.id);
}
...
function closeWlMenu() {
  if (wlMenuAway) {
    document.removeEventListener('pointerdown', /** @type {any} */ (wlMenuAway), true);
    wlMenuAway = null;
  }
  if (wlMenu) {
    wlMenu.remove();
...
function wlMenuAt(x, y, build) {
  ...
  const item = (label, fn) => {
    ...
    r.append(el('span', 'dwg-check', ''), el('span', 'dwg-label', t(label)));
    r.onclick = () => {
      closeWlMenu();
      fn();
    };
  ...
  wlMenuAway = (e) => {
    if (wlMenu && !wlMenu.contains(/** @type {Node} */ (e.target))) closeWlMenu();
  };
...
function openSectionMenu(s, x, y) {
  wlMenuAt(x, y, (m, item) =>
    m.append(
      ...
      el('div', 'dwg-div'),
      item('Add symbol', () =>
        openSymbolSearch((/** @type {string} */ b, /** @type {string} */ sym) => addItemToSection(s.id, b, sym)),
      ),
    ),
  );
}
...
function openSymbolMenu(it, x, y) {
  wlMenuAt(x, y, (m, item) =>
    m.append(
      ...
      item('Add symbol', () =>
        openSymbolSearch((/** @type {string} */ b, /** @type {string} */ sym) => addItem(b, sym)),
      ),
      el('div', 'dwg-div'),
...
function clearDrop() {
  state.listEl
    .querySelectorAll('.wl-drop-before,.wl-drop-after')
    .forEach((r) => r.classList.remove('wl-drop-before', 'wl-drop-after'));
  dropKey = null;
  dropPos = null;
}
...
function rowDnd(row, key) {
  ...
  row.ondragstart = (e) => {
    ...
    try {
      /** @type {DataTransfer} */ (e.dataTransfer).setData('text/plain', key);
    } catch (_) {}
    row.classList.add('wl-drag');
  };
  row.ondragend = () => {
    row.classList.remove('wl-drag');
    clearDrop();
    dragKey = null;
  };
  row.ondragover = (e) => {
    ...
    dropKey = key;
    dropPos = e.clientY - r.top > r.height / 2 ? 'after' : 'before';
...
function doDrop() {
  ...
  if (to < 0) {
    state.items.splice(from, 0, ...moved);
    persist();
    render();
    return;
  } // target was inside the moved span
...
function render() {
  ...
  if (!state.items.length) {
    state.listEl.appendChild(el('div', 'wl-empty', t('No symbols yet — click ＋ to add, ＋☰ for a section.')));
    return;
  }
  let collapsed = false;
  ...
  displayItems().forEach((e) => {
    // sorted-for-display when a column sort is active; the stored order is untouched
    const key = keyOf(e);
    if (e.type === 'section') {
      collapsed = !!e.collapsed;
      state.listEl.appendChild(
        buildSection(e, key, (inp) => {
          toFocus = inp;
        }),
      );
      return;
  ...
  if (toFocus) {
    /** @type {HTMLInputElement} */ (toFocus).focus();
    /** @type {HTMLInputElement} */ (toFocus).select();
  }
}
...
function buildSection(s, key, setFocus) {
  ...
  row.appendChild(tw);
  if (s.id === renamingId) {
    const inp = /** @type {HTMLInputElement} */ (el('input', 'wl-sec-input'));
    inp.value = s.name;
    const commit = () => {
    ...
    const del = el('span', 'wl-sec-del', '✕');
    del.title = t('Remove section (keeps its symbols)');
    del.onclick = (e) => {
      e.stopPropagation();
      removeSection(s.id);
    };
    ...
    row.oncontextmenu = (e) => {
      e.preventDefault();
      openSectionMenu(s, e.clientX, e.clientY);
    };
...
function buildSymbolRow(it, key) {
  ...
  row.oncontextmenu = (e) => {
    e.preventDefault();
    openSymbolMenu(it, e.clientX, e.clientY);
  };
