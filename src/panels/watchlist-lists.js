...
export function renderListBtn() {
  const L = activeList();
  if (state.listBtnName) state.listBtnName.textContent = (L && L.name) || 'Watchlist';
}
...
function switchList(id) {
  ...
  if (cur) cur.items = state.items; // flush any reassigned items back before swapping
  state.activeId = id;
...
function createList(name) {
  stopAll();
  const cur = activeList();
  if (cur) cur.items = state.items;
  const id = newListId();
  state.lists.push({ id, name: name || 'List ' + (state.lists.length + 1), items: [] });
  state.activeId = id;
  state.items = state.lists[state.lists.length - 1].items;
  persist();
  renderListBtn();
  render();
  startAll();
...
function removeList(id) {
  ...
  if (wasActive) {
    ...
    state.items = state.lists[0].items;
    render();
    startAll();
  }
  persist();
...
function closeListMenu() {
  if (listMenuAway) {
    document.removeEventListener('pointerdown', /** @type {any} */ (listMenuAway), true);
    listMenuAway = null;
  }
  if (listMenu) {
    listMenu.remove();
...
export function toggleListMenu(anchor) {
  if (listMenu) closeListMenu();
  else openListMenu(anchor);
}
/** @param {HTMLElement} anchor */
function openListMenu(anchor) {
  ...
  const rebuild = () => {
    ...
    state.lists.forEach((l) => {
      ...
      const al = el('button', 'wl-listmenu-edit wl-listmenu-alert');
      al.innerHTML = SVG_BELL;
      al.title = t('Add alert on the list…');
      al.onclick = (e) => {
        e.stopPropagation();
        closeListMenu();
        openWatchlistAlertDialog({ id: l.id, name: l.name }, getActivePane());
      };
      const ed = el('button', 'wl-listmenu-edit', '✎');
      ed.title = t('Rename watchlist');
      ed.onclick = (e) => {
        e.stopPropagation();
        startListRename(r, l, rebuild);
      };
      const cp = el('button', 'wl-listmenu-copy', '⧉');
      cp.title = t('Copy watchlist');
      cp.onclick = (e) => {
        e.stopPropagation();
        copyList(l.id);
        rebuild();
      };
      const x = el('button', 'wl-listmenu-x', '×');
      x.title = t('Remove watchlist');
      x.onclick = async (e) => {
        e.stopPropagation();
        const ok = await confirmDialog({
          title: t('Delete this watchlist?'),
          message: t('Doing this will permanently delete your watchlist') + ` "${l.name}".`,
          yes: t('Delete'),
          no: t('Cancel'),
        });
        if (!ok) return;
    ...
    const inp = /** @type {HTMLInputElement} */ (el('input', 'wl-listmenu-input'));
    inp.placeholder = t('New list…');
  ...
  m.style.left = rc.left + 'px';
  m.style.top = rc.bottom + 4 + 'px';
  m.style.minWidth = rc.width + 'px';
  listMenuAway = (e) => {
    if (
      listMenu &&
      !listMenu.contains(/** @type {Node} */ (e.target)) &&
...
function startListRename(row, l, rebuild) {
  ...
  const finish = (save) => {
    ...
    if (save) renameList(l.id, inp.value.trim() || l.name);
    rebuild();
  };
  inp.onkeydown = (e) => {
