...
function registerEngine(e) {
  ...
  const walk = (nodes) => (nodes || []).forEach((/** @type {TreeNode} */ n) => { state.engineById.set(n.id, e); if (n.type === 'folder') walk(n.children); });
...
function reconcile(e) {
  ...
  if (e.isolated) {
    ...
    getAllPanes().forEach((/** @type {Pane} */ p) => { if (p.symbol === sym && p.drawings) p.drawings.objects().forEach((/** @type {Drawing} */ d) => ids.add(d.id)); });
  }
  /** @type {Set<string>} */
  ...
  const prune = (nodes) => {
    for (let i = nodes.length - 1; i >= 0; i--) {
      const n = nodes[i];
      if (n.type === 'folder') { prune(n.children || (n.children = [])); }
      else if (ids.has(n.id)) present.add(n.id);
      else { nodes.splice(i, 1); changed = true; }
    }
  };
  ...
  e.objects().forEach((/** @type {Drawing} */ d) => { if (!present.has(d.id)) { active.unshift({ type: 'drawing', id: d.id }); changed = true; } });
...
export function initObjects() {
  ...
  const titleSpan = el('span', null, t('Object tree')); titleSpan.setAttribute('data-i18n', 'Object tree');
  head.append(titleSpan, (() => { const x = el('span', 'lib-x', '✕'); x.onclick = () => rp.toggle('objects', false); return x; })());
  const bar = el('div', 'obj-bar');
  const nf = el('button', 'obj-bar-btn', '🗀﹢'); nf.title = t('New folder (groups the selection)'); nf.setAttribute('data-i18n-title', 'New folder (groups the selection)');
  nf.onclick = () => createFolder([...state.selectedIds]);
  const sv = el('button', 'obj-bar-btn'); sv.title = t('Save all drawings on this chart to a file'); sv.setAttribute('data-i18n-title', 'Save all drawings on this chart to a file');
  sv.appendChild(themeIcon('/images/save.png', 15));
  sv.onclick = saveDrawingSet;
  const ld = el('button', 'obj-bar-btn'); ld.title = t('Load drawings from a file (replaces all drawings on this chart)'); ld.setAttribute('data-i18n-title', 'Load drawings from a file (replaces all drawings on this chart)');
  ...
  listEl.ondragover = (ev) => { if (state.dragId) ev.preventDefault(); };
  listEl.ondrop = (ev) => { if (!state.dragId) return; ev.preventDefault(); state.dropTarget = null; doDrop(); };   // empty area → root end
  ...
  bus.on('rightpanel:shown', (id) => { if (id === 'objects') render(); });

  const refresh = () => { if (rp.isShown('objects') && !state.renamingId && !state.dragId) render(); };
  bus.on('vocab:changed', refresh);   // live vocabulary switch
  bus.on('objects:changed', refresh);
  bus.on('pane:changed', refresh);
  bus.on('pane:active', () => { state.renamingId = null; state.selectedIds.clear(); refresh(); });
...
function _render() {
  ...
  if (e) {
    ...
    if (sig !== lastSel) {
      ...
      if (state.selectedIds.size <= 1 || chartSet.size > 1) { state.selectedIds = chartSet; state.anchorId = e.selectedId; }
      lastSel = sig;
    }
  }
  listEl.innerHTML = '';
  ...
  if (!e) { listEl.appendChild(el('div', 'obj-empty', t('No chart.'))); return; }

  // ---- Drawings section: left-edge LAYER tabs + the active layer's folder tree ----
  registerEngine(e);
  const tree = reconcile(e);
  listEl.appendChild(sectionHead(t('Drawings'), drawingsCollapsed, () => { drawingsCollapsed = !drawingsCollapsed; render(); }));
  if (!drawingsCollapsed) {
    if (e.layers()) {
      ...
      treeCol.ondragover = (ev) => { if (state.dragId) ev.preventDefault(); };
      treeCol.ondrop = (ev) => { if (!state.dragId) return; ev.preventDefault(); state.dropTarget = null; doDrop(); };   // drop in empty tree area → root end
      // render the active layer's tree INTO the tree column (retarget row appends, then restore)
      const prev = listEl; listEl = treeCol;
  ...
  if (studies.length) {
    listEl.appendChild(sectionHead(t('Indicators'), indicatorsCollapsed, () => { indicatorsCollapsed = !indicatorsCollapsed; render(); }));
    if (!indicatorsCollapsed) studies.forEach((/** @type {any} */ a) => {
  ...
  if (state.pendingFocus) { /** @type {HTMLInputElement} */ (state.pendingFocus).focus(); /** @type {HTMLInputElement} */ (state.pendingFocus).select(); }
  ...
  if (p && p.studies) p.studies.list().forEach((/** @type {any} */ a) => { const ce = p.studies.surfaceEngine(a.i); if (ce) subSel.push(...ce.selectedIds()); });
  const selSig = [...(e ? e.selectedIds() : []), ...subSel].sort().join(',');
  if (selSig && selSig !== revealSig) { const sel = listEl.querySelector('.obj-row.sel'); if (sel) sel.scrollIntoView({ block: 'nearest' }); }
  revealSig = selSig;
}
...
const imgAct = (src, title, cls, fn) => { const b = el('button', 'obj-act' + (cls ? ' ' + cls : '')); b.title = title; b.appendChild(themeIcon(src, 14)); b.onclick = (ev) => { ev.stopPropagation(); fn(); }; return b; };
...
function renderTreeNodes(e, nodes, depth, inhHidden, inhLocked) {
  nodes.forEach((node) => {
    if (node.type === 'folder') {
      listEl.appendChild(buildFolderRow(e, node, depth, (/** @type {HTMLInputElement} */ inp) => { state.pendingFocus = inp; }, inhHidden, inhLocked));
      // a folder's own hidden/locked flow down to everything inside it (own-flag inheritance, no cascade)
      if (node.expanded) renderTreeNodes(e, node.children || [], depth + 1, inhHidden || !!node.hidden, inhLocked || !!node.locked);
    } else {
      const d = e.get(node.id); if (!d) return;
      listEl.appendChild(buildDrawingRow(e, d, node, depth, (/** @type {HTMLInputElement} */ inp) => { state.pendingFocus = inp; }, inhHidden, inhLocked));
    }
  });
}
...
function appendSurfaceDrawingRows(ce, baseDepth) {
  ...
  if (!tree.length) { const e = el('div', 'obj-empty', t('No drawings on this pane.')); e.style.paddingLeft = (10 + baseDepth * 16) + 'px'; listEl.appendChild(e); return; }
...
function appendComparePaneGroup(pane) {
  const cmp = pane.compare; if (!cmp) return;
  const head = el('div', 'obj-section obj-pane-head' + (cmp.hidden ? ' hidden' : ''));
  head.append(el('span', 'obj-twirl', comparePaneCollapsed ? '▸' : '▾'), el('span', 'obj-ico', '∿'), el('span', 'obj-section-lbl', cmp.symbol));
  const hacts = el('div', 'obj-acts');
  hacts.append(
    imgAct(cmp.hidden ? '/images/invisible.png' : '/images/visible.png', cmp.hidden ? t('Show') : t('Hide'), cmp.hidden ? 'shown' : '', () => pane.compareSetHidden(!cmp.hidden)),
    imgAct('/images/trash.png', t('Remove pane'), '', () => pane.removeCompare()),
  );
  head.appendChild(hacts);
  head.onclick = (ev) => { if (/** @type {HTMLElement} */ (ev.target).closest('.obj-acts')) return; comparePaneCollapsed = !comparePaneCollapsed; render(); };
...
function buildIndicatorRow(pane, a) {
  ...
  if (a.error) { name.title = a.error; name.append(' ', el('span', 'obj-ind-warn', '⚠')); }
  row.appendChild(name);
  row.onclick = () => openStudySettings(pane, a.i);

  const acts = el('div', 'obj-acts');
  acts.append(
    imgAct(a.hidden ? '/images/invisible.png' : '/images/visible.png', a.hidden ? t('Show') : t('Hide'), a.hidden ? 'shown' : '', () => pane.studies.toggleHidden(a.i)),
...
function buildDrawingRow(e, d, node, depth, setFocus, inhHidden, inhLocked) {
  ...
  const row = el('div', 'obj-row' + ((state.selectedIds.has(d.id) || e.isSelected(d.id)) ? ' sel' : '') + ((effHidden || inhHidden) ? ' hidden' : '') + (inhLocked ? ' obj-inh-lock' : ''));
  row.style.paddingLeft = (10 + depth * 16) + 'px';
  state.flatOrder.push(d.id);
  row.append(el('span', 'obj-twirl'), el('span', 'obj-ico', (tool && tool.glyph) || '•'));

  if (d.id === state.renamingId) { const inp = renameInput(d.id, labelOf(d), (v) => e.rename(d.id, v)); setFocus(inp); row.appendChild(inp); }
  else {
    const name = el('span', 'obj-name', labelOf(d));
    row.appendChild(name);
    row.onclick = (ev) => selectClick(e, d.id, ev, true);
    row.ondblclick = () => { clearTimeout(/** @type {any} */ (renameTimer)); startRename(d.id); };
    row.oncontextmenu = (ev) => { ev.preventDefault(); if (!state.selectedIds.has(d.id)) { state.selectedIds = new Set([d.id]); state.anchorId = d.id; e.select(d.id); render(); } openObjMenu(d.id, ev.clientX, ev.clientY, e); };
  ...
  eye.onclick = (ev) => { ev.stopPropagation(); e.setHidden(d.id, !d.hidden); };
  acts.append(
    eye,
    imgAct('/images/lock.png', d.locked ? t('Unlock') : t('Lock'), d.locked ? 'shown' : '', () => e.setLocked(d.id, !d.locked)),
...
function buildFolderRow(e, node, depth, setFocus, inhHidden, inhLocked) {
  ...
  const row = el('div', 'obj-row obj-folder' + ((state.selectedIds.has(node.id) || containsChartSel) ? ' sel' : '') + (dimmed ? ' hidden' : '') + (inhLocked ? ' obj-inh-lock' : ''));
  row.style.paddingLeft = (10 + depth * 16) + 'px';
  state.flatOrder.push(node.id);
  const tw = el('span', 'obj-twirl', node.expanded ? '▾' : '▸');
  tw.onclick = (ev) => { ev.stopPropagation(); node.expanded = !node.expanded; e.saveTree(); };
  row.append(tw, el('span', 'obj-ico', node.expanded ? '🗁' : '🗀'));

  if (node.id === state.renamingId) { const inp = renameInput(node.id, node.name || 'Folder', (v) => { node.name = v || 'Folder'; e.saveTree(); }); setFocus(inp); row.appendChild(inp); }
  else {
    row.appendChild(el('span', 'obj-name', node.name || t('Folder')));
    row.onclick = (ev) => selectClick(e, node.id, ev, false);
    row.ondblclick = () => { clearTimeout(/** @type {any} */ (renameTimer)); startRename(node.id); };
    row.oncontextmenu = (ev) => { ev.preventDefault(); if (!state.selectedIds.has(node.id)) { state.selectedIds = new Set([node.id]); state.anchorId = node.id; render(); } openFolderMenu(node, ev.clientX, ev.clientY, e); };
    ...
    const acts = el('div', 'obj-acts');
    acts.append(
      imgAct(node.hidden ? '/images/invisible.png' : '/images/visible.png', node.hidden ? t('Show') : t('Hide'), node.hidden ? 'shown' : '', () => { node.hidden = !node.hidden; e.saveTree(); }),
      imgAct('/images/lock.png', node.locked ? t('Unlock') : t('Lock'), node.locked ? 'shown' : '', () => { node.locked = !node.locked; e.saveTree(); }),
      imgAct('/images/trash.png', t('Remove folder & contents'), '', () => { removeFolderDeep(e, node); e.saveTree(); }),
    );
    row.appendChild(acts);
  }
  return row;
...
function selectClick(e, id, ev, isDrawing) {
  ...
  if (ev.shiftKey && state.anchorId != null) {
    const a = state.flatOrder.indexOf(state.anchorId), b = state.flatOrder.indexOf(id);
    if (a >= 0 && b >= 0) { const [lo, hi] = a < b ? [a, b] : [b, a]; state.selectedIds = new Set(state.flatOrder.slice(lo, hi + 1)); } else state.selectedIds = new Set([id]);
  } else if (ev.ctrlKey || ev.metaKey) {
    if (state.selectedIds.has(id)) state.selectedIds.delete(id); else state.selectedIds.add(id);
    state.anchorId = id;
  } else {
    // slow-click rename (Explorer-style): click an already sole-selected row again
    if (state.selectedIds.size === 1 && state.selectedIds.has(id)) { renameTimer = setTimeout(() => startRename(id), 450); return; }
    state.selectedIds = new Set([id]); state.anchorId = id;
  }
