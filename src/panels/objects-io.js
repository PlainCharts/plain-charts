...
/** @returns {Engine} */
const eng = () => { const p = getActivePane(); return p && p.drawings; };

const DRAW_FIELDS = ['tool', 'points', 'style', 'textStyle', 'text', 'name', 'hidden', 'locked', 'z', 'visibility', 'sync'];

export async function saveDrawingSet() {
  const e = eng(); const p = getActivePane();
  if (!e || !p) return;
  const drawings = e.objects().map((/** @type {Drawing} */ d) => {
    /** @type {any} */
    const o = { id: d.id };
    DRAW_FIELDS.forEach((k) => { if (d[k] !== undefined) o[k] = d[k]; });
    return o;
  });
  ...
  if (snap) data.layers = { active: snap.active, list: snap.list.map((/** @type {any} */ ly) => ({ id: ly.id, name: ly.name, hidden: !!ly.hidden, locked: !!ly.locked, nodes: ly.nodes })) };
  ...
  if (/** @type {any} */ (window).showSaveFilePicker) {
    ...
    if (handle) {
      try { const w = await handle.createWritable(); await w.write(json); await w.close(); return; } catch (_) {}
    }
  }
  // Fallback (older browsers): trigger a download with a suggested name.
  const url = URL.createObjectURL(new Blob([json], { type: 'application/json' }));
  const a = /** @type {HTMLAnchorElement} */ (el('a')); a.href = url; a.download = suggested;
...
export function loadDrawingSet(render) {
  ...
  const inp = /** @type {HTMLInputElement} */ (el('input')); inp.type = 'file'; inp.accept = 'application/json,.json';
  inp.onchange = () => {
    const file = inp.files && inp.files[0]; if (!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      let data; try { data = JSON.parse(/** @type {string} */ (reader.result)); } catch (_) { alert('Could not read that file — it is not valid JSON.'); return; }
      if (!data || !Array.isArray(data.drawings)) { alert('That file is not a drawings set.'); return; }
      const layerNote = (data.layers && Array.isArray(data.layers.list) && data.layers.list.length > 1) ? ' and layers' : '';
      if (!confirm('Load ' + data.drawings.length + ' drawing(s)? This replaces ALL drawings' + layerNote + ' on this chart.')) return;
...
function applyDrawingSet(e, data, render) {
  ...
  (data.drawings || []).forEach((/** @type {any} */ d) => {
    ...
    /** @type {any} */
    const params = {};
    DRAW_FIELDS.forEach((k) => { if (k !== 'tool' && d[k] !== undefined) params[k] = d[k]; });
    const nd = e.add(d.tool, params);
    if (nd && d.id) idMap[d.id] = nd.id;   // old id -> new id, to rewrite tree refs
  });
  /** @param {any[]} nodes @returns {any[]} */
  const remap = (nodes) => (nodes || []).map((/** @type {any} */ n) => {
    if (n.type === 'folder') return { type: 'folder', id: n.id, name: n.name, expanded: n.expanded !== false, hidden: !!n.hidden, locked: !!n.locked, children: remap(n.children) };
    const nid = idMap[n.id]; return nid ? { type: 'drawing', id: nid } : null;
  }).filter(Boolean);

  const snap = e.layersSnapshot && e.layersSnapshot();
  if (data.layers && snap) {
    // layered surface: rebuild the WHOLE stack, remapping each layer's node ids
    const list = data.layers.list.map((/** @type {any} */ ly) => ({ id: ly.id, name: ly.name, hidden: !!ly.hidden, locked: !!ly.locked, nodes: remap(ly.nodes) }));
    ...
    const srcTree = data.tree || data.layers.list.reduce((/** @type {any} */ a, /** @type {any} */ ly) => a.concat(ly.nodes || []), []);
    const tree = e.getTree(); tree.length = 0; remap(srcTree).forEach((/** @type {any} */ n) => tree.push(n));
    e.saveTree();
  }
  render();
}
