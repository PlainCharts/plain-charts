...
/** @param {any} ly */
function layerDrawingCount(ly) { let n = 0; (/** @param {any[]} nodes */ function c(nodes) { (nodes || []).forEach((/** @type {any} */ x) => { if (x.type === 'folder') c(x.children); else n++; }); })(ly.nodes); return n; }
...
function openLayerMenu(e, ly, x, y) {
  menu(x, y, (m, item) => {
    ...
    m.appendChild(item(ly.hidden ? 'Show layer' : 'Hide layer', () => { e.setLayerFlag(ly.id, 'hidden', !ly.hidden); render(); }));
    m.appendChild(item(ly.locked ? 'Unlock layer' : 'Lock layer', () => { e.setLayerFlag(ly.id, 'locked', !ly.locked); render(); }));
    if (e.layers().list.length > 1) {
      m.appendChild(el('div', 'dwg-div'));
      m.appendChild(item('Remove layer & drawings', () => {
        const n = layerDrawingCount(ly);
        if (n && !confirm('Remove layer "' + (ly.name || 'Layer') + '" and its ' + n + ' drawing' + (n === 1 ? '' : 's') + '?')) return;
        e.removeLayer(ly.id); render();
      }));
    }
  });
}
...
export function buildLayerTabs(e) {
  ...
  L.list.forEach((/** @type {any} */ ly) => {
    ...
    const tab = el('div', 'obj-ltab' + (renaming ? ' renaming' : '') + (ly.id === L.active ? ' active' : '') + (ly.hidden ? ' hidden' : '') + (ly.locked ? ' locked' : ''));
    if (renaming) { const inp = renameInput(ly.id, ly.name || 'Layer', (v) => e.renameLayer(ly.id, v || 'Layer')); tab.appendChild(inp); state.pendingFocus = inp; }
    else {
      ...
      if (ly.hidden || ly.locked) {
        const st = el('span', 'obj-ltab-state'); st.title = ly.hidden ? 'Hidden' : 'Locked';
      ...
      tab.title = (ly.name || 'Layer') + ' — click to switch, right-click for options, drop drawings here to move them in';
      tab.onclick = () => { e.setActiveLayer(ly.id); render(); };
      tab.ondblclick = (ev) => { ev.stopPropagation(); startRename(ly.id); };
      tab.oncontextmenu = (ev) => { ev.preventDefault(); openLayerMenu(e, ly, ev.clientX, ev.clientY); };
      ...
      tab.ondragover = (ev) => { if (!canDrop()) return; ev.preventDefault(); /** @type {DataTransfer} */ (ev.dataTransfer).dropEffect = 'move'; tab.classList.add('drop-layer'); };
      tab.ondragleave = () => tab.classList.remove('drop-layer');
      tab.ondrop = (ev) => { if (!canDrop()) return; ev.preventDefault(); ev.stopPropagation(); tab.classList.remove('drop-layer'); dropOntoLayer(e, ly.id); };
    }
    col.appendChild(tab);
  });
  const add = el('button', 'obj-ltab-add'); add.textContent = '+'; add.title = 'New layer';
  add.onclick = () => { const ly = e.addLayer(); startRename(ly.id); };
  col.appendChild(add);
  return col;
}
