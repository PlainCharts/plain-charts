...
export function startAll() {
  symbols().forEach((it) => start(it));
}
export function stopAll() {
  symbols().forEach(stop);
}
/** @param {SymbolItem} it */
function start(it) {
  const key = keyOf(it);
...
export function stop(it) {
  const key = keyOf(it);
  const s = state.live.get(key);
  if (s && s.qcb && s.contractId != null) {
    try {
      /** @type {any} */ (broker.for(it.broker)).unsubscribeQuotes(s.contractId, s.qcb);
    } catch (_) {}
  }
...
export function paint(key) {
  const r = state.rowEls.get(key),
    s = state.live.get(key);
  if (!r) return;
