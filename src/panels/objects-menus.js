...
let objAway = null;
export function closeObjMenu() {
  if (objAway) {
    document.removeEventListener('pointerdown', /** @type {any} */ (objAway), true);
    objAway = null;
  }
  if (objMenu) {
    objMenu.remove();
...
export function menu(x, y, build) {
  ...
  objAway = (ev) => {
    if (objMenu && !objMenu.contains(/** @type {Node} */ (ev.target))) closeObjMenu();
  };
...
function goToDrawing(e, id) {
  ...
  const chart = e.pane && e.pane.chart;
  if (!chart) return;
  ...
  if (times.length && cur) {
    const t0 = Math.min(...times),
      t1 = Math.max(...times);
    if (t1 < cur.from || t0 > cur.to) {
      // no overlap with the visible window -> off-screen in time
      const tc = (t0 + t1) / 2;
      let span = cur.to - cur.from;
      const need = (t1 - t0) * 1.2;
      if (need > span) span = need;
      try {
        ts.setTimeWindow({ from: tc - span / 2, to: tc + span / 2 });
        scrolledH = true;
      } catch (_) {}
      try {
        if (e.series && e.series.priceAxis) e.series.priceAxis().configure({ autoScale: true });
      } catch (_) {} // re-fit price to the drawing's neighbourhood
    }
  }
  ...
  if (!scrolledH && prices.length && H > 0 && e.series && e.series.priceToY) {
    const pLo = Math.min(...prices),
      pHi = Math.max(...prices),
      pc = (pLo + pHi) / 2;
    const yHi = e.series.priceToY(pHi),
      yLo = e.series.priceToY(pLo);
    if (yHi != null && yLo != null) {
      const top = Math.min(yHi, yLo),
        bot = Math.max(yHi, yLo);
      if (top < 0 || bot > H) {
        // any part of the drawing is off-screen vertically -> frame it
        const curTop = e.series.yToPrice(0),
          curBot = e.series.yToPrice(H);
        let pSpan = Math.abs((curTop || 0) - (curBot || 0));
        const need = (pHi - pLo) * 1.3;
        if (need > pSpan) pSpan = need;
        if (pSpan > 0) {
          try {
            e.series.priceAxis().configure({ range: [pc + pSpan / 2, pc - pSpan / 2] });
          } catch (_) {}
        }
      }
    }
...
export function openObjMenu(id, x, y, engine) {
  ...
  menu(x, y, (m, item) => {
    ...
    if (!multi) {
      m.appendChild(item(t('Go to'), () => goToDrawing(e, id)));
    ...
    voItem.append(
      el('span', 'dwg-check', ''),
      el('span', 'dwg-label', t('Visual order')),
      el('span', 'dwg-arrow', '▸'),
    );
    ...
    [
      ...
      r.onclick = () => {
        closeObjMenu();
        ids.forEach((i) => e.reorder(i, o.where));
      };
      voSub.appendChild(r);
    });
    ...
    // Organization
    m.appendChild(
      item(t('New folder') + (multi ? ' ' + t('from') + ' ' + ids.length : ''), () => createFolder(ids, e)),
    );
    ...
    m.appendChild(el('div', 'dwg-div'));
    m.appendChild(
      item((anyUnlocked ? t('Lock') : t('Unlock')) + suffix, () => ids.forEach((i) => e.setLocked(i, anyUnlocked))),
    );
    m.appendChild(
      item((anyVisible ? t('Hide') : t('Show')) + suffix, () => ids.forEach((i) => e.setHidden(i, anyVisible))),
    );
...
export function openFolderMenu(node, x, y, engine) {
  ...
  menu(x, y, (m, item) => {
    ...
    m.appendChild(
      item(t('New folder inside'), () => {
        (node.children || (node.children = [])).unshift({
          type: 'folder',
          id: 'f' + Date.now().toString(36) + (state.seq++).toString(36),
          name: 'New folder',
          expanded: true,
          children: [],
        });
