...
export function seedLists() { return { lists: [{ id: 'wl_1', name: 'Watchlist', items: [] }], activeId: 'wl_1', columns: { last: true, chg: true, pct: true, bid: false, ask: false, spread: false }, colOrder: ['last', 'chg', 'pct', 'bid', 'ask', 'spread'], sort: { key: null, dir: 'desc' } }; }
...
export const state = {
  ...
  /** @type {Record<string, boolean>} */ cols: { last: true, chg: true, pct: true, bid: false, ask: false, spread: false },
...
export function persist() { const L = activeList(); if (L) L.items = state.items; store.set('lists', state.lists); store.set('activeId', state.activeId); }
