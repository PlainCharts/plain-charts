...
export function loadColumnPrefs(d) {
  ...
  const validOrder = (Array.isArray(d.colOrder) ? d.colOrder : []).filter((/** @type {string} */ k) =>
    COLS.some((x) => x.key === k),
  );
  state.colOrder = validOrder.concat(COLS.filter((x) => !validOrder.includes(x.key)).map((x) => x.key)); // sanitize + append any missing
  const so = d.sort || {};
  state.sort = {
    key: so.key && ['symbol', ...COLS.map((x) => x.key)].includes(so.key) ? so.key : null,
    dir: so.dir === 'asc' ? 'asc' : 'desc',
  };
}

function saveCols() {
  store.set('columns', state.cols);
  store.set('colOrder', state.colOrder);
...
function sortArrow(key) {
  return state.sort.key === key ? el('span', 'wl-sortarrow', state.sort.dir === 'asc' ? '▲' : '▼') : null;
}
export function renderHeader() {
  if (!state.colhdrEl) return;
  state.colhdrEl.innerHTML = '';
  // Symbol: pinned first, click to sort by name (not draggable)
  const symH = el('span', 'wl-c-sym wl-sortable', t('Symbol'));
  symH.title = t('Click to sort by symbol');
  symH.onclick = () => cycleSort('symbol');
  const symArrow = sortArrow('symbol');
  if (symArrow) symH.appendChild(symArrow);
  state.colhdrEl.appendChild(symH);
  orderedCols().forEach((c) => {
    if (!state.cols[c.key]) return;
    const cell = el('span', 'wl-c-num wl-colhead wl-sortable', t(c.head));
    cell.title = t('Click to sort, drag to reorder');
    const arrow = sortArrow(c.key);
...
function cycleSort(key) {
  ...
  if (state.sort.key !== key) {
    state.sort.key = key;
    state.sort.dir = primary;
  } else if (state.sort.dir === primary) {
    state.sort.dir = reverse;
  } else {
    state.sort.key = null;
  }
  saveCols();
...
export function colCell(key, s, dec) {
  ...
  switch (key) {
    ...
    case 'ask':
      return price(ask);
    case 'spread':
      return price(bid != null && ask != null ? ask - bid : null);
    case 'chg':
      return chg == null
        ? blank
        : { val: chg, text: (chg >= 0 ? '+' : '') + chg.toFixed(dec), cls: chg > 0 ? 'up' : chg < 0 ? 'down' : '' };
    case 'pct': {
      if (chg == null || prev == null) return blank;
      const p = (chg / prev) * 100;
      return { val: p, text: (p >= 0 ? '+' : '') + p.toFixed(2) + '%', cls: p > 0 ? 'up' : p < 0 ? 'down' : '' };
    }
  }
  return blank;
}
...
function clearColDrop() {
  if (state.colhdrEl)
    state.colhdrEl
      .querySelectorAll('.wl-cdrop-before,.wl-cdrop-after')
      .forEach((c) => c.classList.remove('wl-cdrop-before', 'wl-cdrop-after'));
  colDropKey = null;
}
...
function headerCellDnd(cell, key) {
  ...
  cell.ondragstart = (e) => {
    ...
    cell.classList.add('wl-cdrag');
  };
  cell.ondragend = () => {
    cell.classList.remove('wl-cdrag');
    clearColDrop();
    dragCol = null;
  };
  cell.ondragover = (e) => {
    ...
    colDropAfter = e.clientX - r.left > r.width / 2;
    colDropKey = key;
...
function doColDrop() {
  ...
  const from = state.colOrder.indexOf(dk);
  if (from < 0) return;
  state.colOrder.splice(from, 1);
  let to = state.colOrder.indexOf(tk);
  if (to < 0) return;
  if (after) to += 1;
...
function closeColsMenu() {
  if (colsMenuAway) {
    document.removeEventListener('pointerdown', /** @type {any} */ (colsMenuAway), true);
    colsMenuAway = null;
  }
  if (colsMenu) {
    colsMenu.remove();
...
export function toggleColsMenu(anchor) {
  if (colsMenu) closeColsMenu();
  else openColsMenu(anchor);
}
/** @param {HTMLElement} anchor */
function openColsMenu(anchor) {
  closeColsMenu();
  const m = el('div', 'wl-listmenu');
  colsMenu = m;
  ...
  colsMenuAway = (e) => {
    if (
      colsMenu &&
      !colsMenu.contains(/** @type {Node} */ (e.target)) &&
