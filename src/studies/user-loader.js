...
export const studyUrlFor = (id) => {
  const f = fileOf.get(id);
  return f ? new URL('/packages/studies/' + f + '/' + f + '.js', import.meta.url).href : null;
};
/** @param {string} file @returns {string | null} */
export const studyIdForFile = (file) => {
  for (const [id, f] of fileOf) if (f === file) return id;
  return null;
};
/** @param {string} id */
...
async function importFile(folder) {
  ...
  try {
    await import('/packages/studies/' + folder + '/' + folder + '.js?t=' + Date.now());
  } finally {
    // cache-bust so edits reload
  ...
  let meta = null;
  try {
    const r = await fetch('/packages/studies/' + folder + '/meta.json', { cache: 'no-store' });
    if (r.ok) meta = await r.json();
  } catch (_) {}
  for (const id of ids) {
    const st = getStudy(id);
    if (st) {
      st.name = (meta && meta.name) || '';
      st.description = (meta && meta.description) || '';
    }
  }
}
...
export async function loadUserStudies() {
  ...
  let studies = [];
  try {
    studies = (await fetch('/api/user-studies').then((r) => r.json())).studies || [];
  } catch (_) {}
  for (const s of studies) {
    try {
      await importFile(s.id);
    } catch (e) {
      const err = /** @type {any} */ (e);
      log('Indicator "' + s.id + '" failed to load: ' + (err.message || err), true);
    }
  }
}
...
export async function reloadUserFile(name) {
  [...fileOf.entries()].forEach(([id, f]) => {
    if (f === name) fileOf.delete(id);
  });
...
try {
  ...
  rc.onmessage = (e) => {
    ...
    if (m && m.file)
      reloadUserFile(m.file).catch((/** @type {any} */ err) =>
        log('Assistant study load failed: ' + ((err && err.message) || err), true),
      );
  };
} catch (_) {}
