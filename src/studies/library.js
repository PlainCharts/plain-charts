...
export function openLibrary() {
  ...
  const dlg = el('div', 'dialog lib');
  panel = dlg;
  dlg.style.zIndex = '60';
  const head = el('div', 'lib-head');
  const x = el('span', 'lib-x', '✕');
  x.onclick = closeLibrary;
  head.append(el('h3', null, 'Studies'), x);

  const search = /** @type {HTMLInputElement} */ (el('input', 'lib-search'));
...
function renderSide() {
  ...
  write.classList.add('lib-newlist');
  s.append(write);
  const imp = navItem('Import .js', '⬆', false, () => importStudyFile());
  imp.classList.add('lib-newlist');
  s.append(imp);

  s.append(el('div', 'lib-sec', t('My lists')));
  lib.categories().forEach((c) => {
    const r = navItem(c.name, '≡', v.type === 'category' && v.catId === c.id, () =>
      go({ type: 'category', catId: c.id }),
    );
    const tools = el('span', 'lib-nav-tools');
    const ren = el('span', 'lib-ico', '✎');
    ren.title = 'Rename';
    ren.onclick = (e) => {
      e.stopPropagation();
      renameCat(c, r);
    };
    const del = el('span', 'lib-ico', '✕');
    ...
    del.onclick = async (e) => {
      ...
      if (
        !(await confirmDialog({
          title: t('Delete list'),
          message: t('Delete the list') + ` "${c.name}"? ` + t('The studies themselves stay under All studies.'),
          yes: t('Delete'),
          no: t('Cancel'),
        }))
      )
...
function renameCat(c, rowEl) {
  const inp = /** @type {HTMLInputElement} */ (el('input', 'lib-rename'));
  inp.value = c.name;
  rowEl.innerHTML = '';
  ...
  const done = () => {
    ...
    if (v) lib.renameCategory(c.id, v);
    renderSide();
  };
  inp.onblur = done;
  inp.onkeydown = (e) => {
    if (e.key === 'Enter') inp.blur();
    else if (e.key === 'Escape') renderSide();
  };
}
function newCat() {
  const r = el('div', 'lib-nav active');
  const inp = /** @type {HTMLInputElement} */ (el('input', 'lib-rename'));
  inp.placeholder = t('List name');
  r.appendChild(inp);
  /** @type {LibEls} */ (els).side.appendChild(r);
  inp.focus();
  const done = () => {
    const v = inp.value.trim();
    if (v) {
      const id = lib.addCategory(v);
      state.view = { type: 'category', catId: id };
    }
    renderSide();
    renderList();
  };
...
function renderList() {
  ...
  if (q || state.view.type === 'all') ids = listStudies().map((s) => s.id);
    ...
    const c = lib.categories().find((c) => c.id === state.view.catId);
    ids = c ? c.items : [];
  ...
  if (!rows.length) {
    const msg = q
      ...
      : state.view.type === 'favorites'
        ? t('No favorites yet — tap ☆ on any study.')
        : state.view.type === 'category'
          ? t('Empty list — tap the ⊞ on a study to add it here.')
          : t('Nothing here yet.');
  ...
  rows.forEach((s) => {
    ...
    const addto = el('span', 'lib-ico lib-addto', '⊞');
    addto.title = t('Add to a list…');
    addto.onclick = (e) => {
      e.stopPropagation();
      openCatPopover(s.id, addto);
    };
    r.append(star, name, addto);
    if (file) {
      const edit = el('span', 'lib-ico', '✎');
    ...
    r.onclick = () => {
      if (pane) {
        pane.studies.add(s.id);
        flash(r);
        log(t('Added') + ' ' + t(s.name) + '.');
        renderSide();
      }
    };
    L.append(r);
...
async function editStudy(s, folder) {
  const code = await fetch('/api/user-studies/file?name=' + encodeURIComponent(folder))
    .then((x) => x.json())
    .then((d) => d.code)
    .catch(() => '');
  openEditor({
  ...
  if (
    !(await confirmDialog({
      ...
      message:
        t('Delete the study') +
        ` "${t(s.name)}"? ` +
        t('This permanently deletes its package folder from packages/studies.'),
      yes: t('Delete'),
      no: t('Cancel'),
    }))
  )
    return;
  await fetch('/api/user-studies/delete', {
    method: 'POST',
...
function importStudyFile() {
  ...
  inp.onchange = async () => {
    ...
    if (r.error) {
      log(t('Import failed:') + ' ' + r.error, true);
      return;
    }
    try {
      await reloadUserFile(r.file);
    } catch (/** @type {any} */ e) {
      log(t('Imported, but failed to load:') + ' ' + (e.message || e), true);
      return;
    }
...
function openCatPopover(studyId, anchor) {
  ...
  lib.categories().forEach((c) => {
    ...
    row.onclick = () => {
      lib.toggleMember(c.id, studyId);
      openCatPopover(studyId, anchor);
      renderList();
    };
    /** @type {HTMLElement} */ (popover).append(row);
  });
  const add = el('div', 'lib-pop-row lib-pop-new');
  const inp = /** @type {HTMLInputElement} */ (el('input', 'lib-pop-input'));
...
document.addEventListener('click', (e) => {
  if (popover && !popover.contains(/** @type {Node | null} */ (e.target))) closePopover();
});

/** @param {HTMLElement} row */
function flash(row) {
  row.classList.add('added');
  setTimeout(() => row.classList.remove('added'), 500);
}
