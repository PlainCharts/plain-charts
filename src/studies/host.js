...
export class StudyHost extends CoreStudyHost {
  ...
  constructor(pane) {
    super(pane.chart, {
      ...
      intrabar: (/** @type {string} */ lowerTfId, /** @type {number} */ fromMs, /** @type {number} */ toMs) =>
        new Promise((resolve) => {
          const api = pane.api(),
            cid = pane.contractId;
      ...
      subscribeIntrabar: (
        /** @type {string} */ lowerTfId,
        /** @type {number} */ fromMs,
        /** @type {(bars: any[]) => void} */ onBars,
      ) => {
        const api = pane.api(),
          cid = pane.contractId;
      ...
      dropIntrabar: (/** @type {any} */ id) => {
        const api = pane.api();
        if (api && api.drop && id != null) {
          try {
            api.drop(id);
          } catch (_) {}
        }
    ...
    this.on('panemode', (/** @type {any} */ a) => {
      if (a.surface) a.surface.setSuppressed(!this._show(a));
    });
    this.on('moved', () => {
      if (this.pane._captureOrder) this.pane._captureOrder();
      if (this.pane._positionCompareUI) this.pane._positionCompareUI();
    });
  ...
  movePane(a, dir) {
    if (this.pane.board) {
      bus.emit('board:move', { pane: this.pane, dir });
      return;
    }
  ...
  _priceDecimals() {
    return this.pane.priceDecimals;
  }
  _isHidden() {
    return indicatorsHidden();
  }
  _mainSeries() {
    return this.pane.series;
  }
  _candleStyle() {
    const c = this.pane && this.pane.settings && this.pane.settings.candles;
    return c ? { up: c.upColor, down: c.downColor } : null;
  }
  _barTimes() {
    return this.pane.barTimes;
  }
  /** @param {any} series */
  _paneIndexOf(series) {
    return this.pane.paneIndexOf(series);
  }
  _chartSec() {
    const tf = this.pane.tf();
    return tf ? barMs(tf) / 1000 : super._chartSec();
  }
  /** @param {any} study */
  ...
  _ensureIntrabar(lowerTfId) {
    if (this._intrabarCid !== this.pane.contractId) {
      this.resetIntrabar();
      this._intrabarCid = this.pane.contractId;
    }
  ...
  applyTemplate(studies) {
    this.clearAll();
    (studies || []).forEach((/** @type {any} */ s) =>
      this.add(s.id, s.params, s.hidden, undefined, undefined, s.style),
    );
  }
  clearAll() {
  ...
  update(bars) {
    ...
    this._thTimer = setTimeout(() => {
      this._thTimer = 0;
      this._lastCompute = performance.now();
      this.setData(this._pendingBars || []);
    }, due);
  }
  ...
  _onError(a, msg) {
    if (a.errLogged !== msg) {
      a.errLogged = msg;
      log('Indicator "' + a.study.name + '" error: ' + msg, true);
    }
    super._onError(a, msg);
  }
  /** @param {any} a */
  _onRemoved(a) {
    if (a.surface) {
      try {
        this.pane.removePaneSurface(a.surface);
      } catch (_) {}
      a.surface = null;
    }
  ...
  applyVisibility() {
    ...
    this._repaintShapes();
    if (this.pane.skin) this.pane.skin.refresh();
  }
  /** @param {number} i */
  toggleHidden(i) {
  ...
  reindexAndReposition() {
    this._reindexPanes();
    if (this.pane.skin) this.pane.skin.reposition();
  }
  ...
  _updateFutureSpacer() {
    ...
    let maxT = 0;
    this.attached.forEach((a) =>
      (a.shapes || []).forEach((/** @type {any} */ s) => {
        [s.time, s.from, s.to].forEach((/** @type {any} */ t) => {
          if (t != null && t > last) {
    ...
    let refDi = dates[0],
      refCount = -1;
    dates.slice(0, -1).forEach((di) => {
      const c = byDate.get(di).length;
      if (c >= refCount) {
        refCount = c;
        refDi = di;
      }
    ...
    for (let di = lastDi; di * DAY <= maxT + DAY && times.size < 8000; di++) {
      ...
      for (let k = 0; k < refBars.length; k++) {
        const nt = refBars[k] + shift;
        if (nt > last && nt <= maxT) times.add(nt);
      }
    }
    const ws = [...times].sort((x, y) => x - y).map((t) => ({ time: t }));
    if (!this._spacer) {
      try {
        this._spacer = this.pane.chart.addPlot(
          Line,
          { visible: false, showLastValue: false, showPriceLine: false, showCursorMarker: false },
          0,
        );
      } catch (_) {}
    }
    if (this._spacer) {
      try {
        this._spacer.feed(ws);
        this._spacerMax = maxT;
      } catch (_) {}
    }
  }
  ...
  destroy() {
    ...
    this.attached.forEach((a) =>
      a.plots.forEach((/** @type {any} */ s) => {
        try {
          this.pane.chart.removePlot(s);
        } catch (_) {}
      }),
    );
    if (this._spacer) {
      try {
        this.pane.chart.removePlot(this._spacer);
      } catch (_) {}
      this._spacer = null;
      this._spacerMax = 0;
    }
