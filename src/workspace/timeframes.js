...
const UNITS = [
  { label: 'Minutes', unit: 'm' }, { label: 'Hours', unit: 'h' }, { label: 'Days', unit: 'D' },
  { label: 'Weeks', unit: 'W' }, { label: 'Months', unit: 'M' },
];
...
const makeId = (unit, n) => unit === 'm' ? n + 'm' : unit === 'h' ? n + 'h'
                      : unit === 'D' ? (n === 1 ? 'D' : n + 'D')
                      : unit === 'W' ? (n === 1 ? 'W' : n + 'W') : (n === 1 ? 'M' : n + 'M');
...
const parseId = (id) => { const m = /^(\d*)(m|h|D|W|M)$/.exec(String(id || '')); return m ? { unit: /** @type {Unit} */ (m[2]), n: m[1] ? parseInt(m[1], 10) : 1 } : null; };
/** @param {string|null=} id @returns {string|null|undefined} */
const canonId = (id) => { const p = parseId(id); if (!p) return id; const z = normalize(p.unit, p.n); return makeId(z.unit, z.n); };
...
const persist = () => { setSetting('intervals', intervals); setSetting('favoriteTimeframes', favs); };
...
export function initTimeframes() {
  ...
  intervals = (getSetting('intervals') || [])
    .map(/** @param {any} t @returns {Interval|null} */(t) => {
    ...
    .filter(Boolean);
  favs = (getSetting('favoriteTimeframes') || []).map(/** @param {string} f */(f) => idMap[f] || f).filter((/** @type {string} */ f, /** @type {number} */ i, /** @type {string[]} */ a) => a.indexOf(f) === i).filter(byId);
  sortIntervals();
  persist();   // save the cleaned-up list

  tfbar = /** @type {HTMLElement} */ ($('tfbar')); tfAdd = /** @type {HTMLElement} */ ($('tfAdd')); tfMenu = /** @type {HTMLElement} */ ($('tfMenu'));
  tfAdd.onclick = (e) => { e.stopPropagation(); tfMenu.classList.contains('open') ? closeMenu() : openMenu(); };
  document.addEventListener('click', (e) => {
    const t = /** @type {Node} */ (e.target);
    if (!tfMenu.contains(t) && e.target !== tfAdd) closeMenu();
  });
  bus.on('tf:active', (id) => { selectedId = id; renderStrip(); });
  renderStrip();
}

function renderStrip() {
  tfbar.innerHTML = '';
  intervals.filter((t) => favs.includes(t.id)).forEach((tf) => {
...
function toggleFav(id) {
  ...
  persist(); renderStrip(); renderMenu();
}
...
function addCustom(unit, n) {
  ...
  if (byId(id)) { showAddForm = false; renderMenu(); bus.emit('tf:selected', id); return ''; }
...
function renderMenu() {
  ...
  if (!intervals.length) {
    ...
    hint.className = 'tf-add-err'; hint.style.padding = '4px 8px'; hint.style.color = '#888';
  ...
  intervals.forEach((tf) => {
    ...
    star.onclick = (e) => { e.stopPropagation(); toggleFav(tf.id); };
    const lbl = document.createElement('span'); lbl.className = 'lbl'; lbl.textContent = tf.id;
    const del = document.createElement('span');
    del.className = 'del'; del.textContent = '✕'; del.title = 'Delete interval';
    del.onclick = (e) => { e.stopPropagation(); deleteInterval(tf.id); };
    row.append(star, lbl, del);
    row.onclick = () => { bus.emit('tf:selected', tf.id); closeMenu(); };
    tfMenu.appendChild(row);
  });

  const divider = document.createElement('div'); divider.className = 'tf-divider';
  tfMenu.appendChild(divider);

  const addRow = document.createElement('div'); addRow.className = 'tf-add-row';
  addRow.textContent = '+ Add custom interval';
  addRow.onclick = (e) => { e.stopPropagation(); showAddForm = !showAddForm; renderMenu(); };
  tfMenu.appendChild(addRow);

  if (showAddForm) {
    const form = document.createElement('div'); form.className = 'tf-add-form';
    form.onclick = (e) => e.stopPropagation();
    const num = document.createElement('input');
    num.type = 'number'; num.min = '1'; num.value = '7'; num.className = 'tf-num';
    const sel = document.createElement('select'); sel.className = 'tf-unit';
    UNITS.forEach((u) => { const o = document.createElement('option'); o.value = u.unit; o.textContent = u.label; sel.appendChild(o); });
    const btn = document.createElement('button'); btn.textContent = 'Add'; btn.className = 'tf-add-btn';
    const err = document.createElement('div'); err.className = 'tf-add-err';
    const submit = () => { const m = addCustom(/** @type {Unit} */ (sel.value), parseInt(num.value, 10)); if (m) err.textContent = m; };
    btn.onclick = (e) => { e.stopPropagation(); submit(); };
    num.onkeydown = (e) => { if (e.key === 'Enter') { e.preventDefault(); submit(); } };
...
function closeMenu() { tfMenu.classList.remove('open'); }
...
function closeQuickInput() { const el = qiEl; qiEl = null; if (el) { try { el.remove(); } catch (_) {} } }
/** @param {string=} seed @returns {void} */
export function openIntervalQuickInput(seed) {
  closeQuickInput();
  if (!tfbar) return;
  const wrap = document.createElement('div'); wrap.className = 'tf-qi';
  const inp = document.createElement('input'); inp.className = 'tf-qi-input'; inp.value = seed || ''; inp.placeholder = 'e.g. 5, 15, 1h, D';
  ...
  inp.onkeydown = (e) => {
    e.stopPropagation();
    if (e.key === 'Enter') { e.preventDefault(); if (gotoInterval(inp.value)) closeQuickInput(); }
