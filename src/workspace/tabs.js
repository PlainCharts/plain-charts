...
const applyLive = (ws) => {
  applyWorkspace(/** @type {import('../chart/layout.js').Workspace} */ (ws));
  liveTabId = activeId;
};
...
const titleCfg = () => {
  const c = getSetting('tabTitle');
  return Array.isArray(c) && c.length ? c : TITLE_DEFAULT;
};
...
const desktop = /** @type {any} */ (
  typeof window !== 'undefined' && window.desktop && window.desktop.isDesktop ? window.desktop : null
);
...
async function hydrate(t) {
  ...
  if (t.wsId) {
    ...
    if (rec && rec.ws) {
      t.ws = rec.ws;
      if (rec.name != null) t.name = rec.name;
      return t;
    }
  }
...
export async function loadTabs(seedWs) {
  ...
  }); // legacy tabs had no id
  if (!list.length) {
    list = [{ id: genId(), name: '', ws: seedWs || defaultWorkspace() }];
    migrated = true;
  }
  tabs = list;
  ...
  if (realIds.length !== myIds.length) {
    myIds = realIds;
    if (desktop && desktop.claim) desktop.claim(myIds);
  }
  ...
  if (desktop && !claimAll && !myIds.length) {
    ...
    tabs.push(t);
    myIds = [t.id]; // wsId + file created by the hydrate loop below
  ...
  activeId =
    activeParam && myIds.includes(activeParam)
      ? activeParam
      : storedActive && myIds.includes(storedActive)
        ? storedActive
        : myIds[0];
...
export async function openWorkspace(wsId) {
  ...
  if (!rec) return;
  const cur = byId(activeId);
  if (cur && cur.id === liveTabId) {
    cur.ws = getWorkspace();
    if (cur.wsId) saveWorkspace(cur.wsId, cur.name, cur.ws);
    upsert(cur.id);
  } // flush current
...
export const getActiveWsId = () => {
  const t = byId(activeId);
  return t ? t.wsId : null;
};
...
export async function loadWorkspaceHere(wsId) {
  ...
  if (cur.id === liveTabId) {
    cur.ws = getWorkspace();
    if (cur.wsId) saveWorkspace(cur.wsId, cur.name, cur.ws);
  } // flush the old one (only if it's live)
  cur.wsId = rec.id;
  cur.name = rec.name || '';
  cur.ws = rec.ws || defaultWorkspace();
  applyLive(cur.ws);
  upsert(cur.id);
  reportActive();
  render();
...
export function initTabs() {
  ...
  bus.on('workspace:changed', () => {
    ...
    if (t && t.id === liveTabId) {
      t.ws = getWorkspace();
      if (t.wsId) saveWorkspace(t.wsId, t.name, t.ws);
      startLive(t);
      repaint(t.id);
    }
  });
  ...
  if (desktop && desktop.onMaxChange)
    desktop.onMaxChange((/** @type {boolean} */ v) => {
      isMax = v;
      updateMaxBtn();
    });
}
...
function upsert(id) {
  const t = byId(id);
  if (!t) return;
  postJSON('/api/tabs/upsert', {
    id: t.id,
    name: t.name || '',
    wsId: t.wsId,
    active: isPrimary ? activeId : undefined,
  });
}
/** @param {string} id */
...
const primaryOf = (t) => {
  const p = t.ws && t.ws.panes && t.ws.panes[0];
  return p ? { symbol: p.symbol, brokerId: p.broker } : null;
};
...
function titlePieces(t) {
  ...
  titleCfg().forEach(({ key, on }) => {
    ...
    if (key === 'ticker') out.push({ text: sym });
      ...
      if (s && s.last != null) out.push({ text: s.last.toFixed(s.decimals != null ? s.decimals : 2) });
    } else if (key === 'change' && s && s.last != null && s.prevClose) {
      const d = s.last - s.prevClose,
        p = (d / s.prevClose) * 100;
...
function repaint(id) {
  const el = nameEls.get(id),
    t = byId(id);
  if (el && t) fillName(el, t);
}
function repaintAll() {
  nameEls.forEach((el, id) => repaint(id));
}
...
function startLive(t) {
  ...
  const api = broker.for(pr.brokerId);
  if (!api) return;
  /** @type {LiveState} */
  ...
  api.resolveSymbol(pr.symbol, (/** @type {any} */ inst) => {
    ...
    s.contractId = inst.id;
    s.decimals = inst.priceDecimals != null ? inst.priceDecimals : 2;
    ...
    s.qcb = (q) => {
      if (q.last != null) s.last = q.last;
      else if (q.bid != null && q.ask != null) s.last = (q.bid + q.ask) / 2;
      repaint(t.id);
    };
...
function stopLive(id) {
  ...
  if (s && s.qcb && s.contractId != null) {
    try {
      /** @type {any} */ (broker.for(s.brokerId)).unsubscribeQuotes(s.contractId, s.qcb);
    } catch (_) {}
  }
  tabLive.delete(id);
}
...
function syncLive() {
  ...
  [...tabLive.keys()].forEach((id) => {
    if (!vis.has(id)) stopLive(id);
  });
...
function render() {
  ...
  list.forEach((t) => {
    ...
    fillName(name, t);
    nameEls.set(t.id, name);
    name.ondblclick = (e) => {
      e.stopPropagation();
      startRename(t.id, name);
    };
    el.appendChild(name);
...
function appendWindowControls() {
  ...
  if (desktop.winAlwaysOnTopToggle) {
    ...
    const pin = mk('win-pin', '', pinned ? 'Unpin (always on top)' : 'Always on top', function () {
      ...
      this.title = on ? 'Unpin (always on top)' : 'Always on top';
      this.replaceChildren(themeIcon(pinSrc(on), 16));
    });
    pin.appendChild(themeIcon(pinSrc(pinned), 16));
...
function updateMaxBtn() {
  if (maxBtn) maxBtn.title = isMax ? 'Restore' : 'Maximize';
} // icon is static square.png; only the tooltip reflects state
  ...
  el.onpointerdown = (e) => {
    ...
    const move = (/** @type {PointerEvent} */ ev) => {
      if (!dragging) {
        ...
        if (t && t.id === liveTabId) {
          t.ws = getWorkspace();
          if (t.wsId) flushWorkspace(t.wsId, t.name, t.ws);
          upsert(id);
        } // flush the file IMMEDIATELY so the target window reads fresh (only if this tab is live)
...
async function onTabs(ids) {
  ...
  if (added.length)
    activeId = added[added.length - 1]; // focus the docked tab
...
async function reloadContent() {
  ...
  tabs.forEach((t, i) => {
    if (map.has(t.id)) {
      tabs[i] = /** @type {Tab} */ ({ ...map.get(t.id), ws: t.ws });
      map.delete(t.id);
    }
  });
