...
export function initSavedLayouts() {
  nameEl = $('layoutName'); btn = $('btnSaved'); menu = $('savedMenu');
  if (!btn) return;
  btn.title = 'Workspaces';
  const update = () => { if (nameEl) nameEl.textContent = getActiveWorkspaceName(); };
  bus.on('workspace:active', update);   // tabs.js emits this on any active-tab change / rename
  update();
  btn.onclick = (e) => { e.stopPropagation(); (menu && menu.classList.contains('open')) ? close() : open(); };
  document.addEventListener('click', (e) => { const t = /** @type {Node} */ (e.target); if (menu && !menu.contains(t) && !(/** @type {HTMLElement} */ (btn)).contains(t)) close(); });
}

function close() { if (menu) menu.classList.remove('open'); }
...
async function open() {
  ...
  menu.innerHTML = '';
  const h = document.createElement('div'); h.className = 'menu-header'; h.textContent = 'WORKSPACES'; menu.appendChild(h);
  if (!items.length) {
    const e = document.createElement('div'); e.className = 'menu-empty'; e.textContent = 'No workspaces yet — use + to create one.';
    menu.appendChild(e);
  }
  items.forEach((w) => {
    ...
    const nm = document.createElement('div'); nm.className = 'saved-name';
    nm.textContent = (w.name || 'Untitled') + (isCurrent ? '   (current)' : openElsewhere ? '   (open)' : '');
    const sub = document.createElement('div'); sub.className = 'saved-sub'; sub.textContent = w.summary || '';
    info.append(nm, sub); row.appendChild(info);
    row.onclick = () => { if (isCurrent || openElsewhere) return; loadWorkspaceHere(w.id); close(); };
