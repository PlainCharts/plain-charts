...
const linkOn = (/** @type {string} */ kind) => { const l = /** @type {any} */ (getLink()); return !l || l[kind] !== false; };   // the board's own toggle

export function initBoardSync() {
  try { chan = new BroadcastChannel(IPC.UI_BUS); } catch (_) { chan = null; return; }
  ...
  bus.on('pane:range', (src) => {
    ...
    if (isBoard()) { if (anchorAlive === true && !src._openPending && linkOn('range') && src._userRangeAt && Date.now() - src._userRangeAt < 700) chan.postMessage({ t: 'sb-brange', to: getLinkedTo(), pane: getLinkedPane(), range: src.range }); }
  ...
  bus.on('crosshair', ({ source, time, price }) => {
    ...
    if (isBoard()) { if (anchorAlive === true && !(source && source._openPending) && linkOn('crosshair')) chan.postMessage({ t: 'sb-bcross', to: getLinkedTo(), pane: getLinkedPane(), time, price }); }
  ...
  setInterval(() => { if (isBoard()) checkAnchor(); else sendPresence(); }, PRESENCE_MS);
}
...
function sendPresence() {
  ...
  const panes = getAllPanes().map((p, i) => ({ i, tf: p.tfId, range: p.range || null, ready: !p._openPending && !!p.seeded && !p.blanked }));
...
function onMsg(m) {
  if (!m) return;
  if (isBoard()) {
    const aWs = getLinkedTo(), aPane = getLinkedPane();
    if (m.t === 'sb-presence' && m.ws === aWs) {
      ...
      if (pane) {
        ...
        if (!pane.ready) { setAlive(false); return; }
        anchorRange = pane.range || anchorRange;
        const firstAlive = anchorAlive !== true;
        setAlive(true);
        if (pane.tf && pane.tf !== anchorTf) { anchorTf = pane.tf; setBoardTimeframe(pane.tf); }
        if (firstAlive && anchorRange) applyRange(anchorRange);   // align once on (re)connect
      }
      return;
    }
    if (m.t === 'sb-range' && m.ws === aWs && m.pane === aPane && m.range && linkOn('range')) { anchorRange = m.range; applyRange(m.range); return; }
    if (m.t === 'sb-cross' && m.ws === aWs && m.pane === aPane && linkOn('crosshair')) { applyCross(m.time, m.price); return; }
  } else {
    // CHART side: a board anchored to ME scrolled/hovered -> apply to the anchored pane only
    if (m.t === 'sb-brange' && m.to === myWs() && m.range) { const p = getAllPanes()[m.pane]; if (p && !p._openPending) { try { p.chart.timeAxis().setTimeWindow(m.range); } catch (_) {} } return; }   // ignore board moves while the anchor is still opening
    if (m.t === 'sb-bcross' && m.to === myWs()) { const p = getAllPanes()[m.pane]; if (p) { try { p.setCrosshair(m.time, m.price); } catch (_) {} } return; }
  }
}
...
const applyRange = (/** @type {any} */ range) => getAllPanes().forEach((/** @type {any} */ p) => {
  try { p.chart.timeAxis().setTimeWindow(range); } catch (_) {}
  try { if (p.boardEnsureHistory) p.boardEnsureHistory(range); } catch (_) {}
});
const applyCross = (/** @type {any} */ time, /** @type {any} */ price) => getAllPanes().forEach((/** @type {any} */ p) => { try { p.setCrosshair(time, price); } catch (_) {} });

/** @param {boolean} v */
function setAlive(v) {
  if (anchorAlive === v) return;
  anchorAlive = v;
  if (!v) { anchorTf = null; anchorRange = null; }
  bus.emit('board:anchor', v);   // panes blank (false) / unblank (true) -- Stage 2b
}
function checkAnchor() { if (Date.now() - lastAnchorSeen > STALE_MS) setAlive(false); }
