...
const fmtDate = (ms) => { try { return ms ? new Date(ms).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : ''; } catch (_) { return ''; } };
...
let overlay = null;
export function closeWorkspaceManager() { if (overlay) { overlay.remove(); overlay = null; } }
...
export async function openWorkspaceManager() {
  ...
  // Create new
  const createBtn = el('button', 'primary ws-create'); createBtn.textContent = '+  Create new workspace';
  createBtn.onclick = async () => {
    const existing = (await listWorkspaces()).map((w) => w.name);
    const name = await namePrompt({ title: 'Create new workspace', label: 'Workspace name', placeholder: 'My workspace', save: 'Create', existing });
    if (!name) return;
  ...
  const createBoardBtn = el('button', 'ws-create ws-create-board'); createBoardBtn.textContent = '+  Create study board';
  createBoardBtn.onclick = () => { closeWorkspaceManager(); openStudyBoardBuilder(); };
  ...
  SURFACE_KINDS.forEach(([kind, label]) => {
    ...
    b.onclick = async () => { closeWorkspaceManager(); await createSurfaceTab(kind, label); };
  ...
  overlay.onclick = (e) => { if (e.target === overlay) closeWorkspaceManager(); };
...
function escClose(e) { if (e.key === 'Escape') { closeWorkspaceManager(); document.removeEventListener('keydown', escClose); } }
...
async function renderList(listWrap) {
  ...
  items.forEach((w) => {
    ...
    icon.classList.add('ws-icon'); icon.title = w.isBoard ? 'Study board' : 'Chart layout';
    ...
    if (w.isBoard) row.appendChild(actBtn('Edit', async () => {   // reopen the builder to edit rows + re-link
      const rec = await readWorkspace(w.id); if (!rec) return;
      openStudyBoardBuilder({ wsId: w.id, name: rec.name || w.name, ws: rec.ws, rows: boardRowsFromWs(rec.ws), linkedTo: rec.ws && rec.ws.linkedTo, link: rec.ws && rec.ws.link });
      closeWorkspaceManager();
    }));
    row.appendChild(actBtn('Rename', async () => {
      const existing = items.map((i) => i.name).filter((n) => n !== w.name);
      const name = await namePrompt({ title: 'Rename workspace', label: 'Workspace name', value: w.name || '', save: 'Rename', existing });
      if (name) { await renameWorkspace(w.id, name); await renderList(listWrap); }
    }));
    row.appendChild(actBtn('Copy', async () => {
      const name = await namePrompt({ title: 'Copy workspace', label: 'New name', value: (w.name || 'Untitled') + ' copy', save: 'Copy' });
      if (name) { await copyWorkspace(w.id, name); await renderList(listWrap); }
    }));
    row.appendChild(actBtn('Delete', async () => {
      const ok = await confirmDialog({ title: 'Delete workspace', message: `Delete '${w.name || 'Untitled'}'? This can't be undone.`, yes: 'Delete' });
      if (ok) { await deleteWorkspace(w.id); await renderList(listWrap); }
    }));

    row.onclick = async () => { await openWorkspace(w.id); closeWorkspaceManager(); };
