...
let overlay = null;
export function closeStudyBoardBuilder() {
  if (overlay) {
    overlay.remove();
    overlay = null;
  }
...
export function openStudyBoardBuilder(edit) {
  ...
  /** @type {BoardRow[]} */
  const rows =
    edit && edit.rows && edit.rows.length
  ...
  const nameInput = /** @type {HTMLInputElement} */ (el('input', 'sb-name'));
  nameInput.type = 'text';
  nameInput.placeholder = 'Study board name';
  ...
  const noneOpt = /** @type {HTMLOptionElement} */ (el('option', null, '— pick a workspace —'));
  noneOpt.value = '';
  ...
  listWorkspaces()
    .then((wss) => {
      (wss || [])
        .filter((w) => !w.isBoard && w.id !== (edit && edit.wsId))
        .forEach((w) => {
          // anchor only to a CHART workspace, never a board or itself
          const o = /** @type {HTMLOptionElement} */ (el('option', null, w.name || 'Untitled'));
          o.value = w.id;
          linkSel.appendChild(o);
        });
      if (linkedTo && [...linkSel.options].some((o) => o.value === linkedTo)) linkSel.value = linkedTo;
      else linkedTo = null;
      loadAnchor();
    })
    .catch(() => {});
  ...
  async function loadAnchor() {
    ...
    pickGrid.innerHTML = '';
    if (!linkedTo) {
      pickHint.textContent = 'Pick a workspace, then click the chart to anchor the studies to.';
      updateCreate();
      return;
    }
  ...
  function renderPreview() {
    ...
    if (!ws || !panes.length) {
      pickHint.textContent = 'Could not read that workspace.';
      return;
    }
  ...
  const link = {
    range: edit && edit.link ? edit.link.range !== false : true,
  ...
  const taChk = /** @type {HTMLInputElement} */ (el('input'));
  taChk.type = 'checkbox';
  taChk.checked = sharedTimeAxis;
  taChk.onchange = () => {
    sharedTimeAxis = taChk.checked;
  };
  ...
  addBtn.onclick = () => {
    rows.push({ ...base(), studyId: firstOsc });
    renderRows();
  };
  body.appendChild(addBtn);

  const foot = el('div', 'sb-foot');
  const createBtn = /** @type {HTMLButtonElement} */ (
    el('button', 'primary', edit ? 'Save changes' : 'Create study board')
  );
  createBtn.onclick = async () => {
    ...
    const specs = valid.map((r) => ({
      ...
      params: r.params || {},
      _settings: r._settings,
      _range: r._range,
    }));
    const opts = { linkedTo, linkedPane, tf, link, sharedTimeAxis };
    if (edit && edit.wsId)
      updateBoard(edit.wsId, name, mergeStudyBoard(edit.ws, specs, opts)); // preserve state
  ...
  overlay.onclick = (/** @type {MouseEvent} */ e) => {
    if (e.target === overlay) closeStudyBoardBuilder();
  };
  const esc = (/** @type {KeyboardEvent} */ e) => {
    if (e.key === 'Escape') {
      closeStudyBoardBuilder();
  ...
  function rowEl(r, i) {
    ...
    const up = /** @type {HTMLButtonElement} */ (el('button', 'sb-ord-btn', '▲'));
    up.title = 'Move up';
    up.disabled = i === 0;
    up.onclick = () => {
    ...
    const dn = /** @type {HTMLButtonElement} */ (el('button', 'sb-ord-btn', '▼'));
    dn.title = 'Move down';
    ...
    symBtn.onclick = () =>
      openSymbolSearch((/** @type {string} */ bid, /** @type {string} */ sym) => {
        r.broker = bid;
        r.symbol = sym;
        renderRows();
      });
    ...
    const cmpOpt = /** @type {HTMLOptionElement} */ (el('option', null, '＋ Compare (price)'));
    cmpOpt.value = '__compare__';
    stSel.appendChild(cmpOpt); // a price chart of the symbol, not a study
    oscs.forEach((/** @type {any} */ s) => {
      const o = /** @type {HTMLOptionElement} */ (el('option', null, s.name || s.id));
      o.value = s.id;
      stSel.appendChild(o);
    });
    stSel.value = r.compare ? '__compare__' : r.studyId || firstOsc;
    stSel.onchange = () => {
      if (stSel.value === '__compare__') {
        r.compare = true;
        r.studyId = null;
      } else {
        if (stSel.value !== r.studyId) r.params = {};
        r.compare = false;
        r.studyId = stSel.value;
      } // new study -> fresh params
    };

    const rm = /** @type {HTMLButtonElement} */ (el('button', 'sb-rm', '✕'));
    rm.title = 'Remove row';
