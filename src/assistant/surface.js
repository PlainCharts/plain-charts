...
function appRoot() {
  try {
    return decodeURIComponent(new URLSearchParams(location.search).get('root') || '');
  } catch (_) {
    return '';
  }
}
...
function writeTextFile(relPath, text) {
  if (!req) throw new Error('no filesystem');
  const fs = req('fs'),
    path = req('path'),
    ROOT = appRoot();
...
/** @param {string} file */
function signalStudyReload(file) {
  try {
    if (reloadChan) reloadChan.postMessage({ file });
  } catch (_) {}
}
...
/** @param {string} id */
function parseTf(id) {
  const m = /^(\d*)(m|h|D|W|M)$/.exec(String(id || '').trim());
  return m ? { id: String(id), unit: m[2], n: m[1] ? +m[1] : 1 } : null;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const guard = setTimeout(() => {
      if (!done) {
        ...
        reject(new Error('symbol resolve timed out'));
      }
    }, 10000);
    broker.resolveSymbol.bind(broker)(symbol, (/** @type {any} */ inst, /** @type {any} */ err) => {
      if (done) return;
      done = true;
      clearTimeout(guard);
      err ? reject(new Error(String(err))) : resolve(inst);
    });
  });
}
...
export const surface = {
  ...
  async positions() {
    requireRule('read.account');
    return platform.positions.all();
  },
  async orders() {
    requireRule('read.account');
    return platform.orders.all();
  },
  async fills() {
    requireRule('read.account');
    return platform.fills.all();
  },
  async accounts() {
    requireRule('read.account');
    return platform.accounts.all();
  },
  async connections() {
    requireRule('read.account');
  ...
  /** @param {number} [limit] */
  async logs(limit) {
    requireRule('read.diagnostics');
    const h = platform.console.history() || [];
  ...
  async resolveSymbol(symbol) {
    ...
    const inst = await resolveOne(symbol);
  ...
  async bars(p) {
    requireRule('read.market');
    const t = parseTf(p.tf);
    if (!t) throw new Error('bad timeframe: ' + p.tf);
    const inst = await resolveOne(p.symbol);
    if (!inst || inst.id == null) throw new Error('unknown symbol: ' + p.symbol);
    ...
    return new Promise((resolve, reject) => {
      ...
      const finish = () => {
        ...
        clearTimeout(guard);
        resolve(acc.sort((a, b) => a.time - b.time));
      };
      ...
      broker.getBars.bind(broker)({ id: inst.id, tf: t, fromMs: from, toMs: to }, (/** @type {any} */ u) => {
        ...
        if (u.error) {
          ...
          clearTimeout(guard);
          return reject(new Error(String(u.error)));
        }
  ...
  async tabs() {
    ...
    const t = readJsonFile('settings/workspace/tabs.json') || {};
    return { tabs: t.tabs || [], active: t.active || null };
  },
  ...
  async workspace(wsId) {
    ...
    if (!wsId) {
      const t = readJsonFile('settings/workspace/tabs.json') || {};
      const active = (t.tabs || []).find((/** @type {any} */ x) => x.id === t.active);
      wsId = active && active.wsId;
    }
  ...
  async listStudies() {
    requireRule('read.workspace');
    return listDir('packages/studies').filter(
      (id) => readTextFile('packages/studies/' + id + '/' + id + '.js') != null,
    );
  },
  /** @param {string} name */
  async getStudy(name) {
    ...
    if (!s) throw new Error('invalid study name');
    const code = readTextFile('packages/studies/' + s + '/' + s + '.js');
    if (code == null) throw new Error('study not found: ' + name);
    return { name: s, code };
  },
  ...
  async addStudy(p) {
    requireRule('author.workspace');
    return runCommand('addStudy', { paneIndex: p.paneIndex, studyId: p.studyId, params: p.params });
  },
  /** @param {{ paneIndex?: number, symbol: string, broker?: string }} p */
  async setSymbol(p) {
    requireRule('author.workspace');
    return runCommand('setSymbol', { paneIndex: p.paneIndex, symbol: p.symbol, broker: p.broker });
  },
  /** @param {{ paneIndex?: number, tf: string }} p */
  async setTimeframe(p) {
    requireRule('author.workspace');
    return runCommand('setTimeframe', { paneIndex: p.paneIndex, tf: p.tf });
  },

  // ---- author.alerts : a price-level alert on a pane's symbol ----
  /** @param {{ paneIndex?: number, price: number }} p */
  async addAlert(p) {
    requireRule('author.alerts');
    return runCommand('addAlert', { paneIndex: p.paneIndex, price: p.price });
  },

  // ---- read.workspace : what the user is looking at / has selected on the active chart ----
  async getSelection() {
    requireRule('read.workspace');
    return runCommand('getSelection', {});
  },

  // ---- author.appearance : list + apply app themes ----
  async listThemes() {
    requireRule('author.appearance');
    return listDir('packages/themes')
  ...
  async applyTheme(name) {
    requireRule('author.appearance');
    if (!name) throw new Error('theme name required');
    return runCommand('applyTheme', { name });
  },

  // ---- author.drawings : add / list / remove drawings on a pane ----
  /** @param {{ paneIndex?: number, tool: string, points: any[], style?: any }} p */
  async addDrawing(p) {
    requireRule('author.drawings');
    if (!p.tool || !Array.isArray(p.points) || !p.points.length) throw new Error('tool and points required');
    return runCommand('addDrawing', { paneIndex: p.paneIndex, tool: p.tool, points: p.points, style: p.style });
  },
  async listDrawings() {
    requireRule('read.workspace');
    return runCommand('listDrawings', {});
  },
  /** @param {string} id */
  async removeDrawing(id) {
    requireRule('author.drawings');
    if (!id) throw new Error('drawing id required');
    return runCommand('removeDrawing', { id });
  },
  ...
  async listAddons() {
    ...
    return listDir('addons')
      ...
      .map((id) => ({
        ...
        running: !!(status[id] && status[id].running),
        error: (status[id] && status[id].error) || null,
      }));
  },
  ...
  async placeOrder(order) {
    requireRule('execute.orders');
    return sendOrder('placeOrder', [order]);
  },
  /** @param {any} mod */
  async modifyOrder(mod) {
    requireRule('execute.orders');
    return sendOrder('modifyOrder', [mod]);
  },
  /** @param {any} orderId */
  async cancelOrder(orderId) {
    requireRule('execute.orders');
    return sendOrder('cancelOrder', [orderId]);
  },
  /** @param {any} symbol */
  async closePosition(symbol) {
    requireRule('execute.orders');
    return sendOrder('closePosition', [symbol]);
  },

  // ---- control.connections : operational, sensitive (default off) ----
  /** @param {any} account */
  async connect(account) {
    requireRule('control.connections');
    return broker.connect(account);
  },
  /** @param {any} id */
  async disconnect(id) {
    requireRule('control.connections');
    return broker.disconnect(id);
  },
  /** @param {any} id */
  async setActive(id) {
    requireRule('control.connections');
    return broker.setActive(id);
  },
};
