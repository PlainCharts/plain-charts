...
function describe(method, o) {
  ...
  return ('Assistant wants to ' + verb + ' ' + (o.qty != null ? o.qty + ' ' : '') + (o.symbol || '') + price).replace(/\s+/g, ' ').trim();
}

if (chan) chan.onmessage = async (/** @type {MessageEvent} */ e) => {
  const m = e.data;
  if (!m) return;
  // Another window answered, or the host timed out -> dismiss this window's open modal for that request.
  if (m.type === 'reply' || m.type === 'cancel') { const ac = shown.get(m.id); if (ac) { shown.delete(m.id); ac.abort(); } return; }
  if (m.type !== 'request' || shown.has(m.id)) return;
  const ac = new AbortController();
  shown.set(m.id, ac);
  try { if (typeof window !== 'undefined' && window.focus) window.focus(); } catch (_) {}   // best-effort raise
  ...
  try { chan.postMessage({ type: 'reply', id: m.id, approved: !!approved }); } catch (_) {}
};
