...
if (chan) chan.onmessage = (/** @type {MessageEvent} */ e) => {
  ...
  const resolve = pending.get(m.id); if (!resolve) return;
  pending.delete(m.id); resolve(!!m.approved);
};
...
export function requestOrderConfirm(method, order, brokerId) {
  ...
  return new Promise((resolve) => {
    let done = false;
    const finish = (/** @type {boolean} */ ok) => { if (done) return; done = true; pending.delete(id); resolve(ok); };
    pending.set(id, finish);
    chan.postMessage({ type: 'request', id, method, order, brokerId });
    setTimeout(() => { if (!done) { try { chan.postMessage({ type: 'cancel', id }); } catch (_) {} finish(false); } }, TIMEOUT_MS);
  });
}
