...
export function controllerFor(p) {
  ...
  const overAxis = (e) => {
    let w = 0;
    try {
      w = series.priceAxis().width();
    } catch (_) {}
    return e.clientX - rect().left > el.clientWidth - w;
  };
  /** @param {number} y @returns {DragReg|null} */
  const nearest = (y) => {
    /** @type {DragReg|null} */ let best = null;
    let bd = DRAG_TOL;
    for (const r of regs) {
  ...
  const setLock = (on) => {
    ...
    locked = on;
    try {
      p.chart.configure({ handleScroll: !on, handleScale: !on });
    } catch (_) {}
    try {
      el.classList.toggle('drag-cursor-ns', on);
    } catch (_) {}
  };
  ...
  const onHover = (e) => {
    ...
    setLock(!!nearest(yOf(e)));
  };
  /** @param {MouseEvent} e */
  const dragMove = (e) => {
    if (!drag) return;
    const px = series.yToPrice(yOf(e));
    if (px == null) return;
    drag.last = px;
    drag.moved = true;
    drag.setPrice(px);
  ...
  const dragUp = () => {
    ...
    drag = null;
    setLock(false);
    if (d && d.moved && d.onCommit)
      try {
        d.onCommit(/** @type {number} */ (d.last));
      } catch (_) {}
  };
  /** @param {MouseEvent} e */
  const onDown = (e) => {
    if (overAxis(e)) return;
    const hit = nearest(yOf(e));
    if (!hit) return;
    drag = hit;
    drag.moved = false;
    drag.last = series.yToPrice(yOf(e));
...
export function createPriceLine(p, opts = {}) {
  ...
  if (opts.draggable) {
    ...
    reg = {
      yOf: () => {
        try {
          return series.priceToY(state.price);
        } catch (_) {
          return null;
        }
      },
  ...
  const handle = {
    price: () => state.price,
    update: (o = {}) => {
      if (o.price != null) state.price = Number(o.price);
      try {
        ln.configure(o);
      } catch (_) {}
    },
    setVisible: (on) => {
      try {
        ln.configure({ showLine: !!on, showAxisLabel: !!on });
      } catch (_) {}
      if (reg) reg.hidden = !on;
    },
    remove: () => {
      if (ctrl && reg) ctrl.remove(reg);
      try {
        series.removeLevel(ln);
      } catch (_) {}
    },
  };
  return handle;
...
export function createThread(p, opts = {}) {
  ...
  vline.style.cssText =
    'position:absolute;top:0;bottom:0;width:0;pointer-events:auto;cursor:ew-resize;' +
    'border-left:' +
    (st.width || 1) +
    'px ' +
    (st.dash ? 'dashed' : 'solid') +
    ' ' +
    (st.color || 'rgba(150,160,170,0.45)') +
    ';';
  ...
  /** @param {Bead} b */
  const syncLevel = (b) => {
    const be = beadEls.get(b.id);
    if (be && be.level)
      try {
        be.level.configure({ price: b.price });
      } catch (_) {}
  };
  ...
  const lock = (onOff) => {
    try {
      p.chart.configure({ handleScroll: !onOff, handleScale: !onOff });
    } catch (_) {}
  };
  ...
  const snapVert = () => {
    try {
      p.chart.setCursorSnapX(() => t2x(state.time));
    } catch (_) {}
  };
  const snapOff = () => {
    try {
      p.chart.setCursorSnapX(null);
    } catch (_) {}
  };
  ...
  function mountBead(b) {
    ...
    el.style.cssText =
      ...
      ';cursor:' +
      (b.onClick ? 'pointer' : draggable ? 'ns-resize' : 'default') +
      ...
      (b.label
        ? 'color:#fff;text-align:center;font-family:system-ui,sans-serif;font-weight:600;font-size:10px;line-height:' +
          (sz - 4) +
          'px;'
        : '');
    ...
    let level = null;
    try {
      level = p.series.addLevel({
        price: b.price,
        showLine: false,
        showAxisLabel: !b.hidden,
        color: b.color || '#2962ff',
        axisLabelColor: b.color || '#2962ff',
    ...
    el.addEventListener('mousedown', (e) => {
      ...
      const apply = (ev) => {
        const pr = y2p(ev.clientY - rect().top);
      ...
      const up = (ev) => {
        ...
        if (moved && canDrag) {
          ...
          if (pr != null)
            try {
              /** @type {(px: number) => void} */ (b.onCommit || b.onDrag)(pr);
            } catch (_) {}
        } else if (canClick) {
          const r = el.getBoundingClientRect();
          try {
            /** @type {(at: { x: number, y: number, price: number }) => void} */ (b.onClick)({
              x: r.left + r.width / 2,
              y: r.top + r.height / 2,
              price: b.price,
            });
  ...
  function layout() {
    ...
    state.beads.forEach((b) => {
      ...
      if (!be) return;
      const y = p2y(b.price);
      if (x == null || y == null || b.hidden) {
        be.el.style.display = 'none';
        return;
      }
      be.el.style.display = '';
      be.el.style.left = x + 'px';
      be.el.style.top = y + 'px';
    });
  }
  const tick = () => {
    if (dead) return;
  ...
  vline.addEventListener('mouseleave', () => {
    if (!timeDragging) snapOff();
  });
  vline.addEventListener(
    'mousedown',
    startDrag((ev, commit) => {
      const t = x2t(ev.clientX - rect().left);
      if (t == null) return;
      state.time = t;
      timeDragging = !commit; // own the vline position during the drag; release it on commit
      if (commit) snapOff(); // released the line -> normal crosshair
      try {
        (commit ? opts.onMoveCommit : opts.onMove) &&
          /** @type {(t: number) => void} */ (commit ? opts.onMoveCommit : opts.onMove)(t);
      } catch (_) {}
    }),
  );
  ...
  const handle = {
    update: (beadId, patch = {}) => {
      const b = state.beads.find((x) => x.id === beadId);
      if (!b) return;
      const be = beadEls.get(beadId);
      if (patch.price != null && beadId !== draggingId && Number(patch.price) !== b.price) {
        b.price = Number(patch.price);
        if (be && be.level)
          try {
            be.level.configure({ price: b.price });
          } catch (_) {}
      } // don't fight an active drag; skip unchanged prices (an unconditional configure forces an engine repaint per frame under the ride loop)
      if (patch.visible !== undefined) {
        b.hidden = !patch.visible;
        if (be && be.level)
          try {
            be.level.configure({ showAxisLabel: !b.hidden });
          } catch (_) {}
      }
      if (patch.color) {
        b.color = patch.color;
        if (be) {
          be.el.style.background = patch.color;
          if (be.level)
            try {
              be.level.configure({ color: patch.color, axisLabelColor: patch.color });
            } catch (_) {}
        }
      }
      if (patch.tag != null) {
        ...
        if (be && be.level)
          try {
            be.level.configure({ title: patch.tag });
          } catch (_) {}
      }
    },
    addBead: (b) => {
      if (!b || beadEls.has(b.id)) return;
      const nb = /** @type {Bead} */ ({ ...b, price: Number(b.price) });
      state.beads.push(nb);
      mountBead(nb);
    },
    removeBead: (beadId) => {
    ...
    setTime: (t) => {
      if (!timeDragging) state.time = Number(t);
    }, // don't fight an active vline drag
    time: () => state.time,
    setVisible: (onOff) => {
      layer.style.display = onOff ? '' : 'none';
      beadEls.forEach((be) => {
        if (be.level)
          try {
            be.level.configure({ showAxisLabel: !!onOff && !be.b.hidden });
          } catch (_) {}
      });
    },
    remove: () => {
