...
import { pillColor, typeLabel, qtyPicker, pillLayout, pillLine, QTY_PRESET_DEFAULTS, PILL_COLOR_DEFAULTS } from '../order-primitives-config.js';
...
export function createPillView(pane, opts = {}) {
  ...
  const reconcile = (want) => {
    for (const [key, d] of want) {
      ...
      const sidesVis = d.sides ? { left: d.sides.left ? d.sides.left.visible !== false : undefined, right: d.sides.right ? d.sides.right.visible !== false : undefined } : undefined;
      if (cur) { cur.h.update({ price: d.price, color: d.color, segments: d.segs, sides: sidesVis }); continue; }
      const h = createOrderPill(pane, {
        price: d.price, color: d.color, width: d.width, sides: d.sides, layout: pillLayout(), line: pillLine(),
        segments: d.segs.map((/** @type {string} */ t, /** @type {number} */ i) => ({ text: t, onClick: d.cellClicks && d.cellClicks[i] })),
        onClick: d.onClick, onCommit: d.onCommit,
      });
      if (h) pills.set(key, { h });
    }
    for (const [key, v] of [...pills]) { if (!want.has(key)) { v.h.remove(); pills.delete(key); } }
  };
  ...
  const set = (s = {}) => {
    ...
    if (entry != null) {
      const entryKebab = {
        left: opts.onHedgeStop ? { text: t('S'), color: pillColor('stop'), visible: s.hedgeStop == null, onDetach: (/** @type {number} */ px) => /** @type {any} */ (opts.onHedgeStop)(px) } : undefined,
        right: opts.onHedgeTarget ? { text: t('T'), color: pillColor('target'), visible: s.hedgeTarget == null, onDetach: (/** @type {number} */ px) => /** @type {any} */ (opts.onHedgeTarget)(px) } : undefined,
      };
      want.set('entry', { price: entry, sides: entryKebab, color: sideColor(s.side === 'short' ? 'sell' : 'buy'), width: 100,
    ...
    if (hs != null) {
      want.set('hedgeStop', { price: hs, color: pillColor('stop'), width: 100, segs: [String(s.hedgeStopQty != null ? s.hedgeStopQty : (s.qty != null ? s.qty : '')), typeLabel('stopLoss'), t('X')],
        cellClicks: [null, null, opts.onHedgeStopClear && (() => /** @type {any} */ (opts.onHedgeStopClear)())],
        onCommit: opts.onHedgeStop, onClick: opts.onEntry && (() => /** @type {any} */ (opts.onEntry)({ price: hs })) }); }
    const ht = num(s.hedgeTarget);
    if (ht != null) {
      want.set('hedgeTarget', { price: ht, color: pillColor('target'), width: 100, segs: [String(s.hedgeTargetQty != null ? s.hedgeTargetQty : (s.qty != null ? s.qty : '')), typeLabel('takeProfit'), t('X')],
        cellClicks: [null, null, opts.onHedgeTargetClear && (() => /** @type {any} */ (opts.onHedgeTargetClear)())],
        onCommit: opts.onHedgeTarget, onClick: opts.onEntry && (() => /** @type {any} */ (opts.onEntry)({ price: ht })) }); }
    ...
    for (const o of s.orders || []) {
      ...
      const ordKebab = {
        left: opts.onOrderStopCommit ? { text: t('S'), color: pillColor('stop'), visible: o.stopLoss == null, onDetach: (/** @type {number} */ px) => /** @type {any} */ (opts.onOrderStopCommit)(id, px) } : undefined,
        right: opts.onOrderTargetCommit ? { text: t('T'), color: pillColor('target'), visible: o.takeProfit == null, onDetach: (/** @type {number} */ px) => /** @type {any} */ (opts.onOrderTargetCommit)(id, px) } : undefined,
      };
      want.set('ord:' + id, { price: o.price, sides: ordKebab, color: sideColor(o.side), width: 100, segs: [String(o.qty != null ? o.qty : ''), typeLabel(ot), t('X')],
        cellClicks: [
          opts.onOrderQty && ((/** @type {HTMLElement} */ cell) => {
            openNumberPicker(cell, ordQty.get(String(id)) || 1, (v) => /** @type {any} */ (opts.onOrderQty)(id, v), { ...qtyPicker(), confirm: 'Confirm' });
          }),
          null,
      ...
      if (osl != null) {
        want.set('ordStop:' + id, { price: osl, color: pillColor('stop'), width: 100, segs: [String(o.qty != null ? o.qty : ''), typeLabel('stopLoss'), t('X')],
          cellClicks: [null, null, opts.onOrderStopClear && (() => /** @type {any} */ (opts.onOrderStopClear)(id))],
          onClick: opts.onOrderClick && (() => /** @type {any} */ (opts.onOrderClick)(id)),
          onCommit: opts.onOrderStopCommit && ((/** @type {number} */ px) => /** @type {any} */ (opts.onOrderStopCommit)(id, px)) }); }
      const otp = num(o.takeProfit);
      if (otp != null) {
        want.set('ordTarget:' + id, { price: otp, color: pillColor('target'), width: 100, segs: [String(o.qty != null ? o.qty : ''), typeLabel('takeProfit'), t('X')],
          cellClicks: [null, null, opts.onOrderTargetClear && (() => /** @type {any} */ (opts.onOrderTargetClear)(id))],
          onClick: opts.onOrderClick && (() => /** @type {any} */ (opts.onOrderClick)(id)),
          onCommit: opts.onOrderTargetCommit && ((/** @type {number} */ px) => /** @type {any} */ (opts.onOrderTargetCommit)(id, px)) }); }
    }
    ...
    if (proj != null) {
      ...
      const kebab = {
        left: opts.onPlanStop ? { text: t('S'), color: pillColor('planStop'), visible: l0.stop == null, onDetach: (/** @type {number} */ px) => /** @type {any} */ (opts.onPlanStop)(0, px) } : undefined,
        right: opts.onPlanTarget ? { text: t('T'), color: pillColor('planTarget'), visible: l0.target == null, onDetach: (/** @type {number} */ px) => /** @type {any} */ (opts.onPlanTarget)(0, px) } : undefined,
      };
      want.set('proj', { price: proj, sides: kebab, color: sideColor(projSide),
        segs: [String(projQty), projSide === 'sell' ? t('S') : t('B'), s.planArmed ? t('ENTRY') : typeLabel(projType), t('X'), t('V')],
        cellClicks: [
          opts.onProjectionQty && ((/** @type {HTMLElement} */ cell) => {
            openNumberPicker(cell, projQty, (v) => /** @type {any} */ (opts.onProjectionQty)(v), qtyPicker());
          }),
          opts.onProjectionSide && (() => /** @type {any} */ (opts.onProjectionSide)(projSide === 'sell' ? 'buy' : 'sell')),
    ...
    levels.forEach((lv, i) => {
      const st = num(lv && lv.stop), tg = num(lv && lv.target);
      const n = multi ? ' ' + (i + 1) : '';
      ...
      if (tg != null) rung('target', tg, tgtC, t('TGT') + n, (lv && lv.qty != null) ? String(lv.qty) : fullQty, opts.onPlanTarget, opts.onPlanTargetClear);
    });

    reconcile(want);
  };

  return { set, remove: () => { for (const v of pills.values()) v.h.remove(); pills.clear(); } };
}
...
function renderSettings(host, cfg, save) {
  ...
  const cell = (label, key) => {
    ...
    const l = document.createElement('span'); l.style.cssText = 'font-size:13px;flex:1;'; l.textContent = t(label);
    c.append(l, colorSwatch(colors[key] || PILL_COLOR_DEFAULTS[key], (/** @type {string} */ v) => { colors[key] = v; save(); }));
    return c;
  };
  /** @param {HTMLElement[]} cells */
  const grid = (...cells) => { const g = document.createElement('div'); g.style.cssText = 'display:grid;grid-template-columns:1fr 1fr;gap:12px 20px;align-items:center;'; g.append(...cells); host.appendChild(g); };
  /** @param {string} txt @param {boolean} [first] */
  const head = (txt, first) => { const h = document.createElement('div'); h.style.cssText = 'margin:' + (first ? '0' : '14px') + ' 0 8px;font-size:11px;letter-spacing:.08em;color:var(--tx-dim);'; h.textContent = t(txt); host.appendChild(h); };
  head('PLANNING', true);
  ...
  const prow = (label, ctrl) => { const r2 = document.createElement('div'); r2.className = 'sd-row'; const l2 = document.createElement('span'); l2.className = 'sd-label'; l2.textContent = t(label); r2.append(l2, ctrl); host.appendChild(r2); };
  const sideSel = document.createElement('select');
  [['right', 'Right'], ['left', 'Left']].forEach(([v, lbl]) => { const o = document.createElement('option'); o.value = v; o.textContent = t(lbl); sideSel.appendChild(o); });
  sideSel.value = lay.side === 'left' ? 'left' : 'right';
  sideSel.onchange = () => { lay.side = sideSel.value; save(); };
  prow('Position', sideSel);
  const off = document.createElement('input'); off.type = 'number'; off.min = '0'; off.style.width = '60px';
  off.value = String(Number.isFinite(Number(lay.offset)) ? lay.offset : 50);
  off.title = t('0 = flush to the price scale (or the left edge)');
  off.oninput = () => { const n = Number(off.value); lay.offset = Number.isFinite(n) && n >= 0 ? n : 0; save(); };
  prow('Offset (px)', off);
  ...
  prow('Line', strokeSwatch({
    color: { get: () => ln.color || PILL_COLOR_DEFAULTS.buy, set: (/** @type {any} */ v) => { ln.color = v; save(); } },
    width: { get: () => (Number.isFinite(Number(ln.width)) && Number(ln.width) >= 1 ? Number(ln.width) : 1), set: (/** @type {any} */ v) => { ln.width = Number(v) || 1; save(); } },
    lineStyle: { get: () => (ln.style === 'dashed' || ln.style === 'dotted' ? ln.style : 'solid'), set: (/** @type {any} */ v) => { ln.style = v; save(); } },
  }));
  const ext = document.createElement('input'); ext.type = 'checkbox'; ext.checked = !!lay.extend;
  ext.onchange = () => { lay.extend = ext.checked; save(); };
  prow('Extend line', ext);

  head('QTY PICKER');
  const r = document.createElement('div'); r.className = 'sd-row';
  const l = document.createElement('span'); l.className = 'sd-label'; l.textContent = t('Presets');
  const i = document.createElement('input'); i.type = 'text'; i.style.width = '110px';
  i.value = (Array.isArray(cfg.qtyPresets) && cfg.qtyPresets.length ? cfg.qtyPresets : QTY_PRESET_DEFAULTS).join(', ');
  i.oninput = () => { cfg.qtyPresets = i.value.split(',').map((x) => Number(x.trim())).filter((x) => isFinite(x)); save(); };
  r.append(l, i); host.appendChild(r);
}

// PRIMITIVE #2 -- the pill registers itself (primitive-contract.js): pick it in Settings > Trading > Primitives.
registerPrimitive({ id: 'pill', name: 'Pill', description: 'The built-in order view: a compact pill anchored to the price scale.', capabilities: { plan: true }, create: createPillView, renderSettings });
