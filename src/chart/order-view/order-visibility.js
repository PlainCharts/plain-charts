...
ch.onmessage = (/** @type {MessageEvent} */ e) => { const m = e.data; if (m && m.op === 'policy') { policy = norm(m.policy); notify(); } };

/** the global HIDE-ON-ENTRY policy (which categories hide when a position opens). @returns {VisPolicy} */
export function hideOnEntry() { return { ...P() }; }
...
export function setHideOnEntry(patch) {
  const cur = P();
  policy = { entry: patch.entry != null ? !!patch.entry : cur.entry, stop: patch.stop != null ? !!patch.stop : cur.stop, target: patch.target != null ? !!patch.target : cur.target };
  setSetting(KEY, { ...policy });
  try { ch.postMessage({ op: 'policy', policy: { ...policy } }); } catch (_) {}
  notify();
}
/** subscribe to policy changes (the editor UIs re-sync their checkboxes). @param {() => void} fn @returns {() => void} */
export function onHideOnEntryChange(fn) { subs.add(fn); return () => subs.delete(fn); }
...
export function applyEntryVisibility(broker, symbol) { const p = P(); setVis(broker, symbol, { entry: !p.entry, stop: !p.stop, target: !p.target }); }
/** RESET visibility to all-shown at the open -> flat transition. @param {string} broker @param {string} symbol */
export function resetEntryVisibility(broker, symbol) { setVis(broker, symbol, { entry: true, stop: true, target: true }); }
