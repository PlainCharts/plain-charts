...
import { isProjecting, isBracket, isArmed, getPlan, setLevels, setLevel, setProjecting, setArmed, commitStop, subscribe as subscribePlan } from './plan-store.js';   // shared PLAN state (gray projection + bracket ladder), keyed by broker+symbol
...
export function createOrderOverlay(pane) {
  ...
  const projBars = () => { const n = Number(tradeCfg().projBars); return Number.isFinite(n) && n >= 0 ? Math.round(n) : 5; };
  const projFrac = () => { const p = Number(tradeCfg().projHeightPct); return Number.isFinite(p) && p > 0 ? p / 100 : 0.20; };
  ...
  const anchorTime = () => {
    ...
    let barSec = 60; try { const tf = pane.tf && pane.tf(); if (tf) barSec = barMs(tf) / 1000; } catch (_) {}
  ...
  const visibleRange = () => {
    try {
      const s = pane.series; if (!s || !s.yToPrice || !pane.el) return null;
      const h = pane.el.getBoundingClientRect().height;
      let th = 0; try { th = pane.chart.timeAxis().height() || 0; } catch (_) {}
      const plot = h - th; if (!(plot > 0)) return null;
      const top = Number(s.yToPrice(0)), bot = Number(s.yToPrice(plot));
  ...
  /** @param {any} e */
  const jerr = (e) => platform.console.post({ level: 'error', cat: 'journal', src: pane.broker || 'app', msg: 'order command failed: ' + ((e && e.message) || e) });
  ...
  const legsOf = (id) => { const o = /** @type {any} */ (derive().orders.find((x) => x.id === id)) || {}; return { stopLoss: Number(o.stopLoss) || 0, takeProfit: Number(o.takeProfit) || 0 }; };
  ...
  const onOrderCommit = (id, price) => {
    ...
    const l = legsOf(id);
    command({ type: 'modifyOrder', broker: pane.broker, id, price: snap(price), stopLoss: l.stopLoss, takeProfit: l.takeProfit }).catch(jerr).finally(() => schedule());
  };
  ...
  /** @param {any} id @param {number} price */
  const onOrderStopCommit = (id, price) => { if (pane.broker) command({ type: 'modifyOrder', broker: pane.broker, id, stopLoss: snap(price), takeProfit: legsOf(id).takeProfit }).catch(jerr).finally(() => schedule()); };
  /** @param {any} id @param {number} price */
  const onOrderTargetCommit = (id, price) => { if (pane.broker) command({ type: 'modifyOrder', broker: pane.broker, id, stopLoss: legsOf(id).stopLoss, takeProfit: snap(price) }).catch(jerr).finally(() => schedule()); };
  /** @param {any} id */
  const onOrderStopClear = (id) => { if (pane.broker) command({ type: 'modifyOrder', broker: pane.broker, id, stopLoss: 0, takeProfit: legsOf(id).takeProfit }).catch(jerr).finally(() => schedule()); };
  /** @param {any} id */
  const onOrderTargetClear = (id) => { if (pane.broker) command({ type: 'modifyOrder', broker: pane.broker, id, stopLoss: legsOf(id).stopLoss, takeProfit: 0 }).catch(jerr).finally(() => schedule()); };
  // a HEDGING SL/TP dot has no order id -> move the POSITION's SL/TP (setStop/setTarget). Prices snap to the tick grid.
  /** @param {number} price */
  const onHedgeStop = (price) => { if (pane.broker && pane.symbol) command({ type: 'setStop', ctx: { broker: pane.broker, symbol: pane.symbol }, price: snap(price) }).catch(jerr).finally(() => schedule()); };
  /** @param {number} price */
  const onHedgeTarget = (price) => { if (pane.broker && pane.symbol) command({ type: 'setTarget', ctx: { broker: pane.broker, symbol: pane.symbol }, price: snap(price) }).catch(jerr).finally(() => schedule()); };
  ...
  const onHedgeStopClear = () => { if (pane.broker && pane.symbol) command({ type: 'setStop', ctx: { broker: pane.broker, symbol: pane.symbol }, price: 0 }).catch(jerr).finally(() => schedule()); };
  const onHedgeTargetClear = () => { if (pane.broker && pane.symbol) command({ type: 'setTarget', ctx: { broker: pane.broker, symbol: pane.symbol }, price: 0 }).catch(jerr).finally(() => schedule()); };
  ...
  const onEntry = () => {
    const d = /** @type {any} */ (window).desktop; if (!d || !d.openOrderTicket) return;
    const a = derive(); if (a.entry == null) return;
    const lots = /** @type {any[]} */ (platform.positionLots.all()).filter((l) => l.symbol === pane.symbol && (!pane.broker || l.broker === pane.broker) && Number(l.qty) > 0);
    if (lots.length === 1) {
      const l = lots[0];
      d.openOrderTicket({ tab: 'modify', position: { broker: l.broker, accountId: l.accountId, symbol: l.symbol, ticket: l.ticket, side: l.side, qty: l.qty, avgPrice: l.avgPrice, stopLoss: l.stopLoss, takeProfit: l.takeProfit, priceDecimals: l.priceDecimals, tickSize: l.tickSize } });
      return;
    }
    d.openOrderTicket({ tab: 'modify', position: { broker: pane.broker, symbol: pane.symbol, side: a.side, qty: a.qty, avgPrice: a.entry, stopLoss: a.hedgeStop, takeProfit: a.hedgeTarget } });
  };
  ...
  const onProjection = () => {
    const d = /** @type {any} */ (window).desktop; if (!d || !d.openOrderTicket || !pane.symbol) return;
  ...
  /** @param {any} id */
  const onOrderCancel = (id) => { if (pane.broker) command({ type: 'cancel', broker: pane.broker, id }).catch(jerr).finally(() => schedule()); };
  ...
  const onOrderQty = (id, qty) => { if (pane.broker && Number(qty) > 0) command({ type: 'modifyOrder', broker: pane.broker, id, qty: Number(qty) }).catch(jerr).finally(() => schedule()); };
  ...
  const onPositionClose = () => { if (pane.broker && pane.symbol) command({ type: 'closePosition', broker: pane.broker, symbol: pane.symbol }).catch(jerr).finally(() => schedule()); };
  ...
  const onOrderClick = (id) => {
    const d = /** @type {any} */ (window).desktop; if (!d || !d.openOrderTicket || !pane.symbol) return;
    const o = /** @type {any} */ (derive().orders.find((x) => x.id === id)); if (!o) return;
    // hedging (from the order's account) unlocks the SL/TP editor for a resting limit/stop (pending-order exits)
    const acct = /** @type {any[]} */ (platform.accounts.all()).find((a) => a.broker === pane.broker && String(a.accountId) === String(o.accountId));
    d.openOrderTicket({ tab: 'modify', order: { broker: pane.broker, symbol: pane.symbol, id, type: o.type, side: o.side, qty: o.qty, price: o.price, stopLoss: o.stopLoss, takeProfit: o.takeProfit, hedging: !!(acct && acct.hedging) } });
  };
  ...
  const onPlanStop = (i, price) => { commitStop(pane.broker, pane.symbol, i, snap(price), { flip: derive().entry == null && !isArmed(pane.broker, pane.symbol), snap, pivot: livePrice() }); };
  /** @param {number} i @param {number} price */
  const onPlanTarget = (i, price) => { setLevel(pane.broker, pane.symbol, i, { target: snap(price) }); };
  ...
  /** @param {number} i */
  const onPlanStopClear = (i) => { setLevel(pane.broker, pane.symbol, i, { stop: null }); };
  /** @param {number} i */
  const onPlanTargetClear = (i) => { setLevel(pane.broker, pane.symbol, i, { target: null }); };
  ...
  /** @param {number} price */
  const onProjectionMove = (price) => { if (pane.symbol) setLevels(pane.broker, pane.symbol, { ref: snap(price) }); };
  ...
  /** @param {number} q */
  const onProjectionQty = (q) => { const n = Number(q); if (pane.symbol && n > 0) setLevels(pane.broker, pane.symbol, { qty: n }); };
  ...
  /** @param {string} t */
  const onProjectionType = (t) => { if (pane.symbol && (t === 'market' || t === 'limit' || t === 'stop')) setLevels(pane.broker, pane.symbol, { orderType: t }); };
  // switch the planned entry's SIDE (the pill's B/S cell) -> plan.side; the confirm (V) places with it
  /** @param {string} sd */
  const onProjectionSide = (sd) => { if (pane.symbol && (sd === 'buy' || sd === 'sell')) setLevels(pane.broker, pane.symbol, { side: sd }); };
  ...
  const onProjectionCancel = () => { if (pane.symbol) setProjecting(pane.broker, pane.symbol, false); };
  ...
  const onProjectionConfirm = () => {
    ...
    if (p.owner) { setArmed(pane.broker, pane.symbol, true); return; }
    ...
    const bracket = (l0.stop != null || l0.target != null)
      ? { stopLoss: l0.stop != null ? Number(l0.stop) : 0, takeProfit: l0.target != null ? Number(l0.target) : 0 } : null;
    /** @type {any} */
    ...
    if (p.sizing && Number(p.sizing.risk) > 0) cmd.sizing = { risk: Number(p.sizing.risk), stop: Number(p.sizing.stop) };
    if (t !== 'market') {
      ...
      cmd.orderType = t; cmd.price = snap(px);
    }
    command(cmd).then((/** @type {any} */ r) => { if (r && r.ok) setProjecting(pane.broker, pane.symbol, false); }).catch(jerr).finally(() => schedule());
  };
  ...
  const onAnchor = (t, commit) => { if (commit && pane.symbol) { setLevels(pane.broker, pane.symbol, { anchor: Number(t) }); schedule(); } };
  ...
  const ensureView = () => { if (!view) { const prim = getPrimitive(activePrimitiveId()); view = prim ? prim.create(pane, { onEntry, onProjection, onProjectionMove, onProjectionQty, onProjectionType, onProjectionSide, onProjectionCancel, onProjectionConfirm, onPlanStop, onPlanTarget, onPlanStopClear, onPlanTargetClear, onHedgeStop, onHedgeTarget, onHedgeStopClear, onHedgeTargetClear, onOrderCommit, onOrderStopCommit, onOrderTargetCommit, onOrderStopClear, onOrderTargetClear, onOrderQty, onOrderClick, onOrderCancel, onPositionClose, onAnchor }) : null; } return view; };
  const offPrim = subscribePrimitives(() => { if (view) { view.remove(); view = null; } schedule(); });
  ...
  loadPrimitiveModules().then(() => { if (view) { view.remove(); view = null; } schedule(); }).catch(() => {});
  ...
  const livePrice = () => { const c = Number(pane.lastClose); return Number.isFinite(c) ? c : null; };
  ...
  /** @param {number} v */
  const snap = (v) => { const t = Number(pane.tickSize) || 0; const x = t > 0 ? Math.round(Number(v) / t) * t : Number(v); return Number.isFinite(x) ? Number(x.toPrecision(12)) : Number(v); };

  const render = () => {
    raf = 0;
    if (pane.destroyed) return;
    if (!enabled) { if (view) { view.remove(); view = null; } return; }   // PER-CHART toggle off -> draw nothing on this chart
    ...
    if (!hasActive && !planOn) { if (view) { view.remove(); view = null; } return; }   // nothing real and no plan -> clear
    let projection = null;
    /** @type {number|null} */
    ...
    if (planPre) {
      ...
      if (projPx != null) {
        ...
        if (isBracket(pane.broker, pane.symbol) || hasLegs) {
          planLevels = levels.map((lv) => ({ stop: (lv && lv.stop != null) ? snap(Number(lv.stop)) : null, target: (lv && lv.target != null) ? snap(Number(lv.target)) : null, qty: (lv && lv.qty != null) ? Number(lv.qty) : null }));
        }
      }
    } else if (planLive) {
      ...
      const p = getPlan(pane.broker, pane.symbol);
      const levels = Array.isArray(p.levels) ? p.levels : [];
      const ai = Number(p.activeIdx); const doneBelow = Number.isFinite(ai) && ai > 0 ? ai : 0;
      planLevels = levels.map((lv, i) => (i < doneBelow ? { stop: null, target: null, qty: null }
        : { stop: (lv && lv.stop != null) ? snap(Number(lv.stop)) : null, target: (lv && lv.target != null) ? snap(Number(lv.target)) : null, qty: (lv && lv.qty != null) ? Number(lv.qty) : null }));
    }
    ...
    v.set({
      time: anchorTime(),
      entry: show('entry') ? a.entry : null, side: a.side || undefined, qty: a.qty,
      hedgeStop: show('stop') ? a.hedgeStop : null, hedgeStopQty: a.hedgeStopQty,
      hedgeTarget: show('target') ? a.hedgeTarget : null, hedgeTargetQty: a.hedgeTargetQty,
      orders: a.orders,
      projection: show('entry') ? projection : null, projectionQty, projectionType, projectionSide,
      planLevels: planLevels.map((lv) => ({ stop: show('stop') ? lv.stop : null, target: show('target') ? lv.target : null, qty: lv.qty })),
      planArmed: armed, planOwner,
    });
  };
  ...
  const seedTypeRef = () => {
    ...
    const px = livePrice(); if (px == null) return;
  ...
  const projTick = () => {
    projRaf = 0; if (!planActive || pane.destroyed) return;
    seedBracket(); seedTypeRef();
    const px = livePrice();
    if (px !== lastRidePx) { lastRidePx = px; render(); }
    projRaf = requestAnimationFrame(projTick);
  };
  const startRide = () => { if (!projRaf) projRaf = requestAnimationFrame(projTick); };
  const stopRide = () => { if (projRaf) { cancelAnimationFrame(projRaf); projRaf = 0; } };
  ...
  const syncPlan = () => { const on = isProjecting(pane.broker, pane.symbol) || isBracket(pane.broker, pane.symbol); if (on !== planActive) { planActive = on; if (on) startRide(); else stopRide(); } seedBracket(); seedTypeRef(); schedule(); };
  ...
  const onBook = () => {
    ...
    if (has && !hadPosition) applyEntryVisibility(pane.broker, pane.symbol);        // flat -> open: apply the policy
  ...
  return {
    ...
    setEnabled: (on) => { const v = on !== false; if (v === enabled) return; enabled = v; if (!enabled && view) { view.remove(); view = null; } schedule(); },
    refresh: () => { syncPlan(); schedule(); },   // pane calls this on a symbol/broker switch (re-evaluate the plan for the new instrument)
    destroy() { offs.forEach((f) => { try { f(); } catch (_) {} }); offPlan(); offPrim(); stopRide(); if (raf) cancelAnimationFrame(raf); if (view) { view.remove(); view = null; } },
  };
}
