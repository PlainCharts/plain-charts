...
export const tradesMethods = {
  /** @this {TradesCtx} */
  applyTrades() { this._updateTrades(this.settings.trades); const ov = /** @type {any} */ (this).orderView; if (ov) ov.setEnabled(this.settings.trades.showOrders !== false); },
  /** @this {TradesCtx} @param {TradeSettings} t */
  previewTrades(t) { this._updateTrades(t); const ov = /** @type {any} */ (this).orderView; if (ov) ov.setEnabled(!t || t.showOrders !== false); },
  /** @this {TradesCtx} @param {TradeSettings} [t] */
  _updateTrades(t) {
    if (!this._series) return;   // board / oscillator pane: no candle series to mark
    t = t || this.settings.trades;
    if (!t || !t.visible) { this._stopTradeFeed(); try { this._series.setMarkers([]); } catch (_) {} return; }
  ...
  _startTradeFeed() {
    ...
    if (this._tradeSym !== this.symbol) {
      ...
      if (api.getHistory) api.getHistory({ fromMs: Date.now() - 29 * 86400000 }, (/** @type {any} */ fills) => {
        ...
        const keyOf = (f) => (f && f.id != null) ? ('id:' + f.id) : [f && f.contractId, f && f.time, f && f.side, f && f.price, f && f.qty].join('|');
        /** @type {Set<string>} */
        const seen = new Set();
        /** @type {TradeFill[]} */
        const merged = [];
        for (const f of hist.concat(this._trades || [])) { const k = keyOf(f); if (seen.has(k)) continue; seen.add(k); merged.push(f); }
        this._trades = merged;
  ...
  _stopTradeFeed() {
    const api = this.api();
    if (this._tradeCb && api && api.unsubscribeTrade) { try { api.unsubscribeTrade(this._tradeCb); } catch (_) {} }
    this._tradeCb = null;
  },
  ...
  _tradeGroups() {
    const tf = this.tf(); const bar = tf ? barMs(tf) : 60000;
  ...
  _renderTradeMarkers(t) {
    ...
    if (!visibleOnTf({ visibility: t.visibility }, this.tf())) { try { this._series.setMarkers([]); } catch (_) {} return; }
    const dot = t.style === 'dot';
    const th = Math.max(1, t.thickness || 2);
    const markers = this._tradeGroups().map((g) => {
    ...
    try { this._series.setMarkers(markers); } catch (_) {}
  },
  ...
  _hoverTrade(param) {
    ...
    if (!t || !t.visible || !this._series || !param || param.time == null || !param.point) { this._hideTradeTip(); return; }
    if (!visibleOnTf({ visibility: t.visibility }, this.tf())) { this._hideTradeTip(); return; }
    const tf = this.tf(); const bar = tf ? barMs(tf) : 60000;
    const cursorBar = Math.floor((param.time * 1000) / bar) * bar, cy = param.point.y;
    ...
    for (const g of this._tradeGroups()) {
      ...
      /** @type {number|null} */
      let y = null; try { y = this.series.priceToY(g.price); } catch (_) {}
      if (y == null) continue;
      const dy = Math.abs(cy - y);
      if (dy < bestDy) { bestDy = dy; best = g; }
    }
    if (best) this._showTradeTip(param.point, best); else this._hideTradeTip();
  },
  /** @this {TradesCtx} @param {{ x: number, y: number }} point @param {TradeGroup} g */
  _showTradeTip(point, g) {
    if (!this._tradeTip) { this._tradeTip = document.createElement('div'); this._tradeTip.className = 'trade-tip'; this.el.appendChild(this._tradeTip); }
    ...
    const when = new Date(g.time).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' });
    ...
    this._tradeTip.innerHTML = '<b style="color:' + side + '">' + (buy ? 'BUY' : 'SELL') + '</b> ' + g.qty + ' @ ' + Number(g.price).toFixed(dec) + '<br><span class="tt-time" style="color:' + fg + ';opacity:.65">' + when + '</span>';
    const w = 150;
  ...
  /** @this {TradesCtx} */
  _hideTradeTip() { if (this._tradeTip) this._tradeTip.style.display = 'none'; },
};
