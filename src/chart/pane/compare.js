...
export const compareMethods = {
  _effCompareChartType() {
    return this.compareHlMode ? 'line' : this.settings.compare && this.settings.compare.chartType;
  },
  _effCompareSource() {
    return this.compareHlMode || (this.settings.compare && this.settings.compare.line.source);
  },
  _effCompareColor() {
    const ln = this.settings.compare.line;
    return this.compareHlMode === 'high' ? ln.highColor : this.compareHlMode === 'low' ? ln.lowColor : ln.color;
  },
  _compareSource() {
    return PRICE_SOURCES[this._effCompareSource()] || PRICE_SOURCES.close;
  },
  _compareLineOpts() {
    const ln = this.settings.compare.line;
  ...
  _feedCompareLine() {
    ...
    cmp.lineSeries.feed(
      [...cmp.bars.values()].sort((a, b) => a.time - b.time).map((b) => ({ time: b.time, value: src(b) })),
    );
  },
  applyCompareChartType() {
    const cmp = this.compare,
      cfg = this.settings.compare;
    if (!cmp || !cmp.series || !cfg) return;
    if (this._effCompareChartType() === 'line') {
      cmp.series.configure({
      ...
      if (cmp.lineSeries) {
        try {
          this.chart.removePlot(cmp.lineSeries);
        } catch (_) {}
        cmp.lineSeries = null;
      }
  ...
  /** @param {string} t */
  setCompareChartType(t) {
    if (!this.settings.compare) return;
    this.compareHlMode = null;
    this.settings.compare.chartType = t === 'line' ? 'line' : 'candles';
    this.applyCompareChartType();
    if (this._updCmpHLBtn) this._updCmpHLBtn();
    bus.emit('pane:changed');
  },
  /** @param {string} s */
  setCompareLineSource(s) {
    if (!this.settings.compare) return;
    this.compareHlMode = null;
    this.settings.compare.line.source = s;
    this._feedCompareLine();
    if (this._updCmpHLBtn) this._updCmpHLBtn();
    bus.emit('pane:changed');
  },
  /** @param {any} patch */
  setCompareLineStyle(patch) {
    if (!this.settings.compare) return;
    Object.assign(this.settings.compare.line, patch);
    if (this.compare && this.compare.lineSeries) this.compare.lineSeries.configure(this._compareLineOpts());
    bus.emit('pane:changed');
  },
  compareChartTypeTarget() {
    const p = this,
      c = this.settings.compare;
    if (!c) return null;
    return p._ctTarget({
      title: 'Compare: ' + c.symbol,
      ln: () => c.line,
      getType: () => c.chartType,
      setType: (/** @type {string} */ t) => p.setCompareChartType(t),
      setSource: (/** @type {string} */ s) => p.setCompareLineSource(s),
      setStyle: (/** @type {any} */ patch) => p.setCompareLineStyle(patch),
      reColor: (/** @type {string} */ which) => {
        if (p.compareHlMode === which) p.applyCompareChartType();
      }, // compare H/L toggle: added next
    });
  },
  ...
  addCompare(brokerId, symbol) {
    ...
    if (!api) {
      log('Connect a broker to compare symbols.', true);
      return;
    }
    ...
    api.resolveSymbol(
      symbol,
      /** @type {any} */ (
        (/** @type {any} */ inst, /** @type {any} */ err) => {
          if (this.compare !== cmp) return; // replaced/removed meanwhile
          if (!inst) {
            log('[' + symbol + '] not resolved for compare' + (err ? ' (status ' + err.status + ')' : '') + '.', true);
            this.removeCompare();
            return;
          }
          ...
          try {
            const p = this.chart.panes()[this.paneIndexOf(cmp.series)];
            if (p) p.setStretchFactor(0.45);
          } catch (_) {}
          ...
          const store = {
            ...
            save: (/** @type {any[]} */ arr) => {
              if (this.settings.compare) this.settings.compare.drawings = arr;
              bus.emit('pane:changed');
            },
            loadTree: () => (this.settings.compare && this.settings.compare.tree) || [],
            saveTree: (/** @type {any} */ t) => {
              if (this.settings.compare) this.settings.compare.tree = t;
              bus.emit('pane:changed');
            },
          };
  ...
  _compareRequest() {
    ...
    cmp.reqId =
      api &&
      api.subscribeBars({ id: cmp.contractId, tf, fromMs }, (/** @type {any} */ u) => this._compareReport(cmp, u));
  },
  ...
  _compareFollow() {
    ...
    const toMs = cmp.fromMs;
    cmp.fromMs = want; // fetch only the newly-exposed gap
    const api = broker.for(cmp.brokerId);
  ...
  _compareReport(cmp, u) {
    ...
    if (u.error) {
      log('[' + cmp.symbol + '] compare bars failed: ' + u.error, true);
      return;
    }
  ...
  removeCompare() {
    ...
    if (this.compareDrawings) {
      try {
        this.compareDrawings.destroy();
      } catch (_) {}
      this.compareDrawings = null;
    }
  ...
  compareSetHidden(v) {
    ...
    if (cmp.series) {
      try {
        cmp.series.configure({ visible: !cmp.hidden });
      } catch (_) {}
    }
    if (cmp.lineSeries) {
      try {
        cmp.lineSeries.configure({ visible: !cmp.hidden });
      } catch (_) {}
    }
    // hidden compare -> not preserved (its pane vanishes); a collapsed one keeps its bar
    try {
      const p = this.chart.panes()[this.paneIndexOf(cmp.series)];
      if (p && p.setPreserveEmptyPane) p.setPreserveEmptyPane(this.compareMode === 'collapsed' && !cmp.hidden);
    } catch (_) {}
  ...
  _makeCompareHLBtn(mk) {
    ...
    const upd = () => {
      const m = this.compareHlMode,
        ln = this.settings.compare && this.settings.compare.line;
  ...
  _buildCompareUI() {
    ...
    ctrls.className = 'pane-compare-ctrls';
    ...
    this._cmpCollapseBtn = mk('⌄', 'Collapse pane', () =>
      this._setPaneMode(this.compareMode === 'collapsed' ? 'normal' : 'collapsed'),
    );
    const cfg = mk('⚙', 'Chart type (candles / line)', () => bus.emit('charttype:open', this.compareChartTypeTarget()));
    ctrls.append(
      this._makeCompareHLBtn(mk),
      up,
      dn,
      this._cmpMaxBtn,
      this._cmpCollapseBtn,
      cfg,
      mk('🗑', 'Remove comparison', () => this.removeCompare()),
    );
    ...
    this._cmpReposition = () => {
      ...
      this._cmpRaf = requestAnimationFrame(() => {
        this._cmpRaf = 0;
        this._positionCompareUI();
      });
    };
    ...
    try {
      this._cmpRO = new ResizeObserver(this._cmpReposition);
      this._cmpRO.observe(this.el);
    } catch (_) {}
  ...
  _destroyCompareUI() {
    if (this._cmpRO) {
      try {
        this._cmpRO.disconnect();
      } catch (_) {}
      this._cmpRO = null;
    }
    if (this._cmpReposition) {
      this.el.removeEventListener('pointermove', this._cmpReposition);
      this._cmpReposition = null;
    }
    if (this.compareUI) {
      this.compareUI.ctrls.remove();
      this.compareUI = null;
    }
    this._updCmpHLBtn = null;
  },
  _positionCompareUI() {
    const ui = this.compareUI;
    if (!ui) return;
    let topH = 0,
      psw = 0;
    try {
      topH = this.paneTopOf(this.compare && this.compare.series);
    } catch (_) {}
    try {
      psw = this.chart.priceAxis('right').width();
    } catch (_) {}
  ...
  _setPaneMode(mode) {
    ...
    try {
      cmp.series.configure({ visible });
    } catch (_) {}
    if (cmp.lineSeries) {
      try {
        cmp.lineSeries.configure({ visible });
      } catch (_) {}
    }
    // collapse -> keep the (now-invisible) pane as an empty bar; otherwise let it size normally
    try {
      const p = this.chart.panes()[this.paneIndexOf(cmp.series)];
      if (p && p.setPreserveEmptyPane) p.setPreserveEmptyPane(mode === 'collapsed' && !cmp.hidden);
    } catch (_) {}
    if (this.compareDrawings) this.compareDrawings.setSuppressed(!visible);
    this.compareMode = mode;
    if (this._cmpCollapseBtn) {
      const c = mode === 'collapsed';
      this._cmpCollapseBtn.textContent = c ? '⌃' : '⌄';
      this._cmpCollapseBtn.title = c ? 'Expand pane' : 'Collapse pane';
    }
