...
export const appearanceMethods = {
  ...
  /** @param {any} c */
  applyCandlesObj(c) { if (this._series) this._series.configure(candleOptions(c)); },
  applyCandles() { this.applyCandlesObj(this.settings.candles); },
  /** @param {any} c */
  previewCandles(c) { this.applyCandlesObj(c); },
  ...
  tzOffset() { return this.settings.tzOffsetMin != null ? this.settings.tzOffsetMin : getOffsetMin(); },

  // set THIS pane's display offset (minutes east of UTC) and apply it live (bottom-bar tz picker)
  /** @param {number} offsetMin */
  setTz(offsetMin) { this.settings.tzOffsetMin = offsetMin; this.applyTz(); bus.emit('pane:changed'); },
  ...
  applyTz(offOverride) {
    ...
    const daily = () => { const t = this.tf(); return !!t && (t.unit === 'D' || t.unit === 'W' || t.unit === 'M'); };
  ...
  /** @param {any} c */
  applyCanvasObj(c) { this.chart.configure(canvasOptions(c)); this.applyNavButtons(c.navButtons); this.applyPaneButtons(c.paneButtons); },
  /** @param {string} mode */
  ...
  applyCanvas() { this.applyCanvasObj(this.settings.canvas); },
  /** @param {any} c */
  previewCanvas(c) { this.applyCanvasObj(c); },
  ...
  shiftEndOn() { return this.settings.canvas.shiftEnd !== false; },
  toggleShiftEnd() { const c = this.settings.canvas; c.shiftEnd = c.shiftEnd === false; this.applyCanvas(); bus.emit('pane:changed'); return c.shiftEnd; },
  // auto-scroll: follow the latest bar as new bars arrive (only while at the right edge). Returns the new on-state.
  autoScrollOn() { return this.settings.canvas.autoScroll !== false; },
  toggleAutoScroll() { const c = this.settings.canvas; c.autoScroll = c.autoScroll === false; this.applyCanvas(); bus.emit('pane:changed'); return c.autoScroll; },
  ...
  getLineSettings() { const o = /** @type {Record<string, any>} */ ({}); LINE_KEYS.forEach((k) => { o[k] = this.settings[k]; }); return structuredClone(o); },
  /** @param {Record<string, any>} obj */
  applyLineSettings(obj) {
    if (!obj) return;
    LINE_KEYS.forEach((k) => { if (k in obj) this.settings[k] = obj[k]; });
    this.applySettings();
    bus.emit('pane:changed');
  },
  /** @param {any} a */
  previewAppearance(a) { this.previewCandles(a.candles); this.previewCanvas(a.canvas); this.previewStatus(a.statusLine); this.previewIndicators(a.indicators); if (a.trades) this.previewTrades(a.trades); this.applyTz(a.tzOffsetMin); },
  applyAppearance() { this.applyCandles(); this.applyCanvas(); this.applyStatus(); this.applyIndicators(); this.applyTrades(); this.applyTz(); },   // revert to committed
  /** @param {any} a */
  commitAppearance(a) {
    this.settings.candles = a.candles; this.settings.canvas = a.canvas; this.settings.statusLine = a.statusLine; this.settings.indicators = a.indicators;
    ...
    this.applyAppearance(); bus.emit('pane:changed');
  },

  // push the indicator-legend display options to kapelka/skin; bgColor follows the chart background
  applyIndicators() { this.applyIndicatorsObj(this.settings.indicators); },
  /** @param {any} ind */
  previewIndicators(ind) { this.applyIndicatorsObj(ind); },
  /** @param {any} ind */
  applyIndicatorsObj(ind) {
    if (!this.skin || !ind) return;
    this.skin.setLegendOptions({
      title: ind.title !== false, inputs: ind.inputs !== false, values: ind.values !== false,
      bg: ind.bg !== false, bgColor: this.settings.canvas && this.settings.canvas.background,
  ...
  applyStatusObj(s) {
    ...
    /** @type {HTMLElement} */ (this.titleEl).textContent = this.symbol + ' · ' + (this.tfId || '') + (bl ? ' · ' + bl : '');
  ...
  applyStatus() { this.applyStatusObj(this.settings.statusLine); },
  /** @param {any} s */
  previewStatus(s) { this.applyStatusObj(s); },
  ...
  updateValues(bar) {
    ...
    if (!b) { this.valuesEl.textContent = ''; return; }
