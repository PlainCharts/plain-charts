...
export const surfaceMethods = {
  ...
  _orderOccupants() {
    /** @type {Occupant[]} */
    const occ = [{ series: this.series, get: () => this.settings.mainPaneIdx, set: (v) => { this.settings.mainPaneIdx = v; } }];
    if (this.studies) this.studies.attached.forEach((/** @type {any} */ a) => {
      if (a.overlay) return;
      const s = a.plots.values().next().value;
      if (s) occ.push({ series: s, get: () => a._wantOrder, set: (v) => { a._wantOrder = v; } });
    });
    if (this.compare && this.compare.series) occ.push({ series: this.compare.series, get: () => this.settings.compare && this.settings.compare.paneIdx, set: (v) => { if (this.settings.compare) this.settings.compare.paneIdx = v; } });
    return occ;
  },
  ...
  _applyPaneOrder() {
    ...
    occ.forEach((o, i) => { const v = o.get(); o._k = (o.series === this.series) ? -Infinity : (v != null ? v : 1000 + i); });   // unknown → bottom, stable
    occ.sort((a, b) => /** @type {number} */ (a._k) - /** @type {number} */ (b._k));
    occ.forEach((o, target) => {
      const cur = this.paneIndexOf(o.series);
      if (cur >= 0 && cur !== target) { try { this.chart.panes()[cur].moveTo(target); } catch (_) {} }
    });
  ...
  /** @this {SurfaceCtx} */
  _scheduleApplyOrder() { if (this._orderRaf) return; this._orderRaf = requestAnimationFrame(() => { this._orderRaf = 0; this._applyPaneOrder(); }); },
  ...
  paneIndexOf(series) {
    try { const ps = this.chart.panes(); for (let i = 0; i < ps.length; i++) { let l; try { l = ps[i].getSeries(); } catch (_) { l = []; } if (series && l.indexOf(series) !== -1) return i; } } catch (_) {}
    return -1;
  },
  ...
  applyPaneMode(series, mode, state) {
    ...
    const idx = this.paneIndexOf(series); if (idx < 0) return;
    const mainIdx = this.paneIndexOf(this.series);
    /** @param {any} p @returns {number} */
    const sf = (p) => { try { return p.getStretchFactor(); } catch (_) { try { return p.getHeight(); } catch (_2) { return 1; } } };
    /** @param {any} p @returns {number} */
    const px = (p) => { try { return p.getHeight() || 0; } catch (_) { return 0; } };
    /** @param {any} p @param {number} f */
    const set = (p, f) => { try { p.setStretchFactor(Math.max(0.0001, f)); } catch (_) {} };
    ...
    if (mode === 'max') {
      ...
      ps.forEach((/** @type {any} */ p, /** @type {number} */ i) => set(p, i === idx ? COLLAPSED_H : i === absorb ? h[i] + delta : h[i]));
    } else if (state.snap) {                       // restore from max -> the whole layout
      state.snap.forEach((/** @type {number} */ f, /** @type {number} */ i) => { if (ps[i]) set(ps[i], f); });
      state.snap = null; state.collapsedPx = null;
    } else if (state.collapsedPx != null) {        // restore from collapse -> ONLY this pane (+ absorber),
      const h = ps.map(px), grow = Math.max(0, state.collapsedPx - h[idx]);   // so other collapsed panes stay collapsed
      ps.forEach((/** @type {any} */ p, /** @type {number} */ i) => set(p, i === idx ? state.collapsedPx : i === absorb ? Math.max(COLLAPSED_H, h[i] - grow) : h[i]));
  ...
  paneTopOf(series) {
    try {
      const ps = this.chart.panes(); let h = 0;
      for (let i = 0; i < ps.length; i++) {
        let list; try { list = ps[i].getSeries(); } catch (_) { list = []; }
        if (series && list.indexOf(series) !== -1) return h;
        try { h += ps[i].getHeight(); } catch (_) {}
      }
    } catch (_) {}
    return 0;
  },
