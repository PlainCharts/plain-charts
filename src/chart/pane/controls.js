...
export const controlMethods = {
  ...
  addControls() {
    ...
    const nearRect = (/** @type {PointerEvent} */ e, /** @type {Element} */ el) => { const r = el.getBoundingClientRect(); return e.clientX >= r.left - NEAR && e.clientX <= r.right + NEAR && e.clientY >= r.top - NEAR && e.clientY <= r.bottom + NEAR; };
    this.el.addEventListener('pointermove', (/** @type {PointerEvent} */ e) => {
      ...
      this.el.querySelectorAll('.pane-compare-ctrls, .skin-ctrls').forEach((/** @type {Element} */ c) => c.classList.toggle('near', nearRect(e, c)));
    });
    this.el.addEventListener('pointerleave', () => {
      if (this.controls) this.controls.classList.remove('near');
      this.el.querySelectorAll('.pane-compare-ctrls.near, .skin-ctrls.near').forEach((/** @type {Element} */ c) => c.classList.remove('near'));
    });

    // settings gear pinned to the bottom-right corner (time/price scale junction)
    const gear = document.createElement('button');
    gear.className = 'pane-gear-corner';
    gear.textContent = '⚙'; gear.title = 'Chart settings';
    gear.onclick = (e) => { e.stopPropagation(); openChartSettings(this, gear.getBoundingClientRect()); };
    ...
    this.forwardBtn.onclick = (/** @type {MouseEvent} */ e) => { e.stopPropagation(); this.chart.timeAxis().scrollToNow(); };
    ...
    this.titleEl = document.createElement('span'); this.titleEl.className = 'ps-title';
    this.valuesEl = document.createElement('span'); this.valuesEl.className = 'ps-values';
    // market-status dot -- far right of all else in the status line (updateMarketDot colours/shows it)
    this.mktDotEl = document.createElement('span'); this.mktDotEl.className = 'ps-mkt';
    this.mktDotEl.style.cssText = 'display:none;width:8px;height:8px;border-radius:50%;margin-left:7px;vertical-align:middle;cursor:pointer;pointer-events:auto;';   // parent .pane-status is pointer-events:none
    this.mktDotEl.onclick = (/** @type {MouseEvent} */ e) => { e.stopPropagation(); this._toggleMarketPopup(); };   // -> market-hours info popup
  ...
  _buildBoardCtrls() {
    ...
    const mk = (/** @type {string} */ txt, /** @type {string} */ title, /** @type {() => void} */ fn) => { const b = document.createElement('button'); b.className = 'skin-ctrl'; b.textContent = txt; b.title = title; b.onclick = (e) => { e.stopPropagation(); fn(); }; return b; };
    const cluster = document.createElement('div'); cluster.className = 'skin-ctrls board-ctrls';
    const up = mk('↑', 'Move up', () => bus.emit('board:move', { pane: this, dir: -1 }));
    const dn = mk('↓', 'Move down', () => bus.emit('board:move', { pane: this, dir: 1 }));
    const max = mk('⤢', 'Maximize', () => { const m = this.boardMode === 'max' ? 'normal' : 'max'; bus.emit('board:mode', { pane: this, mode: m }); max.title = m === 'max' ? 'Restore' : 'Maximize'; });
    const col = mk('⌄', 'Collapse', () => { const m = this.boardMode === 'collapsed' ? 'normal' : 'collapsed'; bus.emit('board:mode', { pane: this, mode: m }); col.textContent = m === 'collapsed' ? '⌃' : '⌄'; col.title = m === 'collapsed' ? 'Expand' : 'Collapse'; });
    const cfg = mk('⚙', 'Chart type', () => bus.emit('charttype:open', this.chartTypeTarget()));
    cluster.append(this._makeHLBtn(), up, dn, max, col, cfg);
    cluster.style.top = '4px'; cluster.style.right = '60px';   // sensible default until the scale width is known
  ...
  _buildMainHL() {
    ...
    const cluster = document.createElement('div'); cluster.className = 'skin-ctrls board-ctrls main-hl';
    cluster.append(this._makeHLBtn());
    cluster.style.top = '4px'; cluster.style.right = '60px';
    this.el.appendChild(cluster);
    this.boardCtrlsEl = cluster;
    this._positionBoardCtrls();
  },
  _positionBoardCtrls() {
    ...
    let sw = 0; try { sw = this.chart.priceAxis('right').width(); } catch (_) {}
  ...
  setBoardCollapsed(collapsed) {
    ...
    if (collapsed) this._series.configure({ upColor: 'rgba(0,0,0,0)', downColor: 'rgba(0,0,0,0)', showBorder: false, showWick: false });
    else this._series.configure(candleOptions(this.settings.candles));
    if (this.lineSeries) { try { this.lineSeries.configure(collapsed ? { color: 'rgba(0,0,0,0)' } : this._lineOpts()); } catch (_) {} }
  },
};
