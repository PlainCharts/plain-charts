...
export function createOrderPill(pane, opts = {}) {
  ...
  const ln = opts.line || {};
  const lnW = Number.isFinite(Number(ln.width)) && Number(ln.width) >= 1 ? Math.min(6, Math.round(Number(ln.width))) : 1;
  ...
  pill.style.cssText = 'position:absolute;transform:translateY(-50%);display:inline-block;min-width:' + (Number(opts.width) > 0 ? Number(opts.width) : 140) + 'px;text-align:center;white-space:nowrap;overflow:hidden;'
  ...
  segs.forEach((s, i) => {
    ...
    cell.style.cssText = 'display:inline-block;vertical-align:middle;line-height:20px;padding:0 7px;' + (i > 0 ? 'border-left:1px solid rgba(255,255,255,.35);' : '') + (s.onClick ? 'cursor:pointer;' : '');
    if (s.onClick) {
      cell.onmouseenter = () => { cell.style.background = 'rgba(255,255,255,.22)'; };
      cell.onmouseleave = () => { cell.style.background = 'transparent'; };
      cell.onclick = (e) => { e.stopPropagation(); e.preventDefault(); if (justDragged) return; try { /** @type {(c: HTMLElement) => void} */ (s.onClick)(cell); } catch (_) {} };
    }
    cells.push(cell);
  ...
  const mountSide = (side, pos) => {
    ...
    el.style.cssText = 'position:absolute;transform:translate(-50%,-50%);box-sizing:border-box;width:' + SIDE_D + 'px;height:' + SIDE_D + 'px;'
      + 'border-radius:50%;background:' + (side.color || '#7e8a97') + ';border:2px solid #0e0e11;box-shadow:0 0 0 1px rgba(255,255,255,.25);'
      + 'color:#fff;text-align:center;font:600 11px system-ui,sans-serif;line-height:' + (SIDE_D - 4) + 'px;'
    ...
    el.addEventListener('mousedown', (e) => {
      ...
      lock(true);
      /** @param {MouseEvent} ev */
      const move = (ev) => { if (!moved && Math.abs(ev.clientY - sy) > 4) moved = true; if (moved) { rec.dragging = true; curY = ev.clientY - host.getBoundingClientRect().top; el.style.top = curY + 'px'; } };
      const up = () => {
        document.removeEventListener('mousemove', move, true); document.removeEventListener('mouseup', up, true);
        lock(false); rec.dragging = false;
        if (moved && curY != null) { const pr = y2p(curY); if (pr != null) try { rec.side.onDetach(pr); } catch (_) {} }
  ...
  let level = null;
  try { level = pane.series.addLevel({ price: state.price, showLine: false, showAxisLabel: true, color: state.color, axisLabelColor: state.color, axisLabelTextColor: '#fff', title: '', _ordPill: true }); } catch (_) {}
  ...
  const paint = () => {
    ...
    if (level) try { level.configure({ color: state.color, axisLabelColor: state.color }); } catch (_) {}
  };
  paint();
  ...
  const y2p = (y) => { try { return pane.series.yToPrice(y); } catch (_) { return null; } };
  const chartW = () => { const w = pane.chart._chartW; return (w && isFinite(w)) ? w : host.getBoundingClientRect().width; };
  const chartL = () => { const l = pane.chart._chartLeftPx; return (Number.isFinite(l) && l > 0) ? l : 0; };   // plot's left edge (the left price scale's width when the scale sits left, else 0)
  ...
  const layout = () => {
    ...
    if (y == null) { if (lastGeo !== 'hidden') { lastGeo = 'hidden'; pill.style.display = 'none'; line.style.display = 'none'; lineL.style.display = 'none'; sideEls.forEach((s) => { s.el.style.display = 'none'; }); } return; }
    ...
    const geo = y + '|' + left0 + '|' + right0 + '|' + w + '|' + dragSig + '|' + sideEls.map((s) => s.side.visible === false ? 0 : 1).join('');
    ...
    const pillLeft = laySide === 'left'
      ? Math.min(left0 + layOff, Math.max(left0, right0 - w))
      : Math.max(left0, right0 - layOff - w);
    pill.style.top = y + 'px'; pill.style.left = pillLeft + 'px';
    ...
    line.style.top = y + 'px'; line.style.left = (pillLeft + w) + 'px'; line.style.width = rw + 'px';
    const lw = Math.max(0, pillLeft - left0);
    lineL.style.display = (layExtend && lw > 0) ? '' : 'none';
    lineL.style.top = y + 'px'; lineL.style.left = left0 + 'px'; lineL.style.width = lw + 'px';
    // kebab circles ride the pill's y: left one just before the pill, right one just after (on the line)
    sideEls.forEach((s) => {
      if (s.side.visible === false) { s.el.style.display = 'none'; return; }
      s.el.style.display = '';
  ...
  /** @param {boolean} on */
  const lock = (on) => { try { pane.chart.configure({ handleScroll: !on, handleScale: !on }); } catch (_) {} };
  pill.addEventListener('mousedown', (e) => {
    ...
    const apply = (ev) => { const pr = y2p(ev.clientY - host.getBoundingClientRect().top); if (pr == null) return null; state.price = pr; if (level) try { level.configure({ price: pr }); } catch (_) {} return pr; };
    /** @param {MouseEvent} ev */
    const move = (ev) => {
      if (!moved && Math.abs(ev.clientX - sx) + Math.abs(ev.clientY - sy) > 4) moved = true;
      if (moved && draggable) { dragging = true; const pr = apply(ev); if (pr != null) try { opts.onDrag && opts.onDrag(pr); } catch (_) {} }
    };
    /** @param {MouseEvent} ev */
    const up = (ev) => {
      document.removeEventListener('mousemove', move, true); document.removeEventListener('mouseup', up, true);
      ...
      dragging = false;
      if (moved && draggable) {
        justDragged = true; setTimeout(() => { justDragged = false; }, 0);   // the release click must not fire a cell
        const pr = apply(ev); if (pr != null) try { /** @type {(px: number) => void} */ (opts.onCommit || opts.onDrag)(pr); } catch (_) {}
        ...
        if (!onCell) try { opts.onClick(); } catch (_) {}
      }
    };
    document.addEventListener('mousemove', move, true); document.addEventListener('mouseup', up, true);
  });
  ...
  return {
    update: (o = {}) => {
      ...
      if (o.price != null && !dragging && Number(o.price) !== state.price) { state.price = Number(o.price); if (level) try { level.configure({ price: state.price }); } catch (_) {} }
      if (o.label != null) state.label = o.label;
      if (o.color && o.color !== state.color) { state.color = o.color; paint(); }
      if (Array.isArray(o.segments)) o.segments.forEach((t, i) => { if (t != null && cells[i] && cells[i].textContent !== t) cells[i].textContent = t; });
      if (o.sides) sideEls.forEach((s) => { const v = /** @type {any} */ (o.sides)[s.pos]; if (v !== undefined) s.side.visible = !!v; });
      layout();
    },
    remove: () => { dead = true; if (raf) cancelAnimationFrame(raf); try { layer.remove(); } catch (_) {} if (level) try { pane.series.removeLevel(level); } catch (_) {} },
  };
}
