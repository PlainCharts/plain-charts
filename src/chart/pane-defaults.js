...
export const LINE_STYLE_DEFAULT = {
  source: 'close',
  color: '#2962ff',
  lineWidth: 2,
  lineStyle: 'solid',
  highColor: '#26a69a',
  lowColor: '#ef5350',
}; // quick High/Low line-source toggle colours (compare panes)
/**
...
export const CANVAS_DEFAULT = {
  ...
  gridMode: 'both',
  gridColor: '#1a1a20',
  gridStyle: 0, // both|vert|horz|none ; one color, style 0..4 (no width — the engine's grid is 1px)
  crosshairColor: '#9598a1',
  crosshairOpacity: 100,
  crosshairWidth: 1,
  crosshairStyle: 2,
  crosshairTimeLabel: true,
  crosshairPriceLabel: true, // axis labels under the crosshair (each toggled separately)
  crosshairLabelBg: '#9598a1', // background color of the crosshair's axis labels (time + price)
  scaleTextColor: '#dddddd',
  scaleFontSize: 12,
  scaleFontFamily: '', // '' = default font stack
  scaleLineColor: '#333333',
  scaleLineOpacity: 100,
  navButtons: 'hover', // hover | always | never
  paneButtons: 'hover', // sub-pane / compare control rows: hover | always | never
  marginTop: 10,
  marginBottom: 10,
  marginRight: 10, // price %, price %, bars
...
export const STATUS_DEFAULT = {
  ...
  chartValues: false,
  color: '',
  fontSize: 13, // color '' = follow the theme's Text
  bgColor: '#0e0e11',
  bgOpacity: 70,
...
export const LINE_DEFAULTS = {
  ...
  countdownBg: '#363a45', // font / background of the countdown box
...
export const canvasOptions = (c) => {
  ...
  return {
    layout: {
      ...
      fontSize: c.scaleFontSize,
      fontFamily: c.scaleFontFamily || SCALE_FONT_DEFAULT,
    },
    grid: {
      vertLines: {
        visible: c.gridMode === 'both' || c.gridMode === 'vert',
        color: c.gridColor,
        style: c.gridStyle || 0,
      },
      horzLines: {
        visible: c.gridMode === 'both' || c.gridMode === 'horz',
        color: c.gridColor,
        style: c.gridStyle || 0,
      },
    },
    cursor: {
      ...
      vertLine: {
        color: cross,
        width: c.crosshairWidth,
        style: c.crosshairStyle,
        showLabel: c.crosshairTimeLabel !== false,
        labelBg: c.crosshairLabelBg || cross,
      },
      horzLine: {
        color: cross,
        width: c.crosshairWidth,
        style: c.crosshairStyle,
        showLabel: c.crosshairPriceLabel !== false,
        labelBg: c.crosshairLabelBg || cross,
      },
    },
    ...
    timeAxis: {
      borderColor: lineCol,
      rightOffset: c.shiftEnd !== false ? c.marginRight : 0,
      followNewBars: c.autoScroll !== false,
    }, // shift toggle + auto-scroll on new bar
