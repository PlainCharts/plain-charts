...
const GAP = 2, HIT = 6;   // grid gap (px) and gutter hit width (px)

// grid-template-areas -> matrix of area letters, e.g. '"a b" "a c"' -> [[a,b],[a,c]]
/** @param {LayoutDef} def @returns {string[][]} */
function areaMatrix(def) { return (def.areas.match(/"[^"]*"/g) || []).map((r) => r.replace(/"/g, '').trim().split(/\s+/)); }
...
function segments(n, pred) {
  /** @type {number[][]} */
  const out = []; let s = -1;
  for (let k = 0; k < n; k++) { if (pred(k)) { if (s < 0) s = k; } else if (s >= 0) { out.push([s, k - 1]); s = -1; } }
...
function trackEdges(sizes, extent) {
  ...
  /** @type {number[][]} */
  const edges = []; let pos = 0;
  for (let k = 0; k < sizes.length; k++) { const w = sizes[k] / total * content; edges.push([pos, pos + w]); pos += w + GAP; }
  return edges;
}
...
export function createGutters(ctx) {
  ...
  function clear() { gutters.forEach((g) => g.remove()); gutters = []; }

  function build() {
    clear();
    const m = areaMatrix(ctx.def());
    const nr = m.length, nc = (m[0] || []).length;
    for (let j = 0; j < nc - 1; j++)                                   // boundary between col j and j+1
      segments(nr, (i) => m[i][j] !== m[i][j + 1]).forEach(([r0, r1]) => gutters.push(makeGutter('col', j, r0, r1)));
    for (let i = 0; i < nr - 1; i++)                                   // boundary between row i and i+1
  ...
  function makeGutter(axis, index, s0, s1) {
    ...
    g.dataset.axis = axis; g.dataset.index = /** @type {any} */ (index); g.dataset.s0 = /** @type {any} */ (s0); g.dataset.s1 = /** @type {any} */ (s1);   // s0..s1 = cross-axis tracks it spans
  ...
  function position() {
    ...
    const colEdges = trackEdges(ctx.cols(), rect.width), rowEdges = trackEdges(ctx.rows(), rect.height);
    gutters.forEach((g) => {
      if (ctx.isMaximized()) { g.style.display = 'none'; return; }
      g.style.display = '';
      const idx = +/** @type {string} */ (g.dataset.index), s0 = +/** @type {string} */ (g.dataset.s0), s1 = +/** @type {string} */ (g.dataset.s1);
  ...
  function startDrag(e, axis, i) {
    ...
    const move = (ev) => {
      ...
      let na = a0 + dF, nb = b0 - dF;
      if (na < minFrac) { na = minFrac; nb = a0 + b0 - na; }
      if (nb < minFrac) { nb = minFrac; na = a0 + b0 - nb; }
      sizes[i] = na; sizes[i + 1] = nb;
      ctx.applyGrid(); position();
    };
    const up = () => {
