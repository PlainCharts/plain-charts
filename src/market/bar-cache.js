...
export function refreshLibrary() {
  return barCache.library().then((rows) => {
    _lib = Array.isArray(rows) ? rows : [];
    bus.emit('cache:library');
    return _lib;
  });
}
...
export const barCache = {
  ...
  coverage: ({ broker, symbol, tf }) =>
    J('/api/cache/bars?' + new URLSearchParams({ broker, symbol, tf, meta: '1' })).then((d) => d && d.coverage),
