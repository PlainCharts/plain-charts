  ...
  return new Promise((resolve) => {
    ...
    api.getBars(params, (/** @type {FetchUpdate} */ u) => {
      ...
      if (u.error) {
        ...
        resolve({ bars: acc, error: u.error });
        return;
      }
      if (u.bars) acc.push(...u.bars);
      if (u.complete) {
        done = true;
        resolve({ bars: acc, reachedStart: u.reachedStart });
      }
    });
  });
}
...
export async function backfillCache({ brokerId, symbol, id, tf, startMs }, isAlive) {
  ...
  try {
    ...
    /** @param {Bar[]} bars */
    const store = async (bars) => {
      if (bars && bars.length) await barCache.putBars(brokerId, symbol, tf.id, bars);
    };
    ...
    if (cov && cov.to) {
      ...
      while (isAlive() && cursor < Date.now() - step) {
        const winFrom = cursor,
          winTo = Math.min(Date.now(), cursor + step);
    ...
    while (isAlive() && cursor > startMs && hops < GAP_HOPS) {
      const winTo = cursor,
        winFrom = Math.max(startMs, cursor - step);
      const u = await fetchWindow(api, { id, tf, fromMs: winFrom, toMs: winTo });
