...
const postDismiss = () => { if (firedChan) { try { firedChan.postMessage({ kind: 'dismiss' }); } catch (_) {} } };
...
function showToast(msg) {
  ...
  const bell = document.createElement('span'); bell.className = 'alert-toast-bell'; bell.textContent = '🔔';
  const txt = document.createElement('div'); txt.className = 'alert-toast-txt';
  const title = document.createElement('div'); title.className = 'alert-toast-title'; title.textContent = msg.title || 'Alert';
  const body = document.createElement('div'); body.className = 'alert-toast-body'; body.textContent = msg.body || '';
  txt.append(title, body);
  const x = document.createElement('span'); x.className = 'alert-toast-x'; x.textContent = '✕';
  toast.append(bell, txt, x);
  c.appendChild(toast);

  const dismiss = () => { toast.classList.add('out'); setTimeout(() => toast.remove(), 200); };
  x.onclick = () => { dismiss(); postDismiss(); };   // user ✕ also stops the sound (auto-timeout does not)
...
function showPopup(msg) {
  ...
  const head = document.createElement('div'); head.className = 'alert-popup-head';
  const bell = document.createElement('span'); bell.className = 'alert-popup-bell'; bell.textContent = '🔔';
  const title = document.createElement('div'); title.className = 'alert-popup-title'; title.textContent = msg.title || 'Alert';
  head.append(bell, title);
  const body = document.createElement('div'); body.className = 'alert-popup-body'; body.textContent = msg.body || '';
  const foot = document.createElement('div'); foot.className = 'alert-popup-foot';
  const ok = document.createElement('button'); ok.className = 'primary'; ok.textContent = 'Dismiss';
  foot.appendChild(ok);
  card.append(head, body, foot);
  wrap.appendChild(card);
  document.body.appendChild(wrap);
  ok.onclick = () => { wrap.remove(); postDismiss(); };   // dismiss -> also stops the host's sound
  ok.focus();
}
