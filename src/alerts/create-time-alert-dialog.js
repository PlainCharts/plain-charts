...
export function closeCreateTimeAlertDialog() { if (panel) { panel.remove(); panel = null; } }
...
export function openCreateTimeAlertDialog(existing) {
  ...
  const dlg = el('div', 'dialog alert-dlg alert-dlg-time'); panel = dlg; dlg.style.zIndex = '72';

  // ---- header
  const head = el('div', 'aldlg-head');
  head.append(el('div', 'aldlg-title', t(editing ? 'Edit time alert' : 'Create time alert')));
  const x = el('span', 'lib-x', '✕'); x.onclick = closeCreateTimeAlertDialog;
  head.appendChild(x);
  ...
  const timeWidget = segTime(alertHours24(), (p) => { cur.h = p.h; cur.mi = p.mi; }, { seconds: false });
  ...
  const roll = (/** @type {number} */ d) => {
    let m = cur.mi + d, h = cur.h;
    if (m > 59) { m = 0; h = (h + 1) % 24; } else if (m < 0) { m = 59; h = (h + 23) % 24; }
    cur.mi = m; cur.h = h; timeWidget.set({ h, mi: m, s: 0 });
  };
  const spin = el('span', 'aldlg-time-spin');
  const spinBtn = (/** @type {string} */ glyph, /** @type {number} */ d) => { const b = /** @type {HTMLButtonElement} */ (el('button', 'aldlg-spin', glyph)); b.type = 'button'; b.onclick = () => roll(d); return b; };
  spin.append(spinBtn('▲', 1), spinBtn('▼', -1));
  timeWidget.el.appendChild(spin);
  const dateIn = /** @type {HTMLInputElement} */ (el('input', 'aldlg-in aldlg-in-date')); dateIn.type = 'text'; dateIn.readOnly = true; dateIn.value = initDate;
  dateIn.style.cursor = 'pointer';
  dateIn.onclick = () => {
    if (dateIn.disabled) return;
    const [y, m, d] = (dateIn.value || initDate).split('-').map(Number);
    openDateTimePicker({
      time: false, title: t('Pick your date'), value: new Date(y, (m || 1) - 1, d || 1).getTime(),
      onSet: (ms) => { const dt = new Date(ms); dateIn.value = dt.getFullYear() + '-' + p2(dt.getMonth() + 1) + '-' + p2(dt.getDate()); },
    });
  };
  const whenRow = el('div', 'aldlg-when'); whenRow.append(timeWidget.el, dateIn);
  ...
  /** @type {[string, string][]} */
  ([['once', 'Once'], ['daily', 'Daily'], ['weekly', 'Weekly']]).forEach(([v, label]) => { const o = document.createElement('option'); o.value = v; o.textContent = t(label); freqSel.appendChild(o); });
  freqSel.value = initFreq;
  const syncDateEnabled = () => { dateIn.disabled = freqSel.value === 'daily'; };
  freqSel.onchange = syncDateEnabled;
  syncDateEnabled();

  // ---- Message (plain reminder text -- no price/symbol placeholders; a time alert has none)
  const msgIn = /** @type {HTMLTextAreaElement} */ (el('textarea', 'aldlg-in aldlg-msg-short')); msgIn.rows = 3; msgIn.spellcheck = false; msgIn.placeholder = t('Reminder message…');
  ...
  const cancel = el('button', null, t('Cancel')); cancel.onclick = closeCreateTimeAlertDialog;
  ...
  submit.onclick = () => {
    ...
    if (editing) {
      ...
      alertCommand('update', { id: existing.id, patch: { ...fields, rt: {} } }).catch((err) => console.error('[alert] update time alert failed', err));
    } else {
      alertCommand('create', { ...fields, enabled: true }).catch((err) => console.error('[alert] create time alert failed', err));
    }
    closeCreateTimeAlertDialog();
  };
