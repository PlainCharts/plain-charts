...
export const roundPrice = (v, dec) => { const n = Number(v); return Number.isFinite(n) ? Number(n.toFixed(dec != null ? dec : 2)) : n; };
...
const ACTIONS = ['Toast notification', 'System notification', 'Popup window', 'Send email', 'Telegram notification', 'Play sound', 'Webhook URL'];
...
export function section(title, fill, headExtra) {
  ...
  if (fill) fill(body); else sec.classList.add('aldlg-sec-empty');
  head.onclick = () => { const c = sec.classList.toggle('collapsed'); tri.textContent = c ? '▶' : '▼'; };
...
const SVG_PLAY = '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"><circle cx="8" cy="8" r="6.4"/><path d="M6.6 5.4 10.4 8l-3.8 2.6V5.4Z" fill="currentColor"/></svg>';
const SVG_PAUSE = '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"><circle cx="8" cy="8" r="6.4"/><path d="M6.4 5.6v4.8M9.6 5.6v4.8"/></svg>';
...
export function enableToggle(initial = true) {
  ...
  const paint = () => { btn.innerHTML = on ? SVG_PAUSE : SVG_PLAY; btn.title = on ? t('Enabled') : t('Paused'); btn.classList.toggle('off', !on); };
  btn.onclick = (e) => { e.stopPropagation(); on = !on; paint(); };   // don't collapse the section
  paint();
...
export function intervalControl(initId, favIds, allTfs, onChange) {
  ...
  const closeMenu = () => { if (away) { document.removeEventListener('pointerdown', /** @type {any} */ (away), true); away = null; } if (menu) { menu.remove(); menu = null; } };
  /** @param {string} id */
  const setCur = (id) => { cur = id; paint(); if (onChange) onChange(id); };
  const paint = () => {
    closeMenu();
    wrap.innerHTML = '';
    ...
    segs.forEach((id) => {
      const b = /** @type {HTMLButtonElement} */ (el('button', 'aldlg-tf-seg' + (id === cur ? ' active' : ''), id)); b.type = 'button';
      b.onclick = (e) => { e.preventDefault(); setCur(id); };
      wrap.appendChild(b);
    });
    const other = /** @type {HTMLButtonElement} */ (el('button', 'aldlg-tf-other')); other.type = 'button';
    other.append(el('span', null, t('Other')), el('span', 'aldlg-caret', '⌄'));
    other.onclick = () => {
      if (menu) { closeMenu(); return; }
      menu = el('div', 'dwg-menu');
      allTfs.forEach((tf) => { const row = el('div', 'dwg-item'); row.append(el('span', 'dwg-check', tf.id === cur ? '✓' : ''), el('span', 'dwg-label', tf.id)); row.onclick = () => { setCur(tf.id); }; /** @type {HTMLElement} */ (menu).appendChild(row); });
      document.body.appendChild(menu);
      const r = other.getBoundingClientRect();
      menu.style.left = r.left + 'px'; menu.style.top = (r.bottom + 4) + 'px';
      away = (e) => { if (menu && !menu.contains(/** @type {Node} */ (e.target)) && e.target !== other && !other.contains(/** @type {Node} */ (e.target))) closeMenu(); };
...
export function field(label, control) {
  ...
  r.append(el('label', null, label), (() => { const c = el('div', 'aldlg-fctl'); c.appendChild(control); return c; })());
  return r;
}
...
export function selectOf(options) {
  ...
  options.forEach((o) => { const op = /** @type {HTMLOptionElement} */ (el('option', null, t(o))); op.value = o; s.appendChild(op); });
  return s;
}
...
function resolveExpiry(kind, customMs) {
  ...
  if (kind === '1 month') { const d = new Date(now); d.setMonth(d.getMonth() + 1); return d.getTime(); }
...
export function expirationControl(initKind, initMs) {
  ...
  const paint = () => { lbl.textContent = kind === 'Open-ended' ? t('Open-ended') : formatDateTime(/** @type {number} */ (resolveExpiry(kind, customMs))); };

  const closeMenu = () => { if (away) { document.removeEventListener('pointerdown', away, true); away = null; } if (menu) { menu.remove(); menu = null; } };
  /** @param {string} k */
  const pick = (k) => {
    closeMenu();
    if (k === 'Custom date') {
      openDateTimePicker({
        ...
        onSet: (v) => { customMs = v; kind = 'Custom date'; paint(); },
      });
      return;
    }
    kind = k; paint();
  ...
  btn.onclick = () => {
    if (menu) { closeMenu(); return; }
    menu = el('div', 'dwg-menu');
    EXPIRATIONS.forEach((k) => { const row = el('div', 'dwg-item'); row.append(el('span', 'dwg-check', ''), el('span', 'dwg-label', t(k))); row.onclick = () => pick(k); /** @type {HTMLElement} */ (menu).appendChild(row); });
    document.body.appendChild(menu);
    const r = btn.getBoundingClientRect();
    menu.style.left = r.left + 'px'; menu.style.top = (r.bottom + 4) + 'px'; menu.style.minWidth = r.width + 'px';
    away = (e) => { if (menu && !menu.contains(/** @type {Node|null} */ (e.target)) && e.target !== btn && !btn.contains(/** @type {Node|null} */ (e.target))) closeMenu(); };
    setTimeout(() => document.addEventListener('pointerdown', /** @type {(e: PointerEvent) => void} */ (away), true), 0);
  };
  paint();
...
export function conditionsControl(objects, initRows, dec, initMatch, onChange) {
  ...
  const defLeft = OBJ[0], defRight = OBJ[1] || OBJ[0];
  ...
  const rows = (initRows && initRows.length)
    ? initRows.map((/** @type {any} */ r) => ({ left: r.left, op: r.op, right: r.right, value: r.value != null ? r.value : null, percent: r.percent != null ? r.percent : null, amount: r.amount != null ? r.amount : null, lookback: r.lookback != null ? r.lookback : null }))
  ...
  const readAll = () => ({ match: matchSel.value, conditions: rows.map((r) => ({ left: r.left, op: r.op, right: r.right, value: r.value != null ? roundPrice(r.value, dec) : null, percent: r.percent != null ? r.percent : null, amount: r.amount != null ? roundPrice(r.amount, dec) : null, lookback: r.lookback != null ? r.lookback : null })) });
  ...
  const objSel = (val, onset) => {
    ...
    OBJ.forEach((o) => { const op = /** @type {HTMLOptionElement} */ (el('option', null, o)); op.value = o; s.appendChild(op); });
    s.value = val; s.onchange = () => onset(s.value);
    return s;
  };
  ...
  ['All', 'Any'].forEach((o) => { const op = /** @type {HTMLOptionElement} */ (el('option', null, t(o))); op.value = o; matchSel.appendChild(op); });
  matchSel.value = /any/i.test(initMatch || 'all') ? 'Any' : 'All';
  matchLine.append(document.createTextNode(t('If') + ' '), matchSel, document.createTextNode(' ' + t('of the following conditions are met')));
  ...
  hrow.append(el('div', 'aldlg-cond-cell', t('Object')), el('div', 'aldlg-cond-cell', t('Condition')), el('div', 'aldlg-cond-cell', t('Object')), el('div', 'aldlg-cond-cell', t('Value')));
  ...
  const render = () => {
    bodyEl.innerHTML = ''; rowEls.length = 0;
    rows.forEach((r, i) => {
      ...
      const c1 = el('div', 'aldlg-cond-cell'); c1.appendChild(objSel(r.left, (v) => { r.left = v; render(); }));
      const opSel = /** @type {HTMLSelectElement} */ (el('select', 'aldlg-cond-op'));
      [...CONDITIONS, ...MOVE_OPS].forEach((o) => { const op = /** @type {HTMLOptionElement} */ (el('option', null, t(o))); op.value = o; opSel.appendChild(op); });
      opSel.value = r.op; opSel.onchange = () => { r.op = opSel.value; render(); };   // re-render: Moving % swaps the right cells
      const c2 = el('div', 'aldlg-cond-cell'); c2.appendChild(opSel);
      ...
      const numIn = (v, step, set, ph) => {
        const i = /** @type {HTMLInputElement} */ (el('input', 'aldlg-cond-vinput')); i.type = 'number'; i.step = step;
        i.value = v != null ? String(v) : ''; if (ph) i.placeholder = ph;
        i.oninput = () => set(i.value === '' ? null : parseFloat(i.value));
        i.onclick = (e) => e.stopPropagation();
        return i;
      };
      /** @type {HTMLElement} */ let c3; /** @type {HTMLElement} */ let c4;
      if (isMoveOp(r.op)) {
        ...
        if (pct) {
          ...
          c3.append(numIn(r.percent, 'any', (n) => { r.percent = n; }), el('span', 'aldlg-cond-unit', '%'));
        } else {
          c3.append(numIn(r.amount != null ? roundPrice(r.amount, dec) : null, 'any', (n) => { r.amount = n; }, t('Price')));
        }
        c4 = el('div', 'aldlg-cond-cell aldlg-cond-move');
        c4.append(el('span', 'aldlg-cond-unit', t('in')), numIn(r.lookback, '1', (n) => { r.lookback = n == null ? null : Math.trunc(n); }), el('span', 'aldlg-cond-unit', t('bar')));
      } else {
        c3 = el('div', 'aldlg-cond-cell'); c3.appendChild(objSel(r.right, (v) => { r.right = v; render(); }));
        ...
        if (usesValue(r)) c4.appendChild(numIn(r.value != null ? roundPrice(r.value, dec) : null, 'any', (n) => { r.value = n; }, t('Price')));
      }
      row.append(c1, c2, c3, c4);
      row.onclick = () => { sel = i; applySel(); };
      bodyEl.appendChild(row); rowEls.push(row);
    });
    applySel();
    if (onChange) onChange(readAll());   // let the dialog react (e.g. gate watchlist scope on relative-only rows)
  };
  addLink.onclick = () => { rows.push({ left: defLeft, op: 'Crossing', right: defRight, value: null, percent: null, amount: null, lookback: null }); sel = rows.length - 1; render(); };
  removeLink.onclick = () => { if (sel >= 0 && sel < rows.length && rows.length > 1) { rows.splice(sel, 1); sel = Math.min(sel, rows.length - 1); render(); } };
  render();
...
export function actionsControl(initActions) {
  ...
  const actSel = (val, onset) => {
    const s = /** @type {HTMLSelectElement} */ (el('select', 'aldlg-cond-op'));
    ACTIONS.forEach((o) => { const op = /** @type {HTMLOptionElement} */ (el('option', null, t(o))); op.value = o; s.appendChild(op); });
    s.value = val; s.onchange = () => onset(s.value);
    return s;
  };
  const render = () => {
    ...
    rows.forEach((r, i) => {
      const row = el('div', 'aldlg-cond-row');
      const c = el('div', 'aldlg-cond-cell'); c.appendChild(actSel(r.action, (v) => { r.action = v; }));
      row.append(c);
      row.onclick = () => { sel = i; applySel(); };
      bodyEl.appendChild(row); rowEls.push(row);
    });
  ...
  addLink.onclick = () => { rows.push({ action: ACTIONS[0] }); sel = rows.length - 1; render(); };
  removeLink.onclick = () => { if (sel >= 0 && sel < rows.length && rows.length > 1) { rows.splice(sel, 1); sel = Math.min(sel, rows.length - 1); render(); } };
  render();
...
export function messageControl(initial) {
  ...
  ta.value = initial; ta.rows = 3; ta.spellcheck = false; ta.placeholder = t('Alert message');

  const bar = el('div', 'aldlg-tags');
  const btn = /** @type {HTMLButtonElement} */ (el('button', 'aldlg-tag-btn')); btn.type = 'button';
  ...
  btn.onclick = () => {
    if (menu) { closeMenu(); return; }
    menu = el('div', 'dwg-menu');
    PLACEHOLDERS.forEach((p) => { const row = el('div', 'dwg-item'); row.append(el('span', 'dwg-label aldlg-ph-item', p)); row.onclick = () => { insertToken(ta, p); closeMenu(); }; /** @type {HTMLElement} */ (menu).appendChild(row); });
    document.body.appendChild(menu);
    const r = btn.getBoundingClientRect();
    menu.style.left = r.left + 'px'; menu.style.top = (r.bottom + 4) + 'px'; menu.style.minWidth = r.width + 'px';
    away = (e) => { if (menu && !menu.contains(/** @type {Node|null} */ (e.target)) && e.target !== btn && !btn.contains(/** @type {Node|null} */ (e.target))) closeMenu(); };
