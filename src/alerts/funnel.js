...
export function registerAlertHandler(action, fn) { handlers.set(action, fn); }
...
if (!IS_HOST && chan) {
  chan.addEventListener('message', (/** @type {MessageEvent} */ e) => {
    ...
    pending.delete(m.callId); clearTimeout(p.timer);
    if (m.ok) p.resolve(m.result); else p.reject(new Error(m.error || 'alert command failed'));
  });
}
...
export function alertCommand(action, payload) {
  ...
  return new Promise((resolve, reject) => {
    const callId = ++seq;
    const timer = setInterval(() => console.warn('[alert] no ack yet for "' + action + '" -- alert-host busy or down'), SLOW_ACK_MS);
