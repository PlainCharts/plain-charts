...
/** @param {(a:any)=>any} f */ const byStr = (f) => (/** @type {any} */ a, /** @type {any} */ b) =>
  String(f(a) || '').localeCompare(String(f(b) || ''));
/** @param {(a:any)=>number} f */ const byNum = (f) => (/** @type {any} */ a, /** @type {any} */ b) =>
  (f(a) || 0) - (f(b) || 0);
...
export const cmpOf = (key) => {
  const o = ALL_SORTS.find((x) => x.key === key);
  return o ? o.cmp : DEFAULT_CMP;
};
