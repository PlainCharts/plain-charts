...
import {
  ...
  logTypeTime,
  logTypeWatchlist,
} from './alert-display.js'; // alert-owned display prefs (timestamp + row columns)
...
function editAlert(a) {
  if (!a.objectId) {
    openValueAlertDialog(getActivePane ? getActivePane() : null, a);
    return;
  }
...
export function initAlertsPanel() {
  ...
  const tabPrice = el('button', 'al-tab active', t('Price'));
  tabPrice.setAttribute('data-i18n', 'Price');
  const tabTime = el('button', 'al-tab', t('Time'));
  tabTime.setAttribute('data-i18n', 'Time');
  const tabLog = el('button', 'al-tab', t('Log'));
  tabLog.setAttribute('data-i18n', 'Log');
  ...
  const addBtn = el('button', 'al-add', '+');
  addBtn.title = t('Create alert');
  // "+" is producer-aware: on the Time tab it opens the small time-alert dialog; elsewhere the price dialog.
  addBtn.onclick = () => {
    if (tab === 'time') openCreateTimeAlertDialog();
    else openValueAlertDialog(getActivePane ? getActivePane() : null);
  };
  const sortBtn = el('button', 'al-add al-sort', '⇅');
  sortBtn.title = t('Sort alerts');
  sortBtn.style.marginLeft = 'auto';
  const moreBtn = el('button', 'al-add al-more', '⋯');
  moreBtn.title = t('More actions');
  ...
  const showCard = (a, row) => {
    ...
    if (isTime) {
      ...
      if (lines.length) {
        lines.forEach((/** @type {string} */ ln, /** @type {number} */ i) =>
          c.appendChild(el('div', 'al-card-cond', (i ? conn.trim() + ' ' : '') + ln)),
        );
      } else {
  ...
  const renderLog = () => {
    ...
    if (symWanted)
      entries = entries.filter((/** @type {any} */ e) => {
        const a = alertOf(e);
        return (e.symbol || (a && a.symbol)) === symWanted;
      });
    if (tfWanted)
      entries = entries.filter((/** @type {any} */ e) => {
        const a = alertOf(e);
        return a && a.tf === tfWanted;
      });
    ...
    if (!(typeOn.price && typeOn.time && typeOn.watchlist)) {
      entries = entries.filter((/** @type {any} */ e) => {
        const a = alertOf(e);
        return a ? typeOn[alertType(a)] !== false : true;
      });
    }
    if (!entries.length) {
      body.appendChild(el('div', 'al-empty', total ? t('No log entries match the filter.') : t('No alert log yet.')));
      return;
    }
    let lastDay = '';
    entries.forEach((/** @type {any} */ e) => {
      ...
      if (dayKey !== lastDay) {
        lastDay = dayKey;
        body.appendChild(el('div', 'al-logday', strftime(shifted, '%B %-d').toUpperCase()));
      }
      ...
      del.title = t('Remove log entry');
      del.onclick = (ev) => {
        ev.stopPropagation();
        hideCard();
        alertCommand('log-remove', { id: e.id }).catch(() => {});
      };
      ctrls.append(del);
      ...
      if (a) {
        row.addEventListener('mouseenter', () => {
          hoverTimer = setTimeout(() => showCard(a, row), 1000);
        });
  ...
  const render = () => {
    ...
    const full = alertMirror()
      .all()
      .filter((/** @type {any} */ a) => (a.source || 'price') === (tab === 'time' ? 'time' : 'price'));
    let list = full;
    ...
    if (!alerts.length) {
      ...
      body.appendChild(el('div', 'al-empty', msg));
      return;
    }
    ...
    alerts.forEach((/** @type {any} */ a) => {
      ...
      pp.onclick = (e) => {
        e.stopPropagation();
        alertCommand('toggle', { id: a.id }).catch(() => {});
      };
      ctrls.append(pp);
      ...
      edit.onclick = (e) => {
        e.stopPropagation();
        if (sourceOf(a) === 'time') openCreateTimeAlertDialog(a);
        else editAlert(a);
      };
      ctrls.append(edit);
      const del = el('span', 'al-ctrl al-del', '🗑');
      ...
      del.onclick = async (e) => {
        ...
        const ok = await confirmDialog({
          ...
          message: t('This permanently deletes') + ` "${nameOf(a)}".`,
          yes: t('Delete'),
          no: t('Cancel'),
        });
        if (!ok) return;
  ...
  const closeSortMenu = () => {
    if (sortAway) {
      document.removeEventListener('pointerdown', sortAway, true);
      sortAway = null;
    }
    if (sortMenu) {
      sortMenu.remove();
  ...
  const openSortMenu = () => {
    ...
    SORT_GROUPS.forEach((group, gi) => {
      ...
      group.forEach((o) => {
        ...
        item.append(
          el('span', 'dwg-check', o.key === sortKey ? '✓' : ''),
          el('span', 'dwg-label', (o.dir === 'asc' ? '↑' : '↓') + '  ' + t(o.label)),
        );
        item.onclick = () => {
          sortKey = o.key;
          closeSortMenu();
    ...
    sortAway = (e) => {
      if (
        sortMenu &&
        !sortMenu.contains(/** @type {Node} */ (e.target)) &&
        !sortBtn.contains(/** @type {Node} */ (e.target))
      )
        closeSortMenu();
    };
    setTimeout(
      () => document.addEventListener('pointerdown', /** @type {(e: PointerEvent) => void} */ (sortAway), true),
      0,
    );
  };
  sortBtn.onclick = () => {
    if (sortMenu) closeSortMenu();
    else openSortMenu();
  };
  ...
  const inactiveAlerts = () =>
    alertMirror()
      .all()
      .filter((/** @type {any} */ a) => !a.enabled);
  const activeAlerts = () =>
    alertMirror()
      .all()
      .filter((/** @type {any} */ a) => a.enabled);
  // Restart: re-enable + clear the fired latch (restartPatch, the record's one home) so each re-arms fresh.
  const restartAllInactive = () =>
    inactiveAlerts().forEach((a) => alertCommand('update', { id: a.id, patch: restartPatch() }).catch(() => {}));
  // Stop all: force every armed alert off (explicit enabled:false, not a flip).
  const stopAll = () =>
    activeAlerts().forEach((a) => alertCommand('toggle', { id: a.id, enabled: false }).catch(() => {}));
  ...
  const deleteAllInactive = async () => {
    ...
    const ok = await confirmDialog({
      title: t('Delete all inactive alerts?'),
      message: t('This permanently deletes') + ` ${n}.`,
      yes: t('Delete'),
      no: t('Cancel'),
    });
    if (!ok) return;
  ...
  const closeMoreMenu = () => {
    if (moreAway) {
      document.removeEventListener('pointerdown', moreAway, true);
      moreAway = null;
    }
    if (moreMenu) {
      moreMenu.remove();
  ...
  const openMoreMenu = () => {
    ...
    const radio = (key, label, count) => {
      ...
      it.append(
        el('span', 'dwg-check', showFilter === key ? '◉' : '○'),
        el('span', 'dwg-label', t(label)),
        el('span', 'dwg-arrow', String(count)),
      );
      it.onclick = () => {
        showFilter = key;
        render();
    ...
    const build = () => {
      m.innerHTML = '';
      const all = alertMirror().all(),
        activeN = all.filter((/** @type {any} */ a) => a.enabled).length,
        inactiveN = all.length - activeN;
      ...
      filter.section(
        ...
        { check, combo, opt },
        {
          symbols: () => all.map((/** @type {any} */ a) => a.symbol),
          tfs: () => all.map((/** @type {any} */ a) => a.tf),
        },
        { onChange: render, rebuild: build },
      );
    ...
    setTimeout(
      () => document.addEventListener('pointerdown', /** @type {(e: PointerEvent) => void} */ (moreAway), true),
      0,
    );
  };
    ...
    const build = () => {
      ...
      if (has)
        clr.onclick = async () => {
          closeMoreMenu();
          const ok = await confirmDialog({
            title: t('Clear the alert log?'),
            message: t('This permanently deletes all log entries.'),
            yes: t('Clear'),
            no: t('Cancel'),
          });
          if (!ok) return;
      ...
      const linked = alertLogMirror()
        .recent()
        .map((/** @type {any} */ e) => alertOf(e))
        .filter(Boolean);
      m.appendChild(el('div', 'dwg-div'));
      m.appendChild(el('div', 'dwg-head', t('Filter alerts')));
      logFilter.section(
        ...
        { check, combo, opt },
        {
          symbols: () => linked.map((/** @type {any} */ a) => a.symbol),
          tfs: () => linked.map((/** @type {any} */ a) => a.tf),
        },
        { onChange: render, rebuild: build },
      );
      m.appendChild(el('div', 'dwg-div'));
  ...
  bus.on('vocab:changed', () => {
    if (rp.isShown && rp.isShown('alerts')) render();
  }); // live vocabulary switch
  bus.on('alerts:display-changed', () => {
    if (rp.isShown && rp.isShown('alerts')) render();
  }); // date/time/tz pref change
  // live: re-render whenever the engine's alert store changes (create / toggle / remove / fire)
  alertMirror().subscribe(() => {
    if (rp.isShown && rp.isShown('alerts')) render();
  });
  // The unseen-fires badge on the alert rail icon: count of Log entries newer than the last Log visit.
  const updateBadge = () => {
    const n = unseenCount(alertLogMirror().recent(), lastSeenLogAt());
    rp.setRailBadge('alerts', n > 99 ? '99+' : n);
  };
