...
export const alertDateFmt = () => { const d = cfg().dateFmt; return d != null ? d : DATE_FMT_DEFAULT; };
export const alertHours24 = () => { const h = cfg().hours24; return h == null ? true : !!h; };
/** resolved offset used for formatting (null setting = Local -> the live local offset). */
export const alertTzOffsetMin = () => { const o = cfg().tzOffsetMin; return (typeof o === 'number') ? o : localOff(); };
/** the raw stored tz (null = Local) -- for the settings select's current value. */
export const alertTzSetting = () => { const o = cfg().tzOffsetMin; return (typeof o === 'number') ? o : null; };
...
export const alertSoundName = () => { const c = getSetting(SOUND_KEY); return (c && c.name) || ''; };
/** the picked sound's absolute path on disk ('' = none picked). */
export const alertSoundPath = () => { const c = getSetting(SOUND_KEY); return (c && c.path) || ''; };
/** @param {string} path @param {string} name */
export function setAlertSound(path, name) { setSetting(SOUND_KEY, { path, name }); bus.emit('alerts:display-changed'); }
export function clearAlertSoundName() { setSetting(SOUND_KEY, {}); bus.emit('alerts:display-changed'); }
...
export const alertShowName = () => { const v = cfg().showName; return v == null ? true : !!v; };
export const alertShowMessage = () => { const v = cfg().showMessage; return v == null ? true : !!v; };
export const alertShowLastTriggered = () => { const v = cfg().showLastTriggered; return v == null ? true : !!v; };

// Which columns the Log tab row shows (its own "Customize list" toggles, independent of the alerts list).
export const logShowName = () => { const v = cfg().logShowName; return v == null ? true : !!v; };
export const logShowMessage = () => { const v = cfg().logShowMessage; return v == null ? true : !!v; };
...
export const logTypePrice = () => { const v = cfg().logTypePrice; return v == null ? true : !!v; };
export const logTypeTime = () => { const v = cfg().logTypeTime; return v == null ? true : !!v; };
export const logTypeWatchlist = () => { const v = cfg().logTypeWatchlist; return v == null ? true : !!v; };
