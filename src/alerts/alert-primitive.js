...
function drawTrash(c, cx, cy, color) {
  ...
  c.beginPath();
  c.moveTo(x0 - 1, y0 + 2);
  c.lineTo(x0 + w + 1, y0 + 2);
  c.stroke(); // lid
  c.beginPath();
  c.moveTo(x0 + w * 0.32, y0 + 2);
  c.lineTo(x0 + w * 0.32, y0);
  c.lineTo(x0 + w * 0.68, y0);
  c.lineTo(x0 + w * 0.68, y0 + 2);
  c.stroke(); // handle
  c.beginPath();
  c.moveTo(x0 + 0.5, y0 + 2);
  c.lineTo(x0 + 1.5, y0 + h);
  c.lineTo(x0 + w - 1.5, y0 + h);
  c.lineTo(x0 + w - 0.5, y0 + 2);
  c.stroke(); // body
  c.beginPath(); // ribs
  c.moveTo(cx - 2, y0 + 4);
  c.lineTo(cx - 2, y0 + h - 1);
  c.moveTo(cx, y0 + 4);
  c.lineTo(cx, y0 + h - 1);
  c.moveTo(cx + 2, y0 + 4);
  c.lineTo(cx + 2, y0 + h - 1);
...
export function createAlertPrimitive(pane) {
  ...
  const refresh = () => {
    ...
    style = {
      color: S.alertColor || '#f5a623',
      width: S.alertWidth || 1,
      dash: S.alertDash || 'dashed',
      label: S.alertLabel !== false,
    };
  ...
  /** @param {boolean} on */
  const setScroll = (on) => {
    if (on === scrollLocked || !chart) return;
    scrollLocked = on;
    try {
      chart.configure({ handleScroll: !on, handleScale: !on });
    } catch (_) {}
  };
  ...
  const paintCursor = () => {
    ...
    if (!el) return;
    try {
      el.classList.toggle('alert-cursor-ns', !!hoverId && !overTrash);
      el.classList.toggle('alert-cursor-pointer', overTrash);
    } catch (_) {}
  };
  ...
  const drawPill = (c, W, H, a) => {
    ...
    let priceStr;
    try {
      priceStr = series.formatPrice().format(a.level);
    } catch (_) {
      priceStr = String(a.level);
    }
    ...
    const px0 = Math.round(W / 2 - pillW / 2),
      py0 = Math.round(cy - pillH / 2);
    const bg = themeColor('--panel', '#ffffff'),
      fg = themeColor('--tx', '#131722'),
      bd = themeColor('--bd', '#c8cbd0'),
      hov = themeColor('--hover', '#eef0f3');
    // body
    c.beginPath();
    c.roundRect(px0, py0, pillW, pillH, 4);
    c.fillStyle = bg;
    c.fill();
    c.strokeStyle = bd;
    c.lineWidth = 1;
    ...
    const tx0 = px0 + padX + tw + gap - 3,
      tx1 = tx0 + trashW + 6;
    if (overTrash && activeId === a.id) {
      c.beginPath();
      c.roundRect(tx0, py0 + 2, tx1 - tx0, pillH - 4, 3);
      c.fillStyle = hov;
      c.fill();
    }
  ...
  const paneView = {
    ...
    renderer: () => ({
      /** @param {any} target */
      draw: (target) =>
        target.useMediaCoordinateSpace((/** @type {any} */ scope) => {
          ...
          list.forEach((a) => {
            ...
            c.setLineDash(dashOf(style.dash));
            c.beginPath();
            c.moveTo(0, y);
            c.lineTo(W - (BADGE_R * 2 + 4), y);
            c.stroke(); // stop short of the badge
            c.restore();
  ...
  const axisView = (a) => ({
    ...
    text: () => {
      try {
        return series.formatPrice().format(a.level);
      } catch (_) {
  ...
  const overScale = (e) => {
    const el = rootEl();
    if (!el) return false;
    return el.clientWidth - psWidth() - (e.clientX - el.getBoundingClientRect().left) < 0;
  };
  ...
  const onMove = (/** @type {MouseEvent} */ e) => {
    ...
    if (y == null) return;
    if (dragId) {
      // dragging -> preview the new level at the cursor
      const price = series.yToPrice(y);
      if (price != null) {
        dragPreview = { id: dragId, level: round(price) };
  ...
  const onDown = () => {
    if (overTrash) return;
    if (hoverId) dragId = hoverId;
  }; // over the trash -> let click delete
  const onUp = () => {
    if (!dragId) return;
  ...
  const onClick = async () => {
    // quick delete: click the trash in the hover pill -> confirm first
    ...
    let priceStr = '';
    try {
      if (a) priceStr = series.formatPrice().format(a.level);
    } catch (_) {
      priceStr = a ? String(a.level) : '';
    }
    const label = a ? `${sym} ${t(a.op)} ${priceStr}` : sym;
    const ok = await confirmDialog({
      title: t('Delete this alert?'),
      message: t('Doing this will permanently delete your') + ` "${label}" ` + t('alert') + '.',
      yes: t('Delete'),
      no: t('No'),
    });
    if (!ok) return;
  ...
  return {
    ...
    priceAxisViews() {
      return style.label ? list.map(axisView) : [];
    },
    ...
    hitTest(_x, y) {
      ...
      return series && nearestLineId(y, DRAG_TOL)
        ? { cursorStyle: 'ns-resize', externalId: 'alert-primitive', zOrder: 'top' }
        : null;
    },
    ...
    attached(p) {
      ...
      unsub = alertMirror().subscribe(() => {
        if (requestUpdate) requestUpdate();
      }); // repaint on create/move/remove
      const el = rootEl();
      if (el) {
    ...
    detached() {
      ...
      if (el2)
        try {
          el2.classList.remove('alert-cursor-ns', 'alert-cursor-pointer');
        } catch (_) {}
      handlers = null;
      series = null;
      chart = null;
