...
function makeLog(owner) {
  ...
  let bc = null;
  try {
    bc = new BroadcastChannel(IPC.ALERT_LOG);
  } catch (_) {}
  ...
  const applyReset = (entries) => {
    ring = (entries || []).slice(-LOG_CAP);
    notify({ type: 'reset' });
  };
  /** cascade: drop every entry for a removed alert. @param {string} alertId */
  const applyPrune = (alertId) => {
    const n = ring.length;
    ring = ring.filter((x) => x.alertId !== alertId);
    if (ring.length !== n) notify({ type: 'prune', alertId });
  };
  /** drop ONE entry by its id (the alert is untouched -- this only edits the mailbox). @param {string} id */
  const applyRemove = (id) => {
    const n = ring.length;
    ring = ring.filter((x) => x.id !== id);
    if (ring.length !== n) notify({ type: 'remove', id });
  };
  ...
  return {
    ...
    size: () => ring.length,
    /** @param {(ev: LogEvent) => void} fn */ subscribe: (fn) => {
      subs.add(fn);
    ...
    pruneByAlert(alertId) {
      applyPrune(alertId);
      if (bc) {
        try {
          bc.postMessage({ prune: alertId });
        } catch (_) {}
      }
    },
...
export function createAlertLog() {
  return makeLog(true);
}
