...
export function rangeBounds(range) {
  ...
  const parse = (s) => {
    ...
    return m ? Date.UTC(+m[1], +m[2] - 1, +m[3]) - off : null;
  };
  switch (range && range.mode) {
    case 'day':
      return { fromMs: dayStart(now), toMs: null };
    case 'week': {
      const back = (new Date(now + off).getUTCDay() + 6) % 7;
      return { fromMs: dayStart(now) - back * DAY_MS, toMs: null };
    } // Monday-start
    case 'month': {
      const d = new Date(now + off);
      return { fromMs: Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), 1) - off, toMs: null };
    }
    case 'custom': {
      const f = parse(range.from),
        t = parse(range.to);
      return { fromMs: f, toMs: t != null ? t + DAY_MS : null };
    } // inclusive end day
    default:
      return { fromMs: null, toMs: null }; // 'all'
  }
}
...
export function createDateFilter(settingsKey, onChange) {
  ...
  const btn = el('desk-filt date-filt');
  btn.title = tr('Filter by date range');
  ...
  const apply = (r) => {
    range = r;
    setSetting(settingsKey, range);
    lbl.textContent = rangeLabel(range);
    close();
    try {
      onChange();
    } catch (_) {}
  ...
  const open = () => {
    ...
    presets.forEach(([mode, lab]) => {
      const row = el('wl-listmenu-row' + (range.mode === mode ? ' sel' : ''), tr(lab));
      row.onclick = () => apply({ mode });
      m.appendChild(row);
    });
    ...
    applyB.className = 'desk-filt-apply';
    applyB.textContent = tr('Apply range');
    applyB.onclick = () => {
      if (fromIn.value && toIn.value) apply({ mode: 'custom', from: fromIn.value, to: toIn.value });
    };
