...
export function createTableSort({ settingKey, defaultKey, defaultDir = 'desc', valueOf, onChange }) {
  ...
  const compare = (a, b) => {
    const av = valueOf(sortKey, a), bv = valueOf(sortKey, b);
  ...
  /** @param {string} k */
  const setSort = (k) => { if (sortKey === k) sortDir = sortDir === 'asc' ? 'desc' : 'asc'; else { sortKey = k; sortDir = 'desc'; } setSetting(settingKey, { key: sortKey, dir: sortDir }); onChange(); };
  // the header arrow for column k -- an element when k is the active sort, else null
  /** @param {string} k @returns {HTMLElement|null} */
  const arrowFor = (k) => { if (k !== sortKey) return null; const ar = document.createElement('span'); ar.className = 'sort-arrow'; ar.textContent = sortDir === 'asc' ? ' ↑' : ' ↓'; return ar; };
