...
function ensureCss() {
  ...
  l.rel = 'stylesheet';
  l.href = '/node_modules/@xterm/xterm/css/xterm.css';
...
async function loadTermConfig() {
  try {
    const r = await fetch('/settings/ai-workspace/terminal.json', { cache: 'no-store' });
...
export function mountAiWorkspace(root, cfg = {}) {
  ...
  if (!desktop || !desktop.pty) {
    ...
    m.style.cssText = 'padding:24px;color:#888;font:14px system-ui';
  ...
  if (injectChan)
    injectChan.onmessage = (/** @type {MessageEvent} */ e) => {
      const m = e.data;
      if (m && m.text) {
        try {
          desktop.pty.write(sessionId, m.text);
        } catch (_) {}
        if (term) term.focus();
      }
    };
  ...
  (async () => {
    ...
    offData = desktop.pty.onData((/** @type {any} */ m) => {
      if (m && m.sessionId === sessionId && term) term.write(m.data);
    });
    offExit = desktop.pty.onExit((/** @type {any} */ m) => {
      if (m && m.sessionId === sessionId && term) term.write('\r\n\x1b[90m[shell exited: ' + m.code + ']\x1b[0m\r\n');
    });
    desktop.pty.spawn(sessionId, term.cols, term.rows); // new session, or reconnect + buffer replay
    ro = new ResizeObserver(() => {
      try {
        fit.fit();
        desktop.pty.resize(sessionId, term.cols, term.rows);
      } catch (_) {}
    });
    ro.observe(wrap);
    ...
    const clipWrite = (/** @type {string} */ t) => {
      try {
        if (desktop.clipboard) {
          desktop.clipboard.write(t);
          return;
        }
      } catch (_) {}
      try {
    ...
    const doPaste = () => {
      try {
        if (desktop.clipboard) {
          const t = desktop.clipboard.read();
          if (t) term.paste(t);
          return;
        }
      } catch (_) {}
    ...
    wrap.addEventListener('contextmenu', (/** @type {MouseEvent} */ ev) => {
      ev.preventDefault();
      doPaste();
    });
    ...
    wrap.textContent = 'AI Workspace failed to load: ' + ((e && e.message) || e);
  });
