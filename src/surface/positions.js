...
const lookupDerived = (idx, r) => idx.get(r.broker + ':' + (r.accountId != null ? r.accountId : '') + ':' + r.symbol) || idx.get(r.broker + ':' + r.symbol) || null;
...
const markOf = (r) => { const q = quotes.get(markKey(r)); return q ? q.mark : null; };
...
function changePct(r) {
  const mark = r._mark != null ? r._mark : markOf(r), entry = r.avgPrice;
...
const removeLevel = (r, which) => {
  command({ type: 'modifyPosition', broker: r.broker, ticket: r.ticket,
    stopLoss: which === 'sl' ? 0 : (Number(r.stopLoss) || 0),
    takeProfit: which === 'tp' ? 0 : (Number(r.takeProfit) || 0) }).catch(() => {});
};
...
const CAT = [
  ...
  { key: 'avgPrice', label: 'Avg entry', align: 'right', get: (r) => ({ text: fmtPrice(r.avgPrice, r.priceDecimals != null ? r.priceDecimals : 2) }) },
  { key: 'openTime', label: 'Open time', align: 'left', get: (r) => ({ text: fmtTime(r._derived && r._derived.entryTime) }) },
  // --- MARK (live) --- current price from the quote stream, and the signed % move from entry
  { key: 'mark', label: 'Price', align: 'right', get: (r) => ({ text: fmtPrice(r._mark, r.priceDecimals != null ? r.priceDecimals : 2) }) },
  { key: 'changePct', label: 'Change %', align: 'right', get: (r) => { const p = changePct(r); return { text: p == null ? '—' : (p >= 0 ? '+' : '') + p.toFixed(2) + '%', cls: p == null ? '' : p >= 0 ? 'pos' : 'neg' }; } },
  { key: 'profit', label: 'Profit', align: 'right', get: (r) => { const p = profitOf(r); if (!p) return { text: '—' }; const dec = p.currency ? 2 : (r.priceDecimals != null ? r.priceDecimals : 2); const s = p.value.toLocaleString('en-US', { minimumFractionDigits: dec, maximumFractionDigits: dec }); return { text: (p.value >= 0 ? '+' : '') + s + (p.currency ? '' : ' pts'), cls: p.value >= 0 ? 'pos' : 'neg' }; } },
  ...
  { key: 'swap', label: 'Swap', align: 'right', get: (r) => r.swap == null ? { text: '—' } : { text: (Number(r.swap) >= 0 ? '+' : '') + Number(r.swap).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }), cls: Number(r.swap) >= 0 ? 'pos' : 'neg' } },
  ...
  { key: 'stopLoss', label: 'S/L', align: 'right', get: (r) => ({ text: fmtPrice(r.stopLoss, r.priceDecimals != null ? r.priceDecimals : 2), cls: r.stopLoss != null ? 'neg' : '', x: (r.ticket != null && r.stopLoss != null) ? () => removeLevel(r, 'sl') : undefined }) },
  { key: 'takeProfit', label: 'T/P', align: 'right', get: (r) => ({ text: fmtPrice(r.takeProfit, r.priceDecimals != null ? r.priceDecimals : 2), cls: r.takeProfit != null ? 'pos' : '', x: (r.ticket != null && r.takeProfit != null) ? () => removeLevel(r, 'tp') : undefined }) },
];
...
const getCols = () => { const c = getSetting('positionsColumns'); return (Array.isArray(c) && c.length ? c : DEFAULT_COLS).filter((k) => BY[k]); };
...
const RAW = {
  symbol: (r) => r.symbol, accountName: (r) => acctName(r), account: (r) => r.accountId, broker: (r) => r.broker, ticket: (r) => r.ticket,
  side: (r) => r.side, qty: (r) => r.qty, avgPrice: (r) => r.avgPrice,
  openTime: (r) => r._derived && r._derived.entryTime, recon: (r) => recon(r).text,
  stopLoss: (r) => r.stopLoss, takeProfit: (r) => r.takeProfit,
  mark: (r) => r._mark, changePct: (r) => changePct(r),
  profit: (r) => { const p = profitOf(r); return p ? p.value : null; },
  swap: (r) => r.swap,
};
...
export function mountPositions(root) {
  ...
  const title = el('acct-title', 'Positions'); const count = el('acct-count', '');
  const gear = el('acct-gear'); gear.title = 'Configure columns'; gear.innerHTML = GEAR;
  ...
  /** @param {PosRow} r */
  const flatten = (r) => {
    const fail = (/** @type {string} */ msg) => platform.console.post({ level: 'error', cat: 'journal', src: r.broker, msg: 'flatten failed: ' + msg });
    const cmd = r.ticket != null
      ? { type: 'closeLot', broker: r.broker, ticket: r.ticket }
      : { type: 'closePosition', broker: r.broker, symbol: r.symbol };
    command(cmd)
      .then((res) => { if (res && res.error) fail(res.error); })
  ...
  const openModify = (r) => {
    ...
    d.openOrderTicket({ tab: 'modify', position: { broker: r.broker, accountId: r.accountId, symbol: r.symbol, ticket: r.ticket, side: r.side, qty: r.qty, avgPrice: r.avgPrice, stopLoss: r.stopLoss, takeProfit: r.takeProfit, priceDecimals: r.priceDecimals, tickSize: r.tickSize } });
  };
  ...
  const onMenuAway = (ev) => { if (rowMenu && !rowMenu.contains(/** @type {Node} */ (ev.target))) closeRowMenu(); };
  const closeRowMenu = () => { if (!rowMenu) return; rowMenu.remove(); rowMenu = null; document.removeEventListener('pointerdown', /** @type {any} */ (onMenuAway), true); };
  /** @param {number} x @param {number} y @param {PosRow} r */
  const openRowMenu = (x, y, r) => {
    closeRowMenu();
    const m = document.createElement('div');
    m.style.cssText = 'position:fixed; z-index:9999; background:var(--panel); border:1px solid var(--bd); border-radius:4px; box-shadow:0 6px 18px rgba(0,0,0,.45); padding:4px; min-width:120px;';
    /** @param {string} label @param {() => void} onPick @param {string} [color] */
    const mkItem = (label, onPick, color) => {
      const item = document.createElement('div');
      item.textContent = label;
      item.style.cssText = 'padding:6px 12px; font-size:13px; color:' + (color || 'var(--tx)') + '; cursor:pointer; border-radius:3px;';
      item.onmouseenter = () => { item.style.background = 'var(--hover)'; };
      item.onmouseleave = () => { item.style.background = 'transparent'; };
      item.onclick = () => { closeRowMenu(); onPick(); };
      m.appendChild(item);
    };
  ...
  const ensureQuote = (r) => {
    ...
    quotes.set(key, rec);
    api.resolveSymbol(r.symbol, /** @param {any} inst */ (inst) => {
      rec.pending = false;
      if (!inst) { quotes.delete(key); return; }
      ...
      rec.qcb = /** @param {any} q */ (q) => {
        ...
        if (m != null && m !== rec.mark) { rec.mark = m; updateLive(key, m); }
      };
  ...
  const pruneQuotes = (activeKeys) => {
    for (const [key, rec] of [...quotes]) {
      ...
      if (rec.qcb && rec.contractId != null) { try { /** @type {any} */ (broker.for(rec.broker)).unsubscribeQuotes(rec.contractId, rec.qcb); } catch (_) {} }
      quotes.delete(key);
    }
  };
  ...
  const updateLive = (key, m) => {
    const list = liveRows.get(key); if (!list) return;
    for (const e of list) {
      e.r._mark = m;
      for (const k of Object.keys(e.tds)) {
        const col = BY[k]; const v = col.get(e.r); const td = e.tds[k];
  ...
  const schedule = () => {
    ...
    renTimer = setTimeout(() => { renTimer = 0; lastRender = performance.now(); render(); }, due);
  };
  ...
  const render = () => {
    ...
    if (!rows.length) { const tr = document.createElement('tr'); const tdE = document.createElement('td'); tdE.colSpan = cols.length + 1; tdE.className = 'acct-empty'; tdE.textContent = t('Flat — no open positions'); tr.appendChild(tdE); tbody.appendChild(tr); return; }
    rows.forEach((r) => {
      ...
      tr.onclick = (e) => {
        ...
        if (lastTap.key === key && (e.timeStamp - lastTap.t) < 450) { lastTap = { key: '', t: 0 }; openModify(r); }
        else lastTap = { key, t: e.timeStamp };
      };
      // right-click -> context menu with Modify (routing test, independent of the double-click gesture)
      tr.oncontextmenu = (e) => { e.preventDefault(); openRowMenu(e.clientX, e.clientY, r); };
      ...
      cols.forEach((k) => {
        ...
        if (v.x) { const xb = document.createElement('button'); xb.className = 'ord-cancel'; xb.style.marginLeft = '6px'; xb.textContent = '✕'; xb.title = 'Remove level'; xb.onclick = (e) => { e.stopPropagation(); /** @type {() => void} */ (v.x)(); }; td.appendChild(xb); }
        if (LIVE_COLS.has(k)) liveTds[k] = td;   // quote ticks re-fill these in place
        tr.appendChild(td);
      });
      if (Object.keys(liveTds).length) { const mk = markKey(r); const list = liveRows.get(mk) || []; list.push({ r, tds: liveTds }); liveRows.set(mk, list); }
      const act = document.createElement('td'); act.className = 'a-right ord-actions';
      const x = document.createElement('button'); x.className = 'ord-cancel'; x.textContent = '✕'; x.title = t('Flatten position'); x.onclick = () => flatten(r);
      act.appendChild(x); tr.appendChild(act);
      tbody.appendChild(tr);
    });
  };

  gear.onclick = () => openColumnPicker(CAT, cols, /** @param {string[]} next */ (next) => { cols = next; setSetting('positionsColumns', cols); render(); });
  ...
  return { destroy() { closeRowMenu(); acctFilter.destroy(); if (renTimer) clearTimeout(renTimer); [off1, off2, off3, off4, off5].forEach((f) => { try { f(); } catch (_) {} }); pruneQuotes(new Set()); root.innerHTML = ''; } };
}
