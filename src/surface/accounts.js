...
const CAT = [
  {
    ...
    label: 'Status',
    align: 'left',
    get: (r, c) => (c.connected ? { text: t('connected'), dot: 'on' } : { text: t('not connected'), dot: '' }),
  },
  ...
  { key: 'protocol', label: 'Protocol', get: (r) => ({ text: (r.broker || '').toUpperCase() }) },
  {
    ...
    label: 'Type',
    align: 'left',
    get: (r, c) => ({ text: (c.accountType || 'netting').replace(/^./, (s) => s.toUpperCase()) }),
  },
  ...
  { key: 'unrealizedPL', label: 'Unrealized P/L', align: 'right', get: (r) => pl(r.unrealizedPL, r.currency) },
  {
    ...
    label: 'Total P/L',
    align: 'right',
    get: (r) => pl((Number(r.realizedPL) || 0) + (Number(r.unrealizedPL) || 0), r.currency),
  },
  { key: 'marginUsed', label: 'Margin used', align: 'right', get: (r) => ({ text: money(r.marginUsed, r.currency) }) },
  {
    ...
    label: 'Margin avail',
    align: 'right',
    get: (r) => ({ text: money(r.marginAvailable, r.currency) }),
  },
  {
    key: 'purchasingPower',
    label: 'Buying power',
    align: 'right',
    get: (r) => ({ text: money(r.purchasingPower, r.currency) }),
  },
  {
    key: 'positionMargin',
    label: 'Position margin',
    align: 'right',
    get: (r) => ({ text: money(r.positionMargin, r.currency) }),
  },
  ...
  { key: 'cashExcess', label: 'Cash excess', align: 'right', get: (r) => ({ text: money(r.cashExcess, r.currency) }) },
  {
    ...
    label: 'Yesterday bal',
    align: 'right',
    get: (r) => ({ text: money(r.yesterdayBalance, r.currency) }),
  },
...
const getCols = () => {
  const c = getSetting('accountsColumns');
...
const RAW = {
  status: (r, c) => (c.connected ? 1 : 0),
  name: (r, c) => c.name || String(r.accountId || ''),
  protocol: (r) => r.broker,
  accountType: (r, c) => c.accountType || 'netting',
  server: (r, c) => c.server,
  accountId: (r) => r.accountId,
  balance: (r) => r.balance,
  equity: (r) => r.equity,
  realizedPL: (r) => r.realizedPL,
  unrealizedPL: (r) => r.unrealizedPL,
  totalPL: (r) => (Number(r.realizedPL) || 0) + (Number(r.unrealizedPL) || 0),
  marginUsed: (r) => r.marginUsed,
  marginAvailable: (r) => r.marginAvailable,
  purchasingPower: (r) => r.purchasingPower,
  positionMargin: (r) => r.positionMargin,
  mvo: (r) => r.mvo,
  mvf: (r) => r.mvf,
  cashExcess: (r) => r.cashExcess,
  yesterdayBalance: (r) => r.yesterdayBalance,
  workingOrders: (r) => r.workingOrders,
  filledOrders: (r) => r.filledOrders,
...
export function mountAccounts(root) {
  ...
  const title = el('acct-title', 'Accounts');
  ...
  const ctxFor = (r) => {
    const saved = /** @type {Partial<import('../connect/accounts.js').SavedAccount>} */ (
      accounts.listAccounts().find((a) => a.protocol === r.broker) || {}
    );
    return {
      connected: broker.isConnected(r.broker),
      name: saved.name,
      server: saved.server,
  ...
  const sort = createTableSort({
    settingKey: 'accountsSort',
    defaultKey: 'name',
    defaultDir: 'asc',
    valueOf: (k, A) => {
      const acc = RAW[k];
      return acc ? acc(A.r, A.c) : undefined;
    },
    onChange: () => render(),
  });

  const render = () => {
    const rows = platform.accounts
      .all()
      .map((r) => ({ r, c: ctxFor(r) }))
      .sort(sort.compare); // carry context, apply the active sort
    ...
    if (!rows.length) {
      ...
      tdE.textContent = t('No accounts connected');
      tr.appendChild(tdE);
  ...
  gear.onclick = () =>
    openColumnPicker(
      ...
      /** @param {string[]} next */ (next) => {
        cols = next;
        setSetting('accountsColumns', cols);
        render();
      },
    );
