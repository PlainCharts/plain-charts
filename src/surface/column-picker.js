...
export function openColumnPicker(catalog, current, onSave) {
  ...
  const head = el('acct-cfg-head'); head.append(el('acct-cfg-title', 'Configure columns')); const x = el('acct-cfg-x', '✕'); head.appendChild(x);
  let chosen = current.filter((k) => byKey[k]);
  const cols = el('acct-cfg-cols');
  const leftBox = el('acct-cfg-box'); const rightBox = el('acct-cfg-box');
  leftBox.append(el('acct-cfg-lbl', 'Available'));
  rightBox.append(el('acct-cfg-lbl', 'Shown (in order)'));
  const leftList = el('acct-cfg-list'); const rightList = el('acct-cfg-list');
  leftBox.appendChild(leftList); rightBox.appendChild(rightList);
  cols.append(leftBox, rightBox);
  dlg.append(head, cols);
  const foot = el('acct-cfg-foot'); const done = el('acct-cfg-done', 'Done'); foot.appendChild(done); dlg.appendChild(foot);
  ...
  const rerender = () => {
    ...
    catalog.filter((c) => !chosen.includes(c.key)).forEach((c) => {
      const it = el('acct-cfg-item', t(c.label)); const add = el('acct-cfg-btn', '+'); add.title = t('Add'); it.appendChild(add);
      it.onclick = () => { chosen.push(c.key); rerender(); };
      leftList.appendChild(it);
    });
    chosen.forEach((k) => {
      const it = el('acct-cfg-item acct-cfg-drag'); it.draggable = true; it.dataset.key = k;
      it.append(el('acct-cfg-grip', '⠿'), el('acct-cfg-item-lbl', t(byKey[k].label)));
      const rm = el('acct-cfg-btn', '✕'); rm.title = t('Remove'); rm.onclick = (e) => { e.stopPropagation(); chosen = chosen.filter((c) => c !== k); rerender(); };
      const ctrls = el('acct-cfg-ctrls'); ctrls.appendChild(rm); it.appendChild(ctrls);

      it.addEventListener('dragstart', (e) => { dragKey = k; it.classList.add('dragging'); try { const dt = /** @type {DataTransfer} */ (e.dataTransfer); dt.effectAllowed = 'move'; dt.setData('text/plain', k); } catch (_) {} });
      it.addEventListener('dragend', () => { dragKey = null; it.classList.remove('dragging'); rightList.querySelectorAll('.drop-before,.drop-after').forEach((n) => n.classList.remove('drop-before', 'drop-after')); });
      it.addEventListener('dragover', (e) => {
        if (dragKey == null || dragKey === k) return; e.preventDefault();
        const before = (e.clientY - it.getBoundingClientRect().top) < it.offsetHeight / 2;
        it.classList.toggle('drop-before', before); it.classList.toggle('drop-after', !before);
      });
      it.addEventListener('dragleave', () => it.classList.remove('drop-before', 'drop-after'));
      it.addEventListener('drop', (e) => {
        e.preventDefault(); if (dragKey == null || dragKey === k) return;
        const before = (e.clientY - it.getBoundingClientRect().top) < it.offsetHeight / 2;
        chosen = chosen.filter((c) => c !== dragKey);
        let idx = chosen.indexOf(k); if (!before) idx += 1;
        chosen.splice(idx, 0, dragKey); rerender();
      });
  ...
  x.onclick = close; done.onclick = close;
