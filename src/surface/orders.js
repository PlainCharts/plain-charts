...
/** @param {*} v @returns {string} */
const fmtMoney = (v) => (v == null || v === '' || Number.isNaN(Number(v))) ? '—' : Number(v).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
const commOf = (o) => { const f = platform.fills.get(o.broker + ':' + o.id); return f && f.commission != null ? f.commission : null; };
...
const CAT = [
  ...
  // --- ORDER ---
  { key: 'side', label: 'Side', align: 'left', get: (o) => ({ text: (o.side || '').toUpperCase() || '—', cls: o.side === 'sell' ? 'neg' : o.side === 'buy' ? 'pos' : '' }) },
  ...
  { key: 'limitPrice', label: 'Limit price', align: 'right', get: (o) => ({ text: o.limitPrice != null ? fmtPrice(o.limitPrice, decOf(o)) : '—' }) },
  { key: 'stopPrice', label: 'Stop price', align: 'right', get: (o) => ({ text: o.stopPrice != null ? fmtPrice(o.stopPrice, decOf(o)) : '—' }) },
  { key: 'avgFillPrice', label: 'Fill price', align: 'right', get: (o) => ({ text: o.avgFillPrice != null ? fmtPrice(o.avgFillPrice, decOf(o)) : '—' }) },   // what you GOT
  // protective legs ATTACHED to the order (a pending-order S/L / T/P -- the order carries them natively; they activate on fill)
  { key: 'stopLoss', label: 'S/L', align: 'right', get: (o) => ({ text: o.stopLoss != null ? fmtPrice(o.stopLoss, decOf(o)) : '—', cls: o.stopLoss != null ? 'neg' : '' }) },
  { key: 'takeProfit', label: 'T/P', align: 'right', get: (o) => ({ text: o.takeProfit != null ? fmtPrice(o.takeProfit, decOf(o)) : '—', cls: o.takeProfit != null ? 'pos' : '' }) },
  ...
  { key: 'status', label: 'Status', align: 'left', get: (o) => ({ text: txt(o.status), cls: STATUS_CLS[o.status] || '' }) },
  // --- TIME ---
...
const DEFAULT_COLS = ['id', 'symbol', 'side', 'qty', 'limitPrice', 'stopPrice', 'stopLoss', 'takeProfit', 'avgFillPrice', 'status', 'time'];
const getCols = () => { const c = getSetting('ordersColumns'); return (Array.isArray(c) && c.length ? c : DEFAULT_COLS).filter((k) => BY[k]); };
...
const RAW = {
  id: (o) => o.id, symbol: (o) => o.symbol, accountName: (o) => acctName(o), account: (o) => o.accountId,
  broker: (o) => o.broker, side: (o) => o.side, type: (o) => o.type, qty: (o) => o.qty,
  limitPrice: (o) => o.limitPrice, stopPrice: (o) => o.stopPrice, avgFillPrice: (o) => o.avgFillPrice,
  commission: (o) => commOf(o), tif: (o) => o.tif, status: (o) => o.status,
  time: (o) => o.time, updateTime: (o) => o.updateTime, expiry: (o) => o.expiry,
};
...
export function mountOrders(root) {
  ...
  /** @param {OrderRow} o */
  const cancel = (o) => {
    const fail = (/** @type {string} */ msg) => platform.console.post({ level: 'error', cat: 'journal', src: o.broker, msg: 'cancel failed: ' + msg });
    command({ type: 'cancel', broker: o.broker, id: o.id })
      .then((r) => { if (r && r.error) fail(r.error); })
  ...
  const renderFilters = (allRows) => {
    ...
    FILTERS.forEach((f) => {
      ...
      const b = document.createElement('button'); b.className = 'ord-filter' + (active ? ' active' : '');
      b.appendChild(document.createTextNode(t(f.label)));
      if (f.key === 'all' || n > 0) { const c = el('ord-filter-n', String(n)); b.appendChild(c); }   // show the count (All always; others only when non-zero)
  ...
  const render = () => {
    ...
    if (!rows.length) { const tr = document.createElement('tr'); const tdE = document.createElement('td'); tdE.colSpan = cols.length + 1; tdE.className = 'acct-empty'; tdE.textContent = t('No orders'); tr.appendChild(tdE); tbody.appendChild(tr); return; }
    rows.forEach((o) => {
      const tr = document.createElement('tr');
      cols.forEach((k) => {
        const col = BY[k], v = col.get(o);
      ...
      if (cancellable(o)) { const x = document.createElement('button'); x.className = 'ord-cancel'; x.textContent = '✕'; x.title = t('Cancel order'); x.onclick = () => cancel(o); act.appendChild(x); }
      tr.appendChild(act);
      tbody.appendChild(tr);
    });
  };

  gear.onclick = () => openColumnPicker(CAT, cols, /** @param {string[]} next */ (next) => { cols = next; setSetting('ordersColumns', cols); render(); });
