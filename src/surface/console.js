...
/** @type {[string, string][]} */
const FILTERS = [['all', 'All'], ['journal', 'Journal'], ['addon', 'Addons']];
...
export function mountConsole(root, cfg = {}) {
  ...
  FILTERS.forEach(([k, label]) => {
    const b = document.createElement('button'); b.className = 'console-tab'; b.textContent = t(label);
    b.onclick = () => { filter = k; syncTabs(); renderAll(); };
    tabs[k] = b; head.appendChild(b);
  });
  const spacer = el('console-spacer'); head.appendChild(spacer);
  const clearBtn = document.createElement('button'); clearBtn.className = 'console-clear'; clearBtn.textContent = t('Clear');
  ...
  const tbody = document.createElement('tbody'); table.appendChild(tbody); list.appendChild(table);
  const atBottom = () => list.scrollHeight - list.scrollTop - list.clientHeight < 24;
  /** @param {string} cls @param {string} [txt] */
  const td = (cls, txt) => { const d = document.createElement('td'); d.className = cls; if (txt != null) d.textContent = txt; return d; };
  ...
  const rowFor = (e) => {
    ...
    const tr = document.createElement('tr'); tr.className = 'console-row ' + (dir ? 'dir-' + dir + ' ' : '') + (e.level === 'error' ? 'lvl-error' : e.level === 'warn' ? 'lvl-warn' : '');
    const cat = e.cat === 'addon' ? 'addon' : 'journal';
    const dirTd = td('console-dir'); if (dir) dirTd.appendChild(el('dir-chip dir-' + dir, dir === 'out' ? t('OUT') : t('IN')));
    const catTd = td('console-cat'); const chip = el('cat-chip cat-' + cat, t(cat)); catTd.appendChild(chip);
    tr.append(td('console-time', hhmmss(e.t)), dirTd, catTd, td('console-src', e.src || ''), td('console-msg', e.msg));
    return tr;
  };
  /** @param {ConsoleEntry} e */
  const append = (e) => { const stick = atBottom(); tbody.appendChild(rowFor(e)); while (tbody.childElementCount > 2000) tbody.removeChild(/** @type {Node} */ (tbody.firstChild)); if (stick) list.scrollTop = list.scrollHeight; };
  const renderAll = () => { tbody.innerHTML = ''; platform.console.history().forEach((e) => { if (passes(e, filter)) tbody.appendChild(rowFor(e)); }); list.scrollTop = list.scrollHeight; };

  // push the user's OUT/IN direction colours to CSS vars the console rows read (Trade Desk > Colors edits them).
  const applyColors = () => { const c = getDeskColors(); const s = document.documentElement.style; s.setProperty('--dir-out', c.out); s.setProperty('--dir-in', c.in); };
  ...
  const off = platform.console.subscribe((e) => { if (passes(e, filter)) append(e); }, renderAll);
  const offTz = onDeskConfigChange(() => { applyColors(); renderAll(); });   // desk config changed -> re-apply colours + re-render timestamps

  return {
    destroy() { try { off(); } catch (_) {} try { offTz(); } catch (_) {} root.innerHTML = ''; },
    state() { return { filter }; },   // round-trips in the tab's workspace
  };
}
