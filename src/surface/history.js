...
const fmtDur = (a, b) => {
  ...
  let s = Math.max(0, Math.round((Number(b) - Number(a)) / 1000));
...
const signed = (v, dec = 2) => {
  if (v == null || v === '' || Number.isNaN(Number(v))) return { text: '—' };
  const n = Number(v);
  const s = (n > 0 ? '+' : '') + n.toLocaleString('en-US', { minimumFractionDigits: dec, maximumFractionDigits: dec });
...
const decOf = (r) => {
  ...
  const d = f && /** @type {any} */ (f).priceDecimals;
  return d != null && !Number.isNaN(Number(d)) ? Number(d) : 2;
};
...
const CAT = [
  ...
  // Position id: the broker's REAL position id on hedging accounts, else the app's entry-time tag (netting)
  {
    ...
    label: 'Position',
    align: 'left',
    get: (r) => ({ text: r.positionId != null ? String(r.positionId) : fmtTag(r.entryTime) }),
  },
  ...
  // --- P&L (Points = avg price move per contract; Gross/Net in account currency via the contract tick value) ---
  {
    ...
    label: 'Points',
    align: 'right',
    get: (r) => signed(r.entryQty ? r.realizedPricePnl / r.entryQty : null, decOf(r)),
  },
  { key: 'gross', label: 'Gross', align: 'right', get: (r) => signed(r.realizedPnl) },
  {
    ...
    label: 'Comm',
    align: 'right',
    get: (r) => ({ text: r.commission ? fmtPrice(r.commission, 2) : '—' }),
  },
  ...
  // running account balance after this trade (anchored to the LIVE account balance; see render)
  {
    ...
    align: 'right',
    get: (r) => {
      const b = balanceByTrade.get(r);
      return b == null ? { text: '—' } : { text: fmtPrice(b, 2) };
    },
  },
  // --- ACCOUNT ---
  ...
  { key: 'account', label: 'Account #', align: 'left', get: (r) => ({ text: txt(r.accountId) }) },
  {
    ...
    label: 'Broker',
    align: 'left',
    get: (r) => ({ text: String(r.broker || '').toUpperCase() || '—' }),
  },
];
...
const getCols = () => {
  const c = getSetting('historyColumns');
...
const RAW = {
  ...
  side: (r) => r.side,
  qty: (r) => r.entryQty,
  avgEntry: (r) => r.avgEntry,
  avgExit: (r) => r.avgExit,
  entryTime: (r) => r.entryTime,
  exitTime: (r) => r.exitTime,
  duration: (r) => (Number(r.exitTime) || 0) - (Number(r.entryTime) || 0),
  points: (r) => (r.entryQty ? r.realizedPricePnl / r.entryQty : null),
  gross: (r) => r.realizedPnl,
...
const fillCell = (key, f, ctx) => {
  ...
  switch (key) {
    case 'symbol':
      return { text: txt(s.symbol) };
    case 'side':
      return {
        text: (s.side || '').toUpperCase() || '',
        cls: s.side === 'sell' ? 'neg' : s.side === 'buy' ? 'pos' : '',
      };
    case 'qty':
      return { text: fmtNum(s.qty) };
    case 'avgEntry':
      return { text: fmtPrice(s.price, ctx.dec) }; // the fill price (col doubles as Fill Price on children)
    case 'entryTime':
    case 'exitTime':
      return { text: fmtTime(s.time) };
    case 'commission':
      return { text: s.commission ? fmtPrice(s.commission, 2) : '—' };
    case 'points':
    case 'gross':
    case 'net': {
    ...
    default:
      return { text: '' }; // tag / avgExit / duration / balance / account* / broker: parent-only
  }
};
...
export function mountHistory(root) {
  ...
  const title = el('acct-title', 'History');
  const count = el('acct-count', '');
  const gear = el('acct-gear');
  gear.title = t('Configure columns');
  gear.innerHTML = GEAR;
  ...
  const render = () => {
    ...
    balanceByTrade = computeRunningBalances(closed, (k) => {
      const acct = platform.accounts.get(k);
      return acct && acct.balance;
    });
    ...
    if (!rows.length) {
      ...
      tdE.textContent =
        dateFilter.mode() === 'all'
          ? t('No closed trades in the loaded history')
          : t('No closed trades in') + ' ' + dateFilter.label().toLowerCase();
      tr.appendChild(tdE);
    ...
    rows.forEach((r) => {
      ...
      (r.fillPnl || []).forEach(({ isExit, points, gross, net }, i) => {
        ...
        ctr.className = 'hist-child';
        cols.forEach((k) => {
          const v = fillCell(k, f, { dec, isExit, points, gross, net });
          const td = document.createElement('td');
          td.className = 'a-' + (BY[k].align || 'left') + (v.cls ? ' ' + v.cls : '');
          td.textContent = v.text;
          ctr.appendChild(td);
        });
  ...
  gear.onclick = () =>
    openColumnPicker(
      ...
      /** @param {string[]} next */ (next) => {
        cols = next;
        setSetting('historyColumns', cols);
        render();
      },
    );
