...
export function buildModifyEditor(ctx) {
  ...
  function levelRow(kind, label, initPrice) {
    ...
    r.className = 'ot-mod-row';
    r.style.gap = '6px'; // match the account/symbol row gap so the column edges line up
    const lbl = document.createElement('label');
    lbl.className = 'ot-mod-label';
    lbl.style.flex = '0 0 47.3px';
    lbl.textContent = t(label); // == "Account:" width -> the Stop/Target box left edge lands on the account box's
    const price = document.createElement('input');
    price.type = 'number';
    price.className = 'ot-mod-price';
    price.step = String(priceStep);
    price.min = '0';
    ...
    unitLbl.className = 'ot-mod-unit';
    unitLbl.textContent = unit.label;
    ...
    ph.style.cssText =
      'display:inline-grid;justify-items:end;align-items:center;flex:0 0 auto;font-size:12px;color:var(--tx-dim);';
    const phSizer = document.createElement('span');
    phSizer.className = 'ot-label';
    phSizer.textContent = t('Symbol:');
    phSizer.style.cssText = 'grid-area:1/1;visibility:hidden;';
    const phLabel = document.createElement('span');
    phLabel.textContent = t('Dist:');
    phLabel.style.cssText = 'grid-area:1/1;';
    ph.append(phSizer, phLabel);
    const distGroup = document.createElement('div');
    distGroup.style.cssText = 'flex:0 0 120px;display:flex;align-items:center;gap:6px;min-width:0;';
    ...
    if (initPrice != null && Number(initPrice) > 0 && Number.isFinite(entry)) {
      price.value = fmtP(initPrice);
      dist.value = String(toDist(Number(initPrice)));
    } else {
      price.value = fmtP(0);
      dist.value = '0';
    }
    ...
    const xBtn = /** @type {HTMLButtonElement} */ (document.createElement('button'));
    xBtn.type = 'button';
    xBtn.className = 'ot-lvl-x';
    xBtn.textContent = '✕';
    xBtn.title = t('Remove this level');
    const updateX = () => {
      xBtn.style.display = Number(price.value) > 0 ? '' : 'none';
    };
    price.oninput = () => {
      if (!guard && Number.isFinite(entry)) {
        guard = true;
        dist.value = String(toDist(Number(price.value)));
        guard = false;
      }
      updateX();
    };
    dist.oninput = () => {
      if (guard || !Number.isFinite(entry)) return;
      guard = true;
      price.value = fmtP(toPrice(Number(dist.value)));
      guard = false;
      updateX();
    };
    ...
    syncers.push({
      ...
      apply: (/** @type {any} */ v) => {
        ...
        price.value = fmtP(val);
        dist.value = val > 0 && Number.isFinite(entry) ? String(toDist(val)) : '0';
        updateX();
      },
    });
    levelCtl.push({
    ...
    spacer.style.flex = '1 1 auto'; // absorbs the gap between the fixed price box and the right-pinned Dist column
    r.append(lbl, price, xBtn, spacer, ph, distGroup);
    wrap.appendChild(r);
    return price;
  }
  ...
  state.syncModify = () => {
    const live = /** @type {any} */ (
      platform.positionLots
        .all()
        .find((l) => String(l.ticket) === String(ctx.ticket) && (!ctx.broker || l.broker === ctx.broker)) ||
        platform.positions
          .all()
          .find((p) => p.symbol === ctx.symbol && (!ctx.broker || p.broker === ctx.broker) && Number(p.qty) > 0)
    );
  ...
  status.className = 'ot-mod-status';
  ...
  for (const lc of levelCtl) {
    lc.xBtn.onclick = () => {
      ...
      status.textContent = t('Removing…');
      status.className = 'ot-mod-status';
      command({ type: 'modifyPosition', broker: ctx.broker, ticket: ctx.ticket, stopLoss: sl, takeProfit: tp })
        .then((/** @type {any} */ r) => cmdStatus(status, r, t('Removed')))
        .catch((/** @type {any} */ e) => cmdStatus(status, { error: (e && e.message) || String(e) }, ''));
      lc.setZero();
    };
  }
  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'ot-btn-primary';
  btn.textContent = t('Modify');
  btn.onclick = () => {
    const sl = Number(slPrice.value) || 0,
      tp = Number(tpPrice.value) || 0; // 0 = no bracket on that side
    ...
    status.className = 'ot-mod-status';
    command({ type: 'modifyPosition', broker: ctx.broker, ticket: ctx.ticket, stopLoss: sl, takeProfit: tp })
      .then((/** @type {any} */ r) => cmdStatus(status, r, t('Modified')))
      .catch((/** @type {any} */ e) => cmdStatus(status, { error: (e && e.message) || String(e) }, ''));
  };
  ...
  closeBtn.className = 'ot-btn-close';
  closeBtn.textContent = t('Close');
  closeBtn.title = t('Close this position');
  closeBtn.onclick = () => {
    if (ctx.ticket == null) {
      status.textContent = t('no position');
      status.className = 'ot-mod-status err';
      return;
    }
    status.textContent = t('Closing…');
    status.className = 'ot-mod-status';
    command({ type: 'closeLot', broker: ctx.broker, ticket: ctx.ticket })
      .then((/** @type {any} */ r) => cmdStatus(status, r, t('Closed')))
      .catch((/** @type {any} */ e) => cmdStatus(status, { error: (e && e.message) || String(e) }, ''));
  };
  // [Partial] closes X qty of THIS position (closeLotPartial). Qty entered in the adjacent input.
  const qtyIn = document.createElement('input');
  qtyIn.type = 'number';
  qtyIn.className = 'ot-mod-dist ot-mod-qty';
  qtyIn.min = '0';
  qtyIn.step = 'any';
  qtyIn.placeholder = t('Qty');
  qtyIn.title = t('Quantity to close');
  const partialBtn = document.createElement('button');
  partialBtn.type = 'button';
  partialBtn.className = 'ot-btn-close';
  partialBtn.textContent = t('Partial');
  partialBtn.title = t('Close part of this position');
  partialBtn.onclick = () => {
    ...
    if (!(q > 0)) {
      status.textContent = t('enter qty');
      status.className = 'ot-mod-status err';
      return;
    }
    status.textContent = t('Closing') + ' ' + q + '…';
    status.className = 'ot-mod-status';
    command({ type: 'closeLot', broker: ctx.broker, ticket: ctx.ticket, qty: q })
      .then((/** @type {any} */ r) => cmdStatus(status, r, t('Closed') + ' ' + q))
      .catch((/** @type {any} */ e) => cmdStatus(status, { error: (e && e.message) || String(e) }, ''));
  };
...
export function buildOrderModifyEditor(o) {
  ...
  volRow.row.style.gridColumn = '1 / 2';
  volRow.row.style.gridRow = '1';
  const priceRow = mktRow('Price:', price, { step });
  priceRow.row.style.gridColumn = '1 / 2';
  priceRow.row.style.gridRow = '2';
  // Volume/Price are 120px, matching every other input (Qt type / Stake / Stop / Target / Account / Symbol).
  volRow.input.style.flex = '0 0 120px';
  priceRow.input.style.flex = '0 0 120px';
  volRow.input.oninput = () => {
    vol = Math.max(1, Math.round(Number(volRow.input.value) || 1));
  };
  priceRow.input.oninput = () => {
    price = Number(priceRow.input.value) || 0;
  };
  ...
  const slRow = mktRow('Stop Loss:', sl, { step, min: -1 });
  slRow.row.style.gridColumn = '1 / 2';
  slRow.row.style.gridRow = '3';
  const tpRow = mktRow('Take Profit:', tp, { step, min: -1 });
  tpRow.row.style.gridColumn = '1 / 2';
  tpRow.row.style.gridRow = '4';
  ...
  state.syncModify = () => {
    const live = /** @type {any} */ (
      platform.orders.all().find((x) => String(x.id) === String(o.id) && (!o.broker || x.broker === o.broker))
    );
    if (!live) return; // order filled/cancelled -- leave the fields as they are; the mini table shows the new state
    if (document.activeElement !== priceRow.input) {
      const v = Number(live.price) > 0 ? Number(live.price) : 0;
      priceRow.input.value = fmtP(v);
      price = v;
    }
    if (document.activeElement !== slRow.input) {
      const v = Number(live.stopLoss) > 0 ? Number(live.stopLoss) : 0;
      slRow.input.value = v > 0 ? fmtP(v) : '0';
      sl = v;
    }
    if (document.activeElement !== tpRow.input) {
      const v = Number(live.takeProfit) > 0 ? Number(live.takeProfit) : 0;
      tpRow.input.value = v > 0 ? fmtP(v) : '0';
      tp = v;
    }
    if (document.activeElement !== volRow.input) {
      const v = Number(live.qty) > 0 ? Number(live.qty) : vol;
      volRow.input.value = String(v);
      vol = v;
    }
  };
  ...
  const syncSltp = () => {
    const dim = showSltp ? '' : '0.45';
    slRow.row.style.display = '';
    slRow.row.style.opacity = dim;
    slRow.input.disabled = !showSltp;
    tpRow.row.style.display = '';
    tpRow.row.style.opacity = dim;
    tpRow.input.disabled = !showSltp;
  };
  syncSltp();
  ...
  if (ra && ra.resolveSymbol)
    ra.resolveSymbol(
      o.symbol,
      /** @param {any} inst */ (inst) => {
        if (inst) {
          dec = inst.priceDecimals != null ? Number(inst.priceDecimals) : dec;
          step = inst.tickSize ? Number(inst.tickSize) : step;
          priceRow.input.step = String(step);
          slRow.input.step = String(step);
          tpRow.input.step = String(step);
        }
      },
    );

  const actions = document.createElement('div');
  actions.className = 'ot-mod-actions';
  actions.style.gridColumn = '1 / 3';
  actions.style.gridRow = '5'; // span the content columns, below the Target row
  const status = document.createElement('span');
  status.className = 'ot-mod-status';
  const cancelBtn = document.createElement('button');
  cancelBtn.type = 'button';
  cancelBtn.className = 'ot-btn-close';
  cancelBtn.textContent = t('Cancel');
  cancelBtn.title = t('Cancel this order');
  const modBtn = document.createElement('button');
  modBtn.type = 'button';
  modBtn.className = 'ot-btn-primary';
  modBtn.textContent = t('Modify');
  modBtn.onclick = () => {
    if (!(price > 0)) {
      status.textContent = t('enter a price');
      status.className = 'ot-mod-status err';
      return;
    }
    if (!(vol > 0)) {
      status.textContent = t('enter volume');
      status.className = 'ot-mod-status err';
      return;
    }
    status.textContent = t('Sending…');
    ...
    if (showSltp) {
      cmd.stopLoss = sl;
      cmd.takeProfit = tp;
    } // hedging pending order: carry the SL/TP prices
    command(cmd)
      .then((/** @type {any} */ r) => cmdStatus(status, r, t('Modified')))
      .catch((/** @type {any} */ e) => cmdStatus(status, { error: (e && e.message) || String(e) }, ''));
  };
  cancelBtn.onclick = () => {
    status.textContent = t('Canceling…');
    status.className = 'ot-mod-status';
    command({ type: 'cancel', broker: o.broker, id: o.id })
      .then((/** @type {any} */ r) => cmdStatus(status, r, t('Canceled')))
      .catch((/** @type {any} */ e) => cmdStatus(status, { error: (e && e.message) || String(e) }, ''));
  };
