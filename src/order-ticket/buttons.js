...
export function buildButtonBar(getCtx) {
  ...
  let buttons = [];
  const bar = document.createElement('div'); bar.className = 'ot-footer';
  const status = document.createElement('span'); status.className = 'ot-foot-status';
  const barRow = document.createElement('div'); barRow.className = 'ot-bar-row';
  const row = document.createElement('div'); row.className = 'ot-qbtns';
  const gear = document.createElement('button'); gear.type = 'button'; gear.className = 'ot-gear'; gear.textContent = '⚙'; gear.title = t('Configure buttons');

  /** @param {string} msg @param {boolean} [err] */
  const setStatus = (msg, err) => { status.textContent = msg; status.className = 'ot-foot-status' + (err ? ' err' : ''); };
  ...
  const run = (b) => {
    ...
    let trigger = null;
    try { const ops = parseScript(b.script); if (ops.length === 1 && ops[0].op === 'market' && ops[0].trigger) trigger = ops[0].side; } catch (_) {}
    if (trigger) { if (state.fire) { setStatus(''); state.fire(trigger); } else setStatus('no ' + t(trigger) + ' setup on this tab', true); return; }
    setStatus('…');
    command({ type: 'script', script: b.script, ctx: getCtx() })
      .then((/** @type {any} */ r) => { const ok = r && r.ok; setStatus(ok ? 'done' : ((r && r.error) || 'failed'), !ok); })
      .catch((/** @type {any} */ e) => setStatus(e.message, true));
  };

  const fitWindow = () => requestAnimationFrame(() => {
    row.style.flex = '0 0 auto'; const natural = row.offsetWidth; row.style.flex = '';   // measure natural, then restore stretch
    const d = /** @type {any} */ (window).desktop;
    if (natural && d && d.orderTicketWidth) d.orderTicketWidth(natural + 16 + 16 + 8);
  });
  const renderBar = () => {
    row.innerHTML = '';
    for (const b of buttons) { const btn = document.createElement('button'); btn.type = 'button'; btn.className = 'ot-btn-close'; btn.textContent = b.label; btn.title = b.script; btn.onclick = () => run(b); row.appendChild(btn); }
    fitWindow();
  };
  // re-read the active template's buttons and repaint the bar (called after the editor closes / switches template)
  const refresh = () => { buttons = templateButtons(activeTemplateId()); renderBar(); };
  ...
  /** @param {string} combo */
  const fireByCombo = (combo) => { const b = buttons.find((x) => x.hotkey === combo); if (b) run(b); };
