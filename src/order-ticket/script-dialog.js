...
export function openScriptDialog(current, onSave) {
  ...
  ov.className = 'ot-scripted';

  const head = document.createElement('div');
  head.className = 'ot-ed-head';
  const title = document.createElement('span');
  title.textContent = t('Edit script');
  ...
  ta.className = 'ot-scripted-ta';
  ta.rows = 3;
  ta.value = current || '';
  ta.placeholder = 'buy 5 and set stop 20 and set target 40';
  const err = document.createElement('div');
  err.className = 'ot-scripted-err';
  const validate = () => {
    try {
      if (ta.value.trim()) parseScript(ta.value);
      err.textContent = '';
      ta.classList.remove('invalid');
      return true;
    } catch (e) {
      err.textContent = /** @type {any} */ (e).message;
  ...
  tabsBar.className = 'ot-cmd-tabs';
  const cmds = document.createElement('div');
  cmds.className = 'ot-scripted-cmds';
  const append = (/** @type {string} */ snippet) => {
    const v = ta.value.trim();
    ta.value = v ? v + ' and ' + snippet : snippet;
    validate();
    ta.focus();
  };
  /** @param {number} gi */
  const renderCmds = (gi) => {
    ...
    for (const [snippet, desc] of VOCAB_GROUPS[gi].items) {
      ...
      sp.className = 'ot-cmd-t';
      sp.textContent = snippet; // script SYNTAX (English DSL keywords) -- not translated
      const d = document.createElement('span');
      d.className = 'ot-cmd-d';
      d.textContent = desc;
      c.append(sp, d);
  ...
  VOCAB_GROUPS.forEach((g, gi) => {
    ...
    tb.type = 'button';
    tb.className = 'ot-cmd-tab';
    tb.textContent = g.tab; // family names are DSL keywords, not translated
    tb.onclick = () => {
  ...
  foot.className = 'ot-scripted-foot';
  const cancel = document.createElement('button');
  cancel.type = 'button';
  cancel.className = 'ot-btn-close';
  cancel.textContent = t('Cancel');
  cancel.onclick = () => ov.remove();
  const save = document.createElement('button');
  save.type = 'button';
  save.className = 'ot-btn-primary';
