...
function enterPlaced(msg) { state.placed = true; state.placedMsg = msg; render(); }
/** New Order: re-arm the dialog IN PLACE -- reset the form to fresh defaults, drop the projection, un-gray. No close. */
function newOrder() {
  state.placed = false; state.placedMsg = '';
  state.mktVol = 1; state.mktSl = 0; state.mktTp = 0; state.mktStake = 0;
  state.lsPrice = 0; state.lsSl = 0; state.lsTp = 0; state.lsTif = 'gtc'; state.lsGtdDate = '';
  const c = getCtx(); if (c.symbol) setProjecting(c.broker, c.symbol, false);   // clear the placed projection -> clean slate
  render();
}
...
function buildPlacedActions() {
  const wrap = document.createElement('div'); wrap.className = 'ot-mkt-actions';
  const status = document.createElement('div'); status.className = 'ot-foot-status'; status.textContent = state.placedMsg || t('order sent');
  const row = document.createElement('div'); row.className = 'ot-bar-row';
  const nu = document.createElement('button'); nu.type = 'button'; nu.className = 'ot-btn-primary ot-btn-new'; nu.textContent = t('New order'); nu.onclick = newOrder;
  row.append(nu);
...
function applySideGate(buyBtn, sellBtn, side) { buyBtn.disabled = side === 'short'; sellBtn.disabled = side === 'long'; }
...
export function buildMarketForm() {
  const grid = document.createElement('div'); grid.className = 'ot-mkt-grid';
  ...
  vol.row.style.gridColumn = '1 / 2'; vol.row.style.gridRow = '2';
  // Volume edits sync to plan.qty while projecting -- the on-chart pill's qty cell shows/edits the SAME value
  vol.input.oninput = () => { state.mktVol = Math.max(1, Math.round(Number(vol.input.value) || 1)); planControl.setQty(state.mktVol); };
  ...
  const mktRef = () => (state.mktAsk > 0 && state.mktBid > 0) ? (state.mktAsk + state.mktBid) / 2 : (state.mktAsk || state.mktBid || 0);
  const sl = mktRow('Stop Loss:', state.mktSl, { min: -1 }); sl.row.style.gridColumn = '1 / 2'; sl.row.style.gridRow = '3';
  const tp = mktRow('Take Profit:', state.mktTp, { min: -1 }); tp.row.style.gridColumn = '1 / 2'; tp.row.style.gridRow = '4';
  ...
  seedSpinner(sl.input, { getRef: mktRef, getDec, set: (v) => { state.mktSl = v; planControl.setStop(v, mktRef()); if (state.recalcStake) state.recalcStake(); if (state.syncSideGate) state.syncSideGate(); } });
  seedSpinner(tp.input, { getRef: mktRef, getDec, set: (v) => { state.mktTp = v; planControl.setTarget(v); if (state.syncSideGate) state.syncSideGate(); } });
  ...
  state.recalcStake = wireStakePreview(vol.input, () => state.mktAsk || state.mktBid, () => state.mktSl);
  ...
  const slDist = attachDist(sl.input, { kind: 'sl', getRef: mktRef, getDec, onPriceSet: (p) => { state.mktSl = p || 0; planControl.setStop(state.mktSl, mktRef()); } });
  slDist.row.style.gridColumn = '2 / 4'; slDist.row.style.gridRow = '3';
  const tpDist = attachDist(tp.input, { kind: 'tp', getRef: mktRef, getDec, onPriceSet: (p) => { state.mktTp = p || 0; planControl.setTarget(state.mktTp); } });
  tpDist.row.style.gridColumn = '2 / 4'; tpDist.row.style.gridRow = '4';
  ...
  for (const d of [slDist.dist, tpDist.dist]) { d.disabled = true; d.classList.add('ot-dist-ref'); d.title = t('reference only -- a market order uses a fixed price, not a moving distance'); }
  state.recomputeDist = () => { slDist.recompute(); tpDist.recompute(); };
  ...
  const toggles = document.createElement('div'); toggles.className = 'ot-mkt-toggles'; toggles.style.gridColumn = '1 / 3'; toggles.style.gridRow = '5';
  ...
  const stakeRow = document.createElement('div'); stakeRow.className = 'ot-mod-row'; stakeRow.style.justifyContent = 'flex-end';
  stakeRow.style.gridColumn = '2 / 4'; stakeRow.style.gridRow = '1';   // Stake shares the Qt-type row (top), right-pinned -- unchanged size/side
  const stakeLbl = document.createElement('label'); stakeLbl.className = 'ot-mod-label'; stakeLbl.style.flex = '0 0 auto'; stakeLbl.textContent = t('Stake:');
  const stakeIn = document.createElement('input'); stakeIn.type = 'number'; stakeIn.className = 'ot-input'; stakeIn.style.flex = '0 0 120px'; stakeIn.style.textAlign = 'right'; stakeIn.min = '0'; stakeIn.step = 'any'; stakeIn.value = state.mktStake ? String(state.mktStake) : '';
  stakeIn.oninput = () => { state.mktStake = Number(stakeIn.value) || 0; if (state.recalcStake) state.recalcStake(); if (state.syncSideGate) state.syncSideGate(); };
  stakeRow.append(stakeLbl, stakeIn);
  // Qt-type change: show/hide the Stake row AND flip the Volume box between editable (Units) and grayed live-preview (Stake).
  const syncStake = () => { stakeRow.style.display = state.qtType === 'stake' ? '' : 'none'; if (state.recalcStake) state.recalcStake(); if (state.syncSideGate) state.syncSideGate(); };
  const qt = buildQtTypeRow(syncStake); qt.style.gridColumn = '1 / 2'; qt.style.gridRow = '1';   // Qt type in col1, ABOVE Volume
  syncStake();
  ...
  applyMktInst(); syncFields(); resolveMktInst(); state.recalcStake(); state.recomputeDist();   // reflect plan levels + paint the Stake preview + Dist
  return grid;
}
...
function buildQuoteReadout() {
  const el = document.createElement('div'); el.className = 'ot-quote';
  const bidEl = document.createElement('span'); bidEl.className = 'ot-quote-bid';
  const sep = document.createElement('span'); sep.className = 'ot-quote-sep'; sep.textContent = '/';
  const askEl = document.createElement('span'); askEl.className = 'ot-quote-ask';
...
export function buildMarketActions() {
  ...
  const wrap = document.createElement('div'); wrap.className = 'ot-mkt-actions';
  const status = document.createElement('div'); status.className = 'ot-foot-status';
  const btns = document.createElement('div'); btns.className = 'ot-bar-row';
  const buy = document.createElement('button'); buy.type = 'button'; buy.className = 'ot-btn-buy'; buy.textContent = t('Buy');
  const sell = document.createElement('button'); sell.type = 'button'; sell.className = 'ot-btn-sell'; sell.textContent = t('Sell');
  ...
  const fire = (side) => {
    ...
    const res = buildPlaceIntent({ orderType: 'market', ctx: c, side, qty: state.mktVol, qtType: state.qtType, stake: state.mktStake, sl: state.mktSl, tp: state.mktTp });
    if (!res.ok) { status.textContent = t(res.error); status.className = 'ot-foot-status err'; return; }
    const stake = state.qtType === 'stake';
    status.textContent = t(side.toUpperCase()) + ' ' + (stake ? t('sizing') : state.mktVol) + '…'; status.className = 'ot-foot-status';
    command(res.intent)
      .then((/** @type {any} */ r) => { if (r && r.ok) { enterPlaced(t(side.toUpperCase()) + ' ' + (r.qty != null ? r.qty : state.mktVol) + ' ' + c.symbol + ' ' + t('sent')); } else { status.textContent = (r && r.error) || t('rejected'); status.className = 'ot-foot-status err'; } })
      .catch((/** @type {any} */ e) => { status.textContent = (e && e.message) || t('error'); status.className = 'ot-foot-status err'; });
  };
  buy.onclick = () => fire('buy'); sell.onclick = () => fire('sell');
  ...
  state.syncSideGate = () => { const entry = (state.mktAsk > 0 && state.mktBid > 0) ? (state.mktAsk + state.mktBid) / 2 : (state.mktAsk || state.mktBid || 0); applySideGate(buy, sell, sideForSetup(entry, state.mktSl, state.mktTp)); };
  state.syncSideGate();
...
export function buildLimitStopForm(_type) {
  ...
  price.row.style.gridColumn = '1 / 2'; price.row.style.gridRow = '3';
  ...
  price.input.oninput = () => { state.lsPrice = Number(price.input.value) || 0; planControl.setRef(state.lsPrice); if (state.recalcStake) state.recalcStake(); if (state.syncSideGate) state.syncSideGate(); if (state.recomputeDist) state.recomputeDist(); };   // Price is the Dist reference here -> refresh the SL/TP distances when it moves
  ...
  const sl = mktRow('Stop Loss:', state.lsSl, { min: -1 }); sl.row.style.gridColumn = '1 / 2'; sl.row.style.gridRow = '4';
  const tp = mktRow('Take Profit:', state.lsTp, { min: -1 }); tp.row.style.gridColumn = '1 / 2'; tp.row.style.gridRow = '5';
  ...
  const lsSeedDec = () => (state.mktInst && state.mktInst.priceDecimals != null) ? Number(state.mktInst.priceDecimals) : 5;
  seedSpinner(sl.input, { getRef: () => Number(state.lsPrice) || 0, getDec: lsSeedDec, set: (v) => { state.lsSl = v; planControl.setStop(v, Number(state.lsPrice) || 0); if (state.recalcStake) state.recalcStake(); if (state.syncSideGate) state.syncSideGate(); } });
  seedSpinner(tp.input, { getRef: () => Number(state.lsPrice) || 0, getDec: lsSeedDec, set: (v) => { state.lsTp = v; planControl.setTarget(v); if (state.syncSideGate) state.syncSideGate(); } });
  ...
  state.recalcStake = wireStakePreview(vol.input, () => state.lsPrice, () => state.lsSl);
  ...
  const slDist = attachDist(sl.input, { kind: 'sl', getRef: lsRef, getDec: lsDec, onPriceSet: (p) => { state.lsSl = p || 0; planControl.setStop(state.lsSl, lsRef()); if (state.recalcStake) state.recalcStake(); if (state.syncSideGate) state.syncSideGate(); } });
  slDist.row.style.gridColumn = '2 / 3'; slDist.row.style.gridRow = '4';
  const tpDist = attachDist(tp.input, { kind: 'tp', getRef: lsRef, getDec: lsDec, onPriceSet: (p) => { state.lsTp = p || 0; planControl.setTarget(p); if (state.syncSideGate) state.syncSideGate(); } });
  tpDist.row.style.gridColumn = '2 / 3'; tpDist.row.style.gridRow = '5';
  state.recomputeDist = () => { slDist.recompute(); tpDist.recompute(); };
  if (state.lsInputs) { state.lsInputs.slDist = slDist.dist; state.lsInputs.tpDist = tpDist.dist; }
  if (state.lsSltpRows) { state.lsSltpRows.slDist = slDist.row; state.lsSltpRows.tpDist = tpDist.row; }
  ...
  const expRow = document.createElement('div'); expRow.className = 'ot-mod-row';
  expRow.style.gridColumn = '2 / 3'; expRow.style.gridRow = '2'; expRow.style.justifyContent = 'flex-end';   // beside Volume
  const expLbl = document.createElement('label'); expLbl.className = 'ot-mod-label'; expLbl.style.flex = '0 0 auto'; expLbl.textContent = t('Exp:');
  const tif = document.createElement('select'); tif.className = 'ot-input'; tif.style.flex = '0 0 120px';   // == Symbol combo width, so the boxes align
  /** @type {[string,string][]} */ ([['gtc', 'GTC'], ['day', 'DAY'], ['gtd', 'GTD']]).forEach(([v, l]) => { const o = document.createElement('option'); o.value = v; o.textContent = t(l); tif.appendChild(o); });
  ...
  const gDateRow = document.createElement('div'); gDateRow.className = 'ot-mod-row';
  gDateRow.style.gridColumn = '2 / 3'; gDateRow.style.gridRow = '3'; gDateRow.style.justifyContent = 'flex-end';   // beside Price
  const gDate = /** @type {HTMLInputElement} */ (document.createElement('input')); gDate.type = 'date'; gDate.className = 'ot-input'; gDate.style.cssText = 'flex:0 0 120px;min-width:0;'; gDate.value = state.lsGtdDate;
  gDateRow.append(gDate);
  const applyTif = () => { gDateRow.style.display = tif.value === 'gtd' ? '' : 'none'; };
  tif.onchange = () => { state.lsTif = tif.value; applyTif(); };
  gDate.onchange = () => { state.lsGtdDate = gDate.value; };
  applyTif();
  // Stake input under Qt type (col2 row2), shown only when Qt type = Stake -- same shared risk value as the Market tab.
  const stakeRow = document.createElement('div'); stakeRow.className = 'ot-mod-row'; stakeRow.style.justifyContent = 'flex-end';
  stakeRow.style.gridColumn = '2 / 3'; stakeRow.style.gridRow = '1';   // Stake shares the Qt-type row (top), right-pinned -- unchanged size/side
  const stakeLbl = document.createElement('label'); stakeLbl.className = 'ot-mod-label'; stakeLbl.style.flex = '0 0 auto'; stakeLbl.textContent = t('Stake:');
  const stakeIn = document.createElement('input'); stakeIn.type = 'number'; stakeIn.className = 'ot-input'; stakeIn.style.flex = '0 0 120px'; stakeIn.style.textAlign = 'right'; stakeIn.min = '0'; stakeIn.step = 'any'; stakeIn.value = state.mktStake ? String(state.mktStake) : '';
  stakeIn.oninput = () => { state.mktStake = Number(stakeIn.value) || 0; if (state.recalcStake) state.recalcStake(); if (state.syncSideGate) state.syncSideGate(); };
  stakeRow.append(stakeLbl, stakeIn);
  ...
  applyMktInst(); syncFields(); resolveMktInst(); state.recalcStake(); state.recomputeDist();   // steps from the tick; reflect the plan; paint the Stake preview + Dist
  return wrap;
}
...
export function buildLimitStopActions(type) {
  ...
  const fire = (side) => {
    ...
    const res = buildPlaceIntent({ orderType: type, ctx: c, side, qty: state.mktVol, qtType: state.qtType, stake: state.mktStake, sl: state.lsSl, tp: state.lsTp, price: state.lsPrice, tif: state.lsTif, gtdDate: state.lsGtdDate });
    if (!res.ok) { status.textContent = t(res.error); status.className = 'ot-foot-status err'; return; }
    const stake = state.qtType === 'stake';
    status.textContent = t(side.toUpperCase()) + ' ' + t(type) + ' ' + (stake ? t('sizing') : state.mktVol) + '…'; status.className = 'ot-foot-status';
    command(res.intent)
      .then((/** @type {any} */ r) => { if (r && r.ok) { enterPlaced(t(side.toUpperCase()) + ' ' + t(type) + ' ' + (r.qty != null ? r.qty : state.mktVol) + ' ' + c.symbol + ' ' + t('sent')); } else { status.textContent = (r && r.error) || t('rejected'); status.className = 'ot-foot-status err'; } })
      .catch((/** @type {any} */ e) => { status.textContent = (e && e.message) || t('error'); status.className = 'ot-foot-status err'; });
  };
  buy.onclick = () => fire('buy'); sell.onclick = () => fire('sell');
  state.fire = fire;   // a bare "buy"/"sell" DSL trigger (quick-button) fires THIS tab's setup, same as clicking the button
  // gate Buy/Sell from the bracket direction (entry = the order Price); re-run on level/price changes.
  state.syncSideGate = () => { applySideGate(buy, sell, sideForSetup(Number(state.lsPrice) || 0, state.lsSl, state.lsTp)); };
  state.syncSideGate();
  row.append(buy, sell); wrap.append(status, buildQuoteReadout(), row);
  return wrap;
}
