...
import { isProjecting, setProjecting, getPlan, setLevels, subscribe as subscribePlan } from '../chart/order-view/plan-store.js';
...
export const syncToggle = () => { if (state.projectCb && state.projectCb.isConnected) { const c = getCtx(); state.projectCb.checked = isProjecting(c.broker, c.symbol); } };
...
export const syncFields = () => {
  ...
  if (isProjecting(c.broker, c.symbol)) {
    ...
    if (pq > 0) {
      if (state.mktInputs && state.mktInputs.vol !== document.activeElement) { state.mktVol = pq; state.mktInputs.vol.value = String(pq); }
      if (state.lsInputs && state.lsInputs.vol && state.lsInputs.vol !== document.activeElement) { state.mktVol = pq; state.lsInputs.vol.value = String(pq); }
    }
    ...
    if (state.lsInputs && state.lsInputs.price && (p.orderType === 'limit' || p.orderType === 'stop')) {
      ...
      if (ref > 0 && state.lsInputs.price !== document.activeElement) { state.lsPrice = snapTick(ref); state.lsInputs.price.value = String(state.lsPrice); }
    }
  }
  ...
  if (state.mktInputs && l0.stop != null && state.mktInputs.sl !== document.activeElement) { state.mktSl = snapTick(l0.stop); state.mktInputs.sl.value = String(state.mktSl); }
  if (state.mktInputs && l0.target != null && state.mktInputs.tp !== document.activeElement) { state.mktTp = snapTick(l0.target); state.mktInputs.tp.value = String(state.mktTp); }
  // the Limit/Stop tab's SL/TP are the SAME rung-0 bracket -- reflect the pill's drag into those fields too (chart -> fields)
  if (state.lsInputs && state.lsInputs.sl && l0.stop != null && state.lsInputs.sl !== document.activeElement) { state.lsSl = snapTick(l0.stop); state.lsInputs.sl.value = String(state.lsSl); }
  if (state.lsInputs && state.lsInputs.tp && l0.target != null && state.lsInputs.tp !== document.activeElement) { state.lsTp = snapTick(l0.target); state.lsInputs.tp.value = String(state.lsTp); }
...
export const syncTab = () => {
  const c = getCtx(); if (!isProjecting(c.broker, c.symbol)) return;
  const t = getPlan(c.broker, c.symbol).orderType;
  if ((t === 'market' || t === 'limit' || t === 'stop') && t !== state.active && state.active !== 'modify') { state.active = t; render(); }
};
subscribePlan(() => { syncToggle(); syncFields(); syncTab(); });   // any plan change (drag, another window, snapshot) -> refresh the toggles + fields + tab
...
export function buildProjectToggle() {
  const lab = document.createElement('label'); lab.className = 'ot-check'; lab.title = 'Draw a gray planning dot on the chart for this symbol (nothing is sent to a broker)';
  const cb = document.createElement('input'); cb.type = 'checkbox'; cb.className = 'ot-check-box';
  state.projectCb = cb;
  const c = getCtx(); cb.checked = isProjecting(c.broker, c.symbol);
  ...
  cb.onchange = () => { const x = getCtx(); setProjecting(x.broker, x.symbol, cb.checked); if (cb.checked) setLevels(x.broker, x.symbol, { qty: state.mktVol, orderType: 'market' }); };
  const txt = document.createElement('span'); txt.textContent = 'Project order';
  lab.append(cb, txt);
  return lab;
}
