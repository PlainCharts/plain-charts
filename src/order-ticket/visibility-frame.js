...
export function buildVisibilityFrame(getCtx) {
  ...
  const sectionRow = (title, onToggle) => {
    ...
    sec.className = 'ot-vis-sec';
    const ttl = document.createElement('span');
    ttl.className = 'ot-vis-title';
    ttl.textContent = t(title);
    sec.appendChild(ttl);
    const checks = document.createElement('div');
    checks.className = 'ot-vis-checks';
    /** @type {Record<string, HTMLInputElement>} */
    const boxes = {};
    WHICH.forEach(([w, label]) => {
      const lab = document.createElement('label');
      lab.className = 'ot-vis-check';
      const c = document.createElement('input');
      c.type = 'checkbox';
      c.onchange = () => onToggle(w, c.checked);
  ...
  const vis = sectionRow('VISIBILITY', (w, on) => {
    const c = getCtx();
    if (c.symbol) setVis(c.broker, c.symbol, /** @type {any} */ ({ [w]: on }));
  });
  ...
  const syncVis = () => {
    const c = getCtx();
    const v = c.symbol ? getPlan(c.broker, c.symbol).vis || {} : {};
    WHICH.forEach(([w]) => {
      vis.boxes[w].checked = v[w] !== false;
    });
  };
  const syncHide = () => {
    const h = hideOnEntry();
    WHICH.forEach(([w]) => {
      hide.boxes[w].checked = !!h[w];
    });
  };
  ...
  state.syncVis = () => {
    syncVis();
    syncHide();
  }; // window.js render() calls this on a tab / symbol switch (re-target to the new ctx)
  syncVis();
  syncHide();
  return frame;
}
