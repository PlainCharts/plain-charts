...
async function loadWatchSymbols() {
  ...
  for (const l of lists) {
    for (const it of (l && Array.isArray(l.items) ? l.items : [])) {
      ...
      seen.add(key); out.push({ broker: it.broker || '', symbol: it.symbol });
    }
  }
  out.sort((a, b) => a.symbol.localeCompare(b.symbol));
  return out;
}
loadWatchSymbols().then((s) => { watchSymbols = s; });
...
function paintPositionTable(tbl, p) {
  const live = livePosition(p), src = live || p;   // fall back to the opened snapshot once it's gone
  const side = src.side === 'short' ? 'sell' : src.side === 'long' ? 'buy' : (src.side || '');
  const dec = (src.priceDecimals != null ? Number(src.priceDecimals) : (p.priceDecimals != null ? Number(p.priceDecimals) : null));
  const price = src.avgPrice != null ? (dec != null ? Number(src.avgPrice).toFixed(dec) : String(src.avgPrice)) : '';
  /** @type {[string, string][]} */
  const cells = [['id', '#' + p.ticket], ['sym', src.symbol || p.symbol || ''], [side === 'sell' ? 'sell' : 'buy', t(side)], ['qty', src.qty != null ? String(src.qty) : ''], ['px', price]];
  tbl.innerHTML = '';
  for (const [cls, text] of cells) { const c = document.createElement('div'); c.className = 'ot-pcell ot-pcell-' + cls; c.textContent = text; c.title = text; tbl.appendChild(c); }
  tbl.classList.toggle('ot-postable-closed', !live);   // fully closed -> dim the row
}
/** @param {any} p @returns {HTMLElement} */
export function buildPositionTable(p) { const tbl = document.createElement('div'); tbl.className = 'ot-postable'; state.posTableEl = tbl; state.repaintTable = () => paintPositionTable(tbl, p); paintPositionTable(tbl, p); return tbl; }
...
function paintOrderTable(tbl, o) {
  ...
  const pxRaw = src.type === 'stop' ? (src.stopPrice != null ? src.stopPrice : src.price) : src.type === 'limit' ? (src.limitPrice != null ? src.limitPrice : src.price) : (src.price != null ? src.price : o.price);
  ...
  /** @type {[string, string][]} */
  const cells = [['id', '#' + o.id], ['sym', src.symbol || o.symbol || ''], [side === 'sell' ? 'sell' : 'buy', (t(type) + ' ' + t(side)).trim()], ['qty', src.qty != null ? String(src.qty) : ''], ['px', px]];
  tbl.innerHTML = '';
  for (const [cls, text] of cells) { const c = document.createElement('div'); c.className = 'ot-pcell ot-pcell-' + cls; c.textContent = text; c.title = text; tbl.appendChild(c); }
  tbl.classList.toggle('ot-postable-closed', !live);   // filled / canceled -> dim
}
/** @param {any} o @returns {HTMLElement} */
export function buildOrderTable(o) { const tbl = document.createElement('div'); tbl.className = 'ot-postable'; state.posTableEl = tbl; state.repaintTable = () => paintOrderTable(tbl, o); paintOrderTable(tbl, o); return tbl; }
...
export function populateAccounts(sel) {
  ...
  sel.innerHTML = '';
  if (!accts.length) { const o = document.createElement('option'); o.value = ''; o.textContent = t('no account connected'); sel.appendChild(o); sel.disabled = true; state.selectedAccount = null; return; }
  sel.disabled = false;
  ...
  const nameOf = (/** @type {any} */ a) => { const c = listAccounts().find((x) => x.protocol === a.broker); return (c && c.name) || (a.broker || '').toUpperCase(); };
  const names = accts.map(nameOf);
  accts.forEach((a, i) => { const o = document.createElement('option'); o.value = String(i); o.textContent = names.filter((n) => n === names[i]).length > 1 ? names[i] + ' ' + a.accountId : names[i]; sel.appendChild(o); });
  ...
  if (wantBroker) { const j = accts.findIndex((a) => a.broker === wantBroker && (wantAcct == null || String(a.accountId) === String(wantAcct))); if (j >= 0) idx = j; }
  else if (state.selectedAccount) { const cur = state.selectedAccount; const j = accts.findIndex((a) => a.broker === cur.broker && String(a.accountId) === String(cur.accountId)); if (j >= 0) idx = j; }
  sel.value = String(idx);
  const pick = accts[idx]; state.selectedAccount = { broker: pick.broker, accountId: pick.accountId, hedging: !!pick.hedging };
  sel.onchange = () => { const a = accts[Number(sel.value)]; if (a) state.selectedAccount = { broker: a.broker, accountId: a.accountId, hedging: !!a.hedging }; syncToggle(); syncFields(); syncLsSltp(); };
}
function buildAccountSelect() {
  const sel = document.createElement('select'); sel.className = 'ot-input ot-acct-sel'; sel.title = t('Account to execute on');
  state.accountSelEl = sel; populateAccounts(sel);
  return sel;
}
...
export function buildAccountSymbolRow() {
  const row = document.createElement('div'); row.className = 'ot-field ot-acctsym';
  const al = document.createElement('label'); al.className = 'ot-label'; al.textContent = t('Account:');
  const sl = document.createElement('label'); sl.className = 'ot-label'; sl.textContent = t('Symbol:');
...
function buildSymbolCombo() {
  const combo = document.createElement('div'); combo.className = 'ot-combo';
  const input = document.createElement('input'); input.className = 'ot-input'; input.type = 'text';
  input.placeholder = t('Type or pick'); input.value = state.symbolValue;
  const arrow = document.createElement('button'); arrow.type = 'button'; arrow.className = 'ot-combo-arrow'; arrow.textContent = '▾'; arrow.title = t('Pick from watchlists');
  const menu = document.createElement('div'); menu.className = 'ot-combo-menu'; menu.style.display = 'none';
  combo.append(input, arrow, menu);

  const isOpen = () => menu.style.display === 'block';
  const close = () => { menu.style.display = 'none'; };
  /** @param {{broker:string,symbol:string}} s */
  const choose = (s) => { state.symbolValue = s.symbol; state.symbolBroker = s.broker; input.value = s.symbol; close(); syncToggle(); };
  /** @param {string} [filter] */
  const fill = (filter) => {
    menu.innerHTML = '';
    ...
    if (!matches.length) {
      const empty = document.createElement('div'); empty.className = 'ot-combo-empty';
      empty.textContent = watchSymbols.length ? t('No match') : t('No symbols in watchlists');
      menu.appendChild(empty); return;
    }
    for (const s of matches) {
      const opt = document.createElement('div'); opt.className = 'ot-combo-opt';
      const sym = document.createElement('span'); sym.className = 'ot-combo-sym'; sym.textContent = s.symbol; opt.appendChild(sym);
      if (s.broker) { const br = document.createElement('span'); br.className = 'ot-combo-br'; br.textContent = s.broker.toUpperCase(); opt.appendChild(br); }
      opt.onmousedown = (e) => { e.preventDefault(); choose(s); };   // mousedown beats the input's blur
  ...
  arrow.onclick = async () => {
    ...
    fill(''); menu.style.display = 'block'; input.focus();
    watchSymbols = await loadWatchSymbols();   // refresh so newly-added watchlist symbols show
    if (isOpen()) fill('');
  };
  input.oninput = () => { state.symbolValue = input.value; state.symbolBroker = ''; fill(input.value); menu.style.display = 'block'; };
  input.onchange = () => { state.symbolValue = input.value; syncToggle(); };
  return combo;
}
