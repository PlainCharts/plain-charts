...
export function registerPaneCommands() {
  registerCommand({
    id: 'study.add', title: 'Add study', category: 'Studies',
    handler: (args) => {
      const p = paneOf(args); if (!p) return { error: 'no pane' };
  ...
  registerCommand({
    id: 'study.remove', title: 'Remove study', category: 'Studies',
    handler: (args) => {
      const p = paneOf(args); if (!p) return { error: 'no pane' };
  ...
  registerCommand({
    id: 'study.clearAll', title: 'Remove all studies', category: 'Studies',
    handler: (args) => {
      const p = paneOf(args); if (!p) return { error: 'no pane' };
      p.studies.clearAll();
      return { ok: true };
  ...
  registerCommand({
    id: 'symbol.set', title: 'Set symbol', category: 'Chart',
    handler: (args) => {
      const p = paneOf(args); if (!p) return { error: 'no pane' };
  ...
  registerCommand({
    id: 'tf.set', title: 'Set timeframe', category: 'Chart',
    handler: (args) => {
      const p = paneOf(args); if (!p) return { error: 'no pane' };
  ...
  registerCommand({
    id: 'alert.add', title: 'Add price alert', category: 'Alerts',
    handler: (args) => {
      const p = paneOf(args); if (!p || !p.drawings) return { error: 'no pane' };
  ...
  registerCommand({
    id: 'drawing.add', title: 'Add drawing', category: 'Drawings',
    handler: (args) => {
      const p = paneOf(args); if (!p || !p.drawings) return { error: 'no pane' };
      if (!args || !args.tool || !Array.isArray(args.points) || !args.points.length) return { error: 'tool and points required' };
  ...
  registerCommand({
    id: 'drawing.remove', title: 'Remove drawing', category: 'Drawings',
    handler: (args) => {
      const p = paneOf(args); if (!p || !p.drawings) return { error: 'no pane' };
  ...
  registerCommand({
    id: 'drawing.removeInView', title: 'Remove drawings in view', category: 'Drawings',
    handler: (args) => {
      const p = paneOf(args); if (!p || !p.drawings) return { error: 'no pane' };
  ...
  registerCommand({
    id: 'drawing.list', title: 'List drawings', category: 'Drawings',
    handler: (args) => {
      const p = paneOf(args); if (!p) return { error: 'no pane' };
  ...
  registerCommand({
    id: 'selection.get', title: 'Get chart selection', category: 'Chart',
    handler: (args) => {
      const p = paneOf(args); if (!p) return { error: 'no pane' };
      ...
      const selectedDrawings = ids.map((/** @type {any} */ id) => (dr.get ? dr.get(id) : null)).filter(Boolean)
        .map((/** @type {any} */ d) => ({ id: d.id, tool: d.tool, points: d.points, style: d.style }));
      const lb = p.lastBar;
      return {
        symbol: p.symbol, tf: p.tfId, broker: p.broker,
        visibleRange: p.range || null,
        lastBar: lb ? { time: lb.time, open: lb.open, high: lb.high, low: lb.low, close: lb.close, volume: lb.volume } : null,
        selectedDrawings,
      };
    },
  });
