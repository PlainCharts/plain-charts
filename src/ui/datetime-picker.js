...
const SVG_CAL = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="12" height="11" rx="1.5"/><path d="M2 6h12M5 2v2M11 2v2"/></svg>';
const SVG_CLOCK = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6"/><path d="M8 4.6V8l2.4 1.4"/></svg>';
...
export function formatDateTime(ms, h24 = true) {
  ...
  else { const h = d.getHours(); time = (h % 12 || 12) + ':' + pad2(d.getMinutes()) + ' ' + (h < 12 ? 'AM' : 'PM'); }
...
export function openDateTimePicker(opts) {
  ...
  const cancel = () => { closeDateTimePicker(); if (opts.onCancel) opts.onCancel(); };
  overlay.onclick = (e) => { if (e.target === overlay) cancel(); };
  const dlg = el('div', 'dialog dtp-dlg');

  // ---- header: ‹ back + title
  const head = el('div', 'dtp-head');
  const back = el('span', 'dtp-back', '‹'); back.onclick = cancel;
  ...
  const dateIn = /** @type {HTMLInputElement} */ (el('input', 'dtp-in')); dateIn.type = 'text'; dateIn.spellcheck = false;
  const dateField = el('div', 'dtp-field'); dateField.append(dateIn, (() => { const s = el('span', 'dtp-ico'); s.innerHTML = SVG_CAL; return s; })());
  const timeWidget = segTime(h24, (p) => { time.h = p.h; time.mi = p.mi; time.s = 0; }, { seconds: false });
  const timeField = el('div', 'dtp-field dtp-time'); timeField.append(timeWidget.el, (() => { const s = el('span', 'dtp-ico'); s.innerHTML = SVG_CLOCK; return s; })());
  io.append(dateField);
  ...
  setBtn.onclick = () => { const ms = new Date(sel.y, sel.mo, sel.d, time.h, time.mi, 0, 0).getTime(); closeDateTimePicker(); opts.onSet(ms); };
  foot.appendChild(setBtn);
  dlg.appendChild(foot);

  const syncDateField = () => { dateIn.value = fmtDateField(sel); setBtn.textContent = t('Set') + ' ' + fmtDateField(sel); };
  ...
  const renderGrid = () => {
    ...
    for (let day = 1; day <= n; day++) {
      ...
      c.onclick = () => { sel.y = view.y; sel.mo = view.mo; sel.d = day; syncDateField(); renderGrid(); };
      grid.appendChild(c);
    }
  };

  /** @param {number} delta */
  const stepMonth = (delta) => { const m = view.mo + delta; view.y += Math.floor(m / 12); view.mo = ((m % 12) + 12) % 12; renderGrid(); };
  ...
  dateIn.onchange = () => {
    ...
    if (!p) { syncDateField(); return; }
    sel.y = p.y; sel.mo = p.mo; sel.d = Math.min(p.d, daysInMonth(p.y, p.mo));
    view.y = sel.y; view.mo = sel.mo;
    syncDateField(); renderGrid();
  };
  dateIn.addEventListener('keydown', (e) => { if (e.key === 'Enter') dateIn.blur(); });
