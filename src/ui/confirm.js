...
export function confirmDialog({ title = 'Confirmation', message = '', yes = 'Yes', no = 'No', signal } = {}) {
  return new Promise((resolve) => {
    const overlay = el('div', 'modal open'); overlay.style.zIndex = '110';
    const dlg = el('div', 'dialog confirm-dlg');
    const x = el('span', 'lib-x', '✕');
    const head = el('div', 'set-head'); head.append(el('span', 'set-title', t(title)), x);
    ...
    let settled = false;
    /** @param {boolean} v */
    const done = (v) => { if (settled) return; settled = true; if (signal) signal.removeEventListener('abort', onAbort); overlay.remove(); resolve(v); };
    const onAbort = () => done(false);
    x.onclick = () => done(false);
    noBtn.onclick = () => done(false);
    yesBtn.onclick = () => done(true);
    overlay.onclick = (e) => { if (e.target === overlay) done(false); };
    if (signal) { if (signal.aborted) return done(false); signal.addEventListener('abort', onAbort); }
