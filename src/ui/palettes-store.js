...
const chunk = (arr, n) => { /** @type {string[][]} */ const out = []; for (let i = 0; i < arr.length; i += n) out.push(arr.slice(i, i + n)); return out; };
...
const seed = () => ({
  palettes: [
    { id: 'p_standard', name: 'Standard', rows: [
...
const normPalette = (p) => ({
  id: p.id,
  name: p.name || 'Palette',
  rows: Array.isArray(p.rows) ? p.rows.filter(Array.isArray).map((/** @type {string[]} */ r) => r.slice())
...
export async function loadPalettes() {
  ...
  if (!data.palettes.length) { data = seed(); save(); }   // never leave the user with zero palettes
  return data;
}
const save = () => { store.set('palettes', data.palettes); store.set('activeId', data.activeId); store.set('favorites', data.favorites); };
...
export const paletteRows = (id) => { const p = getPalette(id); return p ? p.rows : []; };
/** @param {string|null} id @returns {string[]} */
export const paletteFlat = (id) => { const p = getPalette(id); return p ? p.rows.flat() : []; };
/** @param {string|null} id @returns {void} */
export function setActivePalette(id) { if (getPalette(id)) { data.activeId = id; save(); } }
...
export function createPalette(name) {
  ...
  data.palettes.push(p); data.activeId = id; save();
  return p;
}
/** @param {string} id @param {string} name @returns {void} */
export function renamePalette(id, name) { const p = getPalette(id); if (p && name) { p.name = name; save(); } }
...
export function addColorToPalette(id, color, rowIndex) {
  const p = getPalette(id); if (!p || !color) return -1;
  ...
  let row = p.rows[idx];
  if (!row || row.length >= ROW_MAX) { row = []; p.rows.push(row); idx = p.rows.length - 1; }
  row.push(color); save();
  return idx;
}
...
export function addRowToPalette(id) {
  const p = getPalette(id); if (!p) return;
...
export function toggleFavorite(hex) {
  ...
  if (i >= 0) data.favorites.splice(i, 1); else data.favorites.push(hex);
  save();
}
