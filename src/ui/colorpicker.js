...
export function openColorPicker(anchor, value, onChange, extras) {
  ...
  pop._out = (/** @type {PointerEvent} */ e) => { if (pop && !pop.contains(e.target) && e.target !== anchor && !anchor.contains(/** @type {Node} */ (e.target)) && !creatorContains(e.target)) closeColorPicker(); };
...
function sectionThickness(field) {
  ...
  const wrap = el('div', 'cp-thick'); let wv = field.value || 1;
  [1, 2, 3, 4].forEach((w) => {
    ...
    b.onclick = () => { wv = w; field.onChange(w); wrap.querySelectorAll('.cp-thbtn').forEach((x) => x.classList.remove('sel')); b.classList.add('sel'); };
    wrap.appendChild(b);
  });
...
function sectionLineStyle(field) {
  ...
  const wrap = el('div', 'cp-lsrow'); let lv = /** @type {string} */ (normLS(field.value));
  ['solid', 'dashed', 'dotted'].forEach((sName) => {
    ...
    b.onclick = () => { lv = sName; field.onChange(sName); wrap.querySelectorAll('.cp-lsbtn').forEach((x) => x.classList.remove('sel')); b.classList.add('sel'); };
    wrap.appendChild(b);
  });
  sec.appendChild(wrap); return sec;
}
...
function sectionTextSize(field) {
  ...
  sec.appendChild(sel); return sec;
}
...
function sectionTextStyle(bold, italic) {
  ...
  if (bold) {
    ...
    b.onclick = () => { bv = !bv; bold.onChange(bv); b.classList.toggle('sel', bv); };
    wrap.appendChild(b);
  }
  if (italic) {
    let iv = !!italic.value;
    const i = /** @type {HTMLButtonElement} */ (el('button', 'cp-tsbtn' + (iv ? ' sel' : ''))); i.type = 'button'; i.textContent = 'I'; i.style.fontStyle = 'italic';
    i.onclick = () => { iv = !iv; italic.onChange(iv); i.classList.toggle('sel', iv); };
    wrap.appendChild(i);
  }
  sec.appendChild(wrap); return sec;
}
