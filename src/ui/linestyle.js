...
export function lineStyleControl(value, onChange) {
  const btn = document.createElement('button'); btn.type = 'button'; btn.className = 'ls-btn';
  const prev = document.createElement('span'); prev.className = 'ls-prev ' + norm(value);
  const caret = document.createElement('span'); caret.className = 'ls-caret'; caret.textContent = '▾';
  btn.append(prev, caret);
  btn.onclick = () => openMenu(btn, norm(prev.dataset.k || value), (k) => { prev.className = 'ls-prev ' + k; prev.dataset.k = k; onChange(k); });
...
function openMenu(anchor, cur, onPick) {
  closeLineStyleMenu();
  menu = document.createElement('div'); menu.className = 'ls-menu';
  STYLES.forEach((s) => {
    const row = document.createElement('div'); row.className = 'ls-row' + (s.key === cur ? ' sel' : '');
    const p = document.createElement('span'); p.className = 'ls-prev ' + s.key;
    const n = document.createElement('span'); n.className = 'ls-name'; n.textContent = s.name;
    row.append(p, n);
    row.onclick = () => { onPick(s.key); closeLineStyleMenu(); };
  ...
  /** @type {any} */ (menu)._out = (/** @type {PointerEvent} */ e) => { if (menu && !menu.contains(/** @type {Node} */ (e.target)) && e.target !== anchor && !anchor.contains(/** @type {Node} */ (e.target))) closeLineStyleMenu(); };
