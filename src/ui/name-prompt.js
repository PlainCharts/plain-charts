...
let overlay = null;
export function closeNamePrompt() { if (overlay) { overlay.remove(); overlay = null; } }
...
export function namePrompt({
  title = 'Save', label = 'Name', placeholder = 'Name', value = '',
  existing = [], save = 'Save', replaceMessage,
} = {}) {
  closeNamePrompt();
  return new Promise((resolve) => {
    ...
    overlay = el('div', 'modal open'); overlay.style.zIndex = '100';
    ...
    const inp = /** @type {HTMLInputElement} */ (el('input', 'tmpl-save-input')); inp.type = 'text'; inp.placeholder = t(placeholder); inp.value = value;
    ...
    const done = (/** @type {string|null} */ v) => { closeNamePrompt(); resolve(v); };
    const submit = async () => {
      const name = inp.value.trim();
      if (!name) { inp.focus(); return; }
      if (existing.some((n) => n === name)) {   // overwrite → confirm first
        const ok = await confirmDialog({ message: replaceMessage ? replaceMessage(name) : `'${name}' ` + t('already exists. Replace it?') });
    ...
    const renderSuggest = () => {
      ...
      const names = !q ? [] : existing.filter((n) => { const nl = n.toLowerCase(); return nl.startsWith(q) && nl !== q; });
      suggest.innerHTML = '';
      names.forEach((n) => {
        const r = el('div', 'tmpl-suggest-row', n);
        r.onmousedown = (e) => { e.preventDefault(); inp.value = n; renderSuggest(); inp.focus(); };
    ...
    inp.onkeydown = (e) => { if (e.key === 'Enter') { e.preventDefault(); submit(); } else if (e.key === 'Escape') done(null); };
    renderSuggest();
