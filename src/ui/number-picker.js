...
/** @param {MouseEvent} e */
function onDoc(e) {
  if (pop && !pop.contains(/** @type {any} */ (e.target))) closeNumberPicker();
}
...
export function openNumberPicker(anchor, value, onChange, opts = {}) {
  ...
  pop.style.cssText =
    'position:fixed;z-index:96;display:flex;flex-direction:column;gap:6px;padding:6px;' +
  ...
  const btn = (txt) => {
    ...
    b.textContent = txt;
    b.style.cssText =
      'width:26px;height:26px;border:1px solid var(--bd,#333);background:var(--bg,#14161a);color:var(--tx,#ddd);border-radius:5px;cursor:pointer;font-size:16px;line-height:1;';
    return b;
  };
  const minus = btn('−'),
    plus = btn('+');
  ...
  input.style.cssText =
    'width:64px;height:26px;text-align:center;border:1px solid var(--bd,#333);background:var(--bg,#14161a);color:var(--tx,#ddd);border-radius:5px;font-size:13px;box-sizing:border-box;';
  ...
  input.onchange = () => {
    const nx = parseFloat(input.value);
    commit(isFinite(nx) ? nx : val);
  };
  input.onkeydown = (e) => {
    if (e.key === 'Enter') {
      const nx = parseFloat(input.value);
  ...
  if (presets.length) {
    ...
    const chip =
      'flex:1;min-width:28px;height:24px;border:1px solid var(--bd,#333);background:var(--bg,#14161a);color:var(--tx,#ddd);border-radius:5px;cursor:pointer;font-size:12px;line-height:1;';
    presets.forEach((pv) => {
      const b = document.createElement('button');
      b.type = 'button';
      b.textContent = String(pv);
    ...
    reset.textContent = '↺';
    reset.title = 'Reset';
    reset.style.cssText = chip + 'font-size:14px;';
  ...
  if (confirmLabel) {
    ...
    ok.style.cssText =
      'height:26px;border:1px solid var(--bd,#333);background:var(--active,#2a2d34);color:var(--tx,#ddd);border-radius:5px;cursor:pointer;font-size:12px;line-height:1;width:100%;';
