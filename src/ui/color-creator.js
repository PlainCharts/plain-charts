...
export function openCustomCreator(anchor, initialHex, handlers) {
  ...
  svDot.className = 'cc-dot';
  sv.appendChild(svDot);
  const hue = document.createElement('div');
  hue.className = 'cc-hue';
  const hueDot = document.createElement('div');
  hueDot.className = 'cc-huedot';
  ...
  hexI.className = 'cc-hex';
  hexI.spellcheck = false;
  const pasteBtn = document.createElement('button');
  pasteBtn.type = 'button';
  pasteBtn.className = 'cc-paste';
  pasteBtn.textContent = 'Paste';
  const pickBtn = document.createElement('button');
  pickBtn.type = 'button';
  pickBtn.className = 'cc-pick';
  ...
  const applyHexInput = () => {
    const nv = normHex(hexI.value);
    if (nv) [h, s, v] = hexToHsv(nv);
    sync();
  };
  ...
  creator._out = (/** @type {PointerEvent} */ e) => {
    if (creator && !creator.contains(e.target) && e.target !== anchor) closeCreator();
  };
