...
let pop = null;
/** @param {MouseEvent} e */
function onDoc(e) { if (pop && !pop.contains(/** @type {any} */ (e.target))) closeDropdown(); }
...
export function openDropdown(anchor, items, onSelect, opts = {}) {
  ...
  menu.style.cssText = 'position:fixed;z-index:96;min-width:110px;padding:4px;'
    + 'background:var(--panel,#1b1d22);border:1px solid var(--bd,#333);border-radius:8px;box-shadow:0 8px 30px rgba(0,0,0,.4);';
  items.forEach((it) => {
    const b = document.createElement('button'); b.type = 'button'; b.textContent = it.text;
    const sel = it.value === opts.selected;
    b.style.cssText = 'display:block;width:100%;text-align:left;padding:7px 12px;border:none;border-radius:5px;font-size:13px;cursor:pointer;'
      + 'color:var(--tx,#ddd);background:' + (sel ? 'var(--bg,#14161a)' : 'transparent') + ';';
    b.onmouseenter = () => { if (!sel) b.style.background = 'rgba(255,255,255,.08)'; };
    b.onmouseleave = () => { if (!sel) b.style.background = 'transparent'; };
    b.onclick = (e) => { e.stopPropagation(); e.preventDefault(); closeDropdown(); try { onSelect(it.value); } catch (_) {} };
    menu.appendChild(b);
  });
  document.body.appendChild(menu);

  const r = anchor.getBoundingClientRect(), pr = menu.getBoundingClientRect();
