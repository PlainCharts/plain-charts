...
export function buildPalettePanel(body, host) {
  ...
  const refreshSel = () => grid.querySelectorAll('.cp-sw').forEach((/** @type {any} */ b) => b.classList.toggle('sel', /** @type {boolean} */ (b.dataset.c && sameHex(b.dataset.c, host.currentHex()))));
  ...
  const openCreator = (btn) => openCustomCreator(btn, host.currentHex(), {
