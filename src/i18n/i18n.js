...
export function registerVocab(words) {
  if (words) Object.assign(moduleWords, words);
}
...
/** @param {Record<string, string> | null | undefined} words */
export function setAddonWords(words) {
  addonWords = words || {};
}
...
export function localizeDom(root) {
  ...
  r.querySelectorAll('[data-i18n-ph]').forEach((el) => {
    /** @type {HTMLInputElement} */ (el).placeholder = t(el.getAttribute('data-i18n-ph') || '');
  });
}
...
export function openVocabFolder() {
  postJSON('/api/vocab/open', {});
}
