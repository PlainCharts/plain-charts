...
function start() {
  ...
  if (!ROOT) {
    console.error('[addon-runtime] no root');
    return;
  }
  ...
  function writeStatus() {
    ...
    const s = {};
    running.forEach((r, id) => {
      s[id] = { running: !r.error, error: r.error, logs: r.logs.slice(-200), inputs: r.inputs, config: r.config };
    });
    try {
  ...
  function resolveConfig(dir, inputs) {
    ...
    const cfg = {};
    (inputs || []).forEach((i) => {
      cfg[i.key] = saved && i.key in saved ? saved[i.key] : i.default;
    });
      ...
      log: (...a) => {
        const line =
          ...
          a
            ...
            .join(' ');
        rec.logs.push(line);
        if (rec.logs.length > 300) rec.logs.shift();
        console.log('[addon:' + id + ']', ...a);
        writeStatus();
      },
    };
  }
  ...
  function startAddon(id, rev) {
    ...
    try {
      ...
      rec.error = (e && e.stack) || String(e);
      rec.logs.push('(runtime) start failed: ' + ((e && e.message) || e));
    }
    writeStatus();
  }
  /** @param {string} id */
  function stopAddon(id) {
    ...
    if (!rec) return;
    try {
      const s = rec.mod && rec.mod.stop;
      if (typeof s === 'function') s.call(rec.mod);
    } catch (e) {
      console.error('[addon:' + id + '] stop error', e);
    }
    running.delete(id);
    writeStatus();
  }
