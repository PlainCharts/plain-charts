...
function snapshot(pane) {
  const e = pane && pane.drawings; if (!e) return '{}';
  const drawings = e.objects().map((/** @type {any} */ d) => { const o = /** @type {Record<string, any>} */ ({ id: d.id }); FIELDS.forEach((k) => { if (d[k] !== undefined) o[k] = d[k]; }); return o; });
...
function restore(pane, snapJSON) {
  const e = pane && pane.drawings; if (!e) return;
  /** @type {any} */
  let data; try { data = JSON.parse(snapJSON); } catch (_) { return; }
  restoring = true;
  try {
    e.clear();
    ...
    (data.drawings || []).forEach((/** @type {any} */ d) => {
      ...
      const params = {}; FIELDS.forEach((k) => { if (k !== 'tool' && d[k] !== undefined) params[k] = d[k]; });
      const nd = e.add(d.tool, params);
      if (nd && d.id) idMap[d.id] = nd.id;
    });
    /** @param {any[]} nodes @returns {any[]} */
    const remap = (nodes) => (nodes || []).map((/** @type {any} */ n) => {
      if (n.type === 'folder') return { type: 'folder', id: n.id, name: n.name, expanded: n.expanded !== false, children: remap(n.children) };
      const nid = idMap[n.id]; return nid ? { type: 'drawing', id: nid } : null;
    }).filter(Boolean);
    const tree = e.getTree(); tree.length = 0; remap(data.tree).forEach((/** @type {any} */ n) => tree.push(n));
    e.saveTree();
...
function ensure(pane) {
  ...
  if (!h) { h = { stack: [snapshot(pane)], idx: 0 }; histories.set(pane, h); }
  return h;
}
...
export function undo() {
  const p = getActivePane(); const h = p && histories.get(p);
  if (!h || h.idx <= 0) return;
  h.idx -= 1; restore(p, h.stack[h.idx]); updateButtons();
}
...
export function initUndo() {
  ...
  bus.on('pane:active', () => { const p = getActivePane(); if (p) ensure(p); updateButtons(); });
  ...
  const p = getActivePane(); if (p) ensure(p);   // seed the active chart's baseline
  updateButtons();
}
