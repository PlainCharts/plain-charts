...
/** @returns {any} */
const activeEngine = () => { const p = getActivePane(); return p && p.drawings; };

/** @returns {boolean} */
function copy() {
  const e = activeEngine(); if (!e) return false;
...
/** @returns {boolean} */
function paste() {
  const e = activeEngine(); const clip = getClip();
...
/** @returns {boolean} */
export function hasClipboard() { return hasClip(); }
...
export function selectAll() {
  const e = activeEngine(); if (!e) return false;
  const ids = e.objects()
    .filter((/** @type {any} */ d) => { const t = getTool(d.tool); return t && t.kind === 'draw' && !d.hidden; })
    .map((/** @type {any} */ d) => d.id);
  if (!ids.length) return false;
  if (ids.length === 1) e.select(ids[0]); else e.setSelection(ids);
  return true;
}
...
export function initClipboard() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (k === 'c' && !e.shiftKey) { if (copy()) e.preventDefault(); }
    else if (k === 'v' && !e.shiftKey) { if (paste()) e.preventDefault(); }
    ...
    else if (k === 'a' && !e.shiftKey) { selectAll(); e.preventDefault(); }
  });
}
