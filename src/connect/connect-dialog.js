...
export function initConnectDialog() {
  ...
  const box = /** @type {HTMLElement | null} */ (dlg.querySelector('.dialog')),
    head = /** @type {HTMLElement | null} */ (dlg.querySelector('.conn-head'));
  ...
  const setDlg = (m, err) => {
    dlgStatus.textContent = m || '';
    dlgStatus.className = 'dlg-status' + (err ? ' err' : '');
  };
  ...
  const refreshSaved = () => {
    ...
    accounts.listAccounts().forEach((a) => {
      ...
      o.value = a.name;
      o.textContent = a.name + '  (' + (a.server || a.protocol) + ')';
      sel.appendChild(o);
    });
  };
  ...
  const renderConnList = () => {
    ...
    if (!accts.length) {
      ...
      e.textContent = t('No saved accounts yet — create one below.');
    ...
    accts.forEach((a) => {
      ...
      dot.className = 'conn-dot' + (connected ? ' on' : '');
      const name = document.createElement('div');
      name.className = 'conn-name';
      const nm = document.createElement('span');
      nm.textContent = a.name;
      const pr = document.createElement('span');
      pr.className = 'conn-proto';
      pr.textContent = a.protocol; // just the broker; server (Demo/Practice) is an account detail, not shown here
      name.append(nm, pr);
      ...
      btn.className = 'conn-btn' + (connected ? ' disc' : '');
  ...
  bus.on('connections:changed', () => {
    if (dlg.classList.contains('open')) renderConnList();
  });
  ...
  bus.on('broker:notice', (n) => {
    if (n && n.message) setDlg(n.message, !!n.error);
  });
  ...
  const openModal = () => {
    ...
    if (last) {
      /** @type {HTMLSelectElement} */ ($('f-saved')).value = last.name;
      form.fillForm(last);
    } else form.fillForm({});
    setDlg('');
    renderConnList();
  ...
  const bFolder = $('btnAdaptersFolder');
  if (bFolder) bFolder.onclick = () => fetch('/api/adapters/open', { method: 'POST' }).catch(() => {});
  ...
  /** @type {HTMLElement} */ ($('f-saved')).onchange = () => {
    const a = accounts.listAccounts().find((x) => x.name === /** @type {HTMLSelectElement} */ ($('f-saved')).value);
    form.fillForm(a || { name: '' });
  };
  ...
  /** @type {HTMLElement} */ ($('btnSave')).onclick = async () => {
    ...
    if (missing.length)
      return setDlg(t('Enter') + ' ' + missing.map((f) => t(f.label).toLowerCase()).join(', ') + '.', true);
    ...
    accounts.upsertAccount(a);
    accounts.setLastUsed(a.name);
    await accounts.saveAccounts();
    refreshSaved();
    /** @type {HTMLSelectElement} */ ($('f-saved')).value = a.name;
    renderConnList();
  ...
  /** @type {HTMLElement} */ ($('btnDelete')).onclick = async () => {
    ...
    await accounts.saveAccounts();
    refreshSaved();
    /** @type {HTMLSelectElement} */ ($('f-saved')).value = '';
    form.fillForm({});
    renderConnList();
