...
function makeRange({ id, name, glyph, price, time, description, icon }) {
  ...
  return {
    id, name, glyph, description, icon,
    kind: 'draw',
    points: 2,
    defaultStyle: { color: '#787b86', width: 1, lineStyle: 'solid', fillOn: true, fill: 'rgba(41,98,255,0.12)', stats: statsDefault, labelColor: '#ffffff', labelSize: 12, labelBold: false, labelItalic: false, labelBgOn: false, labelBg: '#1e222d', measureMethod: 'standard' },
    settings: {
      style: [
        { name: 'Measure method', controls: [{ key: 'measureMethod', type: 'select', options: [{ key: 'standard', name: 'Standard (label + arrow)' }, { key: 'conventional', name: 'Conventional (centred value)' }] }] },
        ...
        { fields: [
          { name: 'Label', key: 'labelColor', type: 'color', size: 'labelSize', bold: 'labelBold', italic: 'labelItalic' },
    ...
    marks(d, view) {
      ...
      const P1 = P[0], P2 = P[1];
      const x1 = view.timeToX(P1.time), y1 = view.priceToY(P1.price);
      const x2 = view.timeToX(P2.time), y2 = view.priceToY(P2.price);
      if (x1 == null || y1 == null || x2 == null || y2 == null) return [];
      const xm = (x1 + x2) / 2, ym = (y1 + y2) / 2, color = s.color || '#787b86', bw = s.width || 1;
      ...
      if (s.fillOn !== false) {
        ...
        out.push({ closed: true, fill: s.fill || 'rgba(41,98,255,0.12)', path: [sv(xa, ya), sv(xb, ya), sv(xb, yb), sv(xa, yb)] });
      }
      ...
      if (time) { out.push(bline(sv(x1, y1), sv(x1, y2))); out.push(bline(sv(x2, y1), sv(x2, y2))); }
      if (price) { out.push(bline(sv(x1, y1), sv(x2, y1))); out.push(bline(sv(x1, y2), sv(x2, y2))); }
      ...
      const dP = P2.price - P1.price, pct = P1.price ? (P2.price / P1.price - 1) * 100 : 0, pips = dP * Math.pow(10, Math.max(0, dec - 1));
      ...
      const priceSeg = [has('price') && dP.toFixed(dec), has('percent') && `(${pct.toFixed(2)}%)`, has('pips') && `${pips.toFixed(1)} pip`].filter(Boolean).join(' ');
      ...
      if (s.measureMethod === 'conventional') {
        ...
        const aPriceSeg = [has('price') && Math.abs(dP).toFixed(dec), has('percent') && `(${Math.abs(pct).toFixed(2)}%)`, has('pips') && `${Math.abs(pips).toFixed(1)} pip`].filter(Boolean).join(' ');
        const aTimeSeg = [has('bars') && `${Math.abs(bars)} bars`, has('time') && dur(Math.abs(P2.time - P1.time))].filter(Boolean).join(', ');
        const valStr = [aPriceSeg, aTimeSeg].filter(Boolean).join('   ');
        const mc = measureCtx(); mc.font = (s.labelItalic ? 'italic ' : '') + (s.labelBold ? 'bold ' : '') + fsize + 'px sans-serif';
        const tw = valStr ? mc.measureText(valStr).width : 0, th = fsize;
        ...
        const valMark = (cx, cy) => {
          if (!valStr) return;
          const hw = (rot ? th : tw) / 2 + 4, hh = (rot ? tw : th) / 2 + 3;   // (rotated) text footprint half-extents
          if (s.labelBgOn) out.push({ closed: true, fill: s.labelBg || '#1e222d', path: [sv(cx - hw, cy - hh), sv(cx + hw, cy - hh), sv(cx + hw, cy + hh), sv(cx - hw, cy + hh)] });
          out.push({ text: valStr, at: sv(cx, cy), align: 'center', baseline: 'middle', color: s.labelColor || '#ffffff', size: fsize, bold: !!s.labelBold, italic: !!s.labelItalic, rotate: rot || undefined });
        };
        ...
        if (time) {   // horizontal dimension line at y2; value breaks it (centred on the span)
          const lo = Math.min(x1, x2), hi = Math.max(x1, x2), g0 = xm - tw / 2 - G, g1 = xm + tw / 2 + G;
          if (valStr && g1 > lo && g0 < hi) { if (g0 > lo) out.push(conn(sv(lo, y2), sv(g0, y2))); if (g1 < hi) out.push(conn(sv(g1, y2), sv(hi, y2))); }
          else out.push(conn(sv(lo, y2), sv(hi, y2)));
          arrow([sv(lo + 7, y2 - 4), sv(lo, y2), sv(lo + 7, y2 + 4)]);
          arrow([sv(hi - 7, y2 - 4), sv(hi, y2), sv(hi - 7, y2 + 4)]);
        }
        if (price) {   // vertical dimension line at x2; when this measures price-only, the value breaks it
          const ext = rot ? tw : th, carries = !time;   // value's extent along the line; price-only carries it
          const lo = Math.min(y1, y2), hi = Math.max(y1, y2), g0 = ym - ext / 2 - G, g1 = ym + ext / 2 + G;
          if (carries && valStr && g1 > lo && g0 < hi) { if (g0 > lo) out.push(conn(sv(x2, lo), sv(x2, g0))); if (g1 < hi) out.push(conn(sv(x2, g1), sv(x2, hi))); }
      ...
      let trect = null;
      if (d.text) {
        const ts = d.textStyle || {}, tsize = ts.size || 14, tpad = 5, tha = ts.hAlign || 'center', tva = ts.vAlign || 'middle';
        const tl = String(d.text).split('\n'), tlh = tsize * 1.25;
        const mc = measureCtx(); mc.font = (ts.italic ? 'italic ' : '') + (ts.bold ? 'bold ' : '') + tsize + 'px sans-serif';
        const tw = Math.max(...tl.map((l) => mc.measureText(l).width)), tH = tsize + (tl.length - 1) * tlh;
        const bx1 = Math.min(x1, x2), bx2 = Math.max(x1, x2), by1 = Math.min(y1, y2), by2 = Math.max(y1, y2);
      ...
      if (time) {
        ...
        if (gap && gap[1] > lo && gap[0] < hi) { if (gap[0] > lo) out.push(conn(sv(lo, ym), sv(gap[0], ym))); if (gap[1] < hi) out.push(conn(sv(gap[1], ym), sv(hi, ym))); }
      ...
      if (price) {
        ...
        if (gap && gap[1] > lo && gap[0] < hi) { if (gap[0] > lo) out.push(conn(sv(xm, lo), sv(xm, gap[0]))); if (gap[1] < hi) out.push(conn(sv(xm, gap[1]), sv(xm, hi))); }
      ...
      if (lines.length) {
        const mc = measureCtx(); mc.font = (s.labelItalic ? 'italic ' : '') + (s.labelBold ? 'bold ' : '') + fsize + 'px sans-serif';
        const padX = 8, lh = Math.round(fsize * 1.33), w = Math.max(40, ...lines.map((t) => mc.measureText(t).width + padX * 2)), h = lh * lines.length + 8;
        const ly = (y2 >= y1) ? y2 + 8 : y2 - 8 - h;
        if (s.labelBgOn) out.push({ closed: true, fill: s.labelBg || '#1e222d', path: [sv(xm - w / 2, ly), sv(xm + w / 2, ly), sv(xm + w / 2, ly + h), sv(xm - w / 2, ly + h)] });
        lines.forEach((t, i) => out.push({ text: t, at: sv(xm, ly + 4 + lh / 2 + i * lh), align: 'center', baseline: 'middle', color: s.labelColor || '#ffffff', size: fsize, bold: !!s.labelBold, italic: !!s.labelItalic }));
      }
      return out;
    },
...
function fracIndex(times, t) {
  ...
  if (n < 2) return 0;
  if (t <= times[0]) { const s = times[1] - times[0]; return s > 0 ? (t - times[0]) / s : 0; }
  if (t >= times[n - 1]) { const s = times[n - 1] - times[n - 2]; return (n - 1) + (s > 0 ? (t - times[n - 1]) / s : 0); }
  let lo = 0, hi = n - 1;
...
function dur(sec) {
  ...
  /** @type {[string, number][]} */
  const units = [['Y', 31536000], ['M', 2592000], ['W', 604800], ['D', 86400], ['h', 3600], ['m', 60], ['s', 1]];
  /** @type {string[]} */
  const parts = [];
  for (const [label, size] of units) {
    const v = Math.floor(abs / size);
    if (v > 0) { parts.push(v + label); abs -= v * size; }
