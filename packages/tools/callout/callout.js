...
Tools.register({
  ...
  defaultStyle: {
    lineColor: '#787b86', lineWidth: 1, lineStyle: 'solid',
  ...
  marks(d, view) {
    ...
    const pts = /** @type {ToolScreenPoint[]} */ (P.map((p) => ({ x: view.timeToX(p.time), y: view.priceToY(p.price) })));
    if (pts.some((p) => p.x == null || p.y == null)) return [];
    const box = boxFor(measureCtx(), pts, d);
    if (!box) return [];
    /** @type {any[]} */
    const out = [];
    const tip = pts[0], mid = nearestSideMid(box, tip), s = d.style || {};
    /** @param {number} x @param {number} y */
    const sv = (x, y) => ({ vpx: 0, dx: x, vp: 0, dy: y });
    out.push({ path: [sv(tip.x, tip.y), sv(mid.x, mid.y)], stroke: s.lineColor || '#787b86', width: s.lineWidth || 1, dash: Tools.dash(s.lineStyle) });
    if (s.bgOn !== false && s.bg) out.push({ closed: true, fill: s.bg, path: [sv(box.x, box.y), sv(box.x + box.w, box.y), sv(box.x + box.w, box.y + box.h), sv(box.x, box.y + box.h)] });
    if (s.borderOn !== false) out.push({ closed: true, stroke: s.border || '#2962ff', width: s.borderWidth || 1,
      path: [sv(box.x + 0.5, box.y + 0.5), sv(box.x + box.w - 0.5, box.y + 0.5), sv(box.x + box.w - 0.5, box.y + box.h - 0.5), sv(box.x + 0.5, box.y + box.h - 0.5)] });
    return out;
  },
  /** @param {CanvasRenderingContext2D} c @param {ToolScreenPoint[]} pts @param {ToolDrawing} d */
  drawText(c, pts, d) { const b = boxFor(c, pts, d); if (b) drawBoxText(c, b, d.textStyle); },
  /** @param {CanvasRenderingContext2D} c @param {ToolScreenPoint[]} pts @param {ToolDrawing} d */
  textGeom(c, pts, d) { const b = boxFor(c, pts, d); return b ? boxTextGeom(b, d.style, b.wrap) : null; },

  /** @param {ToolScreenPoint[]} pts @param {ToolDrawing} [d] */
  handles(pts, d) { const b = d && boxCache.get(d); return pts.length >= 2 ? PINS.handles({ pts, box: b }) : pts; },
...
/** @param {ToolDrawing} d */
const keep3 = (d) => (/** @type {ToolDataPoint[]} */ (d.points)[2] ? [{ .../** @type {ToolDataPoint[]} */ (d.points)[2] }] : []);
const PINS = pinHandles([
  // tip: moves the leader-line tip only (box + width point stay put)
  pin({ at: (ctx) => ctx.pts[0], drag: (d, dp) => { d.points = [{ time: dp.time, price: dp.price }, { ...d.points[1] }, ...keep3(d)]; } }),
  ...
  pin({ at: (ctx) => ctx.pts[1], drag: (d, dp) => {
    const p = d.points, dt = dp.time - p[1].time;
  ...
  pin({
    ...
    drag: (d, dp) => { d.points = [{ ...d.points[0] }, { ...d.points[1] }, { time: dp.time, price: d.points[1].price }]; },
  }),
]);
...
function boxFor(c, pts, d) {
  ...
  /** @type {number|null} */
  let wrapW = null;
  if (wrap) { const rightX = pts.length >= 3 ? pts[2].x : pts[1].x + 160; wrapW = Math.max(8, rightX - pts[1].x - PADX * 2); }
...
function nearestSideMid(box, p) {
  ...
  mids.forEach((m) => { const dx = m.x - p.x, dy = m.y - p.y, dd = dx * dx + dy * dy; if (dd < bd) { bd = dd; best = m; } });
  return best;
}
