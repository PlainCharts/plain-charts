...
function makeTrend({ id, name, glyph, extend, description, icon }) {
  return {
    ...
    marks(d, view) {
      ...
      if (s.arrows && s.arrows !== 'none') {
        ...
        if (s.arrows === 'end' || s.arrows === 'both')
          out.push(this.arrowMark(/** @type {ToolScreenPoint} */ (B), ux, uy, sz, s.color));
        if (s.arrows === 'start' || s.arrows === 'both')
          out.push(this.arrowMark(/** @type {ToolScreenPoint} */ (A), -ux, -uy, sz, s.color));
      }
      if (s.midPoint) {
        const mx = (A.x + B.x) / 2,
          my = (A.y + B.y) / 2,
        ...
        for (let k = 0; k <= 20; k++) {
          ...
          pts.push({ vpx: 0, dx: mx + 4 * Math.cos(ang), vp: 0, dy: my + 4 * Math.sin(ang) });
        }
    ...
    textGap(d, pts) {
      ...
      let cx, cy;
      if (ha === 'left') {
        cx = a.x + ux * (pad + textW / 2);
        cy = a.y + uy * (pad + textW / 2);
      } else if (ha === 'right') {
        cx = b.x - ux * (pad + textW / 2);
        cy = b.y - uy * (pad + textW / 2);
      } else {
        cx = (a.x + b.x) / 2;
        cy = (a.y + b.y) / 2;
      }
    ...
    textGeom(c, pts, d) {
      ...
      const ux = dx / len,
        uy = dy / len,
        angle = Math.atan2(uy, ux); // |angle| <= 90°
      let px = -uy,
        py = ux;
      if (py > 0) {
        px = -px;
