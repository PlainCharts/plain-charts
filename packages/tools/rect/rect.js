...
Tools.register({
  ...
  settings: {
    style: [
      { name: 'Border', controls: [{ key: 'color', type: 'color', width: 'width', lineStyle: 'lineStyle' }] },
      {
        name: 'Middle line',
        toggle: 'midLine',
        controls: [{ key: 'midColor', type: 'color', width: 'midWidth', lineStyle: 'midStyle' }],
      },
  ...
  marks(d, view) {
    ...
    if (s.midLine) {
      ...
      const seg = (a, c) => ({
        path: [a, c],
        stroke: s.midColor || '#787b86',
        width: s.midWidth || 1,
        dash: Tools.dash(/** @type {any} */ (s.midStyle)),
      });
      ...
      if (band) {
        ...
        const xL = /** @type {number} */ (view.timeToX(b.left)),
          xR = /** @type {number} */ (view.timeToX(b.right));
        const b0 = Math.max(xL, band[0]),
          b1 = Math.min(xR, band[1]);
  ...
  textBand(d, view) {
    ...
    const b = bounds(d);
    const x1 = view.timeToX(b.left),
      x2 = view.timeToX(b.right);
  ...
  handles(pts) {
    return pts.length >= 2 ? PINS.handles({ bb: bb(pts) }) : pts;
  },
...
const bounds = (d) => {
  ...
  return {
    left: Math.min(p[0].time, p[1].time),
    right: Math.max(p[0].time, p[1].time),
    top: Math.max(p[0].price, p[1].price),
    bottom: Math.min(p[0].price, p[1].price),
  };
};
...
const edit = (d, ch) => {
  ...
  Object.assign(b, ch);
  d.points = [
    { time: b.left, price: b.top },
    { time: b.right, price: b.bottom },
  ];
};
