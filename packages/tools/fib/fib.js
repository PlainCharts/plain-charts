...
Tools.register({
  ...
  defaultStyle: {
    ...
    trendOn: false, trendColor: '#787b86', trendWidth: 1, trendStyle: 'dashed',
    bgOn: true, bgOpacity: 0.1,
    oneColor: false, color: '#787b86',
    extend: 'none',
    showRatio: true, showPrice: true, levelMode: 'values',
    labelHAlign: 'right', labelVAlign: 'middle', labelFontSize: 11,
  },
  settings: {
    style: [
      { name: 'Trend line', toggle: 'trendOn', controls: [{ key: 'trendColor', type: 'color', width: 'trendWidth', lineStyle: 'trendStyle' }] },
      { name: 'Extend', controls: [{ key: 'extend', type: 'select', options: [{ key: 'none', name: "Don't extend" }, { key: 'right', name: 'Extend right' }, { key: 'left', name: 'Extend left' }, { key: 'both', name: 'Extend both' }] }] },
      { controls: [{ key: 'levels', type: 'fiblevels' }] },
      { name: 'Use one color', toggle: 'oneColor', controls: [{ key: 'color', type: 'color' }] },
      { name: 'Background', toggle: 'bgOn', toggleDefault: true, controls: [{ key: 'bgOpacity', type: 'range', min: 0, max: 0.5, step: 0.02 }] },
      { name: 'Prices', toggle: 'showPrice', toggleDefault: true },
      { name: 'Levels', toggle: 'showRatio', toggleDefault: true, controls: [{ key: 'levelMode', type: 'select', options: [{ key: 'values', name: 'Values' }, { key: 'percents', name: 'Percents' }] }] },
      { name: 'Labels', controls: [
        { key: 'labelHAlign', type: 'select', options: [{ key: 'left', name: 'Left' }, { key: 'center', name: 'Center' }, { key: 'right', name: 'Right' }] },
        { key: 'labelVAlign', type: 'select', options: [{ key: 'top', name: 'Top' }, { key: 'middle', name: 'Middle' }, { key: 'bottom', name: 'Bottom' }] },
      ] },
      { name: 'Font size', controls: [{ key: 'labelFontSize', type: 'select', options: [10, 11, 12, 13, 14, 16, 18].map((n) => ({ key: String(n), name: String(n) })) }] },
    ],
  },
  ...
  marks(d, view) {
    ...
    const x1 = view.timeToX(P[0].time), y1 = view.priceToY(P[0].price);
    const x2 = view.timeToX(P[1].time), y2 = view.priceToY(P[1].price);
    if (x1 == null || y1 == null || x2 == null || y2 == null) return [];
    const W = view.width || 0;
    let xa = Math.min(x1, x2), xb = Math.max(x1, x2);
    ...
    const base = P[1].price, span = P[0].price - P[1].price;
    /** @param {number} r */
    ...
    if (s.bgOn !== false && enabled.length > 1) {
      ...
      for (let i = 0; i < enabled.length - 1; i++) {
        const ya = yAt(enabled[i].value), yb = yAt(enabled[i + 1].value), yl = Math.min(ya, yb), yh = Math.max(ya, yb);
        out.push({ closed: true, fill: toAlpha(colOf(enabled[i]), op), path: [sv(xa, yl), sv(xb, yl), sv(xb, yh), sv(xa, yh)] });
      }
    }
    if (s.trendOn) out.push({ path: [sv(x1, y1), sv(x2, y2)], stroke: s.trendColor || '#787b86', width: s.trendWidth || 1, dash: Tools.dash(s.trendStyle) });

    const lha = s.labelHAlign || 'right', lva = s.labelVAlign || 'middle';
    ...
    const mc = measureCtx(); mc.font = fontSize + 'px sans-serif';
    enabled.forEach((l) => {
      const y = yAt(l.value), col = colOf(l);
      const parts = [];
      if (s.showRatio !== false) parts.push(s.levelMode === 'percents' ? `${+(l.value * 100).toFixed(1)}%` : String(l.value));
      if (s.showPrice !== false) parts.push(`(${priceAt(l.value).toFixed(dec)})`);
      const txt = parts.join(' ');
      const width = l.width != null ? l.width : (s.lineWidth || 1), dash = Tools.dash(l.lineStyle != null ? l.lineStyle : s.lineStyle);
      /** @param {any} a @param {any} b */
      const line = (a, b) => ({ path: [a, b], stroke: col, width, dash });
      if (txt && centerGap) {
        const tw = mc.measureText(txt).width, cx = (xa + xb) / 2, g0 = cx - tw / 2 - 6, g1 = cx + tw / 2 + 6;
  ...
  hitTest(pts, x, y, tol, _d) {
    ...
    for (let i = 0; i < 2; i++) if (Tools.geom.dist(x, y, pts[i].x, pts[i].y) <= tol + 3) return { part: 'point', index: i };
