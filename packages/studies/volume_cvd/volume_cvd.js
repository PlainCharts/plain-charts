...
Studies.register({
  ...
  plots() {
    return [{ key: 'cvd', name: 'CVD', type: 'segmented', precision: 0 }];
  },
  shapes() {
    return [{ type: 'hline', price: 0, color: 'rgba(120,123,134,0.5)', width: 1, lineStyle: 'solid' }];
  },
  ...
  step(i, sh, p, ctx, s) {
    ...
    if (sb && sb.length) {
      for (const x of sb) {
        ...
        if (s.cvd > hi) hi = s.cvd;
        if (s.cvd < lo) lo = s.cvd;
      }
    } else {
    ...
    const close = s.cvd,
      col = close >= open ? s.up : s.down;
    return {
      cvd: {
        value: close,
        segments: [{ from: open, to: close, color: col }],
        wicks: [{ from: lo, to: hi, color: col, width: 1 }],
      },
    };
  },
});
