...
const MA_TYPES = [
  { key: 'SMA', name: 'SMA' }, { key: 'EMA', name: 'EMA' }, { key: 'SMMA (RMA)', name: 'SMMA (RMA)' },
  { key: 'WMA', name: 'WMA' }, { key: 'VWMA', name: 'VWMA' },
];
const DEF = [
  { len: 20, color: '#f6c309' }, { len: 50, color: '#fb9800' },
  { len: 100, color: '#fb6500' }, { len: 200, color: '#f60c0c' },
];
...
function movingAverage(bars, srcKey, length, type) {
  const n = bars.length, L = Math.max(1, length | 0);
  /** @type {(number | null)[]} */
  ...
  if (type === 'EMA' || type === 'SMMA (RMA)') {
    ...
    for (let i = 0; i < n; i++) {
      if (i < L - 1) continue;
      if (ma === null) { let s = 0; for (let j = i - L + 1; j <= i; j++) s += src[j]; ma = s / L; }
      else ma = ma + a * (src[i] - ma);
      out[i] = ma;
    }
  } else if (type === 'WMA') {
    const denom = L * (L + 1) / 2;
    for (let i = L - 1; i < n; i++) { let s = 0; for (let k = 0; k < L; k++) s += src[i - k] * (L - k); out[i] = s / denom; }
  } else if (type === 'VWMA') {
    let sv = 0, sw = 0;
    for (let i = 0; i < n; i++) {
      const v = bars[i].volume || 0; sv += src[i] * v; sw += v;
      if (i >= L) { const vo = bars[i - L].volume || 0; sv -= src[i - L] * vo; sw -= vo; }
      if (i >= L - 1) out[i] = sw > 0 ? sv / sw : null;
    }
  } else {   // SMA
    let s = 0;
    for (let i = 0; i < n; i++) { s += src[i]; if (i >= L) s -= src[i - L]; if (i >= L - 1) out[i] = s / L; }
  }
  return out;
}
...
function initMA(type, length) {
  ...
  if (type === 'EMA' || type === 'SMMA (RMA)') return { mode: 'ema', L, a: type === 'EMA' ? 2 / (L + 1) : 1 / L, ma: null, seedSum: 0, idx: -1, seed: [] };
...
function advanceMA(st, price, vol) {
  ...
  if (st.mode === 'ema') {
    ...
    st.idx = /** @type {number} */ (st.idx) + 1; seed.push(price); st.seedSum = /** @type {number} */ (st.seedSum) + price;
    if (seed.length > L) st.seedSum -= /** @type {number} */ (seed.shift());
    if (st.idx < L - 1) return null;
    st.ma = st.ma == null ? /** @type {number} */ (st.seedSum) / L : st.ma + /** @type {number} */ (st.a) * (price - st.ma);
    return st.ma;
  }
  if (st.mode === 'wma') {
    const buf = /** @type {number[]} */ (st.buf); buf.push(price); if (buf.length > L) buf.shift();
    if (buf.length < L) return null;
    let s = 0; for (let k = 0; k < L; k++) s += buf[L - 1 - k] * (L - k);
    return s / /** @type {number} */ (st.denom);
  }
  if (st.mode === 'vwma') {
    const prod = /** @type {number[]} */ (st.prod), vols = /** @type {number[]} */ (st.vols);
    const v = vol || 0, pr = price * v; prod.push(pr); vols.push(v); st.sv = /** @type {number} */ (st.sv) + pr; st.sw = /** @type {number} */ (st.sw) + v;
    if (prod.length > L) { st.sv -= /** @type {number} */ (prod.shift()); st.sw -= /** @type {number} */ (vols.shift()); }
    if (prod.length < L) return null;
    return st.sw > 0 ? /** @type {number} */ (st.sv) / /** @type {number} */ (st.sw) : null;
  }
  const buf = /** @type {number[]} */ (st.buf);   // sma
  buf.push(price); st.sum = /** @type {number} */ (st.sum) + price; if (buf.length > L) st.sum -= /** @type {number} */ (buf.shift());
...
DEF.forEach((d, i) => {
  const n = i + 1;
  inputs.push(
    { key: `show${n}`, type: 'bool', name: `MA #${n}`, default: true, right: [`type${n}`, `source${n}`, `length${n}`, `line${n}`] },
    ...
    { key: `line${n}`, type: 'stroke', name: 'Line', default: { color: d.color, width: 2, style: 'solid' }, hidden: true },
  );
});
...
Studies.register({
  ...
  calc(bars, p) {
    ...
    for (let n = 1; n <= 4; n++) {
      ...
      /** @type {StudyPlotPoint[]} */
      const data = [];
      for (let i = 0; i < bars.length; i++) if (out[i] != null && isFinite(/** @type {number} */ (out[i]))) data.push({ time: bars[i].time, value: /** @type {number} */ (out[i]) });
      plots.push({
        key: `ma${n}`, name: `MA${n} ${type} ${len}`, type: 'line',
        color: ln.color || DEF[n - 1].color, lineWidth: ln.width || 2, lineStyle: ln.style || 'solid', data,
      });
    }
    return { plots };
  },
  ...
  init(p) {
    ...
    for (let n = 1; n <= 4; n++) {
      ...
      const ln = strk(p[`line${n}`] || p[`ma${n}_color`]);
      meta.push({ key: `ma${n}`, name: `MA${n} ${type} ${len}`, color: ln.color || DEF[n - 1].color, width: ln.width || 2, style: ln.style || 'solid', src });
      st.push(initMA(type, len));
    }
    return { meta, st };
  },
  /** @param {Record<string, any>} p @param {Record<string, any>} ctx @param {any} s */
  plots(p, ctx, s) { return s.meta.map((/** @type {any} */ r) => ({ key: r.key, name: r.name, type: 'line', color: r.color, lineWidth: r.width, lineStyle: r.style })); },
