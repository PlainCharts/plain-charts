...
Studies.register({
  ...
  init(p) {
    return { len: Math.max(1, p.length | 0), k: p.mult || 2, src: p.source, buf: /** @type {number[]} */ ([]), sum: 0 };
  },
  plots() {
    return [
  ...
  fills() {
    return [{ top: 'upper', bottom: 'lower', color: 'rgba(33,150,243,0.10)' }];
  },
