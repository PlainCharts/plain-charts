...
const anchorPlot = (data) => [{ key: 'pr', name: '%R', type: 'line', color: 'rgba(255,255,255,0)', lineWidth: 0, data }];
...
function buildScene(bars, p, ctx) {
  ...
  const anchorData = []; for (let c = 0; c < COLS; c++) anchorData.push({ time: bars[idx[c]].time, value: grid[0][c] });
...
function renderFrame(sc, p, g) {
  ...
  m.push({ text: '%R Terrain 3D  ·  fast ' + sc.fast + ' (front) / slow ' + sc.slow + ' (back)', at: { vpx: 0.02, vp: 0.05, dx: 0, dy: 0 }, color: '#d3d6dd', size: 12 });
  return m;
}
