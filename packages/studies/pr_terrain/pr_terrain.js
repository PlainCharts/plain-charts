...
function withAlpha(color, a) {
  ...
  if (m) {
    const q = m[1].split(',').map((s) => s.trim());
    return `rgba(${q[0]}, ${q[1]}, ${q[2]}, ${a})`;
  }
  return color;
}
...
function _vwma(a, vol, n) {
  ...
  for (let i = n - 1; i < a.length; i++) {
    let num = 0,
      ...
      ok = true;
    for (let k = i - n + 1; k <= i; k++) {
      if (a[k] == null || vol[k] == null) {
        ok = false;
        break;
      }
      num += /** @type {number} */ (a[k]) * /** @type {number} */ (vol[k]);
      den += /** @type {number} */ (vol[k]);
    }
...
function movingAverage(series, type, len, volumes) {
  ...
  switch (type) {
    case 'sma':
      return _sma(series, n);
    case 'rma':
      return _ewma(series, 1 / n);
    case 'wma':
      return _wma(series, n);
    case 'vwma':
      return _vwma(series, volumes, n);
    case 'hma': {
      const half = _wma(series, Math.max(1, Math.round(n / 2))),
        full = _wma(series, n);
      const diff = series.map((_, i) =>
        half[i] == null || full[i] == null
          ? null
          : 2 * /** @type {number} */ (half[i]) - /** @type {number} */ (full[i]),
      );
      return _wma(diff, Math.max(1, Math.round(Math.sqrt(n))));
    }
    case 'ema':
    default:
      return _ewma(series, 2 / (n + 1));
  }
}
...
Studies.register({
  ...
  inputs: [
    ...
    { key: 'zones', name: 'Colour by OB/OS zones', type: 'bool', default: true },
    {
      ...
      default: '123',
      options: [
        { key: '12', name: 'Period 1 & 2' },
        { key: '13', name: 'Period 1 & 3' },
        { key: '23', name: 'Period 2 & 3' },
        { key: '123', name: 'All three' },
      ],
    },
    { key: 'threshold', name: 'Exhaustion threshold', type: 'number', default: 30, min: 0, max: 50 },
    {
      ...
      min: -100,
      max: 0,
      default: { value: 0, show: true, color: 'rgba(202,0,23,0.6)', width: 1, style: 'solid' },
    },
    {
      key: 'lvlMid',
      name: 'Middle',
      ...
      min: -100,
      max: 0,
      default: { value: -50, show: true, color: 'rgba(120,123,134,0.5)', width: 1, style: 'solid' },
    },
    {
      key: 'lvlBot',
      name: 'Bottom',
      ...
      min: -100,
      max: 0,
      default: { value: -100, show: true, color: 'rgba(36,102,167,0.65)', width: 1, style: 'solid' },
    },
    // the two derived zone lines (Middle +/- threshold) -- their appearance only; the VALUES stay tied to threshold
    {
      ...
      type: 'stroke',
      tab: 'Levels',
      default: { color: 'rgba(202,0,23,0.7)', width: 1, style: 'dashed' },
    },
    {
      key: 'lowerLine',
      ...
      type: 'stroke',
      tab: 'Levels',
      default: { color: 'rgba(36,102,167,0.7)', width: 1, style: 'dashed' },
    },
    ...
    { key: 'pearlSize', name: 'Pearl size (px)', type: 'number', tab: 'Levels', default: 4, min: 1, max: 14 },
    {
      ...
      name: 'Period 1 line',
      type: 'stroke',
      default: { color: '#ffffff', width: 2, style: 'solid' },
    },
    {
      key: 'slowColor',
      name: 'Period 2 line',
      type: 'stroke',
      default: { color: '#8baeff', width: 2, style: 'solid' },
    },
  ...
  /**
   * The pipeline: buildScene computes everything FRAME-INVARIANT once per recompute (bar math, sampling
   * grid, projection, zone colouring, pearl anchors); renderFrame turns the scene + a height grid into
   * marks; morphOrSnap owns the tween lifecycle (ease the display grid toward the target on the chart's
   * frame clock, or snap when morph is off / no rAF host).
   * @param {StudyBar[]} bars
   * @param {Record<string, any>} p
   * @param {{ visibleRange?:{from:number,to:number}, self?:any, requestFrames?:Function, [k:string]:any }} [ctx]
   */
  calc(bars, p, ctx) {
    const scene = buildScene(bars, p, ctx);
    if (!scene) {
      ...
      return { plots: anchorPlot([]), shapes: [] };
    }
    return morphOrSnap(p, ctx, scene);
  },
});

// faint plot so the study owns a pane + scale (the fast %R line drives it)
/** @param {{ time:number, value:number }[]} data */
const anchorPlot = (data) => [
  { key: 'pr', name: '%R', type: 'line', color: 'rgba(255,255,255,0)', lineWidth: 0, data },
];

// ---- the SCENE -- every frame-invariant piece of the terrain, computed once per recompute ------
// The %R grid over the sampled window, the period ridges, the projection into the ether, the OB/OS zone
// colouring, and the pearl anchors (reversal bars). The morph loop re-renders MANY frames from ONE scene,
// so nothing here may depend on the eased display grid.
/** @typedef {ReturnType<typeof buildScene>} Scene */
/**
 * @param {StudyBar[]} bars
 * @param {Record<string, any>} p
 * @param {{ visibleRange?:{from:number,to:number}, [k:string]:any }} [ctx]
 * @returns {null | { grid:number[][], periods:{ L:number, col:string, wd:number, dash:string, row:number }[],
 *   proj:(c:number, r:number, v:number)=>any, projSig:(t:number, v:number)=>any, cellRgb:(v:number, c:number)=>number[],
 *   topLv:any, midLv:any, botLv:any, upper:number, lower:number, pearls:{ t:number, level:number }[],
 *   fast:number, slow:number, anchorData:{ time:number, value:number }[] }}
 */
function buildScene(bars, p, ctx) {
  const N = bars.length;
  ...
  if (N < slow + COLS + 2) return null; // not enough history -> the caller keeps the anchor alive
  ...
  periods[0].row = 0;
  periods[1].row = midRow;
  periods[2].row = ROWS - 1;
  /** @param {number} r */
  const lookbackAtRow = (r) => {
    if (r <= midRow) {
      const t = midRow ? r / midRow : 0;
      return periods[0].L * Math.pow(periods[1].L / periods[0].L, t);
    }
    const t = (r - midRow) / (ROWS - 1 - midRow);
    return periods[1].L * Math.pow(periods[2].L / periods[1].L, t);
  };
  const Ls = [];
  for (let r = 0; r < ROWS; r++) Ls.push(Math.max(2, Math.round(lookbackAtRow(r))));
  ...
  const src = p.source || 'close',
    smType = p.smoothType || 'ema';
  const fastSm = Math.max(1, p.fastSmooth | 0 || 7),
    slowSm = Math.max(1, p.slowSmooth | 0 || 3),
    p3Sm = Math.max(1, p.p3Smooth | 0 || 5);
  ...
  const smoothAtRow = (r) => {
    if (r <= midRow) {
      const t = midRow ? r / midRow : 0;
      return fastSm + (slowSm - fastSm) * t;
    }
    const t = (r - midRow) / (ROWS - 1 - midRow);
    return slowSm + (p3Sm - slowSm) * t;
  };
  ...
  for (let r = 0; r < ROWS; r++) {
    ...
    for (let c = 0; c < COLS; c++) {
      const gi = idx[c] - w0;
      row.push(gi >= 0 && s[gi] != null ? s[gi] : 0);
    }
    grid.push(row);
  }
  ...
  const pSeries = [
    smoothS(percentR(w, p1, src), fastSm),
    smoothS(percentR(w, p2, src), slowSm),
    smoothS(percentR(w, p3v, src), p3Sm),
  ];
  ...
  const HMAX = p.height | 0 || 120,
    DEP = p.depth | 0 || 120;
  /** @param {number} v */
  const hpx = (v) => (u(v) - 0.5) * 2 * HMAX; // %R -> height px (overbought up, oversold down)
  const pit = ((p.pitch != null ? p.pitch : 42) * Math.PI) / 180,
    cp = Math.cos(pit),
    sp = Math.sin(pit);
  ...
  const proj = (c, r, v) => {
    const Y = hpx(v),
      Z = (r / (ROWS - 1)) * DEP;
    return { t: bars[idx[c]].time, vp: baseY, dy: -(Y * cp + Z * sp) };
  };
  ...
  const lv = (o, dv, dc) =>
    o && typeof o === 'object'
      ? { value: o.value != null ? o.value : dv, color: o.color || dc, show: o.show }
      : { value: dv, color: dc, show: true };
  const topLv = lv(p.lvlTop, 0, 'rgba(202,0,23,0.6)'),
    midLv = lv(p.lvlMid, -50, 'rgba(120,123,134,0.5)'),
    botLv = lv(p.lvlBot, -100, 'rgba(36,102,167,0.65)');
  const upper = midLv.value + th,
    lower = midLv.value - th;
  const hotRgb = parseRgb(topLv.color),
    coldRgb = parseRgb(botLv.color),
    zonesOn = p.zones !== false;
  ...
  const zoneRgb = (v) => {
    if (v >= upper)
      return mix(
        baseRgb,
        hotRgb,
        topLv.value > upper ? Math.min(1, Math.max(0, (v - upper) / (topLv.value - upper))) : 1,
      );
    if (v <= lower)
      return mix(
        baseRgb,
        coldRgb,
        botLv.value < lower ? Math.min(1, Math.max(0, (lower - v) / (lower - botLv.value))) : 1,
      );
    return baseRgb;
  };
  ...
  const obCol = new Array(COLS),
    osCol = new Array(COLS);
  for (let c = 0; c < COLS; c++) {
    const gi = idx[c] - w0;
  ...
  // Exhaustion pearl ANCHORS (frame-invariant -- they don't ease, so they are computed here, not per
  // frame): the %R Trend Exhaustion reversal signal, generalized. overbought = EVERY selected period's
  // %R >= upper; oversold = every selected period's %R <= lower (the `confirm` set picks which periods
  // must agree). A run is a contiguous stretch where that holds; the reversal fires on the FIRST bar the
  // run breaks (any selected period leaves the zone). Only bars in the visible span [lo, hi] fire.
  ...
  if (p.signals !== false && sel.length) {
    ...
    for (let i = 1; i < nW; i++) {
      ...
      if (ob[i - 1] && !ob[i]) pearls.push({ t: w[i].time, level: upper }); // overbought run broke -> pearl on upper line
      if (os[i - 1] && !os[i]) pearls.push({ t: w[i].time, level: lower }); // oversold run broke  -> pearl on lower line
    }
  }

  // faint anchor-plot data (the front-row %R line drives the pane's scale)
  const anchorData = [];
  for (let c = 0; c < COLS; c++) anchorData.push({ time: bars[idx[c]].time, value: grid[0][c] });

  return { grid, periods, proj, projSig, cellRgb, topLv, midLv, botLv, upper, lower, pearls, fast, slow, anchorData };
}

// ---- one FRAME: scene + a height grid `g` -> the mark list ------------------------------------
// Surface + ridges MORPH with g; guides/pearls/text are fixed references from the scene (they don't
// ease). Called once per frame by the tween loop with the eased display grid.
/** @param {NonNullable<Scene>} sc @param {Record<string, any>} p @param {number[][]} g @returns {any[]} */
function renderFrame(sc, p, g) {
  const { periods, proj, projSig, cellRgb, topLv, midLv, botLv, upper, lower, pearls } = sc;
  ...
  const relief = p.relief !== false,
    reliefK = p.reliefStrength != null ? p.reliefStrength : 0.55;
  const Lx = -0.5,
    Ly = 0.72,
    Lz = 0.48,
    HN = 8; // light from the upper-left, slightly toward the viewer
  let gs = g;
  if (relief) {
    // 5-tap blur along columns (the noisy time axis); rows/depth kept as-is
    gs = g.map((row, r) =>
      row.map((_, c) => {
        let s = 0,
          n = 0;
  ...
  for (let r = ROWS - 2; r >= 0; r--) {
    for (let c = 0; c < COLS - 1; c++) {
      ...
      if (relief) {
        ...
        const nx = -gx,
          nz = -gz,
          nl = Math.sqrt(nx * nx + HN * HN + nz * nz) || 1;
  ...
  const guide = (val, color, dash, wd) => {
    const path = [];
    for (let c = 0; c < COLS; c++) path.push(proj(c, 0, val));
    m.push({ path, stroke: color, width: wd || 1, dash });
  };
  ...
  const ul = p.upperLine || {},
    ll = p.lowerLine || {}; // Upper/Lower line appearance (colour/width/style)
  ...
  const ridgeAt = (rf, color, width, dash) => {
    const r0 = Math.floor(rf),
      r1 = Math.min(ROWS - 1, r0 + 1),
      f = rf - r0;
    const path = [];
    for (let c = 0; c < COLS; c++) {
      const v = g[r0][c] + (g[r1][c] - g[r0][c]) * f;
      path.push(proj(c, rf, v));
    }
  ...
  // Exhaustion pearls at the scene's precomputed reversal anchors: a bead on the threshold line (front
  // plane) at the reversal bar -- the orange line runs through it.
  if (pearls.length) {
    const pr = Math.max(1, p.pearlSize | 0 || 4),
      pc = p.pearlColor || '#f3efe6';
    const rim = withAlpha('#000000', 0.35),
      sheen = withAlpha('#ffffff', 0.5);
    /** @param {{ t:number, vp:number, dy:number }} b */
    const pearl = (b) => {
      const ring = [];
      for (let k = 0; k <= 22; k++) {
        const a = (k / 22) * Math.PI * 2;
        ring.push({ t: b.t, vp: b.vp, dx: pr * Math.cos(a), dy: b.dy + pr * Math.sin(a) });
      }
      ...
      for (let k = 0; k <= 12; k++) {
        ...
        hi.push({ t: b.t, vp: b.vp, dx: -pr * 0.3 + hr * Math.cos(a), dy: b.dy - pr * 0.3 + hr * Math.sin(a) });
      }
      m.push({ closed: true, path: hi, fill: sheen });
    };
    for (const s of pearls) pearl(projSig(s.t, s.level));
  }

  m.push({
    text: '%R Terrain 3D  ·  fast ' + sc.fast + ' (front) / slow ' + sc.slow + ' (back)',
    at: { vpx: 0.02, vp: 0.05, dx: 0, dy: 0 },
    color: '#d3d6dd',
    size: 12,
  });
  return m;
...
/** @param {Record<string, any>} p
 * @param {{ self?:any, requestFrames?:Function, [k:string]:any }|undefined} ctx
 * @param {NonNullable<Scene>} sc
 * @returns {{ plots:any[], shapes:any[] }} */
function morphOrSnap(p, ctx, sc) {
  const plots = anchorPlot(sc.anchorData);
  const grid = sc.grid;
  ...
  if (!animate) {
    ...
    return { plots, shapes: [{ marks: renderFrame(sc, p, grid) }] };
  }
  const self = ctx.self;
  ...
  const step = () => {
    ...
    if (maxd < 0.06) {
      for (let r = 0; r < ROWS; r++) {
        const dr = d[r],
          tr = tg[r];
        for (let c = 0; c < COLS; c++) dr[c] = tr[c];
      }
      settled = true;
    } // snap on the last frame, then stop
    return [{ marks: renderFrame(sc, p, d) }];
  };
  ...
  return { plots, shapes: [{ marks: renderFrame(sc, p, self.display) }] };
}
