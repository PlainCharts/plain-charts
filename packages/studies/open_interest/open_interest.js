...
Studies.register({
      ...
      plots: [
        {
          ...
          type: 'line',
          precision: 0,
          color: s.color || '#ffa726',
          lineWidth: s.width || 2,
          lineStyle: s.style || 'solid',
          data,
        },
      ],
    };
  ...
  init(p) {
    const s = p.line || {};
    return { color: s.color || '#ffa726', width: s.width || 2, style: s.style || 'solid' };
  },
  ...
  step(i, sh) {
    const oi = sh.openInterest[i];
    return oi != null ? { oi: oi } : null;
  },
});
