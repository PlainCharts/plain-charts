...
Studies.register({
    ...
    bars.forEach((b, i) => {
      ...
      if (hasSubs) {
        for (const s of sb) {
          ...
          if (bull) up += s.volume || 0;
          else down += s.volume || 0;
        }
      } else {
        // no sub-bars yet (forming / most-recent bars): approximate from the chart bar so it renders
        const vol = b.volume || 0;
        if (!vol) return;
        if (b.close != null && b.open != null ? b.close >= b.open : true) up = vol;
        else down = vol;
      }
      const delta = up - down,
        total = up + down;
  ...
  plots() {
    return [{ key: 'vol', name: 'Up/Dn Vol', type: 'segmented', precision: 0 }];
  },
  ...
  step(i, sh, p, ctx, s) {
    ...
    if (hasSubs) {
      for (const x of sb) {
        const bull = x.open != null && x.close != null ? x.close >= x.open : true;
        if (bull) up += x.volume || 0;
        else down += x.volume || 0;
      }
    } else {
      const vol = sh.volume[i] || 0;
      if (!vol) return null;
      if (sh.close[i] != null && sh.open[i] != null ? sh.close[i] >= sh.open[i] : true) up = vol;
      else down = vol;
    }
    const delta = up - down,
      total = up + down;
    ...
    return {
      vol: {
        ...
        lines: [{ level: delta, color: delta >= 0 ? p.deltaUp || '#0b3d1e' : p.deltaDown || '#5c1212', width: 1 }],
      },
    };
  },
});
