...
RANGE.forEach((d, i) => {
  ...
  inputs.push(
    ...
    { key: `m${n}_lines`, name: 'Show lines', type: 'bool', default: true, right: `m${n}_color`, group: g },
    {
      ...
      type: 'color',
      default: d.color,
      stroke: { width: `m${n}_width`, lineStyle: `m${n}_style` },
      hidden: true,
      group: g,
    },
    {
...
SINGLE.forEach((d, i) => {
  ...
  inputs.push(
    ...
    { key: `s${n}_name`, name: 'Name', type: 'text', default: `Single Marker ${n}`, hidden: true, group: g },
    {
      ...
      type: 'color',
      default: d.color,
      stroke: { width: `s${n}_width`, lineStyle: `s${n}_style` },
      group: g,
    },
    {
      key: `s${n}_style`,
...
function markerDays(refNow, off, past, back, future, fwd) {
  ...
  if (past) {
    let cnt = 0,
      delta = 0,
      g = 0;
    while (cnt <= back && g++ < 600) {
      const d = new Date(Date.UTC(Y, M, D - delta));
      if (d.getUTCDay() % 6 !== 0) {
        push(d);
        cnt++;
      }
...
Studies.register({
  ...
  calc(bars, p) {
    ...
    const past = !!p.show_past,
      future = !!p.show_future;
    const back = p.days_to_show | 0,
      fwd = p.future_days | 0;

    // session range markers
    for (let n = 1; n <= 4; n++) {
      if (!p[`m${n}_enable`]) continue;
      const ses = parseSession(p[`m${n}_session`]);
      if (!ses) continue;
      const off = tzOff(p[`m${n}_tz`]);
      const col = p[`m${n}_color`],
        style = p[`m${n}_style`],
        width = p[`m${n}_width`] | 0 || 2;
      const lines = p[`m${n}_lines`] !== false,
        band = !!p[`m${n}_band`],
        fill = p[`m${n}_band_color`];
    ...
    for (let n = 1; n <= 4; n++) {
      ...
      const hm = parseHM(p[`s${n}_time`]);
      if (!hm) continue;
      const off = tzOff(p[`s${n}_tz`]);
      const col = p[`s${n}_color`],
        style = p[`s${n}_style`],
        width = p[`s${n}_width`] | 0 || 2;
      markerDays(now, off, past, back, future, fwd).forEach(({ y, mo, d }) => {
