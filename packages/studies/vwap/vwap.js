...
function withAlpha(col, a) {
  ...
  if (m) {
    ...
    return `rgba(${q[0] || 0},${q[1] || 0},${q[2] || 0},${a})`;
  }
  return col;
}
...
Studies.register({
  ...
  inputs: [
    ...
    { key: 'resetHour', type: 'number', name: 'Session reset (hour)', default: 0, min: 0, max: 23 },
    {
      ...
      default: 'stdev',
      options: [
        { key: 'stdev', name: 'Standard Deviation' },
        { key: 'pct', name: 'Percentage' },
      ],
    },
    ...
    },
    {
      ...
      type: 'stroke',
      name: 'Band 1 line',
      default: { color: 'rgba(76,175,80,0.9)', width: 1, style: 'solid' },
      tab: 'Bands',
    },
    ...
    },
    {
      ...
      type: 'stroke',
      name: 'Band 2 line',
      default: { color: 'rgba(128,128,0,0.9)', width: 1, style: 'solid' },
      tab: 'Bands',
    },
    ...
    },
    {
      ...
      type: 'stroke',
      name: 'Band 3 line',
      default: { color: 'rgba(0,128,128,0.9)', width: 1, style: 'solid' },
      tab: 'Bands',
    },
  ...
  calc(bars, p) {
    ...
    const cutoffDay =
      p.recentOnly === true && bars.length
        ? dayIdx(bars[bars.length - 1].time) - (Math.max(1, p.recentDays | 0 || 5) - 1)
        : -Infinity;
    ...
    bars.forEach((b) => {
      ...
      const price = Studies.priceOf(b, src),
        vol = b.volume || 0;
      sPV += price * vol;
      sV += vol;
      ...
      for (let k = 0; k < 3; k++) {
        u[k].push({ time: b.time, value: vwap + basis * mult[k] });
        l[k].push({ time: b.time, value: vwap - basis * mult[k] });
      }
    });
    ...
    const bandS = [
      strk(p.band1Line || p.band1Color),
      strk(p.band2Line || p.band2Color),
      strk(p.band3Line || p.band3Color),
    ];
    ...
    const plots = [
      {
        ...
        name: 'VWAP',
        type: 'line',
        color: vs.color || '#2962FF',
        lineWidth: vs.width || 2,
        lineStyle: vs.style || 'solid',
        data: vw,
      },
    ];
    const fills = [];
  ...
  init(p, ctx, sh) {
    ...
    return {
      ...
      vsColor: vs.color || '#2962FF',
      vsWidth: vs.width || 2,
      vsStyle: vs.style || 'solid',
      bandCols: bandS.map((b, k) => b.color || bandDflt[k]),
      bandWds: bandS.map((b) => b.width || 1),
      bandStyles: bandS.map((b) => b.style || 'solid'),
      sPV: 0,
      sV: 0,
      sP2V: 0,
      /** @type {number | null} */ prevKey: null,
    };
  },
  /** @param {Record<string, any>} p @param {Record<string, any>} ctx @param {any} s */
  plots(p, ctx, s) {
    const plots = [
      { key: 'vwap', name: 'VWAP', type: 'line', color: s.vsColor, lineWidth: s.vsWidth, lineStyle: s.vsStyle },
    ];
    for (let k = 0; k < 3; k++) {
      ...
      plots.push({
        ...
        type: 'line',
        color: s.bandCols[k],
        lineWidth: s.bandWds[k],
        lineStyle: s.bandStyles[k],
      });
      plots.push({
        key: 'l' + k,
        name: 'Lower ' + (k + 1),
  ...
  fills(p, ctx, s) {
    const fills = [];
    for (let k = 0; k < 3; k++)
      if (s.shows[k] && s.fillsOn[k])
        fills.push({ top: 'u' + k, bottom: 'l' + k, color: withAlpha(s.bandCols[k], 0.06) });
    return fills;
  },
  ...
  step(i, sh, p, ctx, s) {
    const t = sh.time[i],
      key = s.keyOf(t);
    if (s.prevKey === null || key !== s.prevKey) {
      s.sPV = 0;
      s.sV = 0;
      s.sP2V = 0;
      s.prevKey = key;
    } // anchor reset
    const price = Studies.priceOf(
      { open: sh.open[i], high: sh.high[i], low: sh.low[i], close: sh.close[i], volume: sh.volume[i] },
      s.src,
    );
    const vol = sh.volume[i] || 0;
    s.sPV += price * vol;
    s.sV += vol;
    s.sP2V += price * price * vol;
    ...
    const cutoffDay =
      p.recentOnly === true && sh.n
        ? s.dayIdx(sh.time[sh.n - 1]) - (Math.max(1, p.recentDays | 0 || 5) - 1)
        : -Infinity;
    ...
    for (let k = 0; k < 3; k++)
      if (s.shows[k]) {
        row['u' + k] = { value: vwap + basis * s.mult[k] };
        row['l' + k] = { value: vwap - basis * s.mult[k] };
      }
    return row;
  },
});
