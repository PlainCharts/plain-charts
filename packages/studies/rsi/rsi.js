...
Studies.register({
  ...
  init(p) {
    return {
      len: Math.max(1, p.length | 0),
      src: p.source,
      avgGain: 0,
      avgLoss: 0,
      /** @type {number | null} */ prevSrc: null,
    };
  },
  plots() {
    return [
  ...
  fills() {
    return [{ top: 'upper', bottom: 'lower', color: 'rgba(120,123,134,0.07)' }];
  },
  scale() {
  ...
  step(i, sh, p, ctx, s) {
    ...
    if (i === 0) {
      s.prevSrc = price;
      return row;
    } // no change on the first bar; RSI has no value yet
    const ch = price - s.prevSrc;
    s.prevSrc = price;
    const gain = ch > 0 ? ch : 0,
      loss = ch < 0 ? -ch : 0;
    const len = s.len;
    if (i <= len) {
      ...
      if (i === len) {
        s.avgGain /= len;
        s.avgLoss /= len;
        const rs = s.avgLoss === 0 ? Infinity : s.avgGain / s.avgLoss;
        row.rsi = { value: 100 - 100 / (1 + rs) };
      }
    } else {
      s.avgGain = (s.avgGain * (len - 1) + gain) / len;
      s.avgLoss = (s.avgLoss * (len - 1) + loss) / len;
      const rs = s.avgLoss === 0 ? Infinity : s.avgGain / s.avgLoss;
      row.rsi = { value: 100 - 100 / (1 + rs) };
    }
    return row;
