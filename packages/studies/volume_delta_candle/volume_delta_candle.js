    ...
    bars.forEach((b, i) => {
      ...
      if (sb && sb.length) {
        for (const s of sb) {
          ...
          if (buy) upVol += s.volume || 0;
          else dnVol += s.volume || 0;
        }
      } else {
        ...
        if (!vol) return;
        if (b.close != null && b.open != null ? b.close >= b.open : true) upVol = vol;
        else dnVol = vol;
      }
    ...
    return [
      {
        ...
        legend: false,
        priceScaleId: 'voldelta',
        scaleMargins: { top: s.top, bottom: 0 },
      },
    ];
  },
  ...
  step(i, sh, p, ctx, s) {
    ...
    if (sb && sb.length) {
      for (const x of sb) {
        ...
        if (buy) upVol += x.volume || 0;
        else dnVol += x.volume || 0;
      }
    } else {
      // no sub-bars yet (forming / most-recent bars): approximate from the chart bar's own direction
      const vol = sh.volume[i] || 0;
      if (!vol) return null; // matches calc's per-bar `return` (no point)
      if (sh.close[i] != null && sh.open[i] != null ? sh.close[i] >= sh.open[i] : true) upVol = vol;
      else dnVol = vol;
    }
    const aggregate = Math.max(upVol, dnVol); // aggressor total = full bar height
    const net = Math.abs(upVol - dnVol); // net delta = solid part; opposing = aggregate - net
