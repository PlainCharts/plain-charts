...
// machinery (which weekday a boundary opens, trading-day test, next drawn boundary, week-start weekday,
// the visible trading-day window) is computed once per calc and handed to each feature.
/** @typedef {{ now:number, off:number, tod:number, b0:number, targetWd:number, weeks:number, dayWindow:number,
...
function boundaryContext(bars, p) {
  ...
  return { now, off, tod, b0, targetWd, weeks, dayWindow, fwdWeeks, ownsWeekday, isTrading, nextBoundary };
}

// ---- Mitigation (Hide Mitigated H/L) -----------------------------------------------------------
// A level is "mitigated" once a LATER bar trades through it -- a high taken out from above, a low from
// below. Suffix arrays give the max high / min low over all bars AFTER an index, so a level formed by a
// group ending at endIdx is mitigated when sufHi[endIdx+1] >= its high (or sufLo[endIdx+1] <= its low).
// When the toggle is off, the predicates always say no and nothing is hidden.
/** @param {StudyBar[]} bars @param {Record<string, any>} p
 * @returns {{ hiMitigated:(level:number, endIdx:number)=>boolean, loMitigated:(level:number, endIdx:number)=>boolean }} */
function mitigation(bars, p) {
...
// ---- Separators + day labels (the foundation feature) ------------------------------------------
// Backward pass over the trading-day window (one boundary = one trading day), then the forward
// projection of the next few weeks. Draws day/week vlines + the M T W T F letter strip and returns
...
/** @param {StudyBar[]} bars @param {Record<string, any>} p @param {Boundary} B @param {number} leftEdge
 * @param {ReturnType<typeof mitigation>} M @param {any[]} shapes */
function weeklyHL(bars, p, B, leftEdge, M, shapes) {
  const { off, tod, targetWd } = B;
  /** @param {number} t */
...
/** @param {StudyBar[]} bars @param {Record<string, any>} p @param {Boundary} B @param {any[]} shapes */
...
/** @param {StudyBar[]} bars @param {Record<string, any>} p @param {Boundary} B @param {any[]} shapes */
function weeklyOpenLines(bars, p, B, shapes) {
  const { off, tod, targetWd, weeks } = B;
  /** @param {number} t */
...
/** @param {StudyBar[]} bars @param {Record<string, any>} p @param {{ tickSize?:number }|undefined} ctx
 * @param {Boundary} B @param {any[]} shapes */
function awrTable(bars, p, ctx, B, shapes) {
  const { off, tod, targetWd } = B;
  /** @param {number} t */
...
Studies.register({
  ...
  inputs,
  /**
   * The pipeline: one boundary context, then each enabled feature emits its shapes. Every feature is a
   * named top-level function above; calc only sequences them.
   * @param {StudyBar[]} bars
  ...
  calc(bars, p, ctx) {
    ...
    if (p.enableDailyOpen) dailyOpenLines(bars, p, B, shapes);
    if (p.enableWeeklyOpen) weeklyOpenLines(bars, p, B, shapes);
    if (p.enableAWR) awrTable(bars, p, ctx, B, shapes);
