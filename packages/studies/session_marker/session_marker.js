...
const LAYOUT = [
  { key: 'Layout 1', name: 'Layout 1' },
  { key: 'Layout 2', name: 'Layout 2' },
];
...
const parseSession = (s) => {
  const m = /^(\d{2})(\d{2})-(\d{2})(\d{2})$/.exec(String(s || '').trim());
  return m ? { start: +m[1] * 3600 + +m[2] * 60, end: +m[3] * 3600 + +m[4] * 60 } : null;
};
...
SDEF.forEach((d, i) => {
  ...
  inputs.push(
    ...
    // StudyInput.name's declared `string`, so cast the callback for this one field.
    {
      ...
      tab: 'Quick',
      name: /** @type {any} */ (
        (/** @type {Record<string, any>} */ p) => (p[`name${n}`] ? `Session ${n}: ${p[`name${n}`]}` : `Session ${n}`)
      ),
    },
  );
});
...
SDEF.forEach((d, i) => {
  const n = i + 1,
    tab = `S${n}`;
  const notBands = (/** @type {Record<string, any>} */ P) => P[`viz${n}`] !== 'Bands',
    labOn = `showlab${n}`;
  inputs.push(
    ...
    { key: `viz${n}`, type: 'select', name: 'Display', options: VIZ, default: 'Top & Bottom', tab, inline: `col${n}` },
    {
      ...
      inline: `col${n}`,
      enableWhen: notBands,
      stroke: { width: `bwidth${n}`, lineStyle: `bstyle${n}` },
    },
    { key: `showbg${n}`, type: 'bool', name: 'Bkgnd', default: true, tab, inline: `col${n}` },
    {
      ...
      name: '',
      noLabel: true,
      default: withAlpha(d.color, 0.15),
      tab,
      inline: `col${n}`,
      enableWhen: `showbg${n}`,
    },
    ...
    { key: labOn, type: 'bool', name: 'Show labels', default: true, tab, inline: `lbl${n}` },
    {
      ...
      inline: `lbl${n}`,
      enableWhen: labOn,
      text: { size: `lsize${n}`, bold: `lbold${n}`, italic: `litalic${n}` },
    },
...
Studies.register({
  ...
  calc(bars, p, ctx) {
    ...
    for (let n = 1; n <= 6; n++) {
      if (!p[`en${n}`]) continue;
      const ses = parseSession(p[`session${n}`]);
      if (!ses) continue;
      const off = tzOff(p[`tz${n}`]);
      const viz = p[`viz${n}`] || 'Top & Bottom';
      const bcol = p[`border${n}`] || '#787b86',
        lcol = p[`labelc${n}`] || bcol;
      const showBg = p[`showbg${n}`] !== false,
        bg = p[`bg${n}`];
      const dash = p[`bstyle${n}`] || 'solid',
        bw = p[`bwidth${n}`] | 0 || 2;
      /** @param {number} t */
      const inSes = (t) => {
        const x = (((t + off) % 86400) + 86400) % 86400;
        return ses.start <= ses.end ? x >= ses.start && x < ses.end : x >= ses.start || x < ses.end;
      };
      ...
      const emit = (r) => {
        ...
        if (viz === 'Bands') {
          ...
          shapes.push({
            ...
            top: hi,
            bottom: lo,
            color: showBg ? bg : null,
            borderColor: bcol,
            borderWidth: bw,
            lineStyle: dash,
          });
        } else {
          // Top & Bottom: bg box (no border) + a line at the high and the low
          if (showBg)
            shapes.push({ type: 'box', from, to, top: hi, bottom: lo, color: bg, borderColor: null, borderWidth: 0 });
          shapes.push({
            marks: [
              {
                stroke: bcol,
        ...
        const lines = [];
        const nm = p[`name${n}`] || '',
          priceStr = Number(r.c).toFixed(decimals);
        ...
        shapes.push({
          type: 'label',
          time: lx,
          price: pos === 'Above' ? hi : lo,
          text: txt,
          color: lcol,
          hAlign: align === 'Left' ? 'left' : align === 'Right' ? 'right' : 'center',
          vAlign: pos === 'Above' ? 'bottom' : 'top',
          size: lsz,
          bold: p[`lbold${n}`],
          italic: p[`litalic${n}`],
        });
