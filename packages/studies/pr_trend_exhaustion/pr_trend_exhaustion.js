...
function withAlpha(col, a) {
  ...
  if (rm) {
    const [r, g, b] = rm[1].split(',').map((s) => parseFloat(s));
...
Studies.register({
  ...
  inputs: [
    ...
    // the %R lines (color + width + dash in one swatch)
    {
      ...
      type: 'stroke',
      tab: 'Display',
      default: { color: '#d1d4dc', width: 1, style: 'solid' },
    },
    {
      key: 'slowLine',
      name: 'Period 2 line',
      type: 'stroke',
      tab: 'Display',
      default: { color: '#8baeff', width: 1, style: 'solid' },
    },
    {
      key: 'line3',
      name: 'Period 3 line',
      type: 'stroke',
      tab: 'Display',
      default: { color: '#b39ddb', width: 1, style: 'solid' },
    },
    ...
    // inherit Top's stroke (upper zone) and Bottom's stroke (lower zone). Top/Bottom bound the range.
    {
      ...
      min: -100,
      max: 0,
      default: { value: 0, show: true, color: 'rgba(202,0,23,0.55)', width: 1, style: 'solid' },
    },
    {
      key: 'lvlMid',
      name: 'Middle',
    ...
    },
    {
      ...
      min: -100,
      max: 0,
      default: { value: -100, show: true, color: 'rgba(36,102,167,0.6)', width: 1, style: 'solid' },
    },
  ],
  calc(bars, p) {
    const vols = bars.map((b) => b.volume);
    const on1 = p.show1 !== false,
      on2 = p.show2 !== false,
      on3 = p.show3 !== false;
    ...
    let p3 = null;
    if (on1) {
      fast = percentR(bars, p.fastLength, p.source);
      if (p.fastSmoothLen > 1) fast = movingAverage(fast, p.smoothType, p.fastSmoothLen, vols);
    }
    if (on2) {
      slow = percentR(bars, p.slowLength, p.source);
      if (p.slowSmoothLen > 1) slow = movingAverage(slow, p.smoothType, p.slowSmoothLen, vols);
    }
    if (on3) {
      p3 = percentR(bars, p.period3, p.source);
      if (p.p3SmoothLen > 1) p3 = movingAverage(p3, p.smoothType, p.p3SmoothLen, vols);
    }
    /** @type {StudyPlotPoint[]} */
    const fastData = [];
    /** @type {StudyPlotPoint[]} */
    ...
    for (let i = 0; i < bars.length; i++) {
      if (on1 && /** @type {(number | null)[]} */ (fast)[i] != null)
        fastData.push({
          time: bars[i].time,
          value: /** @type {number} */ (/** @type {(number | null)[]} */ (fast)[i]),
        });
      if (on2 && /** @type {(number | null)[]} */ (slow)[i] != null)
        slowData.push({
          time: bars[i].time,
          value: /** @type {number} */ (/** @type {(number | null)[]} */ (slow)[i]),
        });
      if (on3 && /** @type {(number | null)[]} */ (p3)[i] != null)
        p3Data.push({ time: bars[i].time, value: /** @type {number} */ (/** @type {(number | null)[]} */ (p3)[i]) });
    }

    // guide lines as self-styled hline shapes (non-overlay -> drawn on the study's own sub-pane)
    /** @type {any[]} */
    const shapes = [];
    const hline = (/** @type {number} */ price, /** @type {any} */ s) =>
      shapes.push({ type: 'hline', price, color: s.color, width: s.width || 1, lineStyle: s.style });
    const guide = (/** @type {any} */ lv) => {
      if (lv && typeof lv === 'object' && lv.show !== false && lv.value != null) hline(lv.value, lv);
    };
    ...
    const everyAt = (/** @type {number} */ i, /** @type {(v: number) => boolean} */ cmp) =>
      active.every((s) => s[i] != null && cmp(/** @type {number} */ (s[i])));
    const overbought = signalOn ? bars.map((_, i) => everyAt(i, (v) => v >= upper)) : [];
    const oversold = signalOn ? bars.map((_, i) => everyAt(i, (v) => v <= lower)) : [];
    const hot = p.hotColor || '#CA0017',
      cold = p.coldColor || '#2466A7';
    ...
    if (p.fillZones && th > 0) {
      const band = (/** @type {number} */ hiP, /** @type {number} */ loP, /** @type {string} */ color) =>
        shapes.push({
          marks: [
            {
              back: true,
      ...
      const topColor = (p.lvlTop && p.lvlTop.color) || hot,
        botColor = (p.lvlBot && p.lvlBot.color) || cold;
    ...
    const box = (
      ...
      const L = a,
        R = ended ? b + 1 : b; // window opens on the first exhaustion candle; active run -> right edge = current bar
      shapes.push({
        overlay: true,
        type: 'box',
        from: bars[L].time,
    ...
    const track = (
      ...
      const ended = b < bars.length - 1;
      const from = bars[a].time,
        to = bars[ended ? b + 1 : b].time;
      const vp = up ? 0 : 1,
        s = up ? 1 : -1,
        y0 = s * 3,
        y1 = s * 10; // ~7px strip at the edge
      ...
      if (ended) {
        marks.push({
          ...
          fill: color,
          path: [
            { t: to, vp, dx: -5, dy: s * 12 },
            { t: to, vp, dx: 5, dy: s * 12 },
            { t: to, vp, dx: 0, dy: s * 22 },
          ],
        });
      }
      shapes.push({ marks });
    ...
    const obRuns = runs(overbought),
      osRuns = runs(oversold);
    obRuns.forEach(([a, b]) => track(a, b, hot, true)); // sub-pane markers (always shown)
    osRuns.forEach(([a, b]) => track(a, b, cold, false));
    if (p.drawBoxes !== false) {
      // price-pane boxes (capped to most recent N)
    ...
    /** @type {any[]} */
    const fills = [];
    if (p.shading && on1 && on2) {
      // the gradient fills BETWEEN the Period 1 & 2 lines -- both must be on
      // deep at the extreme, fading out 30 points in (the pine's look): 0 -> -30 (hot), -70 -> -100 (cold)
      fills.push(
        {
          top: 'fast',
          bottom: 'slow',
          gradient: { at: [topVal, topVal - 30], colors: [withAlpha(hot, 0.5), withAlpha(hot, 0)] },
        },
        {
          top: 'fast',
          bottom: 'slow',
          gradient: { at: [botVal + 30, botVal], colors: [withAlpha(cold, 0), withAlpha(cold, 0.5)] },
        },
      );
    }
    ...
    if (on2)
      plots.push({
        ...
        name: 'Period 2 %R',
        type: 'line',
        color: sl.color || '#8baeff',
        lineWidth: sl.width || 1,
        lineStyle: sl.style || 'solid',
        data: slowData,
      });
    if (on3)
      plots.push({
        ...
        name: 'Period 3 %R',
        type: 'line',
        color: l3.color || '#b39ddb',
        lineWidth: l3.width || 1,
        lineStyle: l3.style || 'solid',
        data: p3Data,
      });
    if (on1)
      plots.push({
        ...
        name: 'Period 1 %R',
        type: 'line',
        color: fl.color || '#d1d4dc',
        lineWidth: fl.width || 1,
        lineStyle: fl.style || 'solid',
        data: fastData,
      });
    return {
      plots,
