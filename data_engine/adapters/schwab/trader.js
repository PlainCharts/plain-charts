...
export function stopTradePolling() { tradePollers.forEach((t) => clearInterval(t)); tradePollers.clear(); }
...
const SCHWAB_OSTATUS = { WORKING: 'working', QUEUED: 'working', ACCEPTED: 'working', NEW: 'working', PENDING_ACTIVATION: 'working', AWAITING_PARENT_ORDER: 'working', FILLED: 'filled', CANCELED: 'cancelled', REJECTED: 'rejected', EXPIRED: 'expired', PENDING_CANCEL: 'in_cancel', PENDING_REPLACE: 'in_modify', REPLACED: 'replaced' };
/** @type {Record<string, string>} */
...
function mapPosition(p, acctId) {
  const long = Number((p && p.longQuantity) || 0), short = Number((p && p.shortQuantity) || 0);
  const net = long - short;
  return { symbol: (p && p.instrument && p.instrument.symbol) || '', qty: Math.abs(net), side: net < 0 ? 'short' : 'long', avgPrice: (p && p.averagePrice != null) ? Number(p.averagePrice) : null, accountId: String(acctId || '') };
}
...
function mapOrder(o, acctId) {
  ...
  return { id: String((o && o.orderId != null) ? o.orderId : ''), symbol: (leg.instrument && leg.instrument.symbol) || '', side: /^SELL/.test(instr) ? 'sell' : 'buy', type: SCHWAB_OTYPE[o && o.orderType] || 'market', qty: (o && o.quantity != null) ? Number(o.quantity) : null, price: (o && o.price != null) ? Number(o.price) : null, limitPrice: (o && o.price != null) ? Number(o.price) : null, stopPrice: (o && o.stopPrice != null) ? Number(o.stopPrice) : null, status: SCHWAB_OSTATUS[o && o.status] || 'working', time: (o && o.enteredTime) ? Date.parse(o.enteredTime) : null, updateTime: (o && o.closeTime) ? Date.parse(o.closeTime) : null, accountId: String(acctId || '') };
}
...
export const trader = {
  ...
  /** @param {(a: any) => void} [cb] */
  getAccount(cb) {
    j('/api/schwab/trader/accounts').then((data) => {
      emitRaw('schwab', 'account', data);   // raw /accounts response
      if (!Array.isArray(data)) { mdFail('account', data && data.error); return cb && cb({ error: (data && data.error) || 'no account' }); }
  ...
  getPositions(cb) {
    j('/api/schwab/trader/accounts?fields=positions').then((data) => {
      emitRaw('schwab', 'positions', data);
      if (!Array.isArray(data)) return cb && cb([]);
      /** @type {any[]} */
      const out = [];
      for (const entry of data) { const sa = entry.securitiesAccount || {}; for (const p of (sa.positions || [])) out.push(mapPosition(p, sa.accountNumber)); }
      cb && cb(out);
    }).catch(() => cb && cb([]));
  },
  ...
  getOrders(cb) {
    j('/api/schwab/trader/orders?status=WORKING&maxResults=200').then((data) => {
  ...
  getHistory({ fromMs, toMs }, cb) {
    ...
    j('/api/schwab/trader/orders?fromEnteredTime=' + encodeURIComponent(from) + '&toEnteredTime=' + encodeURIComponent(to) + '&maxResults=1000').then((data) => {
  ...
  subscribeTrade(cb) {
    ...
    const emitChange = (k, obj, ev) => { const s = JSON.stringify(obj); if (last[k] !== s) { last[k] = s; cb(ev); } };
    const tick = () => {
      j('/api/schwab/trader/accounts?fields=positions').then((data) => {
        ...
        for (const entry of data) {
          ...
          emitChange('acct:' + (sa.accountNumber || ''), mapAccount(entry), { kind: 'account', account: mapAccount(entry) });
          for (const p of (sa.positions || [])) { const pos = mapPosition(p, sa.accountNumber); emitChange('pos:' + pos.symbol, pos, { kind: 'position', position: pos }); }
        }
      }).catch(() => {});
      j('/api/schwab/trader/orders?status=WORKING&maxResults=200').then((data) => {
        if (!Array.isArray(data)) return;
        for (const o of data) { const ord = mapOrder(o, o.accountNumber); emitChange('ord:' + ord.id, ord, { kind: 'order', order: ord }); }
      }).catch(() => {});
    };
    tick();
