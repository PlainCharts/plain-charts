...
function pollSubscribe(id, cb) {
  ...
  if (!s) {
    ...
    const poll = async () => {
      ...
      if (out.bid != null || out.ask != null || out.last != null) /** @type {QuotePoller} */ (s).cbs.forEach((c) => c(out));
    };
    poll();
...
function pollUnsubscribe(id, cb) {
  ...
  if (!s) return;
  s.cbs.delete(cb);
  if (!s.cbs.size) { clearInterval(s.timer); quotePollers.delete(id); }
}
...
const adapter = {
  ...
  form: [
    ...
    {
      key: 'authorize', type: 'action', label: 'Authorization', button: 'Authorize with Schwab',
      ...
      run: async (account, ui) => {
        ...
        if (!url) return ui.status('Could not build the authorize URL — check the App Key and that the server is running.', 'err');
        ui.openUrl(url);
        ui.status('A broker tab opened. Log in & approve, then paste that page’s URL below.');
        const pasted = await ui.promptInput({ placeholder: 'Paste the https://127.0.0.1/?code=… URL here', submit: 'Submit code' });
        if (!pasted) return;
        ui.status('exchanging…');
        const r = await adapter.exchangeAuth(pasted, account);
        ui.status(r && r.ok ? '✓ authorized — Save the account, then Connect' : '✗ ' + ((r && r.error) || 'failed'), r && r.ok ? 'ok' : 'err');
      },
      status: async () => { const st = await adapter.authStatus().catch(() => ({})); return { ok: !!(st && st.authorized), text: st && st.authorized ? '✓ authorized (token ~' + st.expiresInSec + 's)' : 'not authorized yet' }; },
    },
  ],
  ...
  async connect(account) {
    ...
    if (account && account.clientId) {
      await j('/api/schwab/creds', {
        ...
        body: JSON.stringify({ clientId: account.clientId, clientSecret: account.clientSecret, redirectUri: account.redirectUri }),
      }).catch(() => {});
    }
    ...
    if (!st.authorized) {
      ...
      const msg = (st.reason === 'transient'
        ? 'Schwab: could not reach the token service — will retry. If this persists, re-Authorize.'
        : 'Schwab session expired (the refresh token lasts 7 days). Open Connect → Authorize with Schwab to re-grant market-data access.') + detail;
  ...
  disconnect() {
    ...
    stopTradePolling();
    [...barStops].forEach((h) => { try { h.stop(); } catch (_) {} });
    try { streamer.close(); } catch (_) {}
  ...
  resolveSymbol(symbol, cb) {
    j('/api/schwab/md/quotes?symbols=' + encodeURIComponent(symbol)).then((data) => {
      emitRaw('schwab', 'quote-rest', data);   // raw /quotes snapshot (resolveSymbol)
      if (!data || data.error) { mdFail('resolve ' + symbol, data && data.error); cb(null, { status: (data && data.error) || 'error' }); return; }
      const key = data[symbol] ? symbol : Object.keys(data)[0];
      const row = key && data[key];
      if (!row) { cb(null, { status: 'not found' }); return; }
      const q = row.quote || {};
      ...
      const tickSize = realTick != null ? realTick : Math.pow(10, -decimalsOf(q.bidPrice, q.askPrice, q.lastPrice, q.closePrice));
      /** @type {Instrument} */
      ...
      if (ref.futureMultiplier != null && Number.isFinite(ref.futureMultiplier)) inst.contractSize = ref.futureMultiplier;
      cb(inst);
    }).catch(() => cb(null, {}));
  },
  ...
  subscribeBars({ id, tf, fromMs }, cb) {
    ...
    if (streamer.available()) {
      ...
      timer = setInterval(async () => {                    // reconcile official OHLC
        if (stopped) return;
        const { bars } = await fetchCandles(id, tf, Date.now() - barSecs * 3000, Date.now());
        if (!stopped && bars.length) { forming = { ...bars[bars.length - 1] }; cb({ bars, complete: true }); }
      }, 30000);
    } else {
    ...
    const handle = { stop: () => { stopped = true; if (timer) clearInterval(timer); if (streamCb) streamer.unsubscribe(id, streamCb); barStops.delete(handle); } };
    barStops.add(handle);
    return handle;
  },
  ...
  async getMarketHours({ id } = {}, cb) {
    try {
      ...
      if (sym[0] === '/') {   // ---- futures ----
        ...
        /** @type {string[]} */
        const fdates = []; for (let i = -1; i <= 4; i++) fdates.push(dstr(i));   // consecutive -- segments are sparse in holiday weeks
        const reps = await Promise.all(fdates.map((date) =>
          j('/api/schwab/md/markets?markets=future&date=' + date).then((d) => { emitRaw('schwab', 'hours', d); return d; }).catch(() => null)));
        /** @type {Set<string>} */
        const seen = new Set();
        /** @type {SessionDay[]} */
        const days = [];
        for (const d of reps) {
          ...
          for (const seg of segs || []) {
            const open = ms(seg.start), close = ms(seg.end);
            if (open == null || close == null || close - open < 30 * 60000) continue;   // drop sub-30m slivers
            const key = open + '|' + close; if (seen.has(key)) continue; seen.add(key);
      ...
      const reports = await Promise.all(dates.map((date) =>
        j('/api/schwab/md/markets?markets=equity&date=' + date).then((d) => { emitRaw('schwab', 'hours', d); return { date, d }; }).catch(() => ({ date, d: null }))));
      /** @type {SessionDay[]} */
      const days = [];
      for (const { date, d } of reports) {
        ...
        if (!sh) continue;
        const pre = sh.preMarket && sh.preMarket[0], reg = sh.regularMarket && sh.regularMarket[0], post = sh.postMarket && sh.postMarket[0];
        const open = ms(pre ? pre.start : reg && reg.start), close = ms(post ? post.end : reg && reg.end);
        if (open == null || close == null) continue;
        days.push({ tradeDate: Date.parse(date + 'T00:00:00Z'), open, close,
          preOpen: ms(pre && pre.start), postClose: ms(post && post.end), rthOpen: ms(reg && reg.start), rthClose: ms(reg && reg.end) });
      }
      cb && cb({ days: days.sort((a, z) => a.open - z.open) });
  ...
  searchSymbols(query, cb) {
    ...
    j('/api/schwab/md/instruments?symbol=' + encodeURIComponent(q) + '&projection=symbol-search').then((data) => {
      emitRaw('schwab', 'search', data);   // raw /instruments response
      if (!data || data.error) { mdFail('symbol search', data && data.error); return cb([]); }
      const ins = Array.isArray(data.instruments) ? data.instruments : [];
      cb(ins.map((/** @type {any} */ i) => ({ symbol: i.symbol, name: i.description || '', category: i.assetType || '' })));
    }).catch((e) => { mdFail('symbol search', String((e && e.message) || e)); cb([]); });
  },
  ...
  async authStatus() { return j('/api/schwab/auth/status').catch(() => ({})); },
  /** @param {any} pasted @param {any} account */
  async exchangeAuth(pasted, account) {
    const v = (pasted || '').trim();
    const body = {
      clientId: account.clientId, clientSecret: account.clientSecret, redirectUri: account.redirectUri,
