...
let rbuf = '',
  ...
  connected = false;
let listening = false,
  boundKey = ''; // is `server` bound, and to which 'host:port' -> decide reuse vs rebind
...
function runGetBars(
  ...
  return new Promise((resolve) => {
    barsQ.push({ symbol, tf, fromSec, toSec, resolve });
    pumpBars();
  });
}
function pumpBars() {
  ...
  let settled = false,
    timer = /** @type {any} */ (null);
  const finish = (/** @type {any} */ r) => {
    if (settled) return;
    settled = true;
    if (timer) clearTimeout(timer);
    rpc.delete(rid);
    barsBusy = false;
    if (pendingBars && pendingBars.id === rid) pendingBars = null;
    job.resolve(r);
    pumpBars();
  };
...
function handleConn(s) {
  ...
  if (sock && verified) {
    log(
      'MT5: second EA connection rejected — one is already active (is the bridge EA attached to more than one chart?)',
      true,
    );
    try {
      s.destroy();
  ...
  } // unverified half-open predecessor -> replace
  sock = s;
  rbuf = '';
  verified = false;
  s.setEncoding('utf8');
  ...
  s.on('timeout', () => {
    if (sock === s) {
      log('MT5 EA link idle >12s — closing the dead socket so the EA can reconnect', true);
      try {
        s.destroy();
      } catch (_) {}
    }
  ...
  s.on('data', (/** @type {string} */ chunk) => {
    ...
    while ((i = rbuf.indexOf('\n')) >= 0) {
      const line = rbuf.slice(0, i).trim();
      rbuf = rbuf.slice(i + 1);
      if (!line) continue;
      let m;
      try {
      ...
      if (!verified) {
        ...
        if (cfg.login && String(m.login) !== String(cfg.login)) {
          try {
            s.destroy();
          } catch (_) {}
          return;
        ...
        verified = true;
        onUp(m); // -> connected + setConn + bus.emit('logon') + route the account
        continue;
      }
      route(m);
    }
...
function startServer() {
  if (!net) {
    setConn('not connected (desktop only)', '#cc0');
    log('MT5: raw TCP unavailable (browser/solo mode) — desktop only.', true);
    return;
  }
  ...
  if (server && listening && boundKey === key) {
    log('MT5 bridge already listening ' + key + ' — reusing');
    return;
  }
  stopServer();
  bindServer(key, 0);
...
function bindServer(key, attempt) {
  ...
  s.on('error', (/** @type {any} */ e) => {
    ...
    if (e && e.code === 'EADDRINUSE' && attempt < 10) {
      ...
      retryTimer = setTimeout(() => {
        if (server === s) bindServer(key, attempt + 1);
      }, 300);
      return;
    }
    listening = false;
    connected = false;
    setConn('not connected', '#c33');
    log(
      'MT5 listen error: ' + (e && e.message) + (e && e.code === 'EADDRINUSE' ? ' (port ' + cfg.port + ' in use)' : ''),
      true,
    );
  });
  s.listen(cfg.port, cfg.host, () => {
    listening = true;
    boundKey = key;
    log('MT5 bridge listening ' + key + ' — waiting for the EA to connect…');
  });
}
function stopServer() {
  if (retryTimer) {
...
function onUp(m) {
  ...
  log('MT5 EA connected (login ' + m.login + ')');
  route(m);
  bus.emit('logon');
}
...
const histOrder = (/** @type {any} */ m) => ({
  ...
  avgFillPrice: Number(m.price),
  status: 'filled',
  tif: 'day',
  time: secToMs(m.time),
  accountId: acct && String(acct.login),
  commission: foldComm(m),
  commissionCurrency: cur(),
  realizedPnl: realizedOf(m),
  positionId: posIdOf(m),
});
const orderOut = (/** @type {any} */ o) => {
  const si = symInfo.get(o.symbol);
  return {
    id: String(o.ticket),
    symbol: o.symbol,
    side: o.side,
    ...
    limitPrice: o.order_type === 'limit' ? Number(o.price) : null,
    stopPrice: o.order_type === 'stop' ? Number(o.price) : null,
    tif: 'day',
    status: 'working',
    time: secToMs(o.time),
    accountId: acct && String(acct.login),
    stopLoss: Number(o.sl) > 0 ? Number(o.sl) : null,
    takeProfit: Number(o.tp) > 0 ? Number(o.tp) : null,
    priceDecimals: si ? Number(si.digits) : null,
    tickSize: si ? Number(si.tick_size) : null,
  };
}; // MT5 pending orders carry native SL/TP; priceDecimals/tickSize from the resolved symbol so prices format at full precision; time = ORDER_TIME_SETUP (null until the EA sends it)
const accountOut = () =>
  acct
    ? {
        accountId: String(acct.login),
        currency: acct.currency,
        balance: Number(acct.balance),
        equity: Number(acct.equity),
        marginUsed: Number(acct.margin),
        marginAvailable: Number(acct.free_margin),
        unrealizedPL: Number(acct.equity) - Number(acct.balance),
        hedging: !!acct.hedging,
      }
    : { accountId: 'default' };
...
const RETCODE = {
  10004: 'requote',
  10006: 'rejected by broker',
  10013: 'invalid request',
  10014: 'invalid volume',
  10015: 'invalid price',
  10016: 'invalid stops (too close to price)',
  ...
  10040: 'too many open positions',
  10041: 'long only',
  10042: 'short only',
  10043: 'close only',
  10044: 'FIFO close required',
  10045: 'hedging prohibited (opposite position open)',
  10046: 'hedging prohibited (opposite position open)',
};
...
function netPosition(symbol) {
  ...
  const si = symInfo.get(symbol),
    decimals = si ? si.digits : null,
    tickSize = si ? si.tick_size : null,
    tickValue = si ? si.tick_value : null;
  if (!lots.length)
    return {
      symbol,
      qty: 0,
  ...
  for (const p of lots) {
    ...
    if (p.side === 'buy') {
      netV += v;
      bV += v;
      bC += v * Number(p.open_price);
    } else {
      netV -= v;
      sV += v;
      sC += v * Number(p.open_price);
    }
  }
  const side = netV >= 0 ? 'long' : 'short',
    qty = Math.abs(netV),
    avgPrice = side === 'long' ? (bV ? bC / bV : null) : sV ? sC / sV : null;
  ...
  if (single) {
    pos.ticket = String(single.ticket);
    if (Number(single.sl) > 0) pos.stopLoss = Number(single.sl);
    if (Number(single.tp) > 0) pos.takeProfit = Number(single.tp);
  }
  return pos;
}
...
function lotEvent(m) {
  ...
  return {
    ...
    lot: {
      ...
      symbol: m.symbol,
      side: m.side === 'sell' ? 'short' : 'long',
      qty: Number(m.volume),
      avgPrice: Number(m.open_price),
      price: Number(m.cur_price),
      stopLoss: Number(m.sl) > 0 ? Number(m.sl) : null,
      takeProfit: Number(m.tp) > 0 ? Number(m.tp) : null,
      unrealizedPnl: m.profit != null ? Number(m.profit) : null,
      accountId: acct && String(acct.login),
      priceDecimals: si ? si.digits : null,
      tickSize: si ? si.tick_size : null,
      tickValue: si ? si.tick_value : null,
    },
  };
...
const emit = (/** @type {any} */ ev) => {
  for (const cb of tradeCbs) {
    try {
      cb(ev);
    } catch (e) {
      log('trade cb error: ' + (e && /** @type {any} */ (e).message), true);
    }
  }
};
...
function route(m) {
  switch (m.type) {
    case 'account':
      acct = m;
      emit({ kind: 'account', account: accountOut() });
      break;
    case 'position':
      posMap.set(String(m.ticket), m);
      emit(lotEvent(m));
      emit({ kind: 'position', position: netPosition(m.symbol) });
      break; // lot FIRST so the feed sees it's hedged and skips the net journal line
    case 'position_closed': {
      const p = posMap.get(String(m.ticket));
      posMap.delete(String(m.ticket));
      if (p) {
        emit({ kind: 'position', position: netPosition(p.symbol) });
        emit({
          kind: 'positionLot',
          lot: { ticket: String(m.ticket), symbol: p.symbol, side: p.side === 'sell' ? 'short' : 'long', qty: 0 },
        });
      }
      break;
    }
    case 'order': {
      const tk = String(m.ticket);
      orderMap.set(tk, m);
      const oo = orderOut(m),
        sig = JSON.stringify(oo);
      if (orderSig.get(tk) !== sig) {
        orderSig.set(tk, sig);
        emit({ kind: 'order', order: oo });
      }
      break;
    } // dedupe the 500ms re-push: emit only when the order actually changed (place/modify)
    case 'order_removed': {
      const tk = String(m.ticket);
      const o = orderMap.get(tk);
      orderMap.delete(tk);
      orderSig.delete(tk);
      if (o) emit({ kind: 'order', order: { ...orderOut(o), status: 'cancelled' } });
      break;
    }
    case 'fill': {
      if (m.historical) {
        if (pendingHist) pendingHist.orders.push(histOrder(m));
        break;
      }
      emit({
        kind: 'fill',
    ...
    case 'quote': {
      const subs = quoteSubs.get(m.symbol);
      if (!subs) break;
      const bid = Number(m.bid),
        ask = Number(m.ask);
      const q = {};
      if (isFinite(bid)) q.bid = bid;
      if (isFinite(ask)) q.ask = ask;
      if (isFinite(bid) && isFinite(ask)) q.last = (bid + ask) / 2;
      subs.forEach((/** @type {any} */ cb) => cb(q));
      break;
    }
    case 'depth': {
      const subs = depthSubs.get(m.symbol);
      if (subs) subs.forEach((/** @type {any} */ cb) => cb({ bids: m.bids || [], asks: m.asks || [] }));
      break;
    }
    case 'bar': {
      if (m.live) {
        const key = m.symbol + '|' + m.tf,
          subs = barSubs.get(key);
        if (!subs) break;
        const T = Number(m.time),
          step = tfStepSec(m.tf),
          cur = barCursor.get(key) || 0;
        if (cur && step && T > cur + step && !barFilling.has(key)) {
          ...
          runGetBars(m.symbol, m.tf, cur + step, T).then((r) => {
            barFilling.delete(key);
            const b = (r && r.bars) || [];
            if (b.length) subs.forEach((/** @type {any} */ cb) => cb({ bars: b, complete: true }));
          });
      ...
      } else if (pendingBars && pendingBars.symbol === m.symbol && pendingBars.tf === m.tf)
        pendingBars.bars.push(barOut(m));
      break;
    }
    case 'bars_end': {
      if (pendingBars && pendingBars.id === m.id) {
        const r = rpc.get(m.id);
        if (r) r.resolve({ bars: pendingBars.bars, complete: true });
      }
      break;
    } // finish() owns cleanup (rpc + pendingBars + pump)
    case 'history_end': {
      if (pendingHist && pendingHist.id === m.id) {
        const r = rpc.get(m.id);
        if (r) {
          r.resolve(pendingHist.orders);
          rpc.delete(m.id);
        }
        pendingHist = null;
      }
    ...
    case 'error': {
      const r = rpc.get(m.id);
      if (r) {
        r.resolve({ ok: false, error: m.error });
        rpc.delete(m.id);
      }
      log('MT5 EA error: ' + m.error, true);
      break;
    }
    default:
      break; // status / pong
...
const adapter = {
  ...
  resolveSymbol(symbol, cb) {
    call({ cmd: 'symbol_info', symbol }).then((m) => {
      ...
      const inst = /** @type {Instrument} */ ({
        id: symbol,
        priceDecimals: Number(m.digits),
        tickSize: Number(m.tick_size),
        tickValue: Number(m.tick_value),
      });
  ...
  subscribeQuotes(id, cb) {
    const s = String(id);
    let set = quoteSubs.get(s);
    if (!set) {
      set = new Set();
      quoteSubs.set(s, set);
      send({ cmd: 'subscribe', symbol: s, id: nextId() });
    }
    set.add(cb);
  },
  /** @param {string|number} id @param {Function} cb */
  unsubscribeQuotes(id, cb) {
    const set = quoteSubs.get(String(id));
    if (set) {
      set.delete(cb);
      if (!set.size) quoteSubs.delete(String(id));
    }
  },
  ...
  subscribeBars({ id, tf, fromMs }, cb) {
    ...
    this.getBars({ id, tf, fromMs, toMs: Date.now() }, (/** @type {any} */ r) => {
      ...
      if (r && r.bars && r.bars.length) {
        const mx = r.bars.reduce((/** @type {number} */ a, /** @type {any} */ b) => Math.max(a, b.time), 0);
        if (mx) barCursor.set(key, Math.max(barCursor.get(key) || 0, mx));
      }
      cb(r);
    });
    let set = barSubs.get(key);
    if (!set) {
      set = new Set();
      barSubs.set(key, set);
      send({ cmd: 'subscribe_bars', symbol: id, tf: code, id: nextId() });
    }
    set.add(cb);
    return {
      stop: () => {
        const s = barSubs.get(key);
        if (s) {
          s.delete(cb);
          if (!s.size) {
            barSubs.delete(key);
            barCursor.delete(key);
            barFilling.delete(key);
            send({ cmd: 'unsubscribe_bars', symbol: id, tf: code, id: nextId() });
          }
        }
      },
    };
  ...
  subscribeDepth(id, cb) {
    const s = String(id);
    let set = depthSubs.get(s);
    if (!set) {
      set = new Set();
      depthSubs.set(s, set);
      send({ cmd: 'subscribe_depth', symbol: s, id: nextId() });
    }
    set.add(cb);
  },
  /** @param {string|number} id @param {Function} cb */
  unsubscribeDepth(id, cb) {
    const set = depthSubs.get(String(id));
    if (set) {
      set.delete(cb);
      if (!set.size) {
        depthSubs.delete(String(id));
        send({ cmd: 'unsubscribe_depth', symbol: String(id), id: nextId() });
      }
    }
  },
  ...
  subscribeTrade(cb) {
    ...
    new Set([...posMap.values()].map((p) => p.symbol)).forEach((sym) =>
      cb({ kind: 'position', position: netPosition(sym) }),
    );
  ...
  /** @param {Function} [cb] */
  unsubscribeTrade(cb) {
    if (cb) tradeCbs.delete(cb);
    else tradeCbs.clear();
  }, // remove ONLY the given cb (clear-all only on a no-arg call)

  /** @param {Function} cb */
  getPositions(cb) {
    cb([...new Set([...posMap.values()].map((p) => p.symbol))].map((sym) => netPosition(sym)).filter((p) => p.qty > 0));
  },
  /** @param {Function} cb */
  getAccount(cb) {
    if (!acct) return cb && cb({ error: 'no account yet (connect + give it a moment)' });
    cb(accountOut());
  }, // no acct -> would key as 'default' (phantom row); trade-feed skips on .error
  /** @param {Function} cb */
  getOrders(cb) {
    cb([...orderMap.values()].map(orderOut));
  },
  ...
  getHistory({ fromMs, toMs }, cb) {
    ...
    new Promise((resolve) => {
      rpc.set(rid, { resolve });
      pendingHist = { id: rid, orders: [] };
      send({
        cmd: 'get_history',
        from: Math.floor(fromMs / 1000),
        to: Math.floor((toMs || Date.now()) / 1000),
        id: rid,
      });
    }).then((orders) => cb(orders));
  },
  ...
  placeOrder({ symbol, side, qty, type = 'market', price, bracket } = {}, cb) {
    const sl = (bracket && bracket.stopLoss) || 0,
      tp = (bracket && bracket.takeProfit) || 0,
      T = String(type).toLowerCase(),
      vol = Math.abs(Number(qty));
    let cmd;
    ...
    call(cmd).then(
      (a) =>
        ...
        cb(
          a && a.ok
            ? { id: String(a.deal || a.order || ''), status: T === 'market' ? 'filled' : 'submitted', price: a.price }
            : { error: tradeErr(a && a.error, 'order failed') },
        ),
    );
  },
  /** @param {any} id @param {Function} [cb] */
  cancelOrder(id, cb) {
    call({ cmd: 'cancel_order', ticket: Number(id) }).then(
      (a) => cb && cb(a && a.ok ? { ok: true } : { error: tradeErr(a && a.error, 'cancel failed') }),
    );
  },
  /** @param {any} mod @param {Function} [cb] */
  modifyOrder(mod, cb) {
    const ticket = Number(mod.id != null ? mod.id : mod.orderId);
    const price = mod.price != null ? mod.price : 0,
      sl = (mod.stopLoss != null ? mod.stopLoss : mod.sl) || 0,
      tp = (mod.takeProfit != null ? mod.takeProfit : mod.tp) || 0;
    ...
    if (cur && vol != null && vol !== Math.abs(Number(cur.volume))) {
      const place = {
        cmd: cur.order_type === 'stop' ? 'place_stop' : 'place_limit',
        symbol: cur.symbol,
        side: cur.side,
        volume: vol,
        price: price || Number(cur.price),
        sl: sl || (Number(cur.sl) > 0 ? Number(cur.sl) : 0),
        tp: tp || (Number(cur.tp) > 0 ? Number(cur.tp) : 0),
      };
      call({ cmd: 'cancel_order', ticket }).then((a) => {
        if (!(a && a.ok)) {
          if (cb) cb({ error: tradeErr(a && a.error, 'resize failed (cancel)') });
          return;
        }
        call(place).then((b) => {
          if (cb)
            cb(
              b && b.ok
                ? { ok: true, id: String(b.order || '') }
                : { error: tradeErr(b && b.error, 'resize failed on re-place -- the order was cancelled') },
            );
        });
      });
      return;
  ...
  closePosition(symbol, cb) {
    call({ cmd: 'close_all', symbol }).then(
      (a) => cb && cb(a && a.ok ? { ok: true } : { error: tradeErr(a && a.error, 'close failed') }),
    );
  },
  /** close ONE hedging position by its ticket (not the whole symbol) @param {any} ticket @param {Function} [cb] */
  closeLot(ticket, cb) {
    call({ cmd: 'close', ticket: Number(ticket) }).then(
      (a) => cb && cb(a && a.ok ? { ok: true } : { error: tradeErr(a && a.error, 'close failed') }),
    );
  },
  /** partially close a hedging position by ticket @param {any} ticket @param {number} qty @param {Function} [cb] */
  closeLotPartial(ticket, qty, cb) {
    call({ cmd: 'partial_close', ticket: Number(ticket), volume: Math.abs(Number(qty)) }).then(
      (a) => cb && cb(a && a.ok ? { ok: true } : { error: tradeErr(a && a.error, 'partial close failed') }),
    );
  },
};
...
try {
  /** @type {any} */ (globalThis).addEventListener &&
    /** @type {any} */ (globalThis).addEventListener('beforeunload', () => stopServer());
} catch (_) {}
