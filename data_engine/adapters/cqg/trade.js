...
const tsObj = (ms) => {
  ...
  return { seconds: s, nanos: Math.round((ms - s * 1000) * 1e6) };
}; // epoch ms -> google.protobuf.Timestamp
...
function goodThruMsOf(o) {
  return tsMs(o.goodThruUtcTimestamp) || (o.goodThruDate ? o.goodThruDate + connection.baseMs() : null);
}
...
export const trading = {
  ...
  positions: () =>
    [...positionsByContract.entries()]
      .filter(([, p]) => p.net !== 0)
      .map(([contractId, p]) => ({
        contractId,
        qty: Math.abs(/** @type {number} */ (p.net)),
        side: /** @type {number} */ (p.net) < 0 ? 'short' : 'long',
        avgPrice: p.avgPrice,
      })),
  ...
  placeOrder({ contractId, side, qty, orderType = 1, duration = 1, goodThru, scaledLimitPrice, scaledStopPrice }, cb) {
    ...
    const order = {
      ...
      duration,
      side,
      qty: { significand: Math.abs(Math.round(qty)), exponent: 0 },
      isManual: true,
    };
    ...
    if (scaledSl != null)
      exits.push({
        order: {
          ...
          qty: q,
          isManual: true,
          scaledStopPrice: scaledSl,
        },
      });
    const child =
      exits.length === 2
        ? { compoundOrder: { type: 2, clCompoundId: 'oco' + stamp + requestId, compoundOrderEntries: exits } }
        : exits[0]; // OCO of both, or the single exit
    const compoundOrder = {
      type: 1,
      ...
      orderRequests: [
        {
          ...
          cancelOrder: {
            orderId: useId,
            accountId: tradeAccountId,
            origClOrderId: origCl,
            clOrderId,
            whenUtcTimestamp: tsObj(Date.now() + (connection.brokerSkewMs() || 0)),
          },
        },
      ],
    });
  ...
  getHistory({ fromMs, toMs } = {}, cb) {
    ...
    // showed in History).
    if (!tradeAccountId) {
      pendingHistory.push({ fromMs, toMs, cb });
      return;
    }
    ...
    tradeSubscriptions: [
      {
        ...
        subscribe: true,
        subscriptionScopes: [1, 2, 4, 5],
        accountSummaryParameters: {
          requestedFields: [6, 7, 8, 9, 10, 11, 12, 13, 15, 16, 17, 18, 19, 20, 21, 22, 28, 29, 30],
        },
      },
    ],
  });
...
function sendHistoryRequest(fromMs, toMs, cb) {
  ...
  const req = {
    fromDate: Math.round(/** @type {number} */ (fromMs) - connection.baseMs()),
    accountIds: [tradeAccountId],
  };
...
function handleHistory(rep) {
  ...
  if (rep.statusCode >= 100) {
    // FAILURE / INVALID_PARAMS / NOT_FOUND etc. (success codes are 0-4)
    histCbs.delete(rep.id);
    h.cb &&
      h.cb({ error: 'history failed (' + rep.statusCode + ')' + (rep.textMessage ? ': ' + rep.textMessage : '') });
    return;
  }
  ...
  (r.orderStatuses || []).forEach((/** @type {any} */ os) => {
    (os.contractMetadata || []).forEach((/** @type {any} */ m) => {
      ...
      if (!h.ticks.has(m.contractId) && (m.tickValue != null || m.tickSize != null))
        h.ticks.set(m.contractId, { tickSize: m.tickSize, tickValue: m.tickValue });
    });
    ...
    h.fills.push({
      ...
      status: OS_NAME[os.status] || 'status_' + os.status,
      orderQty: dec(o.qty),
      qty: dec(os.fillQty),
      price: os.avgFillPriceCorrect,
      scaledLimitPrice: o.scaledLimitPrice,
      scaledStopPrice: o.scaledStopPrice,
      time,
      commission: fc ? fc.commission : null,
      commissionCurrency: fc ? fc.commissionCurrency : null,
    });
  ...
  histCbs.delete(rep.id);
  h.fills.forEach((f) => {
    f.symbol = h.syms.get(f.contractId) || 'contract ' + f.contractId;
    const t = h.ticks.get(f.contractId);
    if (t) {
      f.tickSize = t.tickSize;
      f.tickValue = t.tickValue;
    }
  });
...
function handleAccounts(report) {
  ...
  (report.brokerages || []).forEach((/** @type {any} */ b) =>
    (b.salesSeries || []).forEach((/** @type {any} */ ss) =>
      (ss.accounts || []).forEach((/** @type {any} */ a) => {
        tradeAccounts.push({
          id: a.accountId,
          name: a.name,
          number: a.brokerageAccountNumber,
          brokerage: b.name,
          brokerageType: b.type,
        });
      }),
    ),
  );
...
function handlePosition(ps) {
  ...
  if (!entry) {
    entry = { lots: new Map() };
    positionsByContract.set(cid, entry);
  }
  ...
  (ps.openPositions || []).forEach((/** @type {any} */ op) => {
    const q = dec(op.qty);
    if (!q) {
      entry.lots.delete(op.id);
      return;
    } // deleted lot
    entry.lots.set(op.id, {
      qty: q,
      price: op.priceCorrect || 0,
      isShort: !!op.isShort,
      isYesterday: !!op.isYesterday,
    }); // add/update lot (isYesterday = brought-forward; used by the ghost filter)
  });
  emitPosition(cid);
}
...
function netOf(entry, dropYday) {
  let net = 0,
    tot = 0,
    pxw = 0;
  for (const l of entry.lots.values()) {
    if (dropYday && l.isYesterday) continue;
    net += l.isShort ? -l.qty : l.qty;
    tot += l.qty;
    pxw += l.price * l.qty;
  }
...
function recordFill(cid, chain, side, qty, price, time) {
  ...
  if (chain != null) {
    if (seenFillChains.has(chain)) return;
    seenFillChains.add(chain);
  }
  const a = fillLog.get(cid) || [];
  a.push({ time: time || 0, side, qty, price });
  fillLog.set(cid, a);
}
...
function sendPosition(cid, net, avgPrice) {
  ...
  emitTrade({
    ...
    avgPrice,
    accountId: entry && entry.accountId != null ? entry.accountId : tradeAccountId || null,
  });
}
/** @param {any} cid */
function emitPosition(cid) {
  ...
  if (candidate) {
    if (!ghostTimers.has(cid))
      ghostTimers.set(
        cid,
        setTimeout(() => {
          ghostTimers.delete(cid);
          confirmGhost(cid);
        }, GHOST_DEBOUNCE_MS),
      );
    sendPosition(cid, raw.net, avgPrice); // show the raw position while the ghost-check is pending (no flicker on real opens)
  } else {
    const t = ghostTimers.get(cid);
    if (t) {
      clearTimeout(t);
      ghostTimers.delete(cid);
    } // account moved (real) -> cancel any pending drop
...
function confirmGhost(cid) {
  const entry = positionsByContract.get(cid);
  if (!entry) return;
  if (acctIsFlat() && [...entry.lots.values()].some((l) => l.isYesterday))
    sendPosition(cid, 0, 0); // still flat -> the brought-forward lot is a ghost; emit flat (lots retained, so it un-filters if the account net returns)
...
function handleAccountSummary(s) {
  ...
  [
    ...
    if (s[k] != null) /** @type {Record<string, any>} */ (acctSummary)[k] = s[k];
  });
...
function handleOrderStatus(os) {
  ...
  if (OS_WORKING.has(os.status) && chain) {
    ...
    ordersByChain.set(chain, {
      orderId: os.orderId,
      clOrderId: cl || prev.clOrderId || '',
      chainOrderId: chain,
      contractId: o.contractId,
      side: o.side,
      orderType: o.orderType,
      duration: o.duration,
      goodThruDate: o.goodThruDate,
      expiry: goodThruMsOf(o),
      scaledLimitPrice: o.scaledLimitPrice,
      scaledStopPrice: o.scaledStopPrice,
      qty: dec(o.qty) || dec(os.remainingQty) || prev.qty,
    });
  ...
  const txns = (os.transactionStatuses || []).map((/** @type {any} */ t) => ({
    type: TX_NAME[t.status] || 'TXN_' + t.status,
    fillQty: dec(t.fillQty) || null,
    text: t.textMessage || null,
  }));
  ...
  emitTrade({
    ...
    orderType: o.orderType,
    duration: o.duration,
    qty: dec(o.qty) || dec(os.remainingQty),
    scaledLimitPrice: o.scaledLimitPrice,
    scaledStopPrice: o.scaledStopPrice,
    avgFillPrice: os.avgFillPriceCorrect,
    status: OS_NAME[os.status] || 'status_' + os.status,
    submitTime: tsMs(os.submissionUtcTimestamp),
    statusTime: tsMs(os.statusUtcTimestamp),
    expiry: goodThruMsOf(o),
    accountId: o.accountId != null ? o.accountId : tradeAccountId || null,
    txns,
    rejectText: os.rejectMessage || null,
  });
  if (os.status === 8) {
    const fc = os.fillCommission;
    recordFill(
      ...
      o.side,
      dec(os.fillQty),
      os.avgFillPriceCorrect,
      tsMs(os.fillUtcTimestamp),
    ); // feed the avg-entry reconstruction (live)
    emitTrade({
      kind: 'fill',
      orderId: os.orderId,
      contractId: o.contractId,
      side: o.side,
      qty: dec(os.fillQty),
      avgPrice: os.avgFillPriceCorrect,
      time: tsMs(os.fillUtcTimestamp),
      commission: fc ? fc.commission : null,
      commissionCurrency: fc ? fc.commissionCurrency : null,
      accountId: o.accountId != null ? o.accountId : tradeAccountId || null,
    });
  ...
  if (os.status === OS_REJECTED) {
    const rejMsg =
      os.rejectMessage ||
      (os.transactionStatuses || [])
        .map((/** @type {any} */ t) => t.textMessage)
        .filter(Boolean)
        .join('; ') ||
      'order rejected';
    ...
    if (p.clOrderId && cl === p.clOrderId) {
      ...
      p.cb &&
        p.cb({
          id: os.orderId,
          status: OS_NAME[os.status] || 'status_' + os.status,
          fillQty: dec(os.fillQty),
          avgPrice: os.avgFillPriceCorrect,
          error: rejMsg ? 'rejected: ' + rejMsg : undefined,
          raw: os,
        });
      break;
    }
...
function handleTradeMessage(msg) {
  (msg.informationReports || []).forEach((/** @type {any} */ rep) => {
    if (rep.accountsReport) {
      emitRaw('cqg', 'accounts', rep.accountsReport);
      handleAccounts(rep.accountsReport);
    }
  ...
  (msg.positionStatuses || []).forEach((/** @type {any} */ ps) => {
    emitRaw('cqg', 'position', ps);
    handlePosition(ps);
  });
  (msg.accountSummaryStatuses || []).forEach((/** @type {any} */ s) => {
    emitRaw('cqg', 'account', s);
    handleAccountSummary(s);
  });
  (msg.orderStatuses || []).forEach((/** @type {any} */ os) => {
    emitRaw('cqg', 'order', os);
  ...
  (msg.orderRequestRejects || []).forEach((/** @type {any} */ r) => {
    ...
    if (p) {
      pendingOrders.delete(r.requestId);
      p.cb && p.cb({ error: 'rejected (' + r.rejectCode + ')' + (text ? ': ' + text : '') });
    }
  });
}
...
function resetTrade() {
  ...
  ghostTimers.clear();
  fillLog.clear();
  seenFillChains.clear();
  tradeAccountId = 0;
  tradeAccounts.length = 0;
  positionsByContract.clear();
  acctSummary = null;
  pendingOrders.clear();
  clByOrderId.clear();
  ordersByChain.clear();
  chainByOrderId.clear();
}
