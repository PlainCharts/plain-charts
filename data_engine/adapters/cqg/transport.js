...
function scheduleReconnect() {
  ...
  _retryTimer = /** @type {any} */ (setTimeout(() => { _retryTimer = 0; connection.connect(_account); }, _retryMs));
}
...
function startWatchdog() {
  if (_watchdog) return;
  _watchdog = /** @type {any} */ (setInterval(() => {
...
/** @param {Cb} cb */
export const onMessage = (cb) => { msgHooks.push(cb); };
/** @param {Cb} cb */
export const onLogon = (cb) => { logonHooks.push(cb); };
/** @param {Cb} cb */
export const onReset = (cb) => { resetHooks.push(cb); };
...
export const connection = {
  ...
  /** @param {any} obj */
  send(obj) {
    if (!ws || ws.readyState !== WebSocket.OPEN) {   // never send on a CONNECTING/CLOSING socket (premature resubscribes on the startup race are re-done after logon)
    ...
    try { ws.send(encode(obj)); }
    catch (e) { log('encode error: ' + (/** @type {any} */ (e)).message, true); }
  },

  /** @param {any} account */
  connect(account) {
    _account = account; _userClosed = false; _logonRejected = false;
    if (_retryTimer) { clearTimeout(_retryTimer); _retryTimer = 0; }
    startWatchdog();
    ...
    if (ws) { try { ws.onclose = null; ws.onerror = null; ws.onmessage = null; ws.close(); } catch (_) {} }
    _baseMs = 0;
    ...
    ws.onopen = () => {
      ...
      connection.send({ logon: {
        ...
        clientAppId: (account.appId || 'TradingView').trim(), clientVersion: 'js-lwc-1',
        protocolVersionMajor: 2, protocolVersionMinor: 230,
      }});
    };
    ws.onerror = () => log('WebSocket error (often an Origin rejection).', true);
    ws.onclose = (e) => markDead('offline (' + (e.code || '') + ')');
    ws.onmessage = (ev) => { try { route(decode(ev.data)); } catch (e) { log('decode: ' + (/** @type {any} */ (e)).message, true); } };
  },
  ...
  disconnect() {
    ...
    if (ws) { try { ws.onclose = null; ws.onerror = null; ws.onmessage = null; ws.close(); } catch (_) {} ws = null; }
    _baseMs = 0; _brokerSkewMs = null;
    symCbs.clear(); barCbs.clear(); oneShot.clear(); mdSubs.clear(); sessCbs.clear();
    resetHooks.forEach((cb) => { try { cb(); } catch (_) {} });   // the trade plane clears its own state
  },
  ...
  getBars(params, cb) {
    const id = ++seq;
    barCbs.set(id, cb); oneShot.add(id);
  ...
  subscribeMarketData(contractId, cb) { mdEntry(contractId).quoteCbs.add(cb); applyLevel(contractId); },
  /** @param {any} contractId @param {Cb} cb */
  unsubscribeMarketData(contractId, cb) { const s = mdSubs.get(contractId); if (!s) return; s.quoteCbs.delete(cb); applyLevel(contractId); },
  ...
  /** @param {any} contractId @param {Cb} cb */
  subscribeDepth(contractId, cb) { const s = mdEntry(contractId); s.depthCbs.add(cb); applyLevel(contractId); if (s.bids.size || s.asks.size) { try { cb(buildDom(s)); } catch (_) {} } },
  /** @param {any} contractId @param {Cb} cb */
  unsubscribeDepth(contractId, cb) { const s = mdSubs.get(contractId); if (!s) return; s.depthCbs.delete(cb); applyLevel(contractId); },
  ...
  getTradingDayRanges({ sessionInfoId, fromMs, toMs, count } = {}, cb) {
    ...
    if (count != null && toMs != null) { req.toUtcTime = Math.round(toMs - _baseMs); req.count = count; }
    else {
...
function route(msg) {
  if (msg.ping) {
    connection.send({ pong: {
      token: msg.ping.token, pingUtcTime: msg.ping.pingUtcTime, pongUtcTime: Date.now() - _baseMs,
    }});
  }
  if (msg.logonResult) {
    ...
    log('Logged on.');
    bus.emit('logon');
    logonHooks.forEach((cb) => { try { cb(); } catch (_) {} });   // trade plane: request accounts + subscribe to orders/positions/account-summary
  }
  (msg.informationReports || []).forEach((/** @type {any} */ rep) => {
    if (sessCbs.has(rep.id)) {   // trading-session reply (one-shot; small ranges arrive in one report)
  ...
  (msg.timeBarReports || []).forEach((/** @type {any} */ rep) => {
    ...
    if (oneShot.has(rep.requestId) && rep.isReportComplete !== false) {
      barCbs.delete(rep.requestId); oneShot.delete(rep.requestId);
    }
  });
  (msg.realTimeMarketData || []).forEach((/** @type {any} */ rt) => {
    ...
    if (s.depthCbs.size) { updateDom(s, rt); const dom = buildDom(s); s.depthCbs.forEach((cb) => cb(dom)); }
  });
  ...
  msgHooks.forEach((cb) => { try { cb(msg); } catch (_) {} });
}
...
function mdEntry(cid) {
  ...
  if (!s) { s = { reqId: ++seq, level: 0, quoteCbs: new Set(), depthCbs: new Set(), bids: new Map(), asks: new Map() }; mdSubs.set(cid, s); }
  return s;
}
/** @param {any} cid */
function applyLevel(cid) {
  const s = mdSubs.get(cid); if (!s) return;
  ...
  if (level === 0) { mdSubs.delete(cid); return; }
  s.level = level; s.reqId = ++seq;
...
function updateDom(s, rt) {
  if (rt.isSnapshot) { s.bids.clear(); s.asks.clear(); }
  (rt.quotes || []).forEach((/** @type {any} */ q) => {
    ...
    const vol = q.volume ? (Number(q.volume.significand || 0) * Math.pow(10, Number(q.volume.exponent || 0)))
      : (q.scaledVolume != null ? Number(q.scaledVolume) : 0);
    const book = q.type === 3 ? s.bids : s.asks;
    if (vol > 0) book.set(q.scaledPrice, vol); else book.delete(q.scaledPrice);
  });
}
/** @param {MdSub} s */
function buildDom(s) {
  return {
    bids: [...s.bids.entries()].map(([p, q]) => ({ scaledPrice: p, qty: q })).sort((a, b) => b.scaledPrice - a.scaledPrice),
    asks: [...s.asks.entries()].map(([p, q]) => ({ scaledPrice: p, qty: q })).sort((a, b) => a.scaledPrice - b.scaledPrice),
  };
}
