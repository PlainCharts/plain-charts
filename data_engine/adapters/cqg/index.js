...
const decimalsFor = (cid) => {
  const s = scaleById.get(cid);
  return s != null && s > 0 ? Math.max(0, Math.round(-Math.log10(s))) : 2;
};
...
const tickSizeOf = (cid, sym) => {
  const t = tickSizeById.get(cid);
  if (t != null) return t;
  const pt = PT_TICK[ptRoot(sym)];
  return pt ? pt[0] : undefined;
};
/** @param {any} cid @param {any} sym @returns {number|undefined} */
const tickValueOf = (cid, sym) => {
  const t = tickValueById.get(cid);
  if (t != null) return t;
  const pt = PT_TICK[ptRoot(sym)];
  return pt ? pt[1] : undefined;
};
...
const normAccount = (s) =>
  s && {
    ...
    realizedPL: s.profitLoss,
    marginUsed: s.totalMargin,
    marginAvailable: s.purchasingPower,
    positionMargin: s.positionMargin,
    purchasingPower: s.purchasingPower,
    mvo: s.mvo,
    mvf: s.mvf,
    marginCredit: s.marginCredit,
    cashExcess: s.cashExcess,
    yesterdayBalance: s.yesterdayBalance,
    workingOrders: s.totalWorkingOrders,
    filledOrders: s.totalFilledOrders,
    longQty: s.longQty,
    shortQty: s.shortQty,
...
function toBarUpdate(rep, scale) {
  if (rep.statusCode >= 100) {
    ...
    const statusMap = /** @type {Record<number, string>} */ (BAR_STATUS);
    return {
      bars: [],
      complete: true,
      error: (statusMap[rep.statusCode] || rep.statusCode) + (detail ? ' — ' + detail : ''),
    };
  }
...
const adapter = {
  ...
  resolveSymbol(symbol, cb) {
    transport.resolveSymbol(symbol, (/** @type {any} */ rep) => {
      ...
      if (rep.statusCode >= 2 && !cm) {
        cb(null, { status: rep.statusCode });
        return;
      }
      if (!cm) {
        cb(null, { status: rep.statusCode });
  ...
  searchSymbols(query, cb) {
    ...
    transport.productSearch(q, (/** @type {any} */ rep) => {
      ...
      cb(
        syms
          .filter((/** @type {any} */ s) => !s.deleted)
          .map((/** @type {any} */ s) => ({ symbol: s.name, name: s.description || '', category: 'Futures' })),
      );
    });
  },
  ...
  subscribeQuotes(id, cb) {
    ...
    const wrapped = (/** @type {any} */ rt) => {
      ...
      (rt.quotes || []).forEach((/** @type {any} */ x) => {
        ...
        if (x.type === 0) {
          q.last = px;
          if (s != null) q.lastSize = s;
        } // TYPE_TRADE (was wrongly 4 = a DOM ask level)
        else if (x.type === 1) {
          q.bid = px;
          if (s != null) q.bidSize = s;
        } // TYPE_BESTBID
        else if (x.type === 2) {
          q.ask = px;
          if (s != null) q.askSize = s;
        } // TYPE_BESTASK
      });
  ...
  subscribeDepth(id, cb) {
    const scale = scaleById.get(id) || 1;
    const wrapped = (/** @type {any} */ dom) =>
      cb({
        bids: dom.bids.map((/** @type {any} */ b) => ({ price: b.scaledPrice * scale, qty: b.qty })),
        asks: dom.asks.map((/** @type {any} */ a) => ({ price: a.scaledPrice * scale, qty: a.qty })),
      });
  ...
  unsubscribeDepth(id, cb) {
    const w = depthWrappers.get(cb);
    if (w) {
      transport.unsubscribeDepth(id, w);
      depthWrappers.delete(cb);
    }
  },
  ...
  getPositions(cb) {
    ...
    cb &&
      cb(
        transport.positions().map((/** @type {any} */ p) => ({
          symbol: symOf(p.contractId),
          qty: p.qty,
          side: p.side,
          avgPrice: p.avgPrice,
          accountId: acct,
          priceDecimals: decimalsFor(p.contractId),
          tickSize: tickSizeOf(p.contractId, symOf(p.contractId)),
          tickValue: tickValueOf(p.contractId, symOf(p.contractId)),
        })),
      );
  },
  ...
  subscribeTrade(cb) {
    const w = (/** @type {any} */ ev) => {
      if (ev.kind === 'order') {
        ...
        if (!symByContract.has(ev.contractId) || !scaleById.has(ev.contractId))
          console.error(
            '[cqg] ORDER TRIPWIRE contract ' +
              ev.contractId +
              ' (symbol ' +
              (symByContract.has(ev.contractId) ? 'ok' : 'MISSING') +
              ', scale ' +
              (scaleById.has(ev.contractId) ? 'ok' : 'MISSING') +
              '): ' +
              JSON.stringify(ev),
          );
        ...
        cb(
          event.order({
            id: ev.orderId,
            symbol: symOf(ev.contractId),
            side: ev.side === 2 ? 'sell' : 'buy',
            type: ev.orderType === 2 ? 'limit' : ev.orderType === 3 ? 'stop' : 'market',
            qty: ev.qty,
            price: ev.orderType === 2 ? lim : ev.orderType === 3 ? stp : null,
            limitPrice: lim,
            stopPrice: stp,
            avgFillPrice: ev.avgFillPrice ? ev.avgFillPrice : null,
            tif: DURATION_TO_TIF[ev.duration] || 'day',
            status: ev.status,
            time: ev.submitTime,
            updateTime: ev.statusTime,
            expiry: ev.expiry,
            accountId: ev.accountId,
            priceDecimals: decimalsFor(ev.contractId),
            txns: ev.txns,
            rejectText: ev.rejectText,
          }),
        );
        ...
        cb(
          event.fill({
            ...
            time: ev.time,
            accountId: ev.accountId,
            tickSize: tickSizeOf(ev.contractId, symOf(ev.contractId)),
            tickValue: tickValueOf(ev.contractId, symOf(ev.contractId)),
            commission: ev.commission,
            commissionCurrency: ev.commissionCurrency,
          }),
        );
  ...
  /** @param {Function} cb */
  unsubscribeTrade(cb) {
    const w = tradeWrappers.get(cb);
    if (w) {
      transport.offTrade(w);
      tradeWrappers.delete(cb);
    }
  },
  ...
  getOrders(cb) {
    cb &&
      cb(
        transport.orders().map((/** @type {any} */ o) => {
          ...
          return {
            id: o.orderId,
            symbol: symByContract.get(o.contractId) || 'contract ' + o.contractId,
            side: o.side === 2 ? 'sell' : 'buy',
            type: o.orderType === 2 ? 'limit' : o.orderType === 3 ? 'stop' : 'market',
            qty: o.qty,
            price: o.orderType === 2 ? lim : o.orderType === 3 ? stp : null,
            limitPrice: lim,
            stopPrice: stp,
            tif: DURATION_TO_TIF[o.duration] || 'day',
            status: 'working',
            expiry: o.expiry,
            priceDecimals: decimalsFor(o.contractId),
          };
        }),
      );
  },
        ...
        cb(
          res.map((/** @type {any} */ f) => {
            ...
            return {
              ...
              symbol: symByContract.get(f.contractId) || shortSym(f.symbol),
              side: f.side === 2 ? 'sell' : 'buy',
              type: f.orderType === 2 ? 'limit' : f.orderType === 3 ? 'stop' : 'market',
              status: f.status,
              orderQty: f.orderQty,
              qty: f.qty,
              price: f.price,
              limitPrice: f.scaledLimitPrice ? f.scaledLimitPrice * scale : null,
              stopPrice: f.scaledStopPrice ? f.scaledStopPrice * scale : null,
              time: f.time,
              accountId: acct,
              priceDecimals: decimalsFor(f.contractId),
              tickSize: tickSizeOf(f.contractId, f.symbol),
              tickValue: tickValueOf(f.contractId, f.symbol),
              commission: f.commission,
              commissionCurrency: f.commissionCurrency,
            };
          }),
        );
    });
  ...
  cancelOrder(orderId, cb) {
    transport.cancelOrder(orderId, (/** @type {any} */ r) => cb && cb(r));
  },
  ...
  modifyOrder({ orderId, qty, price } = {}, cb) {
    ...
    if (price != null) {
      if (o.orderType === 2) mod.scaledLimitPrice = Math.round(price / scale);
      else if (o.orderType === 3) mod.scaledStopPrice = Math.round(price / scale);
    }
  ...
  closePosition(symbol, cb) {
    this.resolveSymbol(symbol, (/** @type {any} */ inst) => {
      ...
      transport.placeOrder(
        { contractId: inst.id, side: pos.side === 'long' ? 2 : 1, qty: pos.qty, orderType: 1 },
        (/** @type {any} */ r) => cb && cb(r),
      );
    });
  },
