  ...
  for (const c of clauses) {
    ...
    if (w[0] === 'buy' || w[0] === 'sell') {
      ...
      if (w.length === 1) {
        ...
        if (!(qty > 0))
          throw new Error(
            '"' + c + '": expected a quantity, e.g. "' + w[0] + ' 5" (or a risk amount: "' + w[0] + ' stake 100")',
          );
      ...
      if (!kind)
        throw new Error(
          '"' +
            c +
            '": set what? use "set stop N" or "set target N" (to reposition a stop, use "move stop be" / "move stop N%")',
        );
      ...
      if (!(value > 0))
        throw new Error(
          '"' +
            c +
            '": expected a distance, e.g. "set ' +
            (kind === 'setStop' ? 'stop 20' : 'target 40') +
            '" (to reposition, use "move stop be" / "move stop N")',
        );
      ...
      if (arg === 'be') {
        ...
        const n = Number(arg);
        const unit = w[3] === 'pips' || w[3] === 'points' ? w[3] : undefined;
        ops.push({ op: 'moveStop', mode: 'by', value: n, unit });
      } // move the stop N toward entry
      ...
      else if (what === 'partial') {
        const qty = Number(w[2]);
        if (!(qty > 0)) throw new Error('"' + c + '": close partial needs a quantity, e.g. "close partial 2"');
        ops.push({ op: 'close', what: 'partial', qty });
      } else throw new Error('"' + c + '": use close all | symbol | buy | sell | partial N');
    } else {
      throw new Error(
        '"' + c + '": unknown command (buy, sell, set stop, set target, move stop, close all/symbol/buy/sell/partial)',
      );
    }
  }
...
function validateSemantics(ops) {
  for (let i = 0; i < ops.length; i++) {
    ...
    if (op.op === 'market') {
      ...
      while (
        j + 1 < ops.length &&
        ((ops[j + 1].op === 'setStop' && ops[j + 1].mode === 'dist') || ops[j + 1].op === 'setTarget')
      ) {
        brk.push(ops[j + 1]);
        j++;
      }
      ...
      if (Number(op.stake) > 0 && !brk.some((b) => b.op === 'setStop')) {
        throw new Error(
          '"' +
            ...
            ' stake ' +
            op.stake +
            '" needs a stop -- add "set stop N" (the stop is the risk basis, e.g. "' +
            op.side +
            ' stake ' +
            op.stake +
            ' and set stop 20")',
