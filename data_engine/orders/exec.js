...
const jlog = (src, msg, err) =>
  platform.console.post({ level: err ? 'error' : 'info', cat: 'journal', src: src || 'app', dir: 'out', msg });
const sleep = (/** @type {number} */ ms) => new Promise((r) => setTimeout(r, ms));
/** @param {any} v @param {any} dec */
const fmtPx = (v, dec) =>
  v == null || Number.isNaN(Number(v)) ? '0' : dec != null ? Number(v).toFixed(Number(dec)) : String(v);
...
function resolveInst(brokerId, symbol) {
  return new Promise((resolve, reject) => {
    ...
    if (!a || !a.resolveSymbol) {
      reject(new Error('cannot resolve ' + symbol));
      return;
    }
    let done = false;
    const to = setTimeout(() => {
      if (!done) {
        ...
        reject(new Error('symbol ' + symbol + ' not resolved (3s)'));
      }
    }, 3000);
    a.resolveSymbol(
      symbol,
      /** @param {any} inst */ (inst) => {
        ...
        clearTimeout(to);
        if (!inst) reject(new Error('symbol not found: ' + symbol));
        else resolve(inst);
      },
    );
  });
...
function firstQuote(brokerId, inst) {
  return new Promise((resolve, reject) => {
    const a = /** @type {any} */ (broker.for(brokerId));
    if (!a || !a.subscribeQuotes) {
      reject(new Error('no quotes'));
      return;
    }
    let done = false;
    /** @type {any} */
    ...
    const to = setTimeout(() => {
      if (!done) {
        ...
        if (cb)
          try {
            a.unsubscribeQuotes(inst.id, cb);
          } catch (_) {}
        reject(new Error('no quote (3s)'));
      }
    }, 3000);
    cb = (/** @type {any} */ q) => {
      if (done) return;
...
async function sizeStakeQty(brokerId, ctx, side, sizing, entry, inst) {
  ...
  if (entry == null) {
    ...
    entry = side === 'buy' ? q.ask : q.bid;
  }
  const r = sizeFromStake({
    risk: Number(sizing.risk),
    entryPrice: Number(entry),
    stopPrice: Number(sizing.stop),
  ...
  jlog(
    ...
    'STAKE ' +
      sizing.risk +
      ' @ entry ' +
      fmtPx(entry, inst.priceDecimals) +
      ' stop ' +
      fmtPx(sizing.stop, inst.priceDecimals) +
      ' -> ' +
      r.qty +
      ' ' +
      ctx.symbol +
      ' (' +
      fmtPx(r.riskPerUnit, 2) +
      '/unit)',
  );
...
function placeOrderP(brokerId, order) {
  return new Promise((resolve, reject) => {
    const a = /** @type {any} */ (broker.for(brokerId));
    if (!a || !a.placeOrder) {
      reject(new Error('no order routing on ' + brokerId));
      return;
    }
    a.placeOrder(
      order,
...
function modifyP(brokerId, mod) {
  return new Promise((resolve, reject) => {
    const a = /** @type {any} */ (broker.for(brokerId));
    if (!a || !a.modifyOrder) {
      reject(new Error('no modify on ' + brokerId));
      return;
    }
    a.modifyOrder(
      mod,
...
function bracketPrices(side, entry, inst, brk) {
  ...
  const rnd = (/** @type {number} */ p) =>
    tick ? Math.round(p / tick) * tick : dec != null ? Number(p.toFixed(dec)) : p;
  let sl = 0,
    tp = 0;
  for (const b of brk) {
...
async function placeMarketBracket(brokerId, ctx, side, qty, sl, tp, inst) {
  ...
  await placeOrderP(brokerId, {
    symbol: ctx.symbol,
    side,
    qty,
    type: 'market',
    bracket: { stopLoss: sl || 0, takeProfit: tp || 0 },
  });
}
...
export async function placeMarket(ctx, side, qty, bracket, sizing, onStatus = () => {}) {
  ...
  try {
    ...
    if (!sl && !tp) {
      jlog(brokerId, side.toUpperCase() + ' ' + qty + ' ' + ctx.symbol);
      await placeOrderP(brokerId, { symbol: ctx.symbol, side, qty: Number(qty), type: 'market' });
    } else {
      if (!inst) inst = await resolveInst(brokerId, ctx.symbol);
      await placeMarketBracket(brokerId, ctx, side, Number(qty), sl, tp, inst);
    }
...
export async function placeResting(
  ctx,
  side,
  qty,
  orderType,
  ...
  try {
    ...
    if (t === 'gtd') {
      if (!(Number(goodThru) > 0)) return { ok: false, error: 'GTD needs an expiry date' };
      order.goodThru = Number(goodThru);
    }
    const sl = bracket && Number(bracket.stopLoss) > 0 ? Number(bracket.stopLoss) : 0;
    const tp = bracket && Number(bracket.takeProfit) > 0 ? Number(bracket.takeProfit) : 0;
...
async function execMarketOp(ops, i, ctx, onStatus) {
  ...
  if (op.trigger)
    throw new Error(
      'a bare "' +
        op.side +
        '" fires the order ticket tab -- it only runs from the ticket, give a quantity for a headless order (e.g. "' +
        op.side +
        ' 1")',
    );
  ...
  while (
    i + 1 < ops.length &&
    ((ops[i + 1].op === 'setStop' && ops[i + 1].mode === 'dist') || ops[i + 1].op === 'setTarget')
  ) {
    brk.push(ops[i + 1]);
    i++;
  }
  ...
  if (!brk.length) {
    if (isStake)
      throw new Error(
        'stake needs a stop -- add "set stop N", e.g. "' + op.side + ' stake ' + op.stake + ' and set stop 20"',
      );
...
async function execStopOp(op, ctx, onStatus) {
  ...
  const curStop = posn.net
    ? liveStop
      ? Number(liveStop.stopPrice != null ? liveStop.stopPrice : liveStop.price)
      : null
    : posn.stopLoss != null
      ? Number(posn.stopLoss)
      : null;
  let stop;
  if (op.mode === 'be') {
    const buf = op.buffer ? (isLong ? 1 : -1) * op.buffer * unitSize(dec, op.unit) : 0;
    stop = rnd(posn.entry + buf);
  } // buffer in the profit direction
  else if (op.mode === 'by') {
    if (curStop == null || !isFinite(curStop))
      throw new Error('move stop N needs an existing stop to move -- set one first (e.g. "set stop 10")');
    stop = rnd(curStop + (isLong ? 1 : -1) * op.value * unitSize(dec, op.unit)); // nudge the stop N toward entry (and beyond -> profit lock)
  } else {
    const size = unitSize(dec, op.unit);
    stop = rnd(isLong ? posn.entry - op.value * size : posn.entry + op.value * size);
  }
...
async function execCloseOp(op, ctx) {
  ...
  if (op.what === 'all') {
    ...
    const working = /** @type {any[]} */ (platform.orders.all()).filter(
      (o) => (!ctx.broker || o.broker === ctx.broker) && !isTerminal(o.status),
    );
    if (working.length) {
      jlog(ctx.broker || 'app', 'CANCEL ' + working.length + ' working order' + (working.length === 1 ? '' : 's'));
      working.forEach((o) => {
        cancelById(o.broker, o.id);
      });
    }
    const lots = /** @type {any[]} */ (platform.positionLots.all()).filter(
      (l) => !ctx.broker || l.broker === ctx.broker,
    );
    const nets = /** @type {any[]} */ (platform.positions.all()).filter(
      (p) =>
        (!ctx.broker || p.broker === ctx.broker) &&
        Number(p.qty) > 0 &&
        !lots.some((l) => l.broker === p.broker && l.symbol === p.symbol),
    );
    const targets = [...lots, ...nets];
    jlog(ctx.broker || 'app', 'CLOSE ALL -- ' + targets.length + ' position' + (targets.length === 1 ? '' : 's'));
    targets.forEach((t) => {
      if (t.ticket != null) closeLotById(t.broker, t.ticket);
      else closePositionSym(t.broker, t.symbol);
    });
    return;
  }
  ...
  const net = /** @type {any[]} */ (platform.positions.all()).find(
    (/** @type {any} */ p) => (!ctx.broker || p.broker === ctx.broker) && p.symbol === ctx.symbol && Number(p.qty) > 0,
  );
  ...
  if (op.what === 'symbol') {
    ...
    jlog(
      ctx.broker,
      'PARTIAL CLOSE ' + amount + ' ' + ctx.symbol + (amount < Number(op.qty) ? ' (capped from ' + op.qty + ')' : ''),
    );
    if (posn.net) {
...
export async function setStopPrice(ctx, price, note) {
  ...
  try {
    ...
    const msg = /** @type {any} */ (e).message || String(e);
    jlog(brokerId, verb + ' failed: ' + msg, true);
    return { ok: false, error: msg };
  }
}
...
export async function modifyOrderPrice(brokerId, id, price, qty, stopLoss, takeProfit) {
  try {
    ...
    jlog(
      ...
      'MODIFY #' +
        id +
        (mod.qty != null ? ' qty ' + mod.qty : '') +
        (mod.price != null ? ' @ ' + mod.price : '') +
        (mod.stopLoss ? ' SL ' + mod.stopLoss : '') +
        (mod.takeProfit ? ' TP ' + mod.takeProfit : ''),
    );
    await new Promise((resolve, reject) =>
      a.modifyOrder(mod, (/** @type {any} */ r) => (r && r.error ? reject(new Error(r.error)) : resolve(r))),
    );
    return { ok: true };
  } catch (e) {
    const msg = /** @type {any} */ (e).message || String(e);
    jlog(brokerId, 'MODIFY #' + id + ' failed: ' + msg, true);
    return { ok: false, error: msg };
  }
}
...
export async function modifyPosition(brokerId, ticket, stopLoss, takeProfit) {
  try {
    ...
    await new Promise((resolve, reject) =>
      a.modifyOrder(
        { id: ticket, stopLoss: Number(stopLoss) || 0, takeProfit: Number(takeProfit) || 0 },
        (/** @type {any} */ r) => (r && r.error ? reject(new Error(r.error)) : resolve(r)),
      ),
    );
    return { ok: true };
  } catch (e) {
    const msg = /** @type {any} */ (e).message || String(e);
    jlog(brokerId, 'MODIFY #' + ticket + ' failed: ' + msg, true);
    return { ok: false, error: msg };
  }
}
...
export async function closeLotById(brokerId, ticket, qty) {
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      if (partial) a.closeLotPartial(ticket, Number(qty), cb);
      else a.closeLot(ticket, cb);
    });
    return { ok: true };
  } catch (e) {
    const msg = /** @type {any} */ (e).message || String(e);
    jlog(brokerId, 'CLOSE #' + ticket + ' failed: ' + msg, true);
    return { ok: false, error: msg };
  }
}
...
export async function closePositionSym(brokerId, symbol) {
  try {
    ...
    const lots = /** @type {any[]} */ (platform.positionLots.all()).filter(
      (l) => l.broker === brokerId && l.symbol === symbol && Number(l.qty) > 0,
    );
    const net = /** @type {any} */ (
      /** @type {any[]} */ (platform.positions.all()).find(
        (p) => p.broker === brokerId && p.symbol === symbol && Number(p.qty) > 0,
      )
    );
    if (lots.length) {
        ...
        lots.map(
          (l) =>
            new Promise((resolve, reject) =>
              a.closeLot(l.ticket, (/** @type {any} */ r) => (r && r.error ? reject(new Error(r.error)) : resolve(r))),
            ),
        ),
      );
    } else if (net) {
      ...
      await new Promise((resolve, reject) =>
        a.closePosition(symbol, (/** @type {any} */ r) => (r && r.error ? reject(new Error(r.error)) : resolve(r))),
      );
    } else {
    ...
    const msg = /** @type {any} */ (e).message || String(e);
    jlog(brokerId, 'CLOSE ' + symbol + ' failed: ' + msg, true);
    return { ok: false, error: msg };
  }
}
...
export async function assistantOrder(method, args) {
  ...
  try {
    ...
    jlog(
      brokerId || 'assistant',
      'ASSISTANT ' + method.toUpperCase() + ' ' + (arg && typeof arg === 'object' ? JSON.stringify(arg) : String(arg)),
    );
    const result = await new Promise((resolve, reject) =>
      a[method](arg, (/** @type {any} */ r) => (r && r.error ? reject(new Error(r.error)) : resolve(r))),
    );
    return { ok: true, result };
  } catch (e) {
    const msg = /** @type {any} */ (e).message || String(e);
    jlog(brokerId || 'assistant', 'ASSISTANT ' + method + ' failed: ' + msg, true);
    return { ok: false, error: msg };
  }
}
...
export async function cancelById(brokerId, id) {
  try {
    ...
    await new Promise((resolve, reject) =>
      a.cancelOrder(id, (/** @type {any} */ r) => (r && r.error ? reject(new Error(r.error)) : resolve(r))),
    );
    return { ok: true };
  } catch (e) {
    const msg = /** @type {any} */ (e).message || String(e);
