...
import { execScript, placeMarket, placeResting, setStopPrice, setTargetPrice, modifyOrderPrice, cancelById, modifyPosition, closeLotById, closePositionSym, assistantOrder } from './exec.js';
...
register('echo', (/** @type {any} */ cmd) => { log('echo ' + JSON.stringify(cmd)); return { echoed: cmd, book: { positions: platform.positions.size(), orders: platform.orders.size() } }; });
...
register('script', async (/** @type {any} */ cmd) => {
  let ops;
  try { ops = parseScript(cmd.script); } catch (e) { return { ok: false, error: /** @type {any} */ (e).message }; }
...
register('place', (/** @type {any} */ cmd) => (cmd.orderType === 'limit' || cmd.orderType === 'stop')
  ? placeResting(cmd.ctx || {}, cmd.side, cmd.qty, cmd.orderType, cmd.price, cmd.tif, cmd.goodThru, cmd.bracket || null, cmd.sizing || null)
...
register('modifyOrder', (/** @type {any} */ cmd) => modifyOrderPrice(cmd.broker, cmd.id, cmd.price, cmd.qty, cmd.stopLoss, cmd.takeProfit));   // move/resize a specific resting order by id (keeps a server-side OCO bond); SL/TP for a hedging pending order
register('cancel', (/** @type {any} */ cmd) => cancelById(cmd.broker, cmd.id));
register('modifyPosition', (/** @type {any} */ cmd) => modifyPosition(cmd.broker, cmd.ticket, cmd.stopLoss, cmd.takeProfit));   // hedging position SL/TP together, by ticket
...
let raf = 0;
const snapshot = () => { raf = 0; log('book: ' + platform.positions.size() + ' positions, ' + platform.orders.size() + ' orders, ' + platform.positionLots.size() + ' lots'); };
const nudge = () => { if (!raf) raf = requestAnimationFrame(snapshot); };
