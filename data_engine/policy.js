...
let gate = async () => { throw new Error('assistant execution not permitted (no policy installed)'); };
...
export function setExecGate(fn) { gate = fn; }

/** Run the installed gate; throws to deny. @param {string} method @param {any} arg @param {(string|null)} brokerId */
export function execGate(method, arg, brokerId) { return gate(method, arg, brokerId); }
