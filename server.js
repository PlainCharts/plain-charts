...
const { sendJson, readBody, openFolder, sanitizeModName, readMeta } = require('./server/util.js');
...
const server = http.createServer((req, res) => {
  ...
  if (urlPath === '/api/addons/toggle' && req.method === 'POST')
    return readBody(req, (d) => sendJson(res, 200, addonHost.toggle(d.id, d.enabled)));
  if (urlPath === '/api/addons/reload' && req.method === 'POST')
    return readBody(req, (d) => sendJson(res, 200, addonHost.reload(d.id)));
  if (urlPath === '/api/addons/save' && req.method === 'POST')
    return readBody(req, (d) => sendJson(res, 200, addonHost.save(d.name, d.code)));
  if (urlPath === '/api/addons/delete' && req.method === 'POST')
    return readBody(req, (d) => sendJson(res, 200, addonHost.remove(d.id)));
  if (urlPath === '/api/addons/package' && req.method === 'POST')
    return readBody(req, (d) => sendJson(res, 200, addonHost.installPackage(d.id, d.files)));
  if (urlPath === '/api/addons/open' && req.method === 'POST') {
    openFolder(path.join(ROOT, 'addons'));
    return sendJson(res, 200, { ok: true });
  }
  if (urlPath === '/api/addons/config' && req.method === 'POST')
    return readBody(req, (d) => sendJson(res, 200, addonHost.config(d.id, d.config)));
  if (urlPath === '/api/addons/file' && req.method === 'GET')
    return sendJson(res, 200, addonHost.read(new URL(req.url, 'http://localhost').searchParams.get('id')));
  if (urlPath === '/api/addon-icon' && req.method === 'POST')
    return readBody(req, (d) => sendJson(res, 200, addonHost.setIcon(d.id, d.dataUrl)));
  if (urlPath === '/api/addon-icon' && req.method === 'DELETE')
    return sendJson(res, 200, addonHost.removeIcon(new URL(req.url, 'http://localhost').searchParams.get('id')));
  ...
  if (urlPath === '/api/adapters' && req.method === 'GET') {
    const dir = path.join(ROOT, 'data_engine', 'adapters');
    const adapters = [];
    try {
      for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
        if (e.isDirectory() && fs.existsSync(path.join(dir, e.name, 'index.js'))) {
          const m = readMeta(path.join(dir, e.name));
          adapters.push({
            id: e.name,
            name: m.name || '',
            description: m.description || '',
            url: '/data_engine/adapters/' + e.name + '/index.js',
            icon: m.icon ? '/data_engine/adapters/' + e.name + '/' + m.icon : '',
          });
        }
      }
    } catch (_) {}
  ...
  if (urlPath === '/api/adapters/open' && req.method === 'POST') {
    openFolder(path.join(ROOT, 'data_engine', 'adapters'));
    return sendJson(res, 200, { ok: true });
  }
  if (urlPath === '/api/adapters/package' && req.method === 'POST')
    return readBody(req, (d) => {
      // install a whole adapter FOLDER, tree intact
  ...
  if (urlPath === '/api/adapters/delete' && req.method === 'POST')
    return readBody(req, (d) => {
      const id = sanitizeModName(d.id);
      if (!id) return sendJson(res, 400, { error: 'invalid id' });
      try {
        fs.rmSync(path.join(ROOT, 'data_engine', 'adapters', id), { recursive: true, force: true });
      } catch (_) {}
      return sendJson(res, 200, { ok: true });
    });
  ...
  if (urlPath === '/api/user-order-primitives' && req.method === 'GET') {
    const dir = path.join(ROOT, 'packages', 'primitives');
    const primitives = [];
    try {
      for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
        if (e.isDirectory() && fs.existsSync(path.join(dir, e.name, 'index.js'))) {
          ...
          primitives.push({
            ...
            name: m.name || '',
            description: m.description || '',
            url: '/packages/primitives/' + e.name + '/index.js',
            icon: m.icon ? '/packages/primitives/' + e.name + '/' + m.icon : '',
          });
        }
      }
    } catch (_) {}
  ...
  if (urlPath === '/api/user-order-primitives/open' && req.method === 'POST') {
    openFolder(path.join(ROOT, 'packages', 'primitives'));
    return sendJson(res, 200, { ok: true });
  }
  if (urlPath === '/api/user-order-primitives/package' && req.method === 'POST')
    return readBody(req, (d) => {
      // install a whole primitive FOLDER, tree intact
      const id = sanitizeModName(d.id);
      if (!id) return sendJson(res, 400, { error: 'invalid id' });
      if (!Array.isArray(d.files) || !d.files.length) return sendJson(res, 400, { error: 'no files' });
      const dir = path.join(ROOT, 'packages', 'primitives');
  ...
  if (urlPath === '/api/user-order-primitives/delete' && req.method === 'POST')
    return readBody(req, (d) => {
      const id = sanitizeModName(d.id);
      if (!id) return sendJson(res, 400, { error: 'invalid id' });
      try {
        fs.rmSync(path.join(ROOT, 'packages', 'primitives', id), { recursive: true, force: true });
      } catch (_) {}
      return sendJson(res, 200, { ok: true });
    });
...
server.listen(PORT, HOST, () => {
  ...
  if (HOST === '0.0.0.0')
    console.log(`  LAN: http://<your-LAN-IP>:${PORT}/   (reachable from phones/tablets on this wifi)`);
