...
Studies.register({
  ...
  calc(bars, p, ctx) {
    ...
    const up = p.upColor || '#26a69a', down = p.downColor || '#ef5350';
    const data = [];
    bars.forEach((b, i) => {
      const sb = subs[i];
      let d = 0, hi = 0, lo = 0;   // the delta opens at 0 each bar (so the body is anchored to zero)
      if (sb && sb.length) {
        for (const s of sb) {
        ...
        hi = Math.max(0, d); lo = Math.min(0, d);
      }
      const close = d, col = close >= 0 ? up : down;
      data.push({
        time: b.time,
        value: close,
