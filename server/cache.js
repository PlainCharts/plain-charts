...
function readBars(broker, symbol, tf) {
  try {
    ...
    return { cols: BAR_BASE.slice(), rows: [] };
  }
}
...
function mergeBars(existing, incoming) {
  ...
  incoming.forEach((b) => {
    if (b) for (const k in b) if (!BAR_BASE.includes(k) && b[k] != null) extra.add(k);
  });
  const cols = [...BAR_BASE, ...[...extra].sort()];
  const oldIdx = {};
  existing.cols.forEach((c, i) => {
    oldIdx[c] = i;
  });
  const m = new Map();
  existing.rows.forEach((r) => {
    const row = cols.map((c) => (oldIdx[c] == null ? null : r[oldIdx[c]] == null ? null : r[oldIdx[c]]));
    m.set(row[0], row);
  });
  incoming.forEach((b) => {
    ...
    m.set(
      b.time,
      cols.map((c) => (c === 'volume' ? (b.volume == null ? 0 : b.volume) : b[c] == null ? null : b[c])),
    );
  });
...
const toBarObjs = ({ cols, rows }) =>
  rows.map((r) => {
    const o = {};
    cols.forEach((c, i) => {
      const v = r[i];
      if (BAR_BASE.includes(c) || v != null) o[c] = v;
    });
    return o;
  });
const coverageOf = (rows) =>
  rows.length ? { from: rows[0][0], to: rows[rows.length - 1][0], count: rows.length } : null;

// cached timeframes for a (broker, symbol) with coverage + on-disk size
function tfCoverage(broker, symbol) {
  let files = [];
  try {
    files = fs.readdirSync(barsSymbolDir(broker, symbol)).filter((f) => f.endsWith('.json'));
  } catch (_) {
    return [];
  }
  return files
    .map((f) => {
      ...
      let bytes = 0;
      try {
        bytes = fs.statSync(path.join(barsSymbolDir(broker, symbol), f)).size;
      } catch (_) {}
...
const readLib = () => {
  const d = readSettingsFile('brokers/cache-library.json');
...
function handleCache(req, res) {
  ...
  if (p === '/api/cache/library' && req.method === 'POST') {
    return readBody(req, (d) => {
      ...
      const row = {
        ...
        addedMs: i >= 0 ? rows[i].addedMs : Date.now(),
      };
      if (i >= 0) rows[i] = row;
      else rows.push(row);
      writeLib(rows);
  ...
  if (p === '/api/cache/delete' && req.method === 'POST') {
    return readBody(req, (d) => {
      writeLib(readLib().filter((r) => !(r.broker === d.broker && r.symbol === d.symbol)));
      try {
        fs.rmSync(barsSymbolDir(d.broker, d.symbol), { recursive: true, force: true });
      } catch (_) {}
      sendJson(res, 200, { ok: true });
    });
  }
  if (p === '/api/cache/trim' && req.method === 'POST') {
    return readBody(req, (d) => {
      ...
      tfCoverage(d.broker, d.symbol).forEach(({ tf }) => {
        const s = readBars(d.broker, d.symbol, tf);
        writeBars(d.broker, d.symbol, tf, { cols: s.cols, rows: s.rows.filter((r) => r[0] >= startSec) });
      });
      const rows = readLib();
      const i = rows.findIndex((r) => r.broker === d.broker && r.symbol === d.symbol);
      if (i >= 0) {
        rows[i].startMs = Number(d.startMs) || 0;
        writeLib(rows);
      }
      sendJson(res, 200, { ok: true });
    });
  ...
  if (p === '/api/cache/bars' && req.method === 'GET') {
    ...
    if (from != null) {
      const f = Number(from);
      rows = rows.filter((r) => r[0] >= f);
    }
    if (to != null) {
      const t = Number(to);
      rows = rows.filter((r) => r[0] <= t);
    }
  ...
  if (p === '/api/cache/bars' && req.method === 'POST') {
    return readBody(req, (d) => {
      if (!d || !d.broker || !d.symbol || !d.tf || !Array.isArray(d.bars))
