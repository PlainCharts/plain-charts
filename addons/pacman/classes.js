...
export const CATEGORIES = [
  ...
  { key: 'themes', name: 'App Themes' },
  { key: 'chart-themes', name: 'Chart Themes' },
...
export const LOCAL = {
  ...
  async studies() {
    ...
    return (r.studies || []).map((/** @type {any} */ s) => ({
      id: s.id,
      name: s.name || s.id,
  ...
  async themes() {
    ...
    return (r.themes || []).map((/** @type {any} */ th) => ({
      id: th.name,
      name: th.name,
  ...
  async tools() {
    ...
    return (r.tools || []).map((/** @type {any} */ tl) => ({
      id: tl.folder,
      name: tl.name || tl.folder,
      description: tl.description || '',
      icon: tl.icon ? '/packages/tools/' + tl.folder + '/' + tl.icon : '',
    }));
  },
  /** @returns {Promise<{id:string,name:string}[]>} */
  async addons() {
    const r = await fetch('/api/addons', { cache: 'no-store' }).then((x) => x.json());
    return (r.addons || []).map((/** @type {any} */ a) => ({
      id: a.id,
      name: a.name || a.id,
      description: a.description || '',
      icon: a.hasIcon ? '/addons/' + a.id + '/icon.png' : '',
    }));
  },
  /** @returns {Promise<{id:string,name:string}[]>} */
  async vocab() {
  ...
  async adapters() {
    ...
    return (r.adapters || []).map((/** @type {any} */ a) => ({
      id: a.id,
      name: a.name || a.id,
      description: a.description || '',
      icon: a.icon || '',
  ...
  async primitives() {
    ...
    return (r.primitives || []).map((/** @type {any} */ p) => ({
      id: p.id,
      name: p.name || p.id,
...
export const INSTALL_EP = /** @type {Record<string,string>} */ ({
  studies: '/api/user-studies/package',
  addons: '/api/addons/package',
  tools: '/api/user-tools/package',
  adapters: '/api/adapters/package',
  primitives: '/api/user-order-primitives/package',
});
// Single-file classes: pick one file, save it via the folder-library endpoint ({name, data}).
export const INSTALL_FILE_EP = /** @type {Record<string,{url:string,accept:string}>} */ ({
  themes: { url: '/api/themes/save', accept: '.json' },
  'chart-themes': { url: '/api/chart-themes/save', accept: '.json' },
  vocab: { url: '/api/vocab/save', accept: '.json' },
});
