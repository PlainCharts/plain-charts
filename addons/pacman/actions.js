...
export function createActions({ t, cfg, state, setStatus, refresh }) {
  ...
  async function install(/** @type {any} */ p) {
    ...
    if (!folderEp && !fileEp) {
      setStatus(t('Install is not available for this class yet'));
      return;
    }
    ...
    try {
      if (folderEp) {
        ...
        const r = await fetch(folderEp, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: p.id, files }),
        }).then((x) => x.json());
        if (r && r.error) {
          setStatus(t('Install failed') + ': ' + r.error);
          return;
        }
      } else {
        ...
        const r = await fetch(fileEp.url, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      await refresh();
      setStatus(t('Installed') + ' ' + p.id);
    } catch (e) {
      setStatus(t('Install failed') + ': ' + ((e && /** @type {any} */ (e).message) || e));
    }
  }
  async function uninstall(/** @type {any} */ p) {
    const ep = UNINSTALL_EP[state.category];
    if (!ep) {
      setStatus(t('Uninstall is not available for this class yet'));
      return;
    }
    ...
    try {
      await fetch(ep.url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ [ep.field]: p.id }),
      });
      state.sel = null;
      await refresh();
      setStatus(t('Removed') + ' ' + p.id);
    } catch (_) {
      setStatus(t('Remove failed'));
    }
  }
