...
export function createActions({ t, cfg, state, setStatus, refresh }) {
  async function install(/** @type {any} */ p) {
    if (p.class !== 'studies') { setStatus(t('Only studies install in this build')); return; }
    setStatus(t('Installing') + ' ' + p.id + '…');
    try {
      const code = await fetch(cfg.repo + '/' + p.path + '/' + p.runtime, { cache: 'no-store' }).then((x) => x.text());
      await fetch('/api/user-studies', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: p.id, code }) });
  ...
  function installFile(/** @type {{url:string,accept:string}} */ fep) {
    ...
    inp.onchange = async () => {
      ...
      try {
        ...
        const data = Object.assign({}, obj); delete data.name;
