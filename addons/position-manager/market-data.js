...
export function createMarketData(ot, { quoteLine }) {
  ...
  const wireQuote = () => {
    const gen = ++wireGen;
    if (resolveWd) { clearTimeout(resolveWd); resolveWd = null; }
    const a = /** @type {import('/data_engine/index.js').BrokerAdapter|null} */ (ad()); if (!a) { quoteLine.textContent = t('broker not connected'); return; }   // the broker facade returns a proxy|core union; it IS a conforming adapter
    ...
    const onResolved = (/** @type {import('/data_engine/index.js').Instrument|null} */ inst) => {
      if (gen !== wireGen) { dbg('DROP stale resolve for', cfg.symbol, 'contract', inst && inst.id); return; }   // superseded -> leave the current sub alone
      if (!inst) { quoteLine.textContent = t('symbol not found:') + ' ' + cfg.symbol; return; }
      if (qSub) { try { qSub.api.unsubscribeQuotes(qSub.id, qSub.cb); } catch (_) {} qSub = null; }   // swap: drop the old sub now a replacement is ready
      ...
      const cb = (/** @type {any} */ u) => {
        if (u.bid != null) q.bid = u.bid; if (u.ask != null) q.ask = u.ask; if (u.last != null) q.last = u.last;
        const sp = (q.bid != null && q.ask != null) ? q.ask - q.bid : null;
        const dc = state.domDecimals != null ? state.domDecimals : 2, pxf = (/** @type {number|null} */ v) => (v == null ? '–' : Number(v).toFixed(dc));
        ...
        if (mid != null && q.last != null && Math.abs(q.last - mid) > 5) dbg('STALE last', q.last, 'vs mid', mid, '-> using mid');
      ...
      a.subscribeQuotes(inst.id, cb); qSub = { api: a, id: inst.id, cb };
    };
    ...
    const issueResolve = () => {
      ...
      let answered = false;
      a.resolveSymbol(cfg.symbol, (/** @type {any} */ inst) => { if (gen !== wireGen) return; answered = true; if (resolveWd) { clearTimeout(resolveWd); resolveWd = null; } onResolved(inst); });
      resolveWd = setTimeout(() => { resolveWd = null; if (gen !== wireGen || answered) return; dbg('resolve watchdog re-resolve', cfg.symbol); issueResolve(); }, 2000);
    };
    issueResolve();
  };
  ...
  const teardown = () => {
    if (resolveWd) { clearTimeout(resolveWd); resolveWd = null; }
    if (qSub) { try { qSub.api.unsubscribeQuotes(qSub.id, qSub.cb); } catch (_) {} qSub = null; }
  };
