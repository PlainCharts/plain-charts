...
export function createAuto(ot, { section, autoPane, visPane }) {
  ...
  const addBtn = mk(t('+ Add Level'), () => engine.addLevel()); addBtn.style.marginTop = '6px';
  ...
  const syncTable = () => { if (!engine.active()) return; if (engine.storeLevels().length !== levelCells.length) renderLevels(); else refreshLevelCells(); };
  /** @param {number} i @param {'stop'|'target'} which @param {number|null} v */
  const onLevelPriceEdit = (i, which, v) => {
    const L = engine.curLevels(); if (!L || !L[i]) return;
    if (which === 'stop') engine.setLevelStop(i, /** @type {number} */ (v), true); else engine.setLevelTgt(i, /** @type {number} */ (v), true);
  };
  function refreshLevelCells() {
    const L = engine.curLevels(); if (!L) return;
  ...
  function renderLevels() {
    ...
    const L = engine.curLevels(), ai = engine.curIdx(), st = engine.phaseState();
    ...
    L.forEach((/** @type {Level} */ lv, /** @type {number} */ i) => {
      ...
      const qtyIn = numCell(lv.tgtQty, (v) => { const q = Math.abs(v || 0) || 1; planApi.setLevel(planCtx().b, planCtx().s, i, { qty: q }); qtyIn.value = /** @type {any} */ (q); engine.syncLevelsCfg(); });
      const stopIn = numCell(lv.stop, (v) => onLevelPriceEdit(i, 'stop', v));
      const rm = el('button', 'background:none;border:none;color:var(--tx-dim);cursor:pointer;font-size:15px;line-height:1;padding:0;', '×'); rm.onclick = () => engine.removeLevel(i);
  ...
  function syncButtons() {
    ...
    if (ph === 'position') { const pp = engine.posOf(); pendBtn.textContent = pp ? ((pp.side === 'short' ? t('Short') : t('Long')) + ' ' + fmt(pp.qty)) : t('Position'); grey(pendBtn, true); armBtn.textContent = t('Arm'); grey(armBtn, false); }
    updateSide();
  }
  const onPendBtn = () => { const ph = engine.phase(); if (ph === 'idle') engine.showPending(); else if (ph === 'shown' || ph === 'armed') engine.clearPending(); };
  const onArmBtn = () => { const ph = engine.phase(); if (ph === 'shown') engine.armPending(); else if (ph === 'armed') engine.disarmPending(); else if (ph === 'active') engine.disarmActive(); else if (ph === 'position') engine.armFromPosition(); };
  ...
  return { active: engine.active, syncButtons, onFlat: engine.onFlat, onManualEntry: engine.onManualEntry, resetPlan: engine.resetPlan };
}
