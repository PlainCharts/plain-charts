...
export function createAutoEngine(ot) {
  ...
  const refDir = () => {
    const p = planApi.get(planCtx().b, planCtx().s);
    return {
      ref: p.ref != null ? Number(p.ref) : null,
      dir: /** @type {PositionSide} */ (p.dir === 'short' ? 'short' : 'long'),
    };
  };
  ...
  function applyRules() {
    if (pend && pend.armed && pend.entryRule) {
      ...
      if (ref != null)
        watch.update(pend.entryRule, {
          price: ref,
          dir: dir === 'long' ? 'down' : 'up',
          action: { do: dir === 'long' ? 'buy' : 'sell', qty: unitsQty() },
        });
    }
    if (bracket) {
      const Ls = storeLevels(),
        idx = curIdx(),
        L = Ls[idx],
        last = idx >= Ls.length - 1;
      const { dir } = refDir();
      const sDir = dir === 'long' ? 'down' : 'up',
        tDir = dir === 'long' ? 'up' : 'down';
      if (L && L.stop != null) {
      ...
      } else if (bracket.stopRule) {
        watch.remove(bracket.stopRule);
        bracket.stopRule = null;
      }
      ...
      } else if (bracket.tgtRule) {
        watch.remove(bracket.tgtRule);
  ...
  const posOf = () => {
    ...
    return b.entry != null && b.qty > 0 ? { side: b.side, qty: b.qty, entry: Number(b.entry) } : null;
  };
  const watch = api.watcher({
    ...
    execute: (/** @type {any} */ cmd) => {
      ...
      /** @param {any} r */ const done = (r) => {
        if (r && r.error) clog('order error: ' + r.error, true);
      };
      if (cmd.close) {
  ...
  const applyThreshold = () =>
    watch.setThreshold((Math.abs(Number(cfg.thresholdTicks)) || 1) * (state.domTick || 0.25));
  ...
  /** @type {any} */ (window).__ot = () => ({
    bracket: !!bracket,
    pend: pend ? { armed: pend.armed } : null,
    pending: !!pending,
    plan: planApi.get(planCtx().b, planCtx().s),
    symbol: cfg.symbol,
    lastPx: state.lastPx,
    pos: posOf(),
    rules: watch.rules().map((/** @type {any} */ r) => ({
      id: r.id,
      price: r.price,
      from: r.from,
      do: r.action && r.action.do,
      armed: r.armed,
      fired: r.fired,
    })),
  });
  ...
  const storeLevels = () => {
    const { b, s } = planCtx();
    const pls = planApi.get(b, s).levels;
    return (Array.isArray(pls) ? pls : []).map((/** @type {any} */ pl) => ({
      tgt: pl && pl.target != null ? Number(pl.target) : null,
      tgtQty: pl && pl.qty != null ? Math.abs(Number(pl.qty)) || 1 : 1,
      stop: pl && pl.stop != null ? Number(pl.stop) : null,
    }));
  };
  const curLevels = () => (bracket || pend || pending ? storeLevels() : null);
  const curIdx = () => {
    if (!bracket) return 0;
    const ai = Number(planApi.get(planCtx().b, planCtx().s).activeIdx);
    return Number.isFinite(ai) && ai >= 0 ? ai : 0;
  };
  ...
  const setLevelStop = (i, px, commit) => {
    const L = curLevels();
    if (!L || !L[i] || !commit) return;
    const p = qtick(px);
    const { b, s } = planCtx();
    if (p == null)
      planApi.setLevel(b, s, i, { stop: null }); // cleared cell -> clear the rung's stop
    else if (pend)
      planApi.commitStop(b, s, i, p, {
        flip: !pend.armed,
        snap: (/** @type {number} */ v) => /** @type {number} */ (qtick(v)),
      });
  ...
  const seedOffset = (ref) => {
    const o = offsetPrice();
    const r = Math.abs(Number(ref)) || 0;
    return o > 0 && r > 0 && o < r * 0.5 ? o : Math.max(r * SEED_PCT, 0);
  };
  ...
  const openBracket = (p, entry) => {
    const { b, s } = planCtx();
    planBridge.push();
    planBridge.arm(); // flags on (bridge owns the plan) + LIVE colours; the store levels stay as planned
    if (!storeLevels().length) {
      const off = seedOffset(entry);
      planApi.setLevel(b, s, 0, {
        stop: off ? /** @type {number} */ (qtick(p.dir === 'long' ? entry - off : entry + off)) : null,
        target: off ? /** @type {number} */ (qtick(p.dir === 'long' ? entry + off : entry - off)) : null,
        qty: 1,
      });
    }
    ...
    say(
      p.dir +
        ' @ ' +
        entry +
        (l0.stop != null ? '   stop ' + l0.stop : '') +
        (l0.tgt != null ? '   target ' + l0.tgt : '') +
        (Ls.length > 1 ? '   (+' + (Ls.length - 1) + ' levels)' : ''),
    );
  };
  ...
  const syncLevelsCfg = () => {
    const L = curLevels();
    if (L) {
      cfg.levels = L.map((/** @type {Level} */ l) => ({ tgtQty: Math.abs(Number(l.tgtQty)) || 1 }));
      save();
    }
  };
  ...
  function addLevel() {
    ...
    if (!L || pending) return;
    const { b, s } = planCtx();
    const p = planApi.get(b, s);
    const pls = Array.isArray(p.levels) ? p.levels : [];
    const i = pls.length;
    if (pend) {
      // pre-trade: spaced by the app-seeded rung-0 distance (fallback: the offset), relative to the entry pivot
      ...
      planApi.setLevel(b, s, i, {
        stop: qtick(up ? ref - sDist * (i + 1) : ref + sDist * (i + 1)),
        target: qtick(up ? ref + tDist * (i + 1) : ref - tDist * (i + 1)),
        qty: 1,
      });
    ...
    const e = ref,
      up = dir !== 'short';
    planApi.setLevel(b, s, i, {
      stop: qtick(up ? e - off * (i + 1) : e + off * (i + 1)),
      target: qtick(up ? e + off * (i + 1) : e - off * (i + 1)),
      qty: 1,
    });
    syncLevelsCfg();
  ...
  function removeLevel(i) {
    const L = curLevels();
    if (!L || pending || L.length <= 1) return;
    if (bracket && i <= curIdx()) return say('cannot remove the active or a completed level');
    const { b, s } = planCtx();
    const pls = (planApi.get(b, s).levels || []).slice();
    if (i < pls.length) {
      pls.splice(i, 1);
      planApi.setLadder(b, s, pls);
    }
  ...
  // ----- phase machine -----
  const phase = () =>
    bracket || pending ? 'active' : pend ? (pend.armed ? 'armed' : 'shown') : posOf() ? 'position' : 'idle';
  ...
  function disarmActive() {
    ...
    say(
      posOf()
        ? 'auto layer disarmed — position stays; Arm to re-add stop/target'
        : 'disarmed — automated exits cancelled',
    );
  }
  ...
  function armPending() {
    ...
    const { ref, dir } = refDir();
    if (ref == null) return say('no entry level yet — wait for the projection');
    pend.entryRule = watch.add({
      price: ref,
      dir: dir === 'long' ? 'down' : 'up',
      action: { do: dir === 'long' ? 'buy' : 'sell', qty: unitsQty() },
    });
    pend.armed = true;
    planBridge.arm(); // LIVE: the app redraws the bracket in live colours (planning -> live mode)
    ui.syncButtons();
    ...
    say(
      'ARMED ' +
        ...
        ' ' +
        unitsQty() +
        ' — entry fires at market when price touches the entry bead; the levels activate after the fill',
    );
  }
  function disarmPending() {
    if (!pend || !pend.armed) return;
    if (pend.entryRule) {
      watch.remove(pend.entryRule);
      pend.entryRule = null;
    }
    pend.armed = false;
    planBridge.disarm(); // back to planning mode (plan colours, no live trigger)
    ui.syncButtons();
  ...
  watch.onFire((/** @type {any} */ rule) => {
    dbg(
      'FIRE',
      JSON.stringify({ id: rule.id, price: rule.price, from: rule.from, do: rule.action && rule.action.do }),
      'entryRule=',
      pend && pend.entryRule,
      'stopRule=',
      bracket && bracket.stopRule,
      'tgtRule=',
      bracket && bracket.tgtRule,
      'lastPx=',
      state.lastPx,
    );
    ...
    if (!bracket) return;
    if (rule.id === bracket.stopRule) {
      teardownBracket('stop hit — closing at market');
      return;
    }
    if (rule.id === bracket.tgtRule) {
      const idx = curIdx();
      if (idx >= storeLevels().length - 1) {
        teardownBracket('target ' + (idx + 1) + ' hit — position closed');
        return;
      }
      bracket.tgtRule = null;
      armLevel(idx + 1);
  ...
  const onFlat = () => {
    ...
    planBridge.clear();
    resetVis();
    ui.renderLevels(); // drop any leftover plan projection so a closed trade leaves no stale bracket
  };

  // react to the shared events
  ot.on('fill', (/** @type {any} */ f) => {
    if (pending && f.symbol === cfg.symbol && f.side === pending.entrySide && f.price) {
      const p = pending;
      pending = null;
      openBracket(p, Number(f.price));
    }
  });
  ...
  const syncOwnerGestures = () => {
    ...
    if (pend && !pend.armed && planApi.isArmed(b, s)) {
      armPending();
      if (pend && !pend.armed) planApi.setArmed(b, s, false);
    } else if (pend && pend.armed && !planApi.isArmed(b, s)) disarmPending();
    const pq = Number(planApi.get(b, s).qty);
    if (pend && pq > 0 && pq !== Math.abs(Number(cfg.qty) || 1)) {
      cfg.qty = pq;
      ot.save();
      ot.emit('units-sync');
      ot.emit('units');
  ...
  planApi.subscribe(() => {
    ...
    ui.syncTable();
    ui.syncVisUI();
  }); // ANY store change (seed, chart drag, table edit, other window) -> owner gestures + re-derive the watcher rules + refresh the table + visibility toggles; auto-unsubscribed on close
  ot.on('units', () => {
    if (pend) {
      const { b, s } = planCtx();
      const pls = planApi.get(b, s).levels || [];
      if (pls.length === 1) planApi.setLevel(b, s, 0, { qty: unitsQty() });
      planApi.setLevels(b, s, { qty: unitsQty() });
    }
    applyRules();
    ui.renderLevels();
  }); // new Units -> entry rule qty + a single rung's exit qty + the shared plan.qty (pill controller) follow
