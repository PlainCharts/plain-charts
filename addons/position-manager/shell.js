...
export function createShell(ot) {
  ...
  const updateHeader = () => { hdrSym.textContent = cfg.symbol || '—'; hdrBrk.textContent = (cfg.broker || 'active').toString().toUpperCase(); };
  ...
  const section = (title, host) => { const h = el('div', 'color:var(--tx-dim);font-size:11px;letter-spacing:.06em;margin:12px 0 6px;', title); (host || autoPane).appendChild(h); const box = el('div', ''); (host || autoPane).appendChild(box); return box; };
  ...
  const threshInput = document.createElement('input'); threshInput.type = 'number'; threshInput.value = /** @type {any} */ (cfg.thresholdTicks != null ? cfg.thresholdTicks : 1); threshInput.style.cssText = INP + 'width:52px;';   // match the Units input
  threshInput.onchange = () => { cfg.thresholdTicks = parseFloat(threshInput.value) || 1; emit('rethreshold'); save(); };
  const universalBox = el('div', 'margin-top:14px;padding-top:12px;border-top:1px solid var(--bd-soft);');
  universalBox.append(el('div', 'color:var(--tx-dim);font-size:11px;letter-spacing:.06em;margin-bottom:8px;', t('CONFIGURATION')));
  ...
  const gear = /** @type {HTMLButtonElement} */ (el('button', 'width:28px;height:28px;background:var(--bg);border:1px solid var(--bd);color:var(--tx-dim);border-radius:4px;cursor:pointer;font-size:14px;line-height:1;padding:0;', '⚙'));
  gear.title = t('Button colors');
  gear.onmouseenter = () => { gear.style.color = 'var(--tx)'; gear.style.borderColor = 'var(--tx-dim)'; };
  gear.onmouseleave = () => { gear.style.color = 'var(--tx-dim)'; gear.style.borderColor = 'var(--bd)'; };
  gearRow.append(gear);
  root.appendChild(gearRow);

  const overlay = el('div', 'position:absolute;inset:0;background:rgba(0,0,0,.45);display:none;align-items:center;justify-content:center;z-index:60;');
  const card = el('div', 'background:var(--bg);border:1px solid var(--bd);border-radius:8px;padding:16px;min-width:190px;box-shadow:0 6px 24px rgba(0,0,0,.3);');
  ...
  const colorRow = (label, key) => {
    ...
    const sw = colorSwatch(colors()[key], (/** @type {string} */ v) => { /** @type {any} */ (cfg).colors = { ...colors(), [key]: v }; save(); emit('recolor'); });
  ...
  closeBtn.style.width = '100%'; closeBtn.style.marginTop = '4px';
  closeBtn.onclick = () => { overlay.style.display = 'none'; };
  card.append(closeBtn);
  overlay.append(card);
  overlay.onclick = (/** @type {MouseEvent} */ e) => { if (e.target === overlay) overlay.style.display = 'none'; };
