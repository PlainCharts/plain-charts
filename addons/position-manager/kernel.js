...
export function createKernel(root, api, cfg) {
  ...
  const fmt = (v) => (v == null || v === '' ? '–' : (typeof v === 'number' ? Number(v.toFixed(Math.abs(v) >= 100 ? 2 : 5)).toLocaleString('en-US') : String(v)));
  ...
  /** @param {string} bg */
  const BTN = (bg) => 'flex:1;padding:9px;border:none;border-radius:6px;color:#fff;font:inherit;font-size:13px;font-weight:600;cursor:pointer;background:' + bg + ';';
  ...
  const clog = (m, err) => { try { if (api.console) (err ? api.console.error : api.console.info)(m); } catch (_) {} };
  ...
  const exec = (cmd) => api.trade.command(cmd).catch((/** @type {any} */ e) => ({ ok: false, error: (e && e.message) || String(e) }));
  ...
  const tfSeconds = () => { const id = String((api.chart.timeframe && api.chart.timeframe()) || '5m'); const m = id.match(/(\d+)\s*([smhdw])/i); const n = m ? (Number(m[1]) || 1) : 1; const u = m ? m[2].toLowerCase() : 'm'; return n * (u === 's' ? 1 : u === 'm' ? 60 : u === 'h' ? 3600 : u === 'd' ? 86400 : u === 'w' ? 604800 : 60); };
  ...
  const roundTick = (px) => { const tk = /** @type {number} */ (state.domTick) > 0 ? /** @type {number} */ (state.domTick) : 0.25, dc = state.domDecimals != null ? state.domDecimals : 2; return (Math.round(px / tk) * tk).toFixed(dc); };
  /** snap a price to the instrument's tick, numeric (null passes through) @param {number|null} px @returns {number|null} */
  const qtick = (px) => { if (px == null || !isFinite(px)) return px; const tk = /** @type {number} */ (state.domTick) > 0 ? /** @type {number} */ (state.domTick) : 0.25, dc = state.domDecimals != null ? state.domDecimals : 2; return Number((Math.round(px / tk) * tk).toFixed(dc)); };
  // UNIVERSAL offset -> a PRICE distance, honoring the unit dropdown. ticks -> x tickSize; points -> x 1. Falls back to 10.
  /** @returns {number} */
  const offsetPrice = () => { const n = Math.abs(Number(cfg.offset)); const off = (isFinite(n) && n > 0) ? n : 10; const tk = /** @type {number} */ (state.domTick) > 0 ? /** @type {number} */ (state.domTick) : 0.25; return cfg.offsetUnit === 'points' ? off : off * tk; };
  ...
  const on = (evt, fn) => { let s = listeners.get(evt); if (!s) { s = new Set(); listeners.set(evt, s); } s.add(fn); return () => { const set = listeners.get(evt); if (set) set.delete(fn); }; };
  /** @param {string} evt @param {...any} args */
  const emit = (evt, ...args) => { const s = listeners.get(evt); if (!s) return; for (const fn of [...s]) { try { fn(...args); } catch (e) { clog('handler error (' + evt + '): ' + ((e && /** @type {any} */ (e).message) || e), true); } } };

  return {
    root, api, cfg, state, save,
    fmt, el, BTN, INP, colors,
    clog, ad, exec, tfSeconds,
    roundTick, qtick, offsetPrice,
    on, emit,
