...
module.exports = {
  ...
  ui(root, api) {
    ...
    const out = el(
      'div',
      'margin:8px 0;color:var(--tx-dim);font-size:12px;',
      t('Capture the chart — exactly what is painted.'),
    );
    /** @param {string} m */
    const say = (m) => {
      out.textContent = m;
    ...
    const grab = () => {
      const c = api.chart && api.chart.snapshot && api.chart.snapshot();
      if (!c) say(t('no chart to capture'));
      return c;
    };
    ...
    const copy = () => {
      const c = grab();
      if (!c) return;
      c.toBlob(async (/** @type {Blob | null} */ b) => {
        if (!b) return;
        try {
          await navigator.clipboard.write([new ClipboardItem({ 'image/png': b })]);
          say(t('copied to clipboard'));
        } catch (e) {
          say(t('copy failed:') + ' ' + /** @type {any} */ ((e && /** @type {any} */ (e).message) || e));
        }
      }, 'image/png');
    };
    const print = () => {
      ...
      const doc = /** @type {Window} */ (frame.contentWindow).document;
      doc.open();
      doc.write(
        '<html><head><title>Chart</title><style>@page{size:landscape;margin:10mm;}html,body{margin:0}img{max-width:100%}</style></head><body><img src="' +
          url +
          '"></body></html>',
      );
      doc.close();
      const go = () => {
        try {
          /** @type {Window} */ (frame.contentWindow).focus();
          /** @type {Window} */ (frame.contentWindow).print();
        } catch (_) {}
  ...
  /** @param {any} ctx */
  start(ctx) {
    ctx.log('screenshot addon ready');
  },
  stop() {},
};
