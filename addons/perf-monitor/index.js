...
const EXEC_METHODS = /** @type {Record<string, 1>} */ ({
  placeOrder: 1,
  modifyOrder: 1,
  cancelOrder: 1,
  closePosition: 1,
...
module.exports = {
  ...
  start(ctx) {
    ...
    const req = g.require;
    if (!req || !g.process || g.process.platform !== 'linux') {
      ctx.log('OS sampler off (needs Electron + Linux /proc)');
      return;
    }
    const fs = req('fs');
    ...
    const publish = (key, value) => {
      try {
        perfCh.postMessage({ set: { key, value } });
      } catch (_) {}
    };
    /** @param {string} key */
    const unpublish = (key) => {
      try {
        perfCh.postMessage({ remove: { key } });
      } catch (_) {}
    };
    ...
    const timer = setInterval(() => {
      ...
      while (grew) {
        grew = false;
        for (const [p, pp] of ppids)
          if (tree.has(pp) && !tree.has(p)) {
            tree.add(p);
            grew = true;
          }
      }
  ...
  stop() {
    const t = /** @type {any} */ (this)._osTimer;
    if (t) clearInterval(t);
    const c = /** @type {any} */ (this)._osCh;
    if (c) {
      try {
        c.close();
      } catch (_) {}
  ...
  ui(root, api) {
    ...
    /** @type {any} */
    let platform = null;
    import('/data_engine/index.js').then((m) => {
      platform = m.platform;
    });
    ...
    bb.onmessage = (e) => {
      ...
      if (m.dir === 'out') {
        ...
        if (m.callId != null) {
          ...
          if (p) {
            // first reply only: getBars = first chunk, subs = ack
            pending.delete('r' + m.callId);
            const ms = performance.now() - p.t;
            record(p.key, ms);
            const method = p.key.slice(4);
            if (EXEC_METHODS[method]) {
              captures.unshift({ t: Date.now(), key: p.key, ms });
    ...
    ob.onmessage = (e) => {
      ...
      if (m.dir === 'cmd') {
        counters.cmds++;
        pending.set('c' + m.win + ':' + m.callId, { t: performance.now(), key: 'cmd:' + (m.cmd && m.cmd.type) });
      } else if (m.dir === 'ack') {
        const p = pending.get('c' + m.to + ':' + m.callId);
        if (p) {
          pending.delete('c' + m.to + ':' + m.callId);
          const ms = performance.now() - p.t;
          record(p.key, ms);
          captures.unshift({ t: Date.now(), key: p.key, ms });
          if (captures.length > 8) captures.pop();
    ...
    const gc = setInterval(() => {
      ...
      for (const [k, v] of pending) if (v.t < cut) pending.delete(k);
    }, 30000);
    ...
    status.style.cssText = 'color:var(--tx-dim);margin-right:auto;';
    const reset = document.createElement('button');
    reset.textContent = t('Reset stats');
    reset.style.cssText =
      'background:var(--bg);color:var(--tx);border:1px solid var(--bd);border-radius:4px;padding:2px 8px;cursor:pointer;font:inherit;';
    reset.onclick = () => {
      lat.clear();
      captures.length = 0;
    };
    ...
    const row = (cells, cols, dim) =>
      '<div style="display:grid;grid-template-columns:' +
      cols +
      ';gap:4px;padding:1px 2px;' +
      (dim ? 'opacity:.45;' : '') +
      '">' +
      cells
        .map((c) => '<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + c + '</span>')
        .join('') +
      '</div>';
    /** @param {number} v @param {number} warn @param {number} bad */
    const grade = (v, warn, bad) =>
      v >= bad
        ? '<span style="color:var(--neg,#e66)">' + v + '</span>'
        : v >= warn
          ? '<span style="color:#cc0">' + v + '</span>'
          : String(v);
    /** @param {number} ms */
    ...
    function render() {
      ...
      const wins = all
        ...
        .sort(
          (/** @type {any} */ a, /** @type {any} */ b) =>
            String(a.page).localeCompare(String(b.page)) || String(a.win).localeCompare(String(b.win)),
        );
      ...
      html += row(
        [t('window'), t('cpu%'), t('pss'), t('heap'), t('lag'), t('task'), t('draw/s')].map(
          (h) => '<span style="color:var(--tx-dim)">' + h + '</span>',
        ),
        wcols,
      );
      for (const r of wins) {
      ...
      if (procs.length) {
        ...
        for (const r of rest.sort((/** @type {any} */ a, /** @type {any} */ b) => b.cpu - a.cpu))
          html += row(
            [esc(r.kind), grade(r.cpu, 25, 60), (r.pssMb != null ? r.pssMb : r.rssMb) + 'M', String(r.pid)],
            pcols,
          );
        ...
        const tPss = procs.reduce(
          (/** @type {number} */ s, /** @type {any} */ r) => s + (r.pssMb != null ? r.pssMb : r.rssMb),
          0,
        );
      ...
      html += row(
        [t('broker-bus out'), rate('bbOut', counters.bbOut, dt), t('broker-bus in'), rate('bbIn', counters.bbIn, dt)],
        bcols,
      );
      html += row(
        [t('quote ticks'), rate('quotes', counters.quotes, dt), t('order cmds'), rate('cmds', counters.cmds, dt)],
        bcols,
      );
      html += row(
        [t('sync orders'), rate('s.o', storeMsgs.orders, dt), t('sync fills'), rate('s.f', storeMsgs.fills, dt)],
        bcols,
      );
      html += row(
        [
          t('sync positions'),
          rate('s.p', storeMsgs.positions + storeMsgs.positionLots, dt),
          t('console'),
          rate('s.c', storeMsgs.console, dt),
        ],
        bcols,
      );
      ...
      if (keys.length) {
        ...
        html += row(
          [t('round-trip'), t('n'), t('last'), t('avg'), t('max')].map(
            (h) => '<span style="color:var(--tx-dim)">' + h + '</span>',
          ),
          lcols,
        );
        ...
        html +=
          '<div style="color:var(--tx-dim);padding:1px 2px">' +
          t('waiting for activity (RPCs / order commands are captured as they happen)') +
          '</div>';
      }
      ...
      if (captures.length) {
        ...
        for (const c of captures)
          html += row([new Date(c.t).toTimeString().slice(0, 8), esc(c.key), fmtMs(c.ms) + 'ms'], ccols);
      }
      ...
      if (conns.length) {
        ...
        for (const c of conns) {
          ...
          html += row(
            [
              '<span style="color:' + (c.connected ? 'var(--pos,#6c6)' : 'var(--tx-dim)') + '">●</span>',
              esc(c.name || c.label || c.id),
              c.active ? t('active') : '',
              skew,
            ],
            ncols,
