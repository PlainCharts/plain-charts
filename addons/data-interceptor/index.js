...
module.exports = {
  ...
  ui(root, api) {
    ...
    count.style.cssText = 'color:var(--tx-dim);margin-right:auto;';
    const dl = document.createElement('button');
    dl.textContent = t('Download JSON');
    ...
    function render() {
      ...
      for (const key of keys) {
        const m = /** @type {Map<string, FieldInfo>} */ (reg.get(key));
        total += m.size;
        html +=
          '<div style="padding:5px 2px 2px;color:#5aa9e6;font-weight:bold">' +
          esc(key) +
          ' <span style="color:var(--tx-dim);font-weight:normal">(' +
          m.size +
          ')</span></div>';
        for (const path of [...m.keys()].sort()) {
          const info = /** @type {FieldInfo} */ (m.get(path));
          html +=
            '<div style="display:grid;grid-template-columns:1fr 44px 88px 38px;gap:4px;padding:1px 2px">' +
            ...
            info.type +
            '</span>' +
            '<span style="color:#26a69a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="' +
            esc(fmt(info.value)) +
            '">' +
            esc(fmt(info.value)) +
            '</span>' +
            '<span style="color:var(--tx-dim);text-align:right">' +
            info.count +
            '</span></div>';
        }
      }
      body.innerHTML =
        html ||
        '<div style="color:var(--tx-dim);padding:8px">' + t('Waiting for feed… load a symbol / go live.') + '</div>';
    ...
    dl.onclick = () => {
      ...
      for (const [key, m] of reg) {
        out[key] = {};
        for (const [path, info] of m) out[key][path] = info.type;
      }
      const blob = new Blob([JSON.stringify(out, null, 2)], { type: 'application/json' });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = 'broker-schema.json';
      a.click();
