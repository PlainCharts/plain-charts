...
try {
  ...
  console.error('[pty] node-pty unavailable:', (e && e.message) || e);
}
...
ipcMain.on('pty-spawn', (e, { sessionId, cols, rows } = {}) => {
  if (!ptyLib || !sessionId) {
    try {
      e.sender.send('pty-exit', { sessionId, code: -1, error: ptyLib ? 'no sessionId' : 'node-pty unavailable' });
    } catch (_) {}
    return;
  }
  ...
  if (s) {
    s.owner = e.sender;
    try {
      e.sender.send('pty-data', { sessionId, data: s.buf });
    } catch (_) {}
    return;
  } // reconnect: replay
  ...
  pty.onData((data) => {
    s.buf = (s.buf + data).slice(-PTY_BUF_CAP);
    try {
      if (s.owner && !s.owner.isDestroyed()) s.owner.send('pty-data', { sessionId, data });
    } catch (_) {}
  });
  pty.onExit(({ exitCode }) => {
    ptys.delete(sessionId);
    try {
      if (s.owner && !s.owner.isDestroyed()) s.owner.send('pty-exit', { sessionId, code: exitCode });
    } catch (_) {}
  });
});
ipcMain.on('pty-write', (_e, { sessionId, data } = {}) => {
  const s = ptys.get(sessionId);
  if (s) {
    try {
      s.pty.write(data);
    } catch (_) {}
