...
try {
  const st = fs.statSync(APP_LOG);
  if (st.size > 5 * 1024 * 1024) fs.renameSync(APP_LOG, APP_LOG + '.1');
} catch (_) {} // rotate at 5MB
...
function log(level, sub, msg, meta) {
  ...
  try {
    ...
    line = JSON.stringify({ ts: rec.ts, run: RUN_ID, level, sub, msg: String(msg) });
  }
  line += '\n';
  try {
    fs.appendFileSync(APP_LOG, line);
  } catch (_) {} // SYNC -> flushed before a fatal exit
  try {
...
const shortURL = (/** @type {any} */ wc) => {
  try {
    return (wc.getURL() || '').replace(ORIGIN, '') || 'main';
  } catch (_) {
    return '?';
  }
};
  ...
  try {
    for (const w of BrowserWindow.getAllWindows()) {
      try {
        out.push({
          id: w.id,
          url: shortURL(w.webContents),
          title: w.getTitle(),
          visible: w.isVisible(),
          loading: w.webContents.isLoading(),
          devtools: w.webContents.isDevToolsOpened(),
...
function writeNodeReport(reason) {
  try {
    if (process.report && process.report.writeReport) {
      const f = process.report.writeReport(path.join(LOG_DIR, 'report-' + Date.now() + '.json'));
      log('error', 'report', 'node-report-written', { file: f, reason });
    }
  } catch (e) {
    log('error', 'report', 'node-report-failed', String(e));
  }
}
try {
  crashReporter.start({ submitURL: '', uploadToServer: false });
} catch (e) {
  log('error', 'crashReporter', 'start-failed', String(e));
}
log('info', 'proc', 'boot', {
  pid: process.pid,
...
process.on('uncaughtException', (e) => {
  log('fatal', 'main', 'uncaughtException', {
...
process.on('unhandledRejection', (e) =>
  log('error', 'main', 'unhandledRejection', { stack: (e && /** @type {any} */ (e).stack) || String(e) }),
);
process.on('warning', (w) =>
  log('warn', 'main', 'process-warning', { name: w.name, message: w.message, stack: w.stack }),
);
['SIGINT', 'SIGTERM', 'SIGHUP'].forEach((sig) => {
  try {
    process.on(sig, () => {
      log('warn', 'signal', sig, { windows: dumpWindows() });
      process.exit(0);
    });
  } catch (_) {}
});
...
app.on('render-process-gone', (_e, wc, d) =>
  log('fatal', 'process', 'render-process-gone', {
    url: shortURL(wc),
    reason: d.reason,
    exitCode: d.exitCode,
    windows: dumpWindows(),
...
app.on('child-process-gone', (_e, d) =>
  log('fatal', 'process', 'child-process-gone', {
    type: d.type,
    reason: d.reason,
    exitCode: d.exitCode,
    name: d.name,
...
app.on('web-contents-created', (_e, wc) => {
  ...
  wc.on('console-message', (_ev, level, message, ln, src) => {
    if (level >= 2)
      log(level >= 3 ? 'error' : 'warn', 'renderer-console', message, {
        url: shortURL(wc),
        at: (src || '') + ':' + ln,
      });
  });
  wc.on('did-fail-load', (_ev, code, desc, url) => log('error', 'nav', 'did-fail-load', { code, desc, url }));
  wc.on('did-fail-provisional-load', (_ev, code, desc, url) =>
    log('error', 'nav', 'did-fail-provisional-load', { code, desc, url }),
  );
  wc.on('did-finish-load', () => milestone('page-loaded', { url: shortURL(wc) }));
  wc.on('preload-error', (_ev, p, err) =>
    log('fatal', 'preload', 'preload-error', { preload: p, stack: (err && err.stack) || String(err) }),
  );
});
...
const _memBeat = setInterval(() => {
  try {
    const m = process.memoryUsage();
    log('debug', 'mem', 'heartbeat', { rssMB: Math.round(m.rss / 1048576), heapMB: Math.round(m.heapUsed / 1048576) });
  } catch (_) {}
}, 60000);
