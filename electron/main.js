...
const readDesktop = () => { const d = readJson(DESKTOP_FILE, {}); return { autoRestore: d.autoRestore !== false, windows: Array.isArray(d.windows) ? d.windows : [], orderTicket: d.orderTicket || null }; };
const allTabIds = () => { const t = readJson(TABS_FILE, {}); return (Array.isArray(t.tabs) ? t.tabs : []).map((x) => x && x.id).filter(Boolean); };
...
function writeDesktop(autoRestore, wins) {
  try { fs.mkdirSync(path.dirname(DESKTOP_FILE), { recursive: true }); fs.writeFileSync(DESKTOP_FILE, JSON.stringify({ autoRestore, windows: wins, orderTicket: ticketState }, null, 2)); } catch (_) {}
}
...
function setAutoRestore(on) { writeDesktop(!!on, readDesktop().windows); }
...
function clampBounds(b) {
  ...
  try {
    ...
    width = Math.min(width, wa.width); height = Math.min(height, wa.height);
...
function startServer() {
  ...
  server.on('exit', (code) => { if (code) console.error('[plain-charts] server exited with code', code); });
}

function waitForServer(cb, tries = 0) {
  const req = http.get(ORIGIN, () => { req.destroy(); cb(); });
  req.on('error', () => {
...
function createAppWindow({ tabIds = null, claimAll = false, x, y, bounds, activeId = null, primary = false } = {}) {
  ...
  const opts = {
    ...
    webPreferences: { preload: PRELOAD, contextIsolation: false, nodeIntegration: true, sandbox: false, backgroundThrottling: false },
  };
  const cb = clampBounds(bounds);
  if (cb) { opts.x = cb.x; opts.y = cb.y; opts.width = cb.width; opts.height = cb.height; }
  ...
  bw.webContents.setWindowOpenHandler(({ url }) => {
    if (!url.startsWith(ORIGIN)) { shell.openExternal(url); return { action: 'deny' }; }
    return { action: 'allow' };
  });

  bw.on('move', saveDesktop);
  bw.on('resize', saveDesktop);
  bw.on('maximize', () => { try { bw.webContents.send('win-max', true); } catch (_) {} });
  bw.on('unmaximize', () => { try { bw.webContents.send('win-max', false); } catch (_) {} });
  ...
  bw.on('closed', () => {
    ...
    windows.delete(id);
    if (quitting) return;
    if (windows.size === 0) { app.quit(); return; }   // last visible window == Quit, session preserved
...
const recFor = (sender) => { for (const r of windows.values()) if (r.bw.webContents === sender) return r; return null; };
const idFor = (sender) => { for (const [k, r] of windows) if (r.bw.webContents === sender) return k; return null; };
function sendTabs(winId) { const r = windows.get(winId); if (r) { try { r.bw.webContents.send('win-tabs', r.tabIds); } catch (_) {} } }
...
function removeTab(winId, tabId) { const r = windows.get(winId); if (r) r.tabIds = r.tabIds.filter((t) => t !== tabId); }
...
function removeTabRemote(tabId) {
  try {
    ...
    const req = http.request(`${ORIGIN}api/tabs/remove`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) } });
    req.on('error', () => {});
    req.end(body);
  } catch (_) {}
}
...
ipcMain.on('tabs-claim', (e, { tabIds }) => { const r = recFor(e.sender); if (r) { r.tabIds = Array.isArray(tabIds) ? [...tabIds] : []; saveDesktop(); } });
ipcMain.on('tabs-add', (e, { tabId }) => { const r = recFor(e.sender); if (r && tabId && !r.tabIds.includes(tabId)) { r.tabIds.push(tabId); saveDesktop(); } });
ipcMain.on('tabs-remove', (e, { tabId }) => {
  const id = idFor(e.sender); if (!id) return;
  removeTab(id, tabId);
  const r = windows.get(id);
  if (r && r.tabIds.length === 0 && windows.size > 1) r.bw.close();   // emptied detached window closes
  else saveDesktop();
});
ipcMain.on('tab-active', (e, { tabId } = {}) => { const r = recFor(e.sender); if (r) { r.activeId = tabId; saveDesktop(); } });
...
ipcMain.on('win-always-on-top-toggle', (e) => { const w = BrowserWindow.fromWebContents(e.sender); const on = w ? !w.isAlwaysOnTop() : false; if (w) w.setAlwaysOnTop(on); if (w && w === ticketWin && ticketState) { ticketState.alwaysOnTop = on; saveDesktop(); } e.returnValue = on; });
ipcMain.on('win-always-on-top-get', (e) => { const w = BrowserWindow.fromWebContents(e.sender); e.returnValue = !!(w && w.isAlwaysOnTop()); });
...
function openTicket(payload) {
  ...
  if (ticketWin && !ticketWin.isDestroyed()) { try { ticketWin.show(); ticketWin.focus(); ticketWin.webContents.send('order-ticket-open', ticketPayload); } catch (_) {} return; }
  const rb = ticketState && ticketState.bounds ? clampBounds({ ...ticketState.bounds, width: TICKET_W, height: TICKET_H }) : null;   // remembered position only; size is fixed
  const aot = !!(ticketState && ticketState.alwaysOnTop);
  const base = BrowserWindow.getFocusedWindow();
  const b = base ? base.getBounds() : null;
  ticketWin = new BrowserWindow({
    width: TICKET_W, height: TICKET_H, minWidth: TICKET_W, minHeight: TICKET_H, maxWidth: TICKET_W, maxHeight: TICKET_H,
    resizable: false, title: 'Order',   // FIXED size -- the layout is anchored, so the window never resizes
    icon: APP_ICON,
    x: rb ? rb.x : (b ? b.x + 60 : undefined), y: rb ? rb.y : (b ? b.y + 60 : undefined),
    alwaysOnTop: aot,
    frame: false, autoHideMenuBar: true,   // no OS title bar / menu -- custom title bar like the main app
    webPreferences: { preload: PRELOAD, contextIsolation: false, nodeIntegration: true, sandbox: false, backgroundThrottling: false },
  });
  ...
  const remember = () => { try { if (ticketWin && !ticketWin.isDestroyed()) { ticketState = { bounds: ticketWin.getBounds(), alwaysOnTop: ticketWin.isAlwaysOnTop() }; saveDesktop(); } } catch (_) {} };
  ticketWin.on('move', remember); ticketWin.on('resize', remember);
  ...
  ticketWin.webContents.on('did-finish-load', () => { try { ticketWin.webContents.send('order-ticket-open', ticketPayload); } catch (_) {} });
  ticketWin.on('closed', () => { ticketWin = null; saveDesktop(); });   // persist the final placement; never re-open automatically
  try { ticketState = { bounds: ticketWin.getBounds(), alwaysOnTop: aot }; } catch (_) { ticketState = { bounds: rb, alwaysOnTop: aot }; }
  saveDesktop();
}
...
ipcMain.on('order-ticket-ready', (e) => { try { e.sender.send('order-ticket-open', ticketPayload); } catch (_) {} });
...
const TICKET_W = 400, TICKET_H = 580;   // fixed Order-window content-agnostic frame size
ipcMain.on('order-ticket-width', () => { /* no-op: the Order window is fixed-size */ });

function destroyPreview() { if (preview) { try { preview.destroy(); } catch (_) {} preview = null; } }
function movePreview(x, y) { if (preview && x != null && y != null) { try { preview.setPosition(Math.round(x + PREV_DX), Math.round(y + PREV_DY)); } catch (_) {} } }

ipcMain.on('devtools', (e, { on } = {}) => {
  const apply = (bw) => { if (!bw) return; try { on ? bw.webContents.openDevTools({ mode: 'detach' }) : bw.webContents.closeDevTools(); } catch (_) {} };
...
ipcMain.on('debug-info', (e) => { e.returnValue = { port: DEBUG_PORT, active: DEBUG_ON, dev: DEV }; });
...
ipcMain.on('tab-drag-start', async (e, { tabId, x, y } = {}) => {
  ...
  try {
    ...
    preview = new BrowserWindow({
      width: s.width, height: s.height, x: Math.round((x || 0) + PREV_DX), y: Math.round((y || 0) + PREV_DY),
      frame: false, transparent: true, alwaysOnTop: true, focusable: false, skipTaskbar: true,
    ...
    const html = '<body style="margin:0;overflow:hidden;background:transparent"><img src="' + thumb.toDataURL() +
...
ipcMain.on('tab-drag-end', (e, { x, y } = {}) => {
  ...
  if (targetId && targetId !== d.fromId) {
    ...
    sendTabs(d.fromId); sendTabs(targetId);
    if (from && from.tabIds.length === 0) from.bw.close();   // emptied source window closes
    else try { t.bw.focus(); } catch (_) {}
  } else if (!targetId) {
...
function createDataHost() {
  ...
  dataHost.on('closed', () => { dataHost = null; });
}
...
function createAddonHost() {
  ...
  addonHostWin.on('closed', () => { addonHostWin = null; });
}
...
function createOrderHost() {
  ...
  orderHostWin.on('closed', () => { orderHostWin = null; });
}
...
function createAlertHost() {
  ...
  alertHostWin.on('closed', () => { alertHostWin = null; });
}
...
function bootWindows() {
  ...
  if (!saved.length) { createAppWindow({ claimAll: true }); }
  else {
    const placed = new Set(saved.flatMap((w) => w.tabIds));
    const leftover = allTabIds().filter((tid) => !placed.has(tid));
    saved.forEach((w, i) => {
      createAppWindow({ tabIds: i === 0 ? [...w.tabIds, ...leftover] : w.tabIds, bounds: w.bounds, activeId: w.activeId, primary: i === 0 });
    });
  }
...
app.whenReady().then(() => {
  ...
  waitForServer((err) => {
    if (err) { log('error', 'lifecycle', 'server-did-not-start', err.message); console.error('[plain-charts]', err.message); }
  ...
  app.on('activate', () => { if (!windows.size) createAppWindow({ claimAll: true }); });
});
...
app.on('before-quit', () => { if (quitting) return; quitting = true; saveDesktopNow(); });
app.on('quit', () => { if (server) { try { server.kill(); } catch (_) {} } killAllPtys(); });
