...
### Added
- Alert conditions declared by the study itself: a study can name its own moments (the Fair Value Gap study declares Bullish FVG, Bearish FVG, and FVG mitigated) and the Add-condition dialog offers them when that study is the subject. The sentence is subject-only ("Fair Value Gap: Bullish FVG"), with no second object and no value. Studies with nothing alertable (no line, no declared condition) no longer appear as condition subjects
